...
/*==============================================================================
    Constants
*/
const char *const Qstreambuf::ID = "UA::HiRISE::Qstreambuf ($Revision: 1.6 $ $Date: 2013/06/13 17:42:24 $)";
...
#endif
const int Qstreambuf::BUFFER_SIZE = QSTREAM_BUFFER_SIZE;
...
#endif
const int Qstreambuf::PUTBACK_SIZE = QSTREAM_PUTBACK_SIZE;
...
#endif
int Qstreambuf::Default_Wait_Time = QSTREAM_DEFAULT_WAIT_TIME;

/*==============================================================================
    Constructors
*/
Qstreambuf::Qstreambuf(QIODevice *qiodevice)
    : QObject(), std::streambuf(), QIO_Device(qiodevice), Network_Reply(NULL), Buffer(NULL), Event_Loop(NULL),
      Timer(NULL), Wait_Time(Default_Wait_Time), Timeout(false)
{
    ...
    if (QIO_Device)
    {
        ...
        if (!QIO_Device->isReadable())
        {
            ...
            message << ID << endl << "The QIODevice is not readable.";
...
#if ((DEBUG_SECTION) & DEBUG_CONSTRUCTORS)
        clog << "    " << QSTREAM_BUFFER_SIZE << " character Buffer @ " << (void *)Buffer << " with " << PUTBACK_SIZE
             << " character put-back section" << endl
...
#if ((DEBUG_SECTION) & DEBUG_CONSTRUCTORS)
            clog << "    QIODevice is a QNetworkReply" << endl << "    new QEventLoop" << endl;
#endif
            Event_Loop = new QEventLoop;
            connect(Network_Reply, SIGNAL(finished()), Event_Loop, SLOT(quit()));
            connect(Network_Reply, SIGNAL(readyRead()), Event_Loop, SLOT(quit()));
    ...
    {
        ostringstream message;
        message << ID << endl << "Can't construct a Qstreambuf on a NULL QIODevice.";
...
*/
std::streamsize Qstreambuf::xsgetn(std::streambuf::char_type *data, std::streamsize amount)
{
#if ((DEBUG_SECTION) & DEBUG_STREAMBUF)
    clog << ">>> Qstreambuf:xsgetn: data @ " << (void *)data << " amount = " << amount << endl;
#endif
    std::streamsize got(0), available;
    while (amount)
    {
        ...
        if (available > amount)
            available = amount;
#if ((DEBUG_SECTION) & DEBUG_STREAMBUF)
        clog << "    copying " << available << " bytes from " << (void *)gptr() << " to " << (void *)data << endl;
#endif
...
//	Move last read characters to the put-back section.
#if ((DEBUG_SECTION) & DEBUG_STREAMBUF)
            clog << "    moving " << putback << " put-back characters from " << (void *)(gptr() - putback) << " to "
                 << (void *)(Buffer + (PUTBACK_SIZE - putback)) << endl;
#endif
            traits_type::move(Buffer + (PUTBACK_SIZE - putback), gptr() - putback, putback);

            //	Reset the buffer pointers.
            setg(Buffer + (PUTBACK_SIZE - putback), Buffer + PUTBACK_SIZE, Buffer + PUTBACK_SIZE);
        }

        //	Refill the buffer.
        std::streamsize available = QIO_Device->bytesAvailable();
        if (!available && Network_Reply && Wait_Time)
        {
#if ((DEBUG_SECTION) & DEBUG_STREAMBUF)
        ...
        if (available)
        {
            ...
            clog << "    bytes available = " << available << endl
                 << "    refilling buffer @ " << (void *)(Buffer + PUTBACK_SIZE) << " with "
                 << (QSTREAM_BUFFER_SIZE - PUTBACK_SIZE) << " characters" << endl;
#endif
            available = QIO_Device->read(Buffer + PUTBACK_SIZE, QSTREAM_BUFFER_SIZE - PUTBACK_SIZE);

            //	Reset the buffer pointers.
            setg(Buffer + (PUTBACK_SIZE - putback), Buffer + PUTBACK_SIZE, Buffer + PUTBACK_SIZE + available);
#if ((DEBUG_SECTION) & DEBUG_STREAMBUF)
...
std::streambuf::pos_type Qstreambuf::seekoff(std::streambuf::off_type offset, //	signed integer type.
                                             std::ios_base::seekdir direction, std::ios_base::openmode mode)
{
#if ((DEBUG_SECTION) & DEBUG_STREAMBUF)
    ...
    */
    std::streambuf::pos_type position(-1);
    if (QIO_Device)
    {
        switch (direction)
        {
...
std::streambuf::pos_type Qstreambuf::seekpos(std::streambuf::pos_type position, std::ios_base::openmode /* mode */
)
{
#if ((DEBUG_SECTION) & DEBUG_STREAMBUF)
    ...
    if (QIO_Device)
    {
...
#endif
        if (position < QIO_Device->pos() && position >= (QIO_Device->pos() - (egptr() - eback())))
        {
#if ((DEBUG_SECTION) & DEBUG_STREAMBUF)
        ...
        else if (QIO_Device->seek(position))
        {
...
std::streambuf::int_type Qstreambuf::underflow()
{
    ...
    if (gptr() < egptr() || refill_buffer() == HAS_DATA)
    {
#if ((DEBUG_SECTION) & DEBUG_STREAMBUF)
