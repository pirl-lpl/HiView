...
#endif
int Activity_Indicator::Default_Indicator_Size = PROGRESS_INDICATOR_SIZE,
    Activity_Indicator::Default_Start_Delay = PROGRESS_INDICATOR_START_DELAY,
...
void Activity_Indicator::initialize()
{
    setSizePolicy(QSizePolicy(QSizePolicy::Fixed, QSizePolicy::Fixed));

    State_Color = new QColor[TOTAL_STATE_CONDITIONS];
    for (int index = 0; index < TOTAL_STATE_CONDITIONS; index++)
        State_Color[index] = Default_State_Color[index];

    Update_Timer = new QTimer(this);
    Update_Timer->setSingleShot(true);
    connect(Update_Timer, SIGNAL(timeout()), SLOT(interval()));
...
/*==============================================================================
    Accessors
*/
void Activity_Indicator::default_indicator_size(int size)
{
...
void Activity_Indicator::default_state_color(int condition, QColor color)
{
    ...
    else
    {
        ...
        message << ID << endl
                ...
                << "The maximum state selection is " << (TOTAL_STATE_CONDITIONS - 1);
        throw invalid_argument(message.str());
    }
}
...
/*==============================================================================
    Qt slots
*/
void Activity_Indicator::state(int condition)
{
#if ((DEBUG_SECTION) & DEBUG_STATE)
...
#if ((DEBUG_SECTION) & DEBUG_EVENTS)
        clog << "    NO_PROGRESS - starting timer with update interval " << update_interval << endl;
#endif
...
void Activity_Indicator::paintEvent(QPaintEvent*)
{
    ...
    {
#if ((DEBUG_SECTION) & (DEBUG_UPDATE | DEBUG_EVENTS))
        clog << "    No progress" << endl << "<<< Activity_Indicator::paintEvent" << endl;
#endif
        return;
    }
    ...
    State_Lock.unlock();
#if ((DEBUG_SECTION) & (DEBUG_UPDATE | DEBUG_EVENTS))
    clog << "    pixmap drawn at origin " << origin << "x, " << origin << "y" << endl
...
QPixmap* Activity_Indicator::indicator() const
{
    ...
    {
        conical_gradient.setColorAt(0.0, QColor((QRgb)PROGRESS_INDICATOR_HEAD_COLOR));
