...
HiROC CVS ID: $Id: Dynamic_Image.cc,v 2.38 2014/05/27 17:13:51 guym Exp $
...
const char* const
Dynamic_Image::ID =
"UA::HiRISE::Dynamic_Image ($Revision: 2.38 $ $Date: 2014/05/27 17:13:51 $)";
...
#endif
const QImage::Format
Dynamic_Image::IMAGE_FORMAT = PLASTIC_IMAGE_FORMAT;

#ifndef IMAGE_METADATA_GROUP_NAME
#define IMAGE_METADATA_GROUP_NAME "Image"
#endif
Dynamic_Image::Name_String
Dynamic_Image::IMAGE_METADATA_GROUP = IMAGE_METADATA_GROUP_NAME,
Dynamic_Image::IMAGE_SOURCE_PARAMETER = "Source",
/*
    These parameter names should be the same as those for JP2_Metadata.
*/
Dynamic_Image::IMAGE_WIDTH_PARAMETER = "Width",
Dynamic_Image::IMAGE_HEIGHT_PARAMETER = "Height",
Dynamic_Image::IMAGE_BANDS_PARAMETER = "Image_Bands",
Dynamic_Image::VALUE_BITS_PARAMETER = "Value_Bits";

const Dynamic_Image::Pixel_Datum
Dynamic_Image::UNDEFINED_PIXEL_VALUE = (Dynamic_Image::Pixel_Datum)-1;

const unsigned int
Dynamic_Image::MAXIMUM_PIXEL_DATUM_PRECISION = 16;

const Dynamic_Image::Mapping_Type
Dynamic_Image::NO_MAPPINGS = 0,
Dynamic_Image::IDENTICAL_MAPPINGS = NO_MAPPINGS,
Dynamic_Image::BAND_MAP = (1 << 0),
Dynamic_Image::TRANSFORMS = (1 << 1),
Dynamic_Image::DATA_MAPS = (1 << 2),
Dynamic_Image::ALL_MAPPINGS = (BAND_MAP | TRANSFORMS | DATA_MAPS);


QString
Dynamic_Image::mapping_type_names
(
...
const QString
Dynamic_Image::Rendering_Monitor::Status_Message[] =
{
    // 0: INFO_ONLY
    "Information only.",
    // 1: LOW_QUALITY_DATA
...
QRgb
Dynamic_Image::Default_Background_Color
...
#endif
bool
Dynamic_Image::Default_Auto_Update = DEFAULT_AUTO_UPDATE;

#ifndef DEFAULT_RENDERING_INCREMENT_LINES
#define DEFAULT_RENDERING_INCREMENT_LINES 100
#endif
unsigned int
Dynamic_Image::Default_Rendering_Increment_Lines
    ...
    Cancel_Update(false)
{
    ...
    clog << ">>> Dynamic_Image @ " << reinterpret_cast<void*>(this) << endl
        << "          size = " << size() << endl
        << "      band_map @ " << (band_map) << endl
        << "    transforms @ " << reinterpret_cast<void*>(transforms) << endl
        << "     data_maps @ " << reinterpret_cast<void*>(data_maps) << endl;
#endif
#if (DEBUG_SECTION)
    clog << boolalpha;
#endif
...
void
Dynamic_Image::initialize()
{
#if ((DEBUG_SECTION) & DEBUG_CONSTRUCTORS)
    clog << ">>> Dynamic_Image::initialize" << endl
...
Dynamic_Image::~Dynamic_Image()
{
    ...
    {
#if ((DEBUG_SECTION) & DEBUG_CONSTRUCTORS)
        clog << "    delete Metadata @ " << reinterpret_cast<void*>(Metadata) << endl;
#endif
        delete Metadata;
        Metadata = NULL;
    }
    ...
    {
#if ((DEBUG_SECTION) & DEBUG_CONSTRUCTORS)
        clog << "    delete Band_Map @ " << reinterpret_cast<void*>(Band_Map) << endl;
#endif
        delete[] Band_Map;
    }
    if (Locally_Owned_and_Operated & TRANSFORMS &&
        ...
        while (++band < 3)
        {
            if (Geo_Transforms[band])
            {
                ...
                clog << "    delete Geo_Transforms[" << band << "] @ "
                    << reinterpret_cast<void*>(Geo_Transforms[band]) << endl;
#endif
                delete Geo_Transforms[band];
            }
        }
#if ((DEBUG_SECTION) & DEBUG_CONSTRUCTORS)
        clog << "    delete Geo_Transforms @ " << reinterpret_cast<void*>(Geo_Transforms) << endl;
#endif
        ...
        while (++band < 3)
        {
            if (Data_Maps[band])
            {
                ...
                clog << "    delete Data_Maps[" << band << "] @ "
                    << reinterpret_cast<void*>(Data_Maps[band]) << endl;
#endif
                delete Data_Maps[band];
            }
        }
#if ((DEBUG_SECTION) & DEBUG_CONSTRUCTORS)
        clog << "    delete Data_Maps @ " << reinterpret_cast<void*>(Data_Maps) << endl;
#endif
        delete[] Data_Maps;
    }
#if ((DEBUG_SECTION) & DEBUG_CONSTRUCTORS)
    clog << "<<< ~Dynamic_Image" << endl;
#endif
}
...
*/
bool
Dynamic_Image::source_name
(
    const QString& name
)
{
#if ((DEBUG_SECTION) & DEBUG_METADATA)
    clog << ">>> Dynamic_Image::source_name: " << name << endl;
#endif
    bool
        changed = false;
    if (!closed())
...
#if ((DEBUG_SECTION) & DEBUG_METADATA)
                clog << "    Metadata @ " << reinterpret_cast<void*>(Metadata) << endl
...
#if ((DEBUG_SECTION) & DEBUG_METADATA)
    clog << "<<< Dynamic_Image::source_name: " << changed << endl;
#endif
    return changed;
}

idaeim::PVL::Aggregate*
Dynamic_Image::metadata()
{
#if ((DEBUG_SECTION) & DEBUG_METADATA)
    clog << ">>> Dynamic_Image::metadata" << endl;
#endif
...
#if ((DEBUG_SECTION) & DEBUG_METADATA)
        clog << "          Metadata @ " << reinterpret_cast<void*>(Metadata) << endl
            << "    Image_Metadata @ " << reinterpret_cast<void*>(Image_Metadata) << endl;
#endif
        QSize
...
#if ((DEBUG_SECTION) & DEBUG_METADATA)
    clog << "<<< Dynamic_Image::metadata @ " << reinterpret_cast<void*>(Metadata) << endl;
#endif
    return Metadata;
}
...
bool
Dynamic_Image::add_metadata_monitor
(
...
bool
Dynamic_Image::remove_metadata_monitor
(
    Metadata_Monitor* monitor
)
{
...
void
Dynamic_Image::notify_metadata_monitors()
{
    if (!Closed)
    {
        int
...
)
{
    ...
    clog << ">>> Dynamic_Image::source_band_map:" << band_map << endl
        << "    shared = " << shared << endl
        << "    Band_Map" << Band_Map << endl;
#endif
...
#endif
#if ((DEBUG_SECTION) & DEBUG_BAND_MAP)
        clog << "<<< Dynamic_Image::source_band_map: false" << endl;
#endif
        return false;
    }
    ...
    if (!band_map)
    {
        ...
        clog << "    Band_Map" << Band_Map << endl
            << "<<< Dynamic_Image::source_band_map: " << changed << endl;
#endif
        return changed;
    }
...
bool
Dynamic_Image::source_band_map_reset()
{
#if ((DEBUG_SECTION) & DEBUG_BAND_MAP)
    clog << ">>> Dynamic_Image::source_band_map_reset" << endl;
#endif
    if (Closed)
    {
#if ((DEBUG_SECTION) & DEBUG_BAND_MAP)
...
#endif
#if ((DEBUG_SECTION) & DEBUG_BAND_MAP)
        clog << "<<< Dynamic_Image::source_band_map_reset: false" << endl;
#endif
        return false;
    }
    ...
    changed |= needs_update(changed ? BAND_MAP : NO_MAPPINGS);
#if ((DEBUG_SECTION) & DEBUG_BAND_MAP)
    clog << "<<< Dynamic_Image::source_band_map_reset: " << changed << endl;
#endif
    return changed;
}
...
bool
Dynamic_Image::reset_band_map
(
    unsigned int* band_map
)
{
#if ((DEBUG_SECTION) & DEBUG_BAND_MAP)
    clog << ">>> Dynamic_Image::reset_band_map:" << band_map << endl;
#endif
    bool
        changed = false;
    if (band_map)
...
#if ((DEBUG_SECTION) & DEBUG_BAND_MAP)
        clog << "    band_map @ " << reinterpret_cast<void*>(band_map)
            << ": " << band_map[0]
            << ", " << band_map[1]
            << ", " << band_map[2] << endl;
#endif
    ...
    clog << "    band map" << band_map << endl
        << "<<< Dynamic_Image::reset_band_map: " << changed << endl;
#endif
    return changed;
}
...
bool
Dynamic_Image::different_band_map
(
    const unsigned int* band_map
) const
{
...
*/
bool
Dynamic_Image::set_source_transform
(
...
)
{
    ...
    clog << ">>> Dynamic_Image::set_source_transform:" << endl
        << transform
        << "    band = " << band << endl;
#endif
...
#endif
#if ((DEBUG_SECTION) & DEBUG_TRANSFORMS)
        clog << "<<< Dynamic_Image::set_source_transform: false" << endl;
#endif
        return false;
    }
...
#if ((DEBUG_SECTION) & DEBUG_TRANSFORMS)
    clog << "<<< Dynamic_Image::set_source_transform: " << changed << endl;
#endif
    return changed;
}
...
bool
Dynamic_Image::source_transforms
(
    const QTransform** transforms,
    bool shared
)
{
#if ((DEBUG_SECTION) & DEBUG_TRANSFORMS)
    clog << ">>> Dynamic_Image::source_transforms: @ "
        << reinterpret_cast<void*>(transforms) << endl
        << "    shared = " << shared << endl;
#endif
...
#endif
#if ((DEBUG_SECTION) & DEBUG_TRANSFORMS)
        clog << "<<< Dynamic_Image::source_transforms: false" << endl;
#endif
        return false;
    }
...
#if ((DEBUG_SECTION) & DEBUG_TRANSFORMS)
            clog << "    local Geo_Transforms @ " << reinterpret_cast<void*>(Geo_Transforms) << endl
                << "          replace shared @ " << reinterpret_cast<void*>(transforms) << endl;
#endif
            Locally_Owned_and_Operated |= TRANSFORMS;
            while (++band < 3)
    ...
    changed |= needs_update(changed ? TRANSFORMS : NO_MAPPINGS);
#if ((DEBUG_SECTION) & DEBUG_TRANSFORMS)
    clog << "<<< Dynamic_Image::source_transforms: " << changed << endl;
#endif
    return changed;
}
...
QTransform*
Dynamic_Image::source_transform
(
    int band
) const
{
#if ((DEBUG_SECTION) & DEBUG_TRANSFORMS)
    clog << ">>> Dynamic_Image::source_transform: " << band << endl;
#endif
...
#if ((DEBUG_SECTION) & DEBUG_TRANSFORMS)
    clog << "<<< Dynamic_Image::source_transform" << endl;
#endif
...
QTransform
Dynamic_Image::invert_keeping_origin
(
    const QTransform& transform
)
{
...
QTransform
Dynamic_Image::invert_clipping_origin
(
    const QTransform& transform
)
{
...
bool
Dynamic_Image::source_transform_reset
(
    int band
)
{
#if ((DEBUG_SECTION) & DEBUG_TRANSFORMS)
    clog << ">>> Dynamic_Image::source_transform_reset: " << band << endl;
#endif
...
#endif
#if ((DEBUG_SECTION) & DEBUG_TRANSFORMS)
        clog << "<<< Dynamic_Image::source_transform_reset: false" << endl;
#endif
        return false;
    }
...
#if ((DEBUG_SECTION) & DEBUG_TRANSFORMS)
    clog << "<<< Dynamic_Image::source_transform_reset: " << changed << endl;
#endif
    return changed;
}
...
{
#if ((DEBUG_SECTION) & DEBUG_TRANSFORMS)
    clog << ">>> Dynamic_Image::source_origin: "
...
#endif
#if ((DEBUG_SECTION) & DEBUG_TRANSFORMS)
        clog << "<<< Dynamic_Image::source_origin: false" << endl;
#endif
        return false;
    }
    ...
    changed |= needs_update(changed ? TRANSFORMS : NO_MAPPINGS);
#if ((DEBUG_SECTION) & DEBUG_TRANSFORMS)
    clog << "<<< Dynamic_Image::source_origin: " << changed << endl;
#endif
    return changed;
}
...
#if ((DEBUG_SECTION) & DEBUG_TRANSFORMS)
    clog << ">>> Dynamic_Image::source_origin: " << band << endl;
#endif
    if (band < 0 ||
        band > 2)
    {
...
{
#if ((DEBUG_SECTION) & DEBUG_TRANSFORMS)
    clog << ">>> Dynamic_Image::source_scaling: "
        << scale_horizontal << "x, "
        << scale_vertical << "y; "
        << band << 'b' << endl;
#endif
...
#endif
#if ((DEBUG_SECTION) & DEBUG_TRANSFORMS)
        clog << "<<< Dynamic_Image::source_scaling: false" << endl;
#endif
        return false;
    }
    ...
    changed |= needs_update(changed ? TRANSFORMS : NO_MAPPINGS);
#if ((DEBUG_SECTION) & DEBUG_TRANSFORMS)
    clog << "<<< Dynamic_Image::source_scaling: " << changed << endl;
#endif
    return changed;
}
...
#if ((DEBUG_SECTION) & DEBUG_TRANSFORMS)
    clog << ">>> Dynamic_Image::source_scaling: get x,y for band " << band << endl;
#endif
    if (band < 0 ||
        band > 2)
    {
...
#if ((DEBUG_SECTION) & DEBUG_TRANSFORMS)
    clog << ">>> Dynamic_Image::source_scaling: get QSizeF for band "
        << band << endl;
#endif
    if (band < 0 ||
        band > 2)
...
)
{
    ...
    clog << ">>> Dynamic_Image::source_data_map: @ "
        << (&data_map) << "; " << band << endl;
#endif
...
#endif
#if ((DEBUG_SECTION) & DEBUG_DATA_MAPPING)
        clog << "<<< Dynamic_Image::source_data_map: false" << endl;
#endif
        return false;
    }
    ...
    while (band < bands)
    {
        ...
        if (Data_Maps[band]->size() < static_cast<int>(source_data_map_size()))
...
#if ((DEBUG_SECTION) & DEBUG_DATA_MAPPING)
    clog << "<<< Dynamic_Image::source_data_map: " << changed << endl;
#endif
    return changed;
}
...
bool
Dynamic_Image::source_data_maps
(
    const Data_Map** data_maps,
    bool shared
)
{
#if ((DEBUG_SECTION) & DEBUG_DATA_MAPPING)
    clog << ">>> Dynamic_Image::source_data_maps: @ " << reinterpret_cast<void*>(data_maps) << endl
        << "    shared = " << shared << endl;
#endif
...
#endif
#if ((DEBUG_SECTION) & DEBUG_DATA_MAPPING)
        clog << "<<< Dynamic_Image::source_data_maps: false" << endl;
#endif
        return false;
    }
        ...
        data_maps == const_cast<const Data_Map**>(Data_Maps))
    {
        ...
        clog << "    identical maps; redundant self-assignment" << endl
            << "<<< Dynamic_Image::source_data_maps: false" << endl;
#endif
        return false;
    }
...
#if ((DEBUG_SECTION) & DEBUG_BAND_MAP)
            clog << "    local Data_Maps @ " << reinterpret_cast<void*>(Data_Maps) << endl
                << "     replace shared @ " << reinterpret_cast<void*>(data_maps) << endl;
#endif
            while (++band < 3)
            {
    ...
    changed |= needs_update(changed ? DATA_MAPS : NO_MAPPINGS);
#if ((DEBUG_SECTION) & DEBUG_DATA_MAPPING)
    clog << "<<< Dynamic_Image::source_data_maps: " << changed << endl;
#endif
    return changed;
}


Dynamic_Image::Data_Map*
Dynamic_Image::source_data_map
(
    int band
) const
...
#if ((DEBUG_SECTION) & DEBUG_DATA_MAPPING)
    clog << ">>> Dynamic_Image::source_data_map: " << band << endl;
#endif
    if (band < 0 ||
        band > 2)
    {
...
#if ((DEBUG_SECTION) & DEBUG_DATA_MAPPING)
    clog << "<<< Dynamic_Image::source_data_map" << endl;
#endif
...
bool
Dynamic_Image::source_data_map_reset
(
    int band
)
{
#if ((DEBUG_SECTION) & DEBUG_DATA_MAPPING)
    clog << ">>> Dynamic_Image::source_data_map_reset: " << band << endl;
#endif
...
#endif
#if ((DEBUG_SECTION) & DEBUG_DATA_MAPPING)
        clog << "<<< Dynamic_Image::source_data_map_reset: false" << endl;
#endif
        return false;
    }
...
#if ((DEBUG_SECTION) & DEBUG_DATA_MAPPING)
    clog << "<<< Dynamic_Image::source_data_map_reset: " << changed << endl;
#endif
    return changed;
}
...
bool
Dynamic_Image::reset_data_map
(
    Data_Map& data_map
)
{
#if ((DEBUG_SECTION) & DEBUG_DATA_MAPPING)
    clog << ">>> Dynamic_Image::reset_data_map: @ " << reinterpret_cast<void*>(&data_map) << endl;
#endif
    bool
        changed = false;
    double
...
#if ((DEBUG_SECTION) & DEBUG_DATA_MAPPING)
    clog << dec << setfill(' ')
        << "<<< Dynamic_Image::reset_data_map: " << changed << endl;
#endif
    return changed;
}
...
{
#if ((DEBUG_SECTION) & DEBUG_DATA_MAPPING)
    clog << ">>> Dynamic_Image::assign_data_map: @ "
        << (void*)&to_map << " = @ " << (&from_map) << endl;
#endif
...
#if ((DEBUG_SECTION) & DEBUG_DATA_MAPPING)
    clog << dec << setfill(' ')
        << "<<< Dynamic_Image::assign_data_map: " << changed << endl;
#endif
    return changed;
}
...
bool
Dynamic_Image::different_data_maps
(
    const Data_Map** data_maps
) const
{
#if ((DEBUG_SECTION) & DEBUG_DATA_MAPPING)
    clog << ">>> Dynamic_Image::different_data_maps" << endl
        << "    Data_Maps @ " << reinterpret_cast<void*>(Data_Maps) << endl
        << "    data_maps @ " << reinterpret_cast<void*>(data_maps) << endl;
#endif
    ...
    if (Data_Maps &&
        ...
        while (band--)
        {
            ...
            {
#if ((DEBUG_SECTION) & DEBUG_DATA_MAPPING)
                clog << "<<< Dynamic_Image::different_data_maps: true" << endl;
#endif
                return true;
            }
        }
#if ((DEBUG_SECTION) & DEBUG_DATA_MAPPING)
    clog << "<<< Dynamic_Image::different_data_maps: false" << endl;
#endif
    return false;
}
...
unsigned int
Dynamic_Image::source_data_map_size() const
{
    unsigned int
...
*/
unsigned long long
Dynamic_Image::source_histograms
(
...
) const
{
    ...
    clog << ">>> Dynamic_Image::source_histograms:" << endl
...
#if ((DEBUG_SECTION) & (DEBUG_HISTOGRAMS | DEBUG_PRINT_HISTOGRAMS))
    clog << "    histograms @ "
        << reinterpret_cast<void*>(histograms[0]) << ", "
        << reinterpret_cast<void*>(histograms[1]) << ", "
        << reinterpret_cast<void*>(histograms[2]) << endl;
#endif
    ...
    if (source_region.isEmpty())
    {
        ...
        clog << "    empty source_region" << endl
            << "<<< Dynamic_Image::source_histograms: 0" << endl;
#endif
        return 0;
    }
...
#if ((DEBUG_SECTION) & (DEBUG_HISTOGRAMS | DEBUG_PRINT_HISTOGRAMS))
    clog << "<<< Dynamic_Image::source_histograms: " << max_count << endl;
#endif
    return max_count;
}
...
bool
Dynamic_Image::refresh_source_histogram
(
    QVector<Histogram*>& histograms,
    int band
) const
{
#if ((DEBUG_SECTION) & (DEBUG_HISTOGRAMS | DEBUG_PRINT_HISTOGRAMS))
    clog << ">>> Dynamic_Image::refresh_source_histogram: " << band << endl;
#endif
    bool
        refresh = true;
    int
    ...
    if (other_band >= 0)
    {
        ...
        if (!histograms[band])
        {
            ...
            clog << "    NULL histogram[" << band << "] set to histogram["
                << other_band << "] @ "
                << reinterpret_cast<void*>(histograms[other_band]) << endl;
#endif
        }
        else
    ...
    else
    {
        if (!histograms[band])
        {
            ...
            clog << "    new histogram[ " << band << "] @ "
                << reinterpret_cast<void*>(histograms[band]) << " with "
...
#if ((DEBUG_SECTION) & (DEBUG_HISTOGRAMS | DEBUG_PRINT_HISTOGRAMS))
    clog << ">>> Dynamic_Image::refresh_source_histogram: " << refresh << endl;
#endif
    return refresh;
}
...
unsigned long long
Dynamic_Image::display_histograms
(
    QVector<Histogram*> histograms,
...
{
#if ((DEBUG_SECTION) & (DEBUG_HISTOGRAMS | DEBUG_PRINT_HISTOGRAMS))
    clog << ">>> Dynamic_Image::display_histograms:" << endl
...
#if ((DEBUG_SECTION) & (DEBUG_HISTOGRAMS | DEBUG_PRINT_HISTOGRAMS))
    clog << ">>> Dynamic_Image::display_histograms: " << display_region << endl
        << "    histograms @ "
        << reinterpret_cast<void*>(histograms[0]) << ", "
        << reinterpret_cast<void*>(histograms[1]) << ", "
        << reinterpret_cast<void*>(histograms[2]) << endl;
    ...
    if (display_region.isEmpty())
    {
        ...
        clog << "    empty display_region" << endl
            << "<<< Dynamic_Image::display_histograms: 0" << endl;
#endif
        return 0;
    }
...
#endif
#if ((DEBUG_SECTION) & (DEBUG_HISTOGRAMS | DEBUG_PRINT_HISTOGRAMS))
    clog << "<<< Dynamic_Image::display_histograms: "
...
#endif
#if ((DEBUG_SECTION) & (DEBUG_UPDATE | DEBUG_MANIPULATORS))
    clog << "<<< Dynamic_Image::update: " << up_to_date << endl;
#endif
    return up_to_date;
}
...
{
#if ((DEBUG_SECTION) & (DEBUG_UPDATE | DEBUG_MANIPULATORS))
    clog << ">>> Dynamic_Image::needs_update: " << mapping_type_names(changed)
        << " (" << changed << ')' << endl
        << "    " << *this << endl;
#endif
    bool
...
#endif
#if ((DEBUG_SECTION) & (DEBUG_UPDATE | DEBUG_MANIPULATORS))
    clog << "<<< Dynamic_Image::needs_update: " << updated << endl;
#endif
    return updated;
}


Dynamic_Image::Mapping_Type
Dynamic_Image::needs_update() const
{
...
bool
Dynamic_Image::render_image() noexcept(false)
//throw (Render_Exception, std::bad_exception)
{
#if ((DEBUG_SECTION) & (DEBUG_RENDER | DEBUG_LOCATION))
    clog << ">>> Dynamic_Image::render_image" << endl
        << "    " << *this << endl;
#endif
...
#endif
#if ((DEBUG_SECTION) & DEBUG_RENDER)
        clog << "<<< Dynamic_Image::render_image: false" << endl;
#endif
        return false;
    }
    ...
    */
    ...
    while (++band < 3)
    {
        ...
        try
        {
            transform = invert_keeping_origin
    ...
    clog << "    display_samples = " << display_samples << endl
        ...
        << "       display_data @ " << reinterpret_cast<void*>(display_data) << endl
    ...
    while (++display_line < display_lines)
    {
        ...
        while (++display_sample < display_samples)
        {
            ...
            if (!display_line)
                ...
                << reinterpret_cast<void*>(display_data)
                << " = " << setw(8) << pixel_value
                << dec << setfill(' ') << endl;
#endif
            ...
            ++rendered_lines == rendering_increment)
        {
            ...
            if (display_line == display_lines)
                status = Rendering_Monitor::DONE;
                        ...
                        DEBUG_NOTIFY | \
                        DEBUG_LOCATION))
            clog << "    Dynamic_Image::render_image: notify_rendering_monitors"
                " status " << status << " \""
    ...
    */
    ...
    is_rendering(false);
#if ((DEBUG_SECTION) & (DEBUG_MANIPULATORS | DEBUG_RENDER | DEBUG_LOCATION))
    clog << "<<< Dynamic_Image::render_image: " << completed << endl;
#endif
    return completed;
}
...
bool
Dynamic_Image::cancel_update
(
    bool cancel
)
{
#if ((DEBUG_SECTION) & (DEBUG_NOTIFY | DEBUG_RENDER))
    clog << ">-< Dynamic_Image::cancel_update: " << cancel << endl;
#endif
    bool
...
bool
Dynamic_Image::update_canceled() const
{
...
bool
Dynamic_Image::add_rendering_monitor
(
...
bool
Dynamic_Image::remove_rendering_monitor
(
    Rendering_Monitor* monitor
)
{
...
bool
Dynamic_Image::notify_rendering_monitors
(
...
)
{
    ...
    clog << ">>> Dynamic_Image::notify_rendering_monitors:" << endl
    ...
    ostringstream
        label;
    label << " incr. region @ " << reinterpret_cast<void*>(this) << ": " << region
        << "; st " << status;
...
#endif
#if ((DEBUG_SECTION) & (DEBUG_NOTIFY | DEBUG_RENDER | DEBUG_LOCATION))
    clog << "<<< Dynamic_Image::notify_rendering_monitors: " << (!cancel) << endl;
#endif
    return !cancel;
}
...
void
Dynamic_Image::rendering_increment_lines
(
...
unsigned int
Dynamic_Image::rendering_increment_lines() const
{
...
*/
Dynamic_Image::Rendering_Data::Rendering_Data
(
    const Dynamic_Image& image
)
{
#if ((DEBUG_SECTION) & (DEBUG_CONSTRUCTORS | DEBUG_RENDER | DEBUG_LOCATION))
    LOCKED_LOGGING((
        clog << ">>> Dynamic_Image::Rendering_Data @ " << reinterpret_cast<void*>(this) << endl
        << "    owner - " << image << endl));
#endif
    // Copy the rendering variables.
    Band_Map = new unsigned int[3];
    Geo_Transforms = new QTransform * [3];
    Data_Maps = new Dynamic_Image::Data_Map * [3];
#if ((DEBUG_SECTION) & (DEBUG_CONSTRUCTORS | DEBUG_RENDER | DEBUG_LOCATION))
    LOCKED_LOGGING((
        clog << "          Band_Map @ " << reinterpret_cast<void*>(image.Band_Map) << endl
        << "    Geo_Transforms @ " << reinterpret_cast<void*>(image.Geo_Transforms) << endl
        << "         Data_Maps @ " << reinterpret_cast<void*>(image.Data_Maps) << endl));
#endif
    for (int
         band = 0;
         band < 3;
         band++)
    {
        ...
        */
        Data_Maps[band] =
            new Dynamic_Image::Data_Map(*image.Data_Maps[band]);
    }
    ...
    LOCKED_LOGGING((
        ...
        << "<<< Dynamic_Image::Rendering_Data" << endl));
#endif
}
...
QPointF
Dynamic_Image::Rendering_Data::origin
(
    int band
) const
{
...
QSizeF
Dynamic_Image::Rendering_Data::scaling
(
    int band
) const
{
...
#if ((DEBUG_SECTION) & (DEBUG_RENDER | DEBUG_LOCATION))
    LOCKED_LOGGING((
        clog << ">>> Dynamic_Image::is_rendering: " << rendering << endl));
#endif
...
#if ((DEBUG_SECTION) & (DEBUG_RENDER | DEBUG_LOCATION))
            LOCKED_LOGGING((
                clog << "    new Rendering_Data @ " << reinterpret_cast<void*>(Rendering) << endl
                << "    for " << *this << endl));
#endif
...
#if ((DEBUG_SECTION) & (DEBUG_RENDER | DEBUG_LOCATION))
            LOCKED_LOGGING((
                clog << "    delete Rendering_Data @ " << reinterpret_cast<void*>(Rendering) << endl
                << "    for " << *this << endl));
#endif
            delete Rendering;
            Rendering = NULL;
...
#if ((DEBUG_SECTION) & (DEBUG_RENDER | DEBUG_LOCATION))
    LOCKED_LOGGING((
        clog << "<<< Dynamic_Image::is_rendering: " << was_rendering << endl));
#endif
...
bool
Dynamic_Image::is_rendering() const
{
...
QRgb*
Dynamic_Image::image_data() const
{
...
(
    std::ostream& stream,
    const Dynamic_Image::Triplet& triplet
    )
{
    return stream
        << triplet.Datum[0] << "r, "
...
(
    std::ostream& stream,
    const Dynamic_Image::Histogram* const histogram
    )
{
