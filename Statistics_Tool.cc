...
/*==============================================================================
    Constants
*/
const char *const Statistics_Tool::ID = "UA::HiRISE::Statistics_Tool ($Revision: 1.36 $ $Date: 2012/03/09 02:13:58 $)";
...
#endif
const QSize Statistics_Tool::GRAPH_MIN_SIZE(STATISTICS_TOOL_GRAPH_MIN_WIDTH, STATISTICS_TOOL_GRAPH_MIN_HEIGHT);

const double Statistics_Tool::DEFAULT_OPACITY[3] = {1.00, 0.75, 0.50};
...
/*==============================================================================
    Constructors
*/
Statistics_Tool::Statistics_Tool(const QString &title, QWidget *parent, Qt::WindowFlags flags)
    : QWidget(parent, flags), Limits_Enabled(STATISTICS_TOOL_LIMITS_ENABLED_DEFAULT), Visible_Graph(true)
{
...
//	Dynamic size control.
    ...
    Minimum_Size.rheight() = qMax(Minimum_Size.rheight(), panel->sizeHint().height());
...
/*==============================================================================
    Accessors
*/
void Statistics_Tool::data_structure(int bands, int precision)
{
#if ((DEBUG_SECTION) & (DEBUG_DATA | DEBUG_LIMITS))
    clog << ">>> Statistics_Tool::data_structure " << object_pathname(this) << ": " << endl
         << "    bands = " << bands << endl
    ...
    if (precision > MAX_SOURCE_DATA_PRECISION)
    {
        ...
        message << ID << endl
                ...
                << "The maximum supported is " << MAX_SOURCE_DATA_PRECISION << "-bit data precision.";
#if ((DEBUG_SECTION) & (DEBUG_DATA | DEBUG_LIMITS))
        clog << "!!! " << message.str() << endl << "<<< Statistics_Tool::data_structure" << endl;
#endif
    ...
    int band_count = statistics_sets();
#if ((DEBUG_SECTION) & (DEBUG_DATA | DEBUG_LIMITS))
    ...
    if (precision)
    {
        ...
        while (bands--)
        {
            ...
            {
#if ((DEBUG_SECTION) & (DEBUG_DATA | DEBUG_LIMITS))
                clog << "    Plot[" << bands << "] data size from " << Plot[bands]->data().size() << " to " << precision
                     << endl;
#endif
        ...
        int upper_offset = Statistics.upper_limit();
#if ((DEBUG_SECTION) & (DEBUG_DATA | DEBUG_LIMITS))
...
#if ((DEBUG_SECTION) & (DEBUG_DATA | DEBUG_LIMITS))
        clog << "    lower_limit " << qMin(Statistics.lower_limit(), max_x()) << endl;
#endif
        lower_limit(qMin(Statistics.lower_limit(), max_x()));
#if ((DEBUG_SECTION) & (DEBUG_DATA | DEBUG_LIMITS))
        clog << "          max_x = " << max_x() << endl
             << "    lower_limit = " << lower_limit() << endl
             << "    upper_limit = " << upper_limit() << " (" << Statistics.upper_limit() << ')' << endl;
#endif
    }
#if ((DEBUG_SECTION) & (DEBUG_DATA | DEBUG_LIMITS))
...
int Statistics_Tool::data_precision() const
{
    int bits = 0;
    if (Plot[0])
        for (unsigned int size = Plot[0]->data().size(); size; size >>= 1)
            ++bits;
    return bits;
}

int Statistics_Tool::max_x() const
{
    int max = 0;
    if (Plot[0])
...
int Statistics_Tool::max_y() const
{
    return MAX_DISPLAY_VALUE;
}

void Statistics_Tool::band_map(const unsigned int *bands)
{
    int band = 3;
    while (band--)
        Band_Names[band]->setText(QString::number(HiView_Utilities::band_index_to_number(Band_Map[band] = bands[band]))
                                      .prepend("<b>Band ")
                                      .append("</b>"));
    ...
    */
    ...
    else if (bands[2] == bands[1])
        Display_to_Stats_Map[2] = 1;
    else
        Display_to_Stats_Map[2] = 2;
}

void Statistics_Tool::refresh_band_numbers()
{
    band_map(Band_Map);
}

void Statistics_Tool::band_color(const QColor &color, int band)
{
#if ((DEBUG_SECTION) & DEBUG_ACCESSORS)
    clog << ">>> Statistics_Tool::band_color: " << color << ", " << band << endl;
#endif
    if (band >= 0 && band < 3 && Plot[band])
    {
#if ((DEBUG_SECTION) & DEBUG_ACCESSORS)
...
QColor Statistics_Tool::band_color(int band) const
{
    if (band >= 0 && band < 3 && Plot[band])
...
void Statistics_Tool::band_opacity(int percent, int band)
{
    if (band >= 0 && band < 3 && Plot[band])
    {
        if (percent < 0)
        ...
        QColor color(Plot[band]->bar_color());
        color.setAlphaF(percent / 100.0);
        band_color(color, band);
    }
}

int Statistics_Tool::band_opacity(int band) const
{
    return Opacity[band]->value();
}

void Statistics_Tool::canvas_color(QRgb color)
{
    Graph->setCanvasBackground(QBrush(color));
    Graph->replot();
}

void Statistics_Tool::opacity_changed(int percent)
{
...
/*==============================================================================
    GUI elements
*/
QSize Statistics_Tool::minimumSizeHint() const
{
    return Minimum_Size;
}

QSize Statistics_Tool::sizeHint() const
{
    return Minimum_Size;
}

QWidget *Statistics_Tool::graph_panel()
{
#if ((DEBUG_SECTION) & DEBUG_CONSTRUCTORS)
    ...
    Count_Scale_Max->setToolTip(tr("Maximum Count axis scale value; zero for auto scaling"));
    ...
    connect(Count_Scale_Max, SIGNAL(valueChanged(int)), SLOT(vertical_axis_max(int)));

    //	Panel to contain the graph and its scale control widget.
    Graph_Panel = new QWidget;
    QVBoxLayout *layout = new QVBoxLayout(Graph_Panel);
...
bool Statistics_Tool::visible_graph(bool enabled)
{
...
void Statistics_Tool::vertical_axis_max(int max)
{
    if (!max)
    {
        int max_count, bands = statistics_sets();
        while (bands--)
            if (Plot[bands] && max < (max_count = Plot[bands]->data().calculate_max_count()))
                max = max_count;
    }
...
QWidget *Statistics_Tool::info_panel()
{
#if ((DEBUG_SECTION) & DEBUG_CONSTRUCTORS)
    ...
    QLabel *label;
    int band, row = -1, first_stats_row = 2, col, label_col = 0, x_col = 1, y_col = 2, area_col = 3, red_col = 1,
              green_col = 2, blue_col = 3, spacing_col = 4;
...
#endif
    layout->addWidget(new QLabel(tr("<b>X</b>")), row, x_col, Qt::AlignRight | Qt::AlignBottom);
    layout->addWidget(new QLabel(tr("<b>Y</b>")), row, y_col, Qt::AlignRight | Qt::AlignBottom);
    layout->addWidget(new QLabel(tr("<b>Area</b>")), row, area_col, Qt::AlignRight | Qt::AlignBottom);
...
#endif
    layout->addWidget(new Drawn_Line, row, x_col);
    layout->addWidget(new Drawn_Line, row, y_col);
    layout->addWidget(new Drawn_Line, row, area_col);
...
#endif
    layout->addWidget(label = new QLabel(tr("Origin")), row, label_col, Qt::AlignRight | Qt::AlignVCenter);
...
#endif
    layout->addWidget(label = new QLabel(tr("Size")), row, label_col, Qt::AlignRight | Qt::AlignVCenter);
...
#endif
    layout->addWidget(new Drawn_Line(Heading_Line_Weight), row, label_col, 1, -1);
...
#endif
    for (band = 0, col = red_col; band < 3; band++, col++)
    {
        ...
        Band_Names[band]->setPalette(QPalette(QColor(DISPLAY_BAND_COLORS[band]).lighter()));
        Band_Names[band]->setAutoFillBackground(true);
        layout->addWidget(Band_Names[band], row, col, Qt::AlignVCenter);
    }
...
#endif
    layout->addWidget(label = new QLabel(tr("Values:")), row++, label_col, Qt::AlignRight | Qt::AlignVCenter);
    ...
    horizontal_layout->addWidget(Below_Limit_Label, 0, Qt::AlignRight | Qt::AlignVCenter);
    ...
    connect(Lower_Limit, SIGNAL(valueChanged(int)), SLOT(lower_limit(int)));
    horizontal_layout->addWidget(Lower_Limit, 0, Qt::AlignRight | Qt::AlignVCenter);
    layout->addLayout(horizontal_layout, row++, label_col, Qt::AlignRight | Qt::AlignVCenter);

    horizontal_layout = new QHBoxLayout;
    Above_Limit_Label = new QLabel(tr("Above"));
    Above_Limit_Label->setToolTip(tr("Area above the maximum valid value"));
    horizontal_layout->addWidget(Above_Limit_Label, 0, Qt::AlignRight | Qt::AlignVCenter);
    ...
    connect(Upper_Limit, SIGNAL(valueChanged(int)), SLOT(upper_limit(int)));
    horizontal_layout->addWidget(Upper_Limit, 0, Qt::AlignRight | Qt::AlignVCenter);
    layout->addLayout(horizontal_layout, row++, label_col, Qt::AlignRight | Qt::AlignVCenter);

    layout->addWidget(label = new QLabel(tr("Lowest:")), row++, label_col, Qt::AlignRight | Qt::AlignVCenter);
    label->setToolTip(tr("Lowest image value counted"));
    layout->addWidget(label = new QLabel(tr("Highest:")), row++, label_col, Qt::AlignRight | Qt::AlignVCenter);
    label->setToolTip(tr("Highest image value counted"));
    layout->addWidget(label = new QLabel(tr("Minimum:")), row++, label_col, Qt::AlignRight | Qt::AlignVCenter);
    label->setToolTip(tr("Minimum count"));
    layout->addWidget(label = new QLabel(tr("Maximum:")), row++, label_col, Qt::AlignRight | Qt::AlignVCenter);
    label->setToolTip(tr("Maximum count"));
    layout->addWidget(label = new QLabel(tr("Mean:")), row++, label_col, Qt::AlignRight | Qt::AlignVCenter);
    label->setToolTip(tr("Mean image value"));
    layout->addWidget(label = new QLabel(tr("Median:")), row++, label_col, Qt::AlignRight | Qt::AlignVCenter);
    label->setToolTip(tr("Median image value"));
    layout->addWidget(label = new QLabel(tr("Std. Dev.:")), row++, label_col, Qt::AlignRight | Qt::AlignVCenter);
    label->setToolTip(tr("Standard deviation of image values"));

    for (band = 0, col = red_col; band < 3; band++, col++)
    {
        row = first_stats_row;
        layout->addWidget(Values[band] = new QLabel, row++, col, Qt::AlignRight | Qt::AlignVCenter);
        layout->addWidget(Below_Limit[band] = new QLabel, row++, col, Qt::AlignRight | Qt::AlignVCenter);
        layout->addWidget(Above_Limit[band] = new QLabel, row++, col, Qt::AlignRight | Qt::AlignVCenter);
        layout->addWidget(Lowest_Value[band] = new QLabel, row++, col, Qt::AlignRight | Qt::AlignVCenter);
        layout->addWidget(Highest_Value[band] = new QLabel, row++, col, Qt::AlignRight | Qt::AlignVCenter);
        layout->addWidget(Minimum_Count[band] = new QLabel, row++, col, Qt::AlignRight | Qt::AlignVCenter);
        layout->addWidget(Maximum_Count[band] = new QLabel, row++, col, Qt::AlignRight | Qt::AlignVCenter);
        layout->addWidget(Mean_Value[band] = new QLabel, row++, col, Qt::AlignRight | Qt::AlignVCenter);
        layout->addWidget(Median_Value[band] = new QLabel, row++, col, Qt::AlignRight | Qt::AlignVCenter);
        layout->addWidget(Std_Dev_of_Values[band] = new QLabel, row++, col, Qt::AlignRight | Qt::AlignVCenter);
    }
...
#endif
    layout->addWidget(label = new QLabel(tr("Opacity:")), row, label_col, Qt::AlignRight | Qt::AlignVCenter);
    label->setToolTip(tr("Histogram plot opacity"));
    for (int band = 0, col = red_col; band < 3; band++, col++)
    {
        Opacity[band] = new QSpinBox;
        Opacity[band]->setRange(0, 100);
        Opacity[band]->setSingleStep(1);
        Opacity[band]->setValue(static_cast<int>(DEFAULT_OPACITY[band] * 100.0));
        ...
        connect(Opacity[band], SIGNAL(valueChanged(int)), SLOT(opacity_changed(int)));
        layout->addWidget(Opacity[band], row, col, Qt::AlignRight | Qt::AlignVCenter);
    }
...
void Statistics_Tool::limits(bool enabled)
{
...
int Statistics_Tool::upper_limit() const
{
    return Upper_Limit->value();
}

void Statistics_Tool::upper_limit(int limit)
{
...
int Statistics_Tool::lower_limit() const
{
    return Lower_Limit->value();
}

void Statistics_Tool::lower_limit(int limit)
{
#if ((DEBUG_SECTION) & (DEBUG_LIMITS | DEBUG_SLOTS))
...
void Statistics_Tool::view_band_data(int band, bool enabled)
{
#if ((DEBUG_SECTION) & DEBUG_HELPERS)
    clog << ">>> Statistics_Tool::view_band_data " << object_pathname(this) << ": " << band << ' ' << boolalpha
         << enabled << endl;
#endif
    if (band >= 0 && band < 3)
    {
...
#if ((DEBUG_SECTION) & DEBUG_HELPERS)
            clog << "    attach Plot @ " << (void *)Plot[band] << " to Graph" << endl;
#endif
            Plot[band]->attach(Graph);
//	Bind the histogram vector to the statistics set.
#if ((DEBUG_SECTION) & DEBUG_HELPERS)
            clog << "    bind plot histogram @ " << (void *)(Plot[band]->data_vector()) << " to Statistics" << endl;
#endif
...
#if ((DEBUG_SECTION) & DEBUG_HELPERS)
            clog << "    unbind plot histogram @ " << (void *)(Statistics.histogram(band)) << " from Statistics"
                 << endl;
#endif
            Statistics.histogram(NULL, band);
#if ((DEBUG_SECTION) & DEBUG_HELPERS)
            clog << "    detach Plot @ " << (void *)Plot[band] << " from Graph and delete" << endl;
#endif
...
Histogram_Plot *Statistics_Tool::new_plot(int band)
{
    Histogram_Plot *plot = new Histogram_Plot;
#if ((DEBUG_SECTION) & DEBUG_HELPERS)
    clog << ">-< Statistics_Tool::new_plot " << object_pathname(this) << " @ " << (void *)plot << endl;
#endif
    plot->attribute_set(Histogram_Plot::VERTICAL_BARS);
    QColor color(DISPLAY_BAND_COLORS[band]);
...
bool Statistics_Tool::refresh(const QRect &selected_region)
{
#if ((DEBUG_SECTION) & (DEBUG_REFRESH | DEBUG_SLOTS))
    clog << ">>> Statistics_Tool::refresh: " << object_pathname(this) << ": " << selected_region << endl;
#endif
    //	Check for invalid histogram data.
    if (!Statistics.histogram(0) || Statistics.histogram(0)->at(0) == static_cast<Histogram::value_type>(-1))
    {
#if ((DEBUG_SECTION) & (DEBUG_REFRESH | DEBUG_SLOTS))
        clog << "    no histogram" << (Statistics.histogram(0) ? " data" : "") << endl
    ...
    if (selected_region.isNull())
        ...
        area = (unsigned long long)selected_region.width() * selected_region.height();
#if ((DEBUG_SECTION) & (DEBUG_REFRESH | DEBUG_SLOTS))
    clog << "    sampled area = " << value << endl << "     actual area = " << area << endl;
#endif
    if (value == 0 && area != 0)
    {
#if ((DEBUG_SECTION) & (DEBUG_REFRESH | DEBUG_SLOTS))
        clog << "    no area measured" << endl << "<<< Statistics_Tool::refresh: false" << endl;
#endif
        return false;
    }
    if (value != area)
    ...
    for (int band = 0; band < bands; band++)
    {
        if (Plot[band])
        {
            ...
            Values[band]->setNum(Statistics.values_counted(band));
            Below_Limit[band]->setText(QString::number(Statistics.area(band, Stats::LOWER_RANGE)));
            Lowest_Value[band]->setNum(Statistics.lowest_value(band));
            Highest_Value[band]->setNum(Statistics.highest_value(band));
            Above_Limit[band]->setText(QString::number(Statistics.area(band, Stats::UPPER_RANGE)));
            Minimum_Count[band]->setText(QString::number(Statistics.minimum_count(band)));
            Maximum_Count[band]->setText(QString::number(Statistics.maximum_count(band)));
            Mean_Value[band]->setText(QString::number(Statistics.mean_value(band), 'f', 1));
            Median_Value[band]->setText(QString::number(Statistics.median_value(band), 'f', 1));
            Std_Dev_of_Values[band]->setText(QString::number(Statistics.standard_deviation_of_values(band), 'f', 1));
#if ((DEBUG_SECTION) & (DEBUG_REFRESH | DEBUG_SLOTS))
            clog << "      values_counted = " << Statistics.values_counted(band) << endl
                 << "      below " << Lower_Limit->value() << " = " << Statistics.area(band, Stats::LOWER_RANGE) << endl
                 << "        lowest_value = " << Statistics.lowest_value(band) << endl
                 << "       highest_value = " << Statistics.highest_value(band) << endl
                 << "      above " << Upper_Limit->value() << " = " << Statistics.area(band, Stats::UPPER_RANGE) << endl
                 << "       minimum_count = " << Statistics.minimum_count(band) << endl
                 << "       maximum_count = " << Statistics.maximum_count(band) << endl
                 << "          mean_value = " << Statistics.mean_value(band) << endl
                 << "        median_value = " << Statistics.median_value(band) << endl
                 << "       std deviation = " << Statistics.standard_deviation_of_values(band) << endl
                 << "                area = " << Statistics.area(band) << endl;
#endif
        }
    }

#if ((DEBUG_SECTION) & (DEBUG_REFRESH | DEBUG_SLOTS))
    clog << "    Y axis max_value = " << max_value << endl << "    replot and update" << endl;
#endif
