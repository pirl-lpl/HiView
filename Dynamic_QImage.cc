...
HiROC CVS ID: $Id: Dynamic_QImage.cc,v 1.30 2012/06/15 01:16:07 castalia Exp $
...
const char* const
Dynamic_QImage::ID =
"UA::HiRISE::Dynamic_QImage ($Revision: 1.30 $ $Date: 2012/06/15 01:16:07 $)";

/*==============================================================================
 Constructors
*/
Dynamic_QImage::Dynamic_QImage
(
QImage* image,
const QSize& size,
...
Dynamic_QImage::Dynamic_QImage
(
const Dynamic_QImage& image,
const QSize& size,
Mapping_Type shared_mappings
)
    :
    Dynamic_Image(size.isValid() ? size : image.size(),
...
{
#if ((DEBUG_SECTION) & DEBUG_CONSTRUCTORS)
    clog << ">>> ~Dynamic_QImage @ " << (void*)this << endl;
#endif
    if (Source)
    {
#if ((DEBUG_SECTION) & DEBUG_CONSTRUCTORS)
...
*/
Dynamic_QImage*
Dynamic_QImage::clone
(
const QSize& size,
Mapping_Type shared_mappings
) const
{
    return new Dynamic_QImage(*this, size, shared_mappings);
}


const void*
Dynamic_QImage::source() const
{
    return Source;
}
...
QSize
Dynamic_QImage::source_size() const
{
...
unsigned int
Dynamic_QImage::source_bands() const
{
    unsigned int
        bands = 0;
    if (Source)
...
unsigned int
Dynamic_QImage::source_precision_bits() const
{
    unsigned int
        precision = 0;
    if (Source)
...
Dynamic_QImage::source_pixel_value
(
unsigned int x,
unsigned int y,
unsigned int band
...
const
{
    ...
    clog << ">-< Dynamic_QImage::source_pixel_value: "
        << x << "x, " << y << "y, " << band << "b = ";
#endif
    Pixel_Datum
        value;
...
Dynamic_QImage::source_pixel
(
const QPoint& point
) const
{
#if ((DEBUG_SECTION) & DEBUG_PIXEL_DATA)
    clog << ">-< Dynamic_QImage::source_pixel: " << point << " = ";
#endif
    Triplet
        triplet;
    if (Source &&
