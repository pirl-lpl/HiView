...
#define Apply_Button_Icon HiView_Config::Apply_Button_Icon
#define Defaults_Button_Icon HiView_Config::Defaults_Button_Icon
#define Reset_Button_Icon HiView_Config::Reset_Button_Icon
...
//	Help Documentation.
...
#endif
    for (int index = 0; Default_Documentation_Search_Locations[index]; ++index)
    {
        Documentation_Search_Locations.append(Default_Documentation_Search_Locations[index]);
#if ((DEBUG_SECTION) & (DEBUG_CONSTRUCTORS | DEBUG_GENERAL))
        clog << "      " << index << ": " << Documentation_Search_Locations.last() << endl;
#endif
    }
    ...
    {
        Documentation_Location = settings.value(DOCUMENTATION_LOCATION_KEY, QString()).toString();
#if ((DEBUG_SECTION) & (DEBUG_CONSTRUCTORS | DEBUG_GENERAL))
        clog << "      Documentation_Location setting = " << Documentation_Location << endl;
#endif
    }
    else
    {
...
#if ((DEBUG_SECTION) & (DEBUG_CONSTRUCTORS | DEBUG_GENERAL))
        clog << "      Documentation_Location = " << Documentation_Location << endl;
#endif
...
//	Layout Restoration.
    ...
    connect(Restore_Layout_CheckBox, SIGNAL(toggled(bool)), SLOT(restore_layout(bool)));
    vertical_layout->addWidget(Restore_Layout_CheckBox, 0, Qt::AlignLeft | Qt::AlignVCenter);
    ...
    connect(Restore_Last_Source_CheckBox, SIGNAL(toggled(bool)), SLOT(restore_last_source(bool)));
    vertical_layout->addWidget(Restore_Last_Source_CheckBox, 0, Qt::AlignLeft | Qt::AlignVCenter);

    ++row;
    grid_layout->addLayout(vertical_layout, row, 0, 1, -1, Qt::AlignLeft | Qt::AlignVCenter);
...
//	Band Numbering.
    ...
    connect(Band_Numbers_Indexed_Button, SIGNAL(toggled(bool)), SLOT(band_numbers_indexed(bool)));
...
//	Help Documentation.
    ...
    //		Heading Label.
    ++row;
    grid_layout->addWidget(new QLabel(tr("<b>Help Documentation</b>")), row, 0, 1, -1,
    ...
    connect(push_button, SIGNAL(clicked()), SLOT(find_documentation_location()));
    horizontal_layout->addWidget(push_button, 0, Qt::AlignVCenter);
    ++row;
    grid_layout->addLayout(horizontal_layout, row, 0, Qt::AlignLeft | Qt::AlignVCenter);
    ...
    ++row;
    grid_layout->addWidget(new QLabel(tr("<b>Coordinate Display Format</b>")), row, 0, 1, -1,
                           Qt::AlignLeft | Qt::AlignVCenter);
    ...
    connect(Latitude_Units_ComboBox, SIGNAL(currentIndexChanged(int)), SLOT(latitude_format(int)));
...
void General_Section::restore_layout(bool enabled)
{
...
void General_Section::documentation_location(const QString &location)
{
    if (Documentation_Location_lineEdit)
    {
        ...
        Documentation_Location_Reset_Button->setVisible(location != Documentation_Location);
        reset_modifier_buttons();
    }
}
...
void General_Section::documentation_location_changed()
{
...
bool General_Section::find_documentation_location()
{
...
void General_Section::reset_modifier_buttons()
{
    reset_defaults_button();
    Apply_Button->setEnabled(has_changed());
}

bool General_Section::has_changed() const
{
#if ((DEBUG_SECTION) & DEBUG_GENERAL)
    bool changed =
#else
    return
#endif
        (Restore_Layout_CheckBox->isChecked() != Restore_Layout) ||
        (Restore_Last_Source_CheckBox->isChecked() != Restore_Last_Source) ||
        (Band_Numbers_Indexed_Button->isChecked() != Band_Numbers_Indexed) ||
        (Get_PDS_Metadata_CheckBox->isChecked() != Get_PDS_Metadata) ||
        (Documentation_Location_lineEdit->text() != Documentation_Location) ||
        (Longitude_Units_ComboBox->currentIndex() != Longitude_Units) ||
        (Longitude_Direction_ComboBox->currentIndex() != Longitude_Direction) ||
    ...
    clog << ">-< General_Section::has_changed:" << endl
         ...
         << "                     Get_PDS_Metadata = " << Get_PDS_Metadata << endl
         << "               Documentation_Location = " << Documentation_Location << endl
         << "                              changed = " << changed << endl;
    return changed;
#endif
}

void General_Section::reset()
{
...
/*=*****************************************************************************
    Sources_Section
*/
const char *Sources_Section::SOURCE_LIST_CAPACITY_KEY = "Source_List_Capacity",
           *Sources_Section::SOURCE_LIST_KEY = "Source_List";

#ifndef DEFAULT_SOURCE_LIST_CAPACITY
#define DEFAULT_SOURCE_LIST_CAPACITY 32
#endif
int Sources_Section::Default_Source_List_Capacity = DEFAULT_SOURCE_LIST_CAPACITY;
...
//	Layout controls.
...
#endif
    connect(Capacity_spinBox, SIGNAL(valueChanged(int)), SLOT(reset_modifier_buttons()));
    connect(Capacity_spinBox, SIGNAL(editingFinished()), SLOT(source_list_capacity_change()));
    ...
    connect(Capacity_Reset_Button, SIGNAL(clicked()), SLOT(source_list_capacity_reset()));
    horizontal_layout->addWidget(Capacity_Reset_Button);

    //		Padding
    horizontal_layout->addStretch(100);
    grid_layout->addLayout(horizontal_layout, row, 0);
    ...
    Source_List_Widget->setHorizontalScrollMode(QAbstractItemView::ScrollPerPixel);
    Source_List_Widget->setVerticalScrollMode(QAbstractItemView::ScrollPerItem);
    Source_List_Widget->setTextElideMode(Qt::ElideNone);
    Source_List_Widget->setDragEnabled(true);
    Source_List_Widget->viewport()->setAcceptDrops(true);
    Source_List_Widget->setDragDropMode(QAbstractItemView::InternalMove);
    ...
    connect(Source_List_Reset_Button, SIGNAL(clicked()), SLOT(source_list_reset()));
    Source_List_Widget->setVerticalScrollBarPolicy(Qt::ScrollBarAlwaysOn);
    Source_List_Widget->setHorizontalScrollBarPolicy(Qt::ScrollBarAlwaysOn);
    Source_List_Widget->setCornerWidget(Source_List_Reset_Button);
    Source_List_Reset_Button->setVisible(false);
    grid_layout->addWidget(Source_List_Widget, row, 0);
    grid_layout->setRowStretch(row, 100);
    ...
    connect(Remove_Button, SIGNAL(clicked()), SLOT(source_list_remove_items()));
    vertical_layout->addWidget(Remove_Button);

    //		Spacing
    vertical_layout->addStretch(100);
    grid_layout->addLayout(vertical_layout, row, 1);
...
void Sources_Section::source_list(const QStringList &list)
{
    ...
    if (*Source_List != list)
    {
        ...
        *Source_List = list;
        ...
        int here, there;
        for (here = 0; here < (Source_List->count() - 1); here++)
        {
...
void Sources_Section::source_list_selection_changed()
{
...
void Sources_Section::source_list_remove_items()
{
#if ((DEBUG_SECTION) & DEBUG_SOURCES)
...
#if ((DEBUG_SECTION) & DEBUG_SOURCES)
            clog << "     viewport rect = " << Source_List_Widget->viewport()->rect() << endl
                 << "    searching for top item in viewport -" << endl;
#endif
            int top_row;
            for (top_row = 0; top_row < entries; top_row++)
            {
...
#endif
            delete Source_List_Widget->takeItem(Source_List_Widget->row(selected_items.at(count)));
        }
        Entries->setNum(Source_List_Widget->count());

        if (top_item)
            //	Scroll the top item to the top of the viewport.
            Source_List_Widget->scrollToItem(top_item, QAbstractItemView::PositionAtTop);
...
void Sources_Section::reset_modifier_buttons()
{
    ...
    Defaults_Button->setEnabled(Default_Source_List_Capacity != Capacity_spinBox->value());
}

void Sources_Section::apply()
{
    QSettings settings;
    if (Capacity != Capacity_spinBox->value())
    {
    ...
    {
        Source_List->clear();
        for (int index = 0; index < Source_List_Widget->count(); index++)
...
#endif
double Rendering_Section::Default_Min_Scale = DEFAULT_MIN_SCALE;
#ifndef MIN_SCALING
...
#endif
double Rendering_Section::Default_Max_Scale = DEFAULT_MAX_SCALE;
#ifndef MAX_SCALING
...
#endif
double Rendering_Section::Default_Scaling_Major_Increment = DEFAULT_SCALING_MAJOR_INCREMENT;
...
#endif
int Rendering_Section::Default_Tile_Size = DEFAULT_TILE_SIZE;
...
//	Initialize values.
    ...
    */
    ...
    double min_scaling = Min_Scale = settings.value(MIN_SCALE_KEY, Default_Min_Scale).toDouble(&OK);
#if ((DEBUG_SECTION) & (DEBUG_CONSTRUCTORS | DEBUG_RENDERING))
...
#if ((DEBUG_SECTION) & (DEBUG_CONSTRUCTORS | DEBUG_RENDERING))
    clog << "    " << SCALING_MINOR_INCREMENT_KEY << " = " << Scaling_Minor_Increment << endl;
#endif
    if (!OK || Scaling_Minor_Increment <= 0)
    {
        ...
        settings.setValue(SCALING_MINOR_INCREMENT_KEY, Default_Scaling_Minor_Increment);

    double major_increment = Scaling_Major_Increment =
        settings.value(SCALING_MAJOR_INCREMENT_KEY, Default_Scaling_Major_Increment).toDouble(&OK);
#if ((DEBUG_SECTION) & (DEBUG_CONSTRUCTORS | DEBUG_RENDERING))
    clog << "    " << SCALING_MAJOR_INCREMENT_KEY << " = " << Scaling_Major_Increment << endl;
#endif
    if (!OK || Scaling_Major_Increment <= 0)
    {
        ...
        settings.setValue(SCALING_MAJOR_INCREMENT_KEY, Default_Scaling_Major_Increment);
...
*/
    ...
    int tile_extent = Tile_Size = settings.value(TILE_SIZE_KEY, Default_Tile_Size).toInt(&OK);
#if ((DEBUG_SECTION) & (DEBUG_CONSTRUCTORS | DEBUG_RENDERING))
...
#if ((DEBUG_SECTION) & (DEBUG_CONSTRUCTORS | DEBUG_RENDERING))
    clog << "    " << RENDERING_INCREMENT_LINES_KEY << " = " << Rendering_Increment_Lines << endl;
#endif
    if (!OK)
    {
        QMessageBox::warning((isVisible() ? this : NULL), Title,
        ...
        settings.setValue(RENDERING_INCREMENT_LINES_KEY, Default_Rendering_Increment_Lines);
...
*/
    ...
    line = new Drawn_Line(Heading_Line_Weight);
    line->alignment(Qt::AlignBottom);
    grid_layout->addWidget(line, row, col, 1, -1, Qt::AlignBottom);
#if ((DEBUG_SECTION) & (DEBUG_CONSTRUCTORS | DEBUG_RENDERING))
...
#endif
    connect(Initial_Scale_doubleSpinBox, SIGNAL(valueChanged(double)), SLOT(changing()));
    connect(Initial_Scale_doubleSpinBox, SIGNAL(editingFinished()), SLOT(initial_scale_change()));
    ...
    connect(Initial_Scale_Reset_Button, SIGNAL(clicked()), SLOT(initial_scale_reset()));
    grid_layout->addWidget(Initial_Scale_Reset_Button, row, col, Qt::AlignLeft);
#if ((DEBUG_SECTION) & (DEBUG_CONSTRUCTORS | DEBUG_RENDERING))
...
#if ((DEBUG_SECTION) & (DEBUG_CONSTRUCTORS | DEBUG_RENDERING))
    clog << "    reset buttons icon size = " << Initial_Scale_Reset_Button->iconSize() << endl;
#endif
...
#endif
    connect(Min_Scale_doubleSpinBox, SIGNAL(valueChanged(double)), SLOT(changing()));
    connect(Min_Scale_doubleSpinBox, SIGNAL(editingFinished()), SLOT(min_scale_change()));
    ...
    connect(Min_Scale_Reset_Button, SIGNAL(clicked()), SLOT(min_scale_reset()));
    grid_layout->addWidget(Min_Scale_Reset_Button, row, col, Qt::AlignLeft);
#if ((DEBUG_SECTION) & (DEBUG_CONSTRUCTORS | DEBUG_RENDERING))
    ...
    Scaling_Minor_Increment_doubleSpinBox->setToolTip(tr("Scaling factor small increment"));
#ifndef QT_NO_SHORTCUT
...
#endif
    connect(Max_Scale_doubleSpinBox, SIGNAL(valueChanged(double)), SLOT(changing()));
    connect(Max_Scale_doubleSpinBox, SIGNAL(editingFinished()), SLOT(max_scale_change()));
    ...
    connect(Max_Scale_Reset_Button, SIGNAL(clicked()), SLOT(max_scale_reset()));
    ...
    Scaling_Major_Increment_doubleSpinBox->setToolTip(tr("Scaling factor large increment"));
#ifndef QT_NO_SHORTCUT
    ...
    //		Heading label.
    ++row;
    grid_layout->addWidget(new QLabel(tr("<b>Default Contrast Stretch</b>")), row, col, 1, -1,
                           Qt::AlignLeft | Qt::AlignVCenter);
#if ((DEBUG_SECTION) & (DEBUG_CONSTRUCTORS | DEBUG_RENDERING))
    ...
    for (int band = 0; band < 3; ++band)
    {
        ...
        Contrast_Stretch_Upper_doubleSpinBox[band]->setDecimals(PERCENT_DECIMAL_PLACES);
    ...
    for (int band = 0; band < 3; ++band)
    {
        ...
        Contrast_Stretch_Lower_doubleSpinBox[band]->setDecimals(PERCENT_DECIMAL_PLACES);
    ...
    ++row;
    col = 0;
    grid_layout->addWidget(new Drawn_Line(Heading_Line_Weight), row, col, 1, -1);
#if ((DEBUG_SECTION) & (DEBUG_CONSTRUCTORS | DEBUG_RENDERING))
    clog << "    " << row << ',' << col << ": heading line" << endl;
#endif
    ...
    //			Label.
    col = 0;
    grid_layout->addWidget(label = new QLabel(tr("Background &Color:"), this), row, col,
    ...
    Background_Color_lineEdit->setToolTip(tr("Background color to use in the image display"));
#ifndef QT_NO_SHORTCUT
    ...
    connect(Background_Color_Reset_Button, SIGNAL(clicked()), SLOT(background_color_reset()));
    ...
    Tile_Size_spinBox->setToolTip(tr("Maximum size, in pixels, of an image display tile"));
#ifndef QT_NO_SHORTCUT
    label->setBuddy(Tile_Size_spinBox);
#endif
    connect(Tile_Size_spinBox, SIGNAL(valueChanged(int)), SLOT(changing()));
    connect(Tile_Size_spinBox, SIGNAL(editingFinished()), SLOT(tile_size_change()));
    ...
    connect(Tile_Size_Reset_Button, SIGNAL(clicked()), SLOT(tile_size_reset()));
    grid_layout->addWidget(Tile_Size_Reset_Button, row, col, Qt::AlignLeft);
#if ((DEBUG_SECTION) & (DEBUG_CONSTRUCTORS | DEBUG_RENDERING))
    ...
    //			Label.
    col = 0;
    grid_layout->addWidget(label = new QLabel(tr("Incremental &Lines:"), this), row, col,
                           Qt::AlignRight | Qt::AlignVCenter);
#if ((DEBUG_SECTION) & (DEBUG_CONSTRUCTORS | DEBUG_RENDERING))
    ...
    connect(action, SIGNAL(triggered()), Apply_Button, SLOT(click()));
    addAction(action);
    grid_layout->addWidget(buttons, row, col, 1, -1);
#if ((DEBUG_SECTION) & (DEBUG_CONSTRUCTORS | DEBUG_RENDERING))
...
void Rendering_Section::min_scale_change()
{
#if ((DEBUG_SECTION) & DEBUG_RENDERING)
...
void Rendering_Section::max_scale_change()
{
#if ((DEBUG_SECTION) & DEBUG_RENDERING)
...
void Rendering_Section::scaling_minor_increment(double increment)
{
    ...
    Scaling_Minor_Increment_Reset_Button->setVisible(increment != Scaling_Minor_Increment);

    reset_modifier_buttons();
}

void Rendering_Section::scaling_minor_increment_change()
{
#if ((DEBUG_SECTION) & DEBUG_RENDERING)
...
void Rendering_Section::scaling_major_increment(double increment)
{
    ...
    Scaling_Major_Increment_Reset_Button->setVisible(increment != Scaling_Major_Increment);

    reset_modifier_buttons();
}

void Rendering_Section::scaling_major_increment_change()
{
#if ((DEBUG_SECTION) & DEBUG_RENDERING)
...
/*------------------------------------------------------------------------------
    Contrast Stretch
*/
double Rendering_Section::contrast_stretch_upper(int band) const
{
    if (band < 0 || band > 2)
    {
...
double Rendering_Section::contrast_stretch_lower(int band) const
{
    if (band < 0 || band > 2)
    {
        ostringstream message;
...
void Rendering_Section::contrast_stretch_upper(double percent, int band)
{
    ...
    Contrast_Stretch_Upper_Reset_Button[band]->setVisible(percent != Contrast_Stretch_Upper[band]);
    reset_modifier_buttons();
#if ((DEBUG_SECTION) & DEBUG_RENDERING)
...
void Rendering_Section::contrast_stretch_lower(double percent, int band)
{
    ...
    Contrast_Stretch_Lower_Reset_Button[band]->setVisible(percent != Contrast_Stretch_Lower[band]);
    reset_modifier_buttons();
#if ((DEBUG_SECTION) & DEBUG_RENDERING)
...
void Rendering_Section::contrast_stretch_change()
{
#if ((DEBUG_SECTION) & DEBUG_RENDERING)
...
/*------------------------------------------------------------------------------
    Miscellaneous
*/
void Rendering_Section::tile_size(int size)
{
#if ((DEBUG_SECTION) & DEBUG_RENDERING)
...
void Rendering_Section::tile_size_change()
{
#if ((DEBUG_SECTION) & DEBUG_RENDERING)
...
void Rendering_Section::rendering_increment_lines(int lines)
{
    ...
    Rendering_Increment_Lines_Reset_Button->setVisible(lines != Rendering_Increment_Lines);

    reset_modifier_buttons();
}

void Rendering_Section::rendering_increment_lines_change()
{
#if ((DEBUG_SECTION) & DEBUG_RENDERING)
...
#if ((DEBUG_SECTION) & DEBUG_RENDERING)
    clog << "<<< Rendering_Section::background_color_is_valid: " << accepted << endl;
#endif
    return accepted;
}
...
void Rendering_Section::background_color_changed()
{
    //	Avoid redundant editingFinished signals.
    static bool OK = true;
...
void Rendering_Section::defaults()
{
...
void Rendering_Section::reset_modifier_buttons()
{
#if ((DEBUG_SECTION) & DEBUG_RENDERING)
...
void Rendering_Section::reset_defaults_button()
{
    bool enabled = false;
    for (int band = 0; band < 3; ++band)
    {
...
void Rendering_Section::changing()
{
    ...
    changed |= change;
    Max_Scale_Reset_Button->setVisible(change = (Max_Scale_doubleSpinBox->value() != Max_Scale));
    changed |= change;
    Scaling_Minor_Increment_Reset_Button->setVisible(
        change = (Scaling_Minor_Increment_doubleSpinBox->value() != Scaling_Minor_Increment));
    changed |= change;
    Scaling_Major_Increment_Reset_Button->setVisible(
        change = (Scaling_Major_Increment_doubleSpinBox->value() != Scaling_Major_Increment));
    changed |= change;
    ...
    Rendering_Increment_Lines_Reset_Button->setVisible(
        change = (Rendering_Increment_Lines_spinBox->value() != Rendering_Increment_Lines));
    changed |= change;
    //	Text fields manage their Reset_Buttons.
    changed |= (Background_Color_lineEdit->text() != Background_Color_Text);
...
bool Rendering_Section::has_changed() const
{
#if ((DEBUG_SECTION) & DEBUG_RENDERING)
    clog << ">-< Rendering_Section::has_changed" << endl
         << "    Background_Color_lineEdit->text = " << Background_Color_lineEdit->text() << endl
         << "              Background_Color_Text = " << Background_Color_Text << endl;
#endif
    bool changed = false;
    for (int band = 0; band < 3; ++band)
    {
        if (Contrast_Stretch_Upper_doubleSpinBox[band]->value() != Contrast_Stretch_Upper[band] ||
            Contrast_Stretch_Lower_doubleSpinBox[band]->value() != Contrast_Stretch_Lower[band])
        {
            changed = true;
            break;
        }
...
void Rendering_Section::apply()
{
    ...
    if (Scaling_Minor_Increment != Scaling_Minor_Increment_doubleSpinBox->value())
    {
        Scaling_Minor_Increment = Scaling_Minor_Increment_doubleSpinBox->value();
        Scaling_Minor_Increment_Reset_Button->setVisible(false);
        settings.setValue(SCALING_MINOR_INCREMENT_KEY, Scaling_Minor_Increment);
        //	>>> SIGNAL <<<
    ...
    if (Scaling_Major_Increment != Scaling_Major_Increment_doubleSpinBox->value())
    {
        Scaling_Major_Increment = Scaling_Major_Increment_doubleSpinBox->value();
        Scaling_Major_Increment_Reset_Button->setVisible(false);
        settings.setValue(SCALING_MAJOR_INCREMENT_KEY, Scaling_Major_Increment);
        //	>>> SIGNAL <<<
    ...
    while (band--)
    {
        if (Contrast_Stretch_Upper[band] != Contrast_Stretch_Upper_doubleSpinBox[band]->value())
        {
            Contrast_Stretch_Upper[band] = Contrast_Stretch_Upper_doubleSpinBox[band]->value();
            Contrast_Stretch_Upper_Reset_Button[band]->setVisible(false);
            //	>>> SIGNAL <<<
            emit contrast_stretch_upper_changed(Contrast_Stretch_Upper[band], band);
        }
    ...
    while (band--)
    {
        if (Contrast_Stretch_Lower[band] != Contrast_Stretch_Lower_doubleSpinBox[band]->value())
        {
            Contrast_Stretch_Lower[band] = Contrast_Stretch_Lower_doubleSpinBox[band]->value();
            Contrast_Stretch_Lower_Reset_Button[band]->setVisible(false);
            //	>>> SIGNAL <<<
            emit contrast_stretch_lower_changed(Contrast_Stretch_Lower[band], band);
        }
        values.append(QVariant(Contrast_Stretch_Lower[band]));
    }
    settings.setValue(CONTRAST_STRETCH_LOWER_KEY, values);

    QString text = Background_Color_lineEdit->text();
    if (Background_Color_Text != text && background_color_is_valid(text))
    {
        Background_Color_Text = text;
        Background_Color = color_value(text);
        Background_Color_Reset_Button->setVisible(false);
#if ((DEBUG_SECTION) & DEBUG_RENDERING)
        clog << "    Background_Color_Text_Pending = " << Background_Color_Text_Pending << endl
             << "            Background_Color_Text = " << Background_Color_Text << endl
             << "                 Background_Color = " << color_text(Background_Color) << endl;
#endif
    ...
    if (Rendering_Increment_Lines != Rendering_Increment_Lines_spinBox->value())
    {
        Rendering_Increment_Lines = Rendering_Increment_Lines_spinBox->value();
        Rendering_Increment_Lines_Reset_Button->setVisible(false);
        settings.setValue(RENDERING_INCREMENT_LINES_KEY, Rendering_Increment_Lines);
        //	>>> SIGNAL <<<
...
#endif
int JPIP_Section::Default_JPIP_Server_Port = DEFAULT_JPIP_SERVER_PORT;
...
#endif
int JPIP_Section::Default_JPIP_Request_Timeout = DEFAULT_JPIP_REQUEST_TIMEOUT;
...
#endif
int JPIP_Section::Default_Max_Source_Image_Area_MB = DEFAULT_MAX_SOURCE_IMAGE_AREA_MB;
...
#if ((DEBUG_SECTION) & DEBUG_HELPERS)
    clog << "<<< possible_abbreviated_home_path: " << changed << " - " << name << endl;
#endif
    return changed;
}
} // namespace
...
//	Initialize values.
    ...
    int image_area = Max_Source_Image_Area_MB =
        settings.value(MAX_SOURCE_IMAGE_AREA_MB_KEY, Default_Max_Source_Image_Area_MB).toInt(&OK);
#if ((DEBUG_SECTION) & (DEBUG_CONSTRUCTORS | DEBUG_JPIP))
    clog << "    " << MAX_SOURCE_IMAGE_AREA_MB_KEY << " = " << Max_Source_Image_Area_MB << endl;
#endif
    if (!OK)
    {
        QMessageBox::warning((isVisible() ? this : NULL), Title,
...
//	Layout controls.
    ...
    Port_spinBox->setToolTip(tr("Server port to use when none specified"));
#ifndef QT_NO_SHORTCUT
    label->setBuddy(Port_spinBox);
#endif
    connect(Port_spinBox, SIGNAL(valueChanged(int)), SLOT(changing()));
    ...
    connect(Port_Reset_Button, SIGNAL(clicked()), SLOT(JPIP_server_port_reset()));
    ...
    Proxy_lineEdit->setToolTip(tr("Proxy URL for JPIP server access"));
#ifndef QT_NO_SHORTCUT
    ...
    connect(Proxy_Reset_Button, SIGNAL(clicked()), SLOT(JPIP_proxy_reset()));
    grid_layout->addWidget(Proxy_Reset_Button, row, col, Qt::AlignLeft);
    ...
    Cache_Directory_lineEdit->setToolTip(tr("JPIP codestream data bin cache directory"));
#ifndef QT_NO_SHORTCUT
    ...
    connect(Cache_Directory_Select_Button, SIGNAL(clicked()), SLOT(select_JPIP_cache_directory()));
    ...
    connect(Cache_Directory_Reset_Button, SIGNAL(clicked()), SLOT(JPIP_cache_directory_reset()));
    grid_layout->addWidget(Cache_Directory_Reset_Button, row, col, Qt::AlignLeft);
    ...
    Request_Timeout_spinBox->setToolTip(tr("Maximum time to wait for a server response"));
#ifndef QT_NO_SHORTCUT
    label->setBuddy(Request_Timeout_spinBox);
#endif
    connect(Request_Timeout_spinBox, SIGNAL(valueChanged(int)), SLOT(changing()));
    ...
    connect(Request_Timeout_Reset_Button, SIGNAL(clicked()), SLOT(JPIP_request_timeout_reset()));
    ...
    Max_Source_Image_Area_MB_spinBox->setToolTip(tr("Maximum time to wait for a server response"));
#ifndef QT_NO_SHORTCUT
    label->setBuddy(Max_Source_Image_Area_MB_spinBox);
#endif
    connect(Max_Source_Image_Area_MB_spinBox, SIGNAL(valueChanged(int)), SLOT(changing()));
...
void JPIP_Section::JPIP_proxy_changed()
{
    //	Avoid redundant editingFinished signals.
    static bool OK = true;
    if (OK && Proxy_lineEdit->text() != Proxy_Pending && !Defaults_Button->hasFocus())
    {
        OK = false;
...
void JPIP_Section::JPIP_cache_directory_changed()
{
    //	Avoid redundant editingFinished signals.
    static bool OK = true;
...
void JPIP_Section::select_JPIP_cache_directory()
{
...
void JPIP_Section::max_source_image_area_MB(int area)
{
    ...
    Max_Source_Image_Area_MB_Reset_Button->setVisible(area != Max_Source_Image_Area_MB);
    reset_modifier_buttons();
}
...
void JPIP_Section::reset_modifier_buttons()
{
    reset_defaults_button();
    Apply_Button->setEnabled(has_changed());
}
...
void JPIP_Section::apply()
{
    ...
    if (Proxy != text && JPIP_proxy_is_valid(text))
    {
        Proxy = text;
        Proxy_Reset_Button->setVisible(false);
    ...
    if (Cache_Directory != text && JPIP_cache_directory_is_valid(text))
    {
    ...
    if (Max_Source_Image_Area_MB != Max_Source_Image_Area_MB_spinBox->value())
    {
        ...
        Max_Source_Image_Area_MB_Reset_Button->setVisible(false);
        settings.setValue(MAX_SOURCE_IMAGE_AREA_MB_KEY, Max_Source_Image_Area_MB);
        //	>>> SIGNAL <<<
...
//	Initialize values.
    ...
    int sensitivity = Selection_Sensitivity =
        settings.value(SELECTION_SENSITIVITY_KEY, Default_Selection_Sensitivity).toInt(&OK);
#if ((DEBUG_SECTION) & (DEBUG_CONSTRUCTORS | DEBUG_GRAPHS))
    ...
    if (!OK)
    {
        ...
        settings.setValue(SELECTION_SENSITIVITY_KEY, Default_Selection_Sensitivity);
...
//	Layout controls.
    ...
    Canvas_Color_lineEdit->setToolTip(tr("Color of graph canvas"));
#ifndef QT_NO_SHORTCUT
    ...
    connect(Canvas_Color_Select_Button, SIGNAL(clicked()), SLOT(select_canvas_color()));
    ...
    connect(Canvas_Color_Reset_Button, SIGNAL(clicked()), SLOT(canvas_color_reset()));
...
void Graphs_Section::selection_sensitivity(int size)
{
...
void Graphs_Section::selection_sensitivity_change()
{
#if ((DEBUG_SECTION) & DEBUG_GRAPHS)
...
void Graphs_Section::canvas_color_changed()
{
    //	Avoid redundant editingFinished signals.
    static bool OK = true;
...
void Graphs_Section::changing()
{
    bool changed = false;
    Selection_Sensitivity_Reset_Button->setVisible(
        changed = (Selection_Sensitivity_spinBox->value() != Selection_Sensitivity));
    //	Text fields manage their Reset_Buttons.
    changed |= (Canvas_Color_lineEdit->text() != Canvas_Color_Text);

    reset_defaults_button();
    Apply_Button->setEnabled(changed);
}

bool Graphs_Section::has_changed() const
{
    return Selection_Sensitivity_spinBox->value() != Selection_Sensitivity ||
           Canvas_Color_lineEdit->text() != Canvas_Color_Text;
}

void Graphs_Section::apply()
{
#if ((DEBUG_SECTION) & DEBUG_GRAPHS)
    ...
    if (Canvas_Color_Text != text && canvas_color_is_valid(text))
    {
        ...
        Canvas_Color_Reset_Button->setVisible(false);
#if ((DEBUG_SECTION) & DEBUG_GRAPHS)
        clog << "    Canvas_Color_Text_Pending = " << Canvas_Color_Text_Pending << endl
             << "            Canvas_Color_Text = " << Canvas_Color_Text << endl
             << "                 Canvas_Color = " << color_text(Canvas_Color) << endl;
#endif
