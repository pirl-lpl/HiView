...
#define DEBUG_DEFAULT (DEBUG_ALL & ~DEBUG_EVENTS & ~DEBUG_MOUSE_EVENTS & ~DEBUG_SIGNALS)
...
#define Reticule_Cursor HiView_Config::Reticule_Cursor
#define Shift_Cursor HiView_Config::Shift_Cursor
#define Scale_Cursor HiView_Config::Scale_Cursor
...
#endif
double Image_Viewer::Scaling_Minor_Increment = SCALING_MINOR_INCREMENT;

#ifndef SCALING_MAJOR_INCREMENT
#define SCALING_MAJOR_INCREMENT .1
#endif
double Image_Viewer::Scaling_Major_Increment = SCALING_MAJOR_INCREMENT;
...
#endif
bool Image_Viewer::Default_Scaling_Immediate = DEFAULT_SCALING_IMMEDIATE;
      ...
      Mouse_Drag_Image_Position(-1, -1), Default_Cursor(NULL), Projector(NULL), Block_Image_Updates(true)
{
    ...
    LRC_Widget->setFixedSize(Vertical_Scrollbar_Width, Horizontal_Scrollbar_Height);
    LRC_Widget->setVisible(false);

    //	Scrollbars movement connections.
    connect(Horizontal_Scrollbar, SIGNAL(valueChanged(int)), SLOT(scrollbar_value_changed()));
    connect(Vertical_Scrollbar, SIGNAL(valueChanged(int)), SLOT(scrollbar_value_changed()));
    ...
    connect(Sliding_Scale, SIGNAL(valueChanged(int)), SLOT(sliding_scale_value_changed(int)));

    Sliding_Scale_Value = new QLabel(this);
    sliding_scale_value(1.0);
    QFont scale_font(Sliding_Scale_Value->font());
    ...
    Sliding_Scale_Width = qMax(Sliding_Scale_Width, (Sliding_Scale_Value->sizeHint()).width());
    Sliding_Scale_Value->setFixedSize(Sliding_Scale_Width, Horizontal_Scrollbar_Height);
...
//	Image_Display
    ...
    OBJECT_CONDITIONAL(clog << "    new Tiled_Image_Display ..." << endl;)
#endif
...
bool Image_Viewer::image(const QImage &source_image, const QSize &display_size)
{
    ...
    {
#if ((DEBUG_SECTION) & DEBUG_LOAD_IMAGE)
        OBJECT_CONDITIONAL(clog << "    wrapping the QImage in a Plastic_Image" << endl;)
#endif
...
//	>>> SIGNAL <<<
        ...
        OBJECT_CONDITIONAL(LOCKED_LOGGING((clog << "^^^ Image_Viewer::loaded: "
                                                   "emit image_loaded "
...
/*==============================================================================
    Layout
*/
void Image_Viewer::layout_display(QSize display_size)
{
#if ((DEBUG_SECTION) & (DEBUG_LAYOUT | DEBUG_EVENTS))
    OBJECT_CONDITIONAL(clog << ">>> Image_Viewer::layout_display " << object_pathname(this) << endl
                            << "     display_size = " << display_size << endl;)
#endif
...
#if ((DEBUG_SECTION) & (DEBUG_LAYOUT | DEBUG_EVENTS))
    OBJECT_CONDITIONAL(clog << "             viewport size = " << size() << endl
    ...
    if (Scrollbars_Enabled && !scaled_image_size.isEmpty())
    {
        ...
        sliding_scale_x = frame_margin + display_size.rwidth();
    ...
    {
#if ((DEBUG_SECTION) & (DEBUG_LAYOUT | DEBUG_EVENTS))
        OBJECT_CONDITIONAL(clog << "    Image_Viewer::layout_display: adjusting ..." << endl;)
#endif
        updateGeometry();
    }
#if ((DEBUG_SECTION) & (DEBUG_LAYOUT | DEBUG_EVENTS))
...
QSize Image_Viewer::sizeHint() const
{
    ...
    if (Horizontal_Scrollbar && Horizontal_Scrollbar->isVisible())
        display_size.rheight() += Horizontal_Scrollbar_Height;
#if ((DEBUG_SECTION) & DEBUG_SIZE_HINT)
    OBJECT_CONDITIONAL(clog << "<<< Image_Viewer::sizeHint: " << display_size << endl;)
#endif
    return display_size;
}

QSize Image_Viewer::viewport_size() const
{
#if ((DEBUG_SECTION) & (DEBUG_LAYOUT | DEBUG_LOAD_IMAGE))
    OBJECT_CONDITIONAL(clog << ">>> Image_Viewer::viewport_size" << endl;)
#endif
    QSize preferred_size(Image_Display->size());
#if ((DEBUG_SECTION) & (DEBUG_LAYOUT | DEBUG_LOAD_IMAGE))
...
#if ((DEBUG_SECTION) & (DEBUG_LAYOUT | DEBUG_LOAD_IMAGE))
        OBJECT_CONDITIONAL(clog << "    adjusting for scrollbars" << endl;)
#endif
        if (Vertical_Scrollbar && Vertical_Scrollbar->isVisible())
...
#if ((DEBUG_SECTION) & (DEBUG_LAYOUT | DEBUG_LOAD_IMAGE))
    OBJECT_CONDITIONAL(clog << "<<< Image_Viewer::viewport_size: " << preferred_size << endl;)
#endif
...
bool Image_Viewer::shift_image(const QSize &offsets, int band)
{
    ...
    if (!offsets.isNull())
    {
        QPoint origin(round_down(displayed_image_origin(band)));
#if ((DEBUG_SECTION) & DEBUG_MOVE)
        OBJECT_CONDITIONAL(clog << "    from origin = " << origin << endl;)
#endif
        origin.rx() -= offsets.width();
        origin.ry() -= offsets.height();
#if ((DEBUG_SECTION) & DEBUG_MOVE)
        OBJECT_CONDITIONAL(clog << "      to origin = " << origin << endl;)
#endif
        moved = move_image(origin, band);
    }
#if ((DEBUG_SECTION) & DEBUG_MOVE)
    OBJECT_CONDITIONAL(clog << "<<< Image_Viewer::shift_image: " << boolalpha << moved << endl;)
#endif
    return moved;
}
...
bool Image_Viewer::shift_display(const QSize &offsets, int band)
{
    ...
    if (!offsets.isNull())
    {
        QSizeF scaled_offsets(image_scaling(band));
#if ((DEBUG_SECTION) & DEBUG_MOVE)
...
void Image_Viewer::adjust_scrollbars_range()
{
    ...
    */
    QSize display_size(Image_Display->size());
    QSizeF scaling(image_scaling());
#if ((DEBUG_SECTION) & (DEBUG_SCROLLBARS | DEBUG_LAYOUT))
    OBJECT_CONDITIONAL(clog << "              display size = " << display_size << endl
                            << "             image_scaling = " << scaling << endl;)
#endif
    display_size.rwidth() = display_size.rwidth() / scaling.rwidth();
...
//	static_cast<int>(ceil (display_size.rheight () / scaling.rheight ()));
    ...
    */
    QSize size_of_image(image_size());
#if ((DEBUG_SECTION) & (DEBUG_SCROLLBARS | DEBUG_LAYOUT))
...
void Image_Viewer::scrollbar_value_changed()
{
    if (Image_Display && !Block_Image_Updates)
    {
...
void Image_Viewer::scrollbars(bool enabled)
{
...
bool Image_Viewer::scale_image(const QSizeF &scaling, const QPoint &center, int band)
{
    ...
    OBJECT_CONDITIONAL(clog << ">>> Image_Viewer::scale_image: " << scaling << endl
                            ...
                            << "      band = " << band << endl;)
#endif
    bool scaled = Image_Display->scale_image(scaling, center, band);
    if (scaled)
    {
        /*	Block image updates.
...
#if ((DEBUG_SECTION) & (DEBUG_SCALE | DEBUG_SLOTS))
    OBJECT_CONDITIONAL(clog << "<<< Image_Viewer::scale_image: " << boolalpha << scaled << endl;)
#endif
    return scaled;
}
...
bool Image_Viewer::scale_by(const QSizeF &scaling_factors)
{
...
#if ((DEBUG_SECTION) & (DEBUG_SCALE | DEBUG_SLOTS))
    OBJECT_CONDITIONAL(clog << "    scaling = " << scaling << endl;)
#endif
...
#if ((DEBUG_SECTION) & (DEBUG_SCALE | DEBUG_SLOTS))
        OBJECT_CONDITIONAL(clog << "      menu display position = " << display_position << endl;)
#endif
    }
    else
    {
...
#if ((DEBUG_SECTION) & (DEBUG_SCALE | DEBUG_SLOTS))
        OBJECT_CONDITIONAL(clog << "    cursor display position = " << display_position << endl;)
#endif
    }

    QRect display_region(image_display_region());
#if ((DEBUG_SECTION) & (DEBUG_SCALE | DEBUG_SLOTS))
    OBJECT_CONDITIONAL(clog << "       image display region = " << display_region << endl;)
#endif
...
#if ((DEBUG_SECTION) & (DEBUG_SCALE | DEBUG_SLOTS))
        OBJECT_CONDITIONAL(clog << "    using the display center = " << display_position << endl;)
#endif
    }
    ...
    if (fixed_position && scaled)
    {
        ...
        if (display_position != position)
        {
            ...
            {
#if ((DEBUG_SECTION) & (DEBUG_SCALE | DEBUG_SLOTS))
                OBJECT_CONDITIONAL(clog << "     set to left edge" << endl;)
#endif
                position.rx() = 0;
            }
            else if (position.rx() >= Image_Display->width())
            {
#if ((DEBUG_SECTION) & (DEBUG_SCALE | DEBUG_SLOTS))
                OBJECT_CONDITIONAL(clog << "     set to right edge" << endl;)
#endif
            ...
            {
#if ((DEBUG_SECTION) & (DEBUG_SCALE | DEBUG_SLOTS))
                OBJECT_CONDITIONAL(clog << "     set to top edge" << endl;)
#endif
                position.ry() = 0;
            }
            else if (position.ry() >= Image_Display->height())
            {
#if ((DEBUG_SECTION) & (DEBUG_SCALE | DEBUG_SLOTS))
                OBJECT_CONDITIONAL(clog << "     set to bottom edge" << endl;)
#endif
...
#if ((DEBUG_SECTION) & (DEBUG_SCALE | DEBUG_SLOTS))
    OBJECT_CONDITIONAL(clog << "<<< Image_Viewer::scale_by: " << boolalpha << scaled << endl;)
#endif
    return scaled;
}

bool Image_Viewer::scale_up(double factor)
{
    if (factor <= 0.0)
    {
        if (factor < 0.0)
        {
            ...
            QSizeF image_region(displayed_image_region().size());
            factor = 1.0 - qMin((image_region.rwidth() / (image_region.rwidth() + 1)),
...
bool Image_Viewer::scale_down(double factor)
{
    if (factor <= 0.0)
    {
        if (factor < 0.0)
        {
            //	Change the scaled image size by at most one pixel.
            QSizeF image_region(displayed_image_region().size());
            factor = 1.0 - qMin(((image_region.rwidth() - 1) / image_region.rwidth()),
...
bool Image_Viewer::actual_size()
{
    QSizeF scaling_factors(1, 1);
    return scale_by(scaling_factors -= image_scaling());
}

bool Image_Viewer::fit_image_to_window()
{
#if ((DEBUG_SECTION) & (DEBUG_SCALE | DEBUG_LAYOUT | DEBUG_SLOTS))
    OBJECT_CONDITIONAL(clog << ">>> Image_Viewer::fit_image_to_window" << endl;)
#endif
    bool scaled = false;
    QSize display_viewport(viewport_size());
#if ((DEBUG_SECTION) & (DEBUG_SCALE | DEBUG_LAYOUT | DEBUG_SLOTS))
    OBJECT_CONDITIONAL(clog << "    display viewport_size = " << display_viewport << endl;)
#endif
...
bool Image_Viewer::fit_to_width()
{
#if ((DEBUG_SECTION) & (DEBUG_SCALE | DEBUG_LAYOUT | DEBUG_SLOTS))
    OBJECT_CONDITIONAL(clog << ">>> Image_Viewer::fit_image_width_to_window" << endl;)
#endif
    bool scaled = false;
    QSize size_of_display(viewport_size()), size_of_image(image_size());
#if ((DEBUG_SECTION) & (DEBUG_SCALE | DEBUG_LAYOUT | DEBUG_SLOTS))
    OBJECT_CONDITIONAL(clog << "    display viewport_size = " << size_of_display << endl
                            << "               image size = " << size_of_image << endl;)
#endif
    if (!size_of_display.isEmpty() && !size_of_image.isEmpty())
    {
        double scale = (double)size_of_display.width() / size_of_image.rwidth();
#if ((DEBUG_SECTION) & (DEBUG_SCALE | DEBUG_LAYOUT | DEBUG_SLOTS))
...
#if ((DEBUG_SECTION) & (DEBUG_SCALE | DEBUG_LAYOUT | DEBUG_SLOTS))
        OBJECT_CONDITIONAL(clog << "            applied scale = " << scale << endl;)
#endif
        if ((scaled = scale_image(QSizeF(scale, scale), QPoint(0, 0))))
        {
#if ((DEBUG_SECTION) & (DEBUG_SCALE | DEBUG_LAYOUT | DEBUG_SLOTS))
...
bool Image_Viewer::fit_to_height()
{
#if ((DEBUG_SECTION) & (DEBUG_SCALE | DEBUG_LAYOUT | DEBUG_SLOTS))
    OBJECT_CONDITIONAL(clog << ">>> Image_Viewer::fit_image_height_to_window" << endl;)
#endif
    bool scaled = false;
    QSize size_of_display(viewport_size()), size_of_image(image_size());
#if ((DEBUG_SECTION) & (DEBUG_SCALE | DEBUG_LAYOUT | DEBUG_SLOTS))
    ...
    if (!size_of_display.isEmpty() && !size_of_image.isEmpty())
    {
        double scale = (double)size_of_display.height() / size_of_image.height();
#if ((DEBUG_SECTION) & (DEBUG_SCALE | DEBUG_LAYOUT | DEBUG_SLOTS))
        OBJECT_CONDITIONAL(clog << "                    scale = " << scale << endl;)
#endif
...
void Image_Viewer::sliding_scale_value_changed(int value)
{
    QSizeF scaling(image_scaling());
...
void Image_Viewer::sliding_scale_value(double value)
{
#if ((DEBUG_SECTION) & (DEBUG_LAYOUT | DEBUG_SCALE))
...
void Image_Viewer::update_sliding_scale()
{
#if ((DEBUG_SECTION) & (DEBUG_LAYOUT | DEBUG_SCALE))
...
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_MAP_BANDS))
    OBJECT_CONDITIONAL(clog << "<<< Image_Viewer::map_bands: " << boolalpha << changed << endl;)
#endif
    return changed;
}
...
#endif
    bool changed = Image_Display->map_data(maps);
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_MAP_DATA))
    OBJECT_CONDITIONAL(clog << "<<< Image_Viewer::map_data: " << boolalpha << changed << endl;)
#endif
    return changed;
}
...
/*==============================================================================
    Menus
*/
void Image_Viewer::create_menus()
{
    ...
    connect(Normal_Size_Action, SIGNAL(triggered()), SLOT(actual_size()));
    ...
    connect(Fit_to_Window_Action, SIGNAL(triggered()), SLOT(fit_image_to_window()));
    ...
    connect(Fit_to_Width_Action, SIGNAL(triggered()), SLOT(fit_to_width()));
    ...
    connect(Fit_to_Height_Action, SIGNAL(triggered()), SLOT(fit_to_height()));
...
void Image_Viewer::update_scaling_actions()
{
...
void Image_Viewer::update_window_fit_actions()
{
    Fit_to_Window_Action->setEnabled(!display_fit_to_image());
    QSize display_size(image_display_size()), scaled_size(scaled_image_size());
    Fit_to_Width_Action->setEnabled(display_size.rwidth() != scaled_size.rwidth() &&
                                    display_size.rwidth() != (scaled_size.rwidth() - 1) &&
                                    display_size.rwidth() != (scaled_size.rwidth() + 1));
    Fit_to_Height_Action->setEnabled(display_size.height() != scaled_size.height() &&
...
bool Image_Viewer::display_fit_to_image() const
{
#if ((DEBUG_SECTION) & (DEBUG_MENUS | DEBUG_LAYOUT))
    OBJECT_CONDITIONAL(clog << ">>> Image_Viewer::display_fit_to_image" << endl;)
#endif
    bool fit = true;
    if (Image_Display)
    {
...
#if ((DEBUG_SECTION) & (DEBUG_MENUS | DEBUG_LAYOUT))
        OBJECT_CONDITIONAL(clog << "    image display size = " << display_size << endl
...
void Image_Viewer::contextMenuEvent(QContextMenuEvent *event)
{
    ...
    OBJECT_CONDITIONAL(clog << ">>> Image_Viewer::contextMenuEvent:" << endl
    ...
    Menu_Position.rx() = Menu_Position.ry() = -1;
#if ((DEBUG_SECTION) & DEBUG_EVENTS)
    OBJECT_CONDITIONAL(clog << "<<< Image_Viewer::contextMenuEvent" << endl;)
#endif
}
...
#endif
    bool accepted = false;
    if (Control_Mode == SHIFT_MODE && event->buttons() == Qt::LeftButton)
    {
        QPoint display_position = (Image_Display->mapFromGlobal(event->globalPosition())).toPoint();
#if ((DEBUG_SECTION) & DEBUG_EVENTS)
        OBJECT_CONDITIONAL(clog << "     image display position = " << display_position << endl;)
#endif
        if (image_display_region().contains(display_position))
        {
            Mouse_Drag_Image_Position = (round_down(map_display_to_image(display_position)));
            accepted = true;
        }
    }
    else
    {
        Mouse_Drag_Image_Position.rx() = Mouse_Drag_Image_Position.ry() = -1;
    }
    ...
    if (Control_Mode == SHIFT_MODE && event->buttons() == Qt::LeftButton)
    {
        ...
        if (Mouse_Drag_Image_Position.rx() == -1 && Mouse_Drag_Image_Position.ry() == -1)
        {
            ...
            accepted = event->isAccepted();
                 ...
                 !shift_image(QSize(position.rx() - Mouse_Drag_Image_Position.rx(),
...
void Image_Viewer::mouseReleaseEvent(QMouseEvent *event)
{
    ...
    {
#if ((DEBUG_SECTION) & (DEBUG_EVENTS | DEBUG_MOUSE_EVENTS))
        OBJECT_CONDITIONAL(clog << "    SCALE_MODE" << endl;)
#endif
        accepted = true;
    ...
    if (delta)
    {
        if (Control_Mode == SCALE_MODE)
        {
            ...
            if (delta)
            {
                ...
                {
                    scale = (max_scaling > scale) ? (max_scaling - scaling_minor_increment())
                                                  : (max_scaling + scaling_minor_increment());
#if ((DEBUG_SECTION) & (DEBUG_EVENTS | DEBUG_MOUSE_EVENTS))
                    OBJECT_CONDITIONAL(clog << "        min change scale = " << scale << endl;)
#endif
                }
        ...
        {
#if ((DEBUG_SECTION) & (DEBUG_EVENTS | DEBUG_MOUSE_EVENTS))
            OBJECT_CONDITIONAL(clog << "    SHIFT_MODE - offset = " << delta << endl;)
#endif
            accepted = true;
...
//	>>> SIGNAL <<
    ...
    OBJECT_CONDITIONAL(clog << "^^^ Image_Viewer::cursor_moved: emit image_cursor_moved" << endl
...
//	>>> SIGNAL <<
#if ((DEBUG_SECTION) & (DEBUG_MOUSE_EVENTS | DEBUG_SIGNALS))
        OBJECT_CONDITIONAL(clog << "^^^ Image_Viewer::cursor_moved: emit image_pixel_value" << endl
                                << "    display_pixel = " << Plastic_Image::Triplet() << endl
                                << "      image_pixel = " << Plastic_Image::Triplet() << endl;)
#endif
        emit image_pixel_value(Plastic_Image::Triplet(), Plastic_Image::Triplet());
    }
    else
    {
//	>>> SIGNAL <<
...
#endif
        emit image_pixel_value(display_pixel(display_position), image_pixel(image_position));
    }
}
...
//	Arrow key actions
#if ((DEBUG_SECTION) & (DEBUG_EVENTS | DEBUG_KEY_EVENTS))
        OBJECT_CONDITIONAL(clog << "    arrow key" << endl;)
#endif
        if (Control_Mode == SCALE_MODE)
        {
...
#if ((DEBUG_SECTION) & (DEBUG_EVENTS | DEBUG_KEY_EVENTS))
                    OBJECT_CONDITIONAL(clog << "      Shift/Alt Modifier" << endl
...
//	Cursor position.
#if ((DEBUG_SECTION) & (DEBUG_EVENTS | DEBUG_KEY_EVENTS))
            OBJECT_CONDITIONAL(clog << "      NO_CONTROL_MODE" << endl;)
#endif
            bool nudge = true;
            QPoint position(Image_Display->mapFromGlobal(QCursor::pos()));
    ...
    else if (key == Qt::Key_PageUp || key == Qt::Key_PageDown)
    {
        ...
        {
#if ((DEBUG_SECTION) & (DEBUG_EVENTS | DEBUG_KEY_EVENTS))
            OBJECT_CONDITIONAL(clog << "    non-autorepeat" << endl;)
#endif
...
#if ((DEBUG_SECTION) & (DEBUG_EVENTS | DEBUG_KEY_EVENTS))
                OBJECT_CONDITIONAL(clog << "    control/command key combination" << endl;)
#endif
                switch (key)
                {
                case Qt::Key_Plus:
...
#if ((DEBUG_SECTION) & (DEBUG_EVENTS | DEBUG_KEY_EVENTS))
                        OBJECT_CONDITIONAL(clog << "    Control_Mode = NO_CONTROL_MODE" << endl;)
#endif
                        Control_Mode = NO_CONTROL_MODE;
                        Image_Display->setCursor(*Default_Cursor);
                        releaseKeyboard();
...
QString Image_Viewer::control_mode_description(int control_mode)
{
