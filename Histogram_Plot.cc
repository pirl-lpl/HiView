...
#endif
QColor Histogram_Plot::Default_Bar_Color = Qt::HISTOGRAM_BAR_COLOR;

#ifndef HISTOGRAM_PLOT_Z
#define HISTOGRAM_PLOT_Z 20.0
#endif
double Histogram_Plot::Default_Plot_Z = HISTOGRAM_PLOT_Z;

/*==============================================================================
    Constructors
*/
Histogram_Plot::Histogram_Plot(const QwtText &title)
    : QwtPlotItem(title), Data(new Count_Sequence), Attributes(Histogram_Plot::VERTICAL_BARS),
...
Histogram_Plot::Histogram_Plot(const QString &title)
    : QwtPlotItem(QwtText(title)), Data(new Count_Sequence), Attributes(Histogram_Plot::VERTICAL_BARS),
      Bar_Color(Default_Bar_Color)
{
#if ((DEBUG_SECTION) & DEBUG_CONSTRUCTORS)
...
void Histogram_Plot::initialize()
{
...
/*==============================================================================
    Accessors
*/
Histogram_Plot &Histogram_Plot::data(const Count_Sequence &count_sequence)
{
    *Data = count_sequence;
...
Histogram_Plot &Histogram_Plot::data(Count_Sequence *count_sequence)
{
    if (count_sequence && count_sequence != Data)
    {
        delete Data;
        Data = count_sequence;
        itemChanged(); //	Assume the data changed.
...
Histogram_Plot &Histogram_Plot::data(QVector<unsigned long long> &data_sequence)
{
...
QVector<unsigned long long> *Histogram_Plot::data_vector()
{
    return &(Data->data());
}

Histogram_Plot &Histogram_Plot::bar_color(const QColor &color)
{
    if (Bar_Color != color)
    {
        Bar_Color = color;
...
Histogram_Plot &Histogram_Plot::attribute_set(Attribute attribute, bool enabled)
{
#if ((DEBUG_SECTION) & DEBUG_ACCESSORS)
    clog << ">>> Histogram_Plot::attribute_set: " << attribute << ' ' << boolalpha << enabled << endl;
#endif
...
bool Histogram_Plot::attribute_is_set(Attribute attribute) const
{
    if (attribute == VERTICAL_BARS)
...
*/
QRectF Histogram_Plot::boundingRect() const
{
    QRectF rectangle = Data->boundingRect();
    if (rectangle.isValid() && attribute_is_set(HORIZONTAL_BARS))
        //	Horizontal values, increasing upwards.
        rectangle.setRect(0.0, rectangle.height(), rectangle.height(), rectangle.width());
    return rectangle;
}

int Histogram_Plot::rtti() const
{
    return QwtPlotItem::Rtti_PlotHistogram;
}

void Histogram_Plot::draw(QPainter *painter, const QwtScaleMap &xMap, const QwtScaleMap &yMap, const QRectF &) const
{
#if ((DEBUG_SECTION) & DEBUG_DRAW)
    clog << ">>> Histogram_Plot @ " << (void *)this << "::draw" << endl
         << "    Attributes = " << Attributes << endl
         << "    orientation = " << (attribute_is_set(HORIZONTAL_BARS) ? "HORIZONTAL_BARS" : "VERTICAL_BARS") << endl;
#endif
    int bars = (int)Data->size();
    if (!bars)
    {
#if ((DEBUG_SECTION) & DEBUG_DRAW)
        clog << "    no bars" << endl << "<<< Histogram_Plot::draw" << endl;
#endif
        return;
    }
    bool horizontal = attribute_is_set(HORIZONTAL_BARS);
    double base = Data->base(), increment = Data->increment();
#if ((DEBUG_SECTION) & DEBUG_DRAW)
    clog << "         base = " << base << endl << "    increment = " << increment << endl;
#endif
    ...
    */
    QRect bar_rect;
    int bottom, last_edge, last_top, last_length = 0, next_edge, next_top, next_length;
...
#if ((DEBUG_SECTION) & DEBUG_DRAW)
    clog << "       bottom = " << bottom << endl << "         bars = " << bars << endl;
#endif

    for (int bar = 0; bar <= bars; bar++)
    {
#if ((DEBUG_SECTION) & DEBUG_DRAW)
        clog << "    bar " << bar << " -" << endl << "      value = " << Data->y(bar) << endl;
#endif
        if (horizontal)
        {
...
#if ((DEBUG_SECTION) & DEBUG_DRAW)
        clog << "      last = " << last_edge << " edge, " << last_top << " top" << endl
             << "      next = " << next_edge << " edge, " << next_top << " top" << endl;
#endif

        base += increment;
        if (last_top == next_top && bar < bars)
        {
#if ((DEBUG_SECTION) & DEBUG_DRAW)
            clog << "      no length change" << endl << "        no bar drawn" << endl;
#endif
            continue;
        }
        ...
        if (horizontal)
            bar_rect.setRect(qMin(last_top, bottom), qMin(next_edge, last_edge), qAbs(last_top - bottom) + 1,
                             qAbs(next_edge - last_edge));
        else
            bar_rect.setRect(qMin(next_edge, last_edge), qMin(last_top, bottom), qAbs(next_edge - last_edge),
                             qAbs(last_top - bottom) + 1);
#if ((DEBUG_SECTION) & DEBUG_DRAW)
