...
/*==============================================================================
    Constants
*/
const char *const Stats::ID = "UA::HiRISE::Stats ($Revision: 1.15 $ $Date: 2014/08/05 17:58:09 $)";

/*==============================================================================
    Constructors
*/
Stats::Stats() : Lower_Limit(0), Upper_Limit(0), Histograms(MAX_STATISTICS_SETS, NULL)
{
    clear();
}
...
Stats &Stats::operator=(const Stats &stats)
{
    if (this != &stats)
    {
        for (int index = 0; index < MAX_STATISTICS_SETS; index++)
        {
...
/*==============================================================================
    Accessors
*/
void Stats::limits(int min, int max)
{
    if (min < 0)
        min = 0;
    if (max < 0)
        max = 0;
    bool recalculate = (min != Lower_Limit || max != Upper_Limit);
    Lower_Limit = min;
    Upper_Limit = max;
    if (recalculate)
        calculate();
...
Stats &Stats::histogram(Histogram *data, int index)
{
...
int Stats::sets() const
{
    int count = 0, index = MAX_STATISTICS_SETS;
    while (index--)
        if (Histograms[index])
            ++count;
    return count;
...
unsigned long long Stats::area(int index, Range_Selection range) const
{
    index_range_check(index);
    return ((range == VALID_RANGE) ? Valid_Area[index]
                                   : ((range == LOWER_RANGE) ? Lower_Area[index] : Upper_Area[index]));
}

double Stats::upper_percent_at_value(int value, int index) const
{
#if ((DEBUG_SECTION) & DEBUG_VALUE_AT)
    clog << ">>> Stats::upper_percent_at_value: value " << value << ", index " << index << endl;
#endif
    index_range_check(index);
    ...
    if (value < 0)
        value = 0;
    double percent = 0;
    if (Histograms[index] && Valid_Area[index] && value < Highest_Value[index])
    {
        Histogram *histogram = Histograms[index];
        unsigned long long count = 0;
        for (int bin = Highest_Value[index]; bin > value; --bin)
...
double Stats::lower_percent_at_value(int value, int index) const
{
    index_range_check(index);
    ...
    if (value < 0)
        value = 0;
    double percent = 0;
    if (Histograms[index] && Valid_Area[index] && value > Lowest_Value[index])
    {
        Histogram *histogram = Histograms[index];
        unsigned long long count = 0;
        for (int bin = Lowest_Value[index]; bin < value; ++bin)
            count += histogram->at(bin);
        percent = ((double)count / Valid_Area[index]) * 100.0;
    }
    return percent;
...
int Stats::upper_value_at_percent(double percent, int index) const
{
    index_range_check(index);

    int value;
    if (Histograms[index] && percent < 100.0)
    {
        value = Highest_Value[index];

        if (Valid_Area[index] && percent > 0.0)
        {
            Histogram *histogram = Histograms[index];
            unsigned long long count = histogram->at(value),
                               area = static_cast<unsigned long long>((percent / 100.0) * Valid_Area[index]);
            while (count < area)
...
int Stats::lower_value_at_percent(double percent, int index) const
{
    index_range_check(index);
...
/*==============================================================================
    Manipulators
*/
void Stats::calculate(int index)
{
    ...
    Histogram *histogram;
    int value, counted_values, lowest_counted, highest_counted, upper_limit_value;
    Histogram::value_type count, min_count, max_count;
    unsigned long long area_count, lower_area_count, upper_area_count;
    double mean, offset, accumulator;
    ...
    while (++index < last_index)
    {
        ...
        if (Histograms[index])
        {
            ...
            while (--value > upper_limit_value)
            {
                ...
                clog << "     " << setw(5) << value << ": " << histogram->at(value) << endl;
#endif
            ...
            if (counted_values)
            {
                            ...
                            if (count > min_count)
                            {
                                ...
                                */
                                ...
                                if (min_count || max_count)
                                {
                                    ...
                                    Median_Value[index] += (min_count + (max_count / 2.0)) / count;
                                }
                            }
                            else
                                min_count -= count;
                ...
                Std_Dev_of_Values[index] = sqrt(accumulator / counted_values);
            }
        }
#if ((DEBUG_SECTION) & (DEBUG_CALCULATE | DEBUG_VALUES))
...
/*==============================================================================
    Helpers
*/
int Stats::index_range_check(int index, int max)
{
    if (max == 0)
    ...
    if (index < 0 || index >= max)
    {
        ...
        message << Stats::ID << endl << "Invalid index number: " << index << endl << "Maximum index number: " << max;
