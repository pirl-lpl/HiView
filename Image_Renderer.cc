...
/*==============================================================================
    Constants
*/
const char* const Image_Renderer::ID =
        ...
        if (Owner)
        {
                ...
                (status & Plastic_Image::Rendering_Monitor::RENDERED_DATA_MASK))
            {
                ...
                if (Owner->Active_Tile->is_high_priority())
                {
                    ...
                    char input[4];
                    clog << "vis. tile " << Owner->Active_Tile->Tile_Coordinate << " region "
                         << tile_region << " > ";
                    cin.getline(input, 2);
                    if (input[0] == 'q') exit(7);
#endif
...
#if ((DEBUG_SECTION) & (DEBUG_RENDER | DEBUG_NOTIFY | DEBUG_NOTIFY_VISIBLE | DEBUG_LOCATION))
        clog << "<++ Image_Renderer::Image_Renderer_Rendering_Monitor::notification " << thread_ID
             << ": true" << endl;
#endif
        return true;
    }
...
void Image_Renderer::send_status_notice(const QString& message)
{
    ...
    void* thread_ID = (void*)QThread::currentThreadId();
    LOCKED_LOGGING((clog << "    Image_Renderer::Image_Renderer_Rendering_Monitor::notification "
      ...
      Image_Rendering_Monitor(new Image_Renderer_Rendering_Monitor(this))
{
    ...
    LOCKED_LOGGING((clog << ">-< Image_Renderer @ " << (void*)this << ": " << object_pathname(this)
                         << endl
...
void Image_Renderer::immediate_mode(bool enable)
{
    ...
    LOCKED_LOGGING((clog << ">-< Image_Renderer::immediate_mode "
...
/*==============================================================================
    Queue management
*/
void Image_Renderer::queue(Plastic_Image* image, const QPoint& tile_coordinate,
                           const QRect& tile_region, bool cancelable)
{
    ...
    {
#if ((DEBUG_SECTION) & DEBUG_QUEUE)
        LOCKED_LOGGING((clog << "    Image_Renderer::queue " << thread_ID << ": adding new tile"
                             << endl
                             << "    in " << pathname << endl));
#endif
        add_tile(image_tile);
...
#if ((DEBUG_SECTION) & DEBUG_QUEUE)
        LOCKED_LOGGING((clog << "    Image_Renderer::queue " << thread_ID
...
//	Change of priority; remove current entry and add new tile.
...
#if ((DEBUG_SECTION) & DEBUG_QUEUE)
            LOCKED_LOGGING((clog << "    Image_Renderer::queue " << thread_ID << ": add new tile"
                                 << endl
                                 << "    in " << pathname << endl));
#endif
            add_tile(image_tile);
    ...
    print_queue(Render_Queue);
    clog << "    Image_Renderer::queue " << thread_ID << ": unlock Queue_Lock" << endl
         << "    in " << pathname << endl;
    UNLOCK_LOG;
#endif
...
#if ((DEBUG_SECTION) & DEBUG_QUEUE)
            LOCKED_LOGGING((clog << "    Image_Renderer::cancel " << thread_ID
...
#if ((DEBUG_SECTION) & DEBUG_QUEUE)
    LOCKED_LOGGING((clog << "<<< Image_Renderer::cancel " << thread_ID << ": " << boolalpha
                         << canceled << endl));
#endif
    return canceled;
}
...
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_QUEUE))
    LOCKED_LOGGING(
        (clog << "<<< Image_Renderer::cancel " << thread_ID << ": " << boolalpha << done << endl));
#endif
    return done;
}
...
bool Image_Renderer::reset(int cancel_options)
{
    ...
    LOCKED_LOGGING((clog << ">>> Image_Renderer::reset " << thread_ID << ": " << cancel_options
                         << " - " << description << endl
                         << "    in " << pathname << endl
                         << "    lock Queue_Lock" << endl));
#endif
...
#if ((DEBUG_SECTION) & DEBUG_QUEUE)
    LOCKED_LOGGING((clog << "    in " << pathname << endl
                         << "<<< Image_Renderer::reset " << thread_ID << ": " << boolalpha << done
                         << endl));
#endif
    return done;
}
...
#if ((DEBUG_SECTION) & DEBUG_QUEUE)
        LOCKED_LOGGING((clog << "    Image_Renderer::add_tile " << thread_ID
...
//	Start rendering.
#if ((DEBUG_SECTION) & DEBUG_QUEUE)
        LOCKED_LOGGING((clog << "    Image_Renderer::add_tile " << thread_ID << ": start rendering"
                             << endl
                             << "    in " << pathname << endl));
#endif
        start_rendering();
...
//	>>> CAUTION: The Queue_Lock is expected to be locked.
    ...
    QString pathname(object_pathname(this)),
        description(cancel_options_descriptions(cancel_options));
    LOCK_LOG;
    clog << ">>> Image_Renderer::clear " << thread_ID << ": " << cancel_options << " - "
         << description << endl
         << "    in " << pathname << endl;
    if (Active_Tile) clog << "    Active_Tile -" << endl << "    " << *Active_Tile << endl;
    clog << "    Render_Queue -" << endl;
...
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_QUEUE))
    LOCKED_LOGGING((clog << "    in " << pathname << endl
                         << "<<< Image_Renderer::clear " << thread_ID << ": " << boolalpha << done
                         << endl));
#endif
    return done;
}
...
//	>>> CAUTION: The Queue_Lock is expected to be locked.
    ...
    QString pathname(object_pathname(this)),
        description(cancel_options_descriptions(cancel_options));
    LOCKED_LOGGING((clog << ">>> Image_Renderer::abort " << thread_ID << ": " << cancel_options
                         << " - " << description << endl
                         << "    in " << pathname << endl));
#endif
    bool done = true;
...
#if ((DEBUG_SECTION) & DEBUG_QUEUE)
                LOCKED_LOGGING((clog << "!!! Image_Renderer::abort " << thread_ID
                                     << ": failed to obtain Rendering_Lock after " << Wait_Seconds
                                     << " seconds!" << endl
                                     << "    in " << pathname << endl));
#endif
                done = false;
...
#if ((DEBUG_SECTION) & DEBUG_QUEUE)
            LOCKED_LOGGING((clog << "    Image_Renderer::abort " << thread_ID << ": didn't wait"
                                 << endl
                                 << "    in " << pathname << endl));
#endif
            done = false;
...
#if ((DEBUG_SECTION) & DEBUG_QUEUE)
    LOCKED_LOGGING((clog << "    in " << pathname << endl
                         << "<<< Image_Renderer::abort " << thread_ID << ": " << boolalpha << done
                         << endl));
#endif
    return done;
}
...
#if ((DEBUG_SECTION) & (DEBUG_DELETE_TILES | DEBUG_QUEUE))
    LOCKED_LOGGING((clog << "<<< Image_Renderer::delete_image " << thread_ID << ": " << boolalpha
                         << deleted << endl));
#endif
    return deleted;
}
...
//	>>> CAUTION: The Queue_Lock is expected to be locked.
    ...
    void* thread_ID = (void*)QThread::currentThreadId();
    QString pathname(object_pathname(this));
    LOCKED_LOGGING((clog << ">>> Image_Renderer::delete_tile " << thread_ID << ": " << *image_tile
                         << endl
                         << "    in " << pathname << endl));
#endif
...
#if ((DEBUG_SECTION) & (DEBUG_DELETE_TILES | DEBUG_QUEUE))
    LOCKED_LOGGING((clog << "    Image_Renderer::delete_tiles " << thread_ID
                         << ": unlock Queue_Lock" << endl
                         << "    in " << pathname << endl));
#endif
...
void Image_Renderer::clean_up() { delete_tiles(); }
...
#if ((DEBUG_SECTION) & DEBUG_RENDER)
    LOCKED_LOGGING((clog << "    Image_Renderer::run_rendering " << thread_ID
...
bool Image_Renderer::is_ready()
{
    ...
    {
#if ((DEBUG_SECTION) & (DEBUG_RENDER | DEBUG_SIGNALS | DEBUG_STATUS))
        LOCKED_LOGGING((clog << "^^^ Image_Renderer::is_ready " << thread_ID << ": emit status "
...
#if ((DEBUG_SECTION) & DEBUG_RENDER)
    LOCKED_LOGGING((clog << "    Image_Renderer::is_ready " << thread_ID << ": unlock Ready_Lock"
                         << endl
                         << "    in " << pathname << endl));
#endif
    Ready_Lock.unlock();
#if ((DEBUG_SECTION) & DEBUG_RENDER)
    LOCKED_LOGGING((clog << "    in " << pathname << endl
                         << "<<< Image_Renderer::is_ready " << thread_ID << ": " << boolalpha
...
#endif
#if ((DEBUG_SECTION) & DEBUG_RENDER)
    LOCKED_LOGGING((clog << ">>> Image_Renderer::render " << thread_ID << endl
                         << "    in " << pathname << endl));
#endif
...
#if ((DEBUG_SECTION) & DEBUG_RENDER)
        LOCKED_LOGGING((clog << "    Image_Renderer::render " << thread_ID << ": lock Queue_Lock"
                             << endl
                             << "    in " << pathname << endl));
#endif
        Queue_Lock.lock();
...
#endif
#if ((DEBUG_SECTION) & (DEBUG_RENDER | DEBUG_QUEUE | DEBUG_OVERVIEW))
        LOCKED_LOGGING((clog << "    Image_Renderer::render " << thread_ID << ": Active_Tile "
                             << *Active_Tile << endl
...
*/
#if ((DEBUG_SECTION) & DEBUG_RENDER)
        LOCKED_LOGGING((clog << "    Image_Renderer::render " << thread_ID << ": unlock Queue_Lock"
                             << endl
                             << "    in " << pathname << endl));
#endif
        Queue_Lock.unlock();
        ...
        {
#if ((DEBUG_SECTION) & DEBUG_RENDER)
            LOCKED_LOGGING((clog << "^^^ Image_Renderer::render " << thread_ID << ": emit status "
...
*/
#if ((DEBUG_SECTION) & DEBUG_RENDER)
        LOCKED_LOGGING((clog << "    Image_Renderer::render " << thread_ID << ": relock Queue_Lock"
                             << endl
                             << "    in " << pathname << endl));
#endif
        Queue_Lock.lock();
...
#endif
#if ((DEBUG_SECTION) & DEBUG_RENDER)
    LOCKED_LOGGING((clog << "    Image_Renderer::start_rendering " << thread_ID
                         << ": lock Ready_Lock" << endl
                         << "    in " << pathname << endl));
#endif
    Ready_Lock.lock();
...
bool Image_Renderer::suspend_rendering(bool wait)
{
    ...
    void* thread_ID = (void*)QThread::currentThreadId();
    QString pathname(object_pathname(this));
    LOCKED_LOGGING((clog << ">>> Image_Renderer::suspend_rendering " << thread_ID
...
//	Wait for any rendering to complete.
        ...
        LOCKED_LOGGING((clog << "    Image_Renderer::suspend_rendering " << thread_ID
                             ...
                             << "    wait up to " << Wait_Seconds << " seconds" << endl));
#endif
        if ((done = Rendering_Lock.tryLock(Wait_Seconds * 1000)))  // NOLINT
        {
#if ((DEBUG_SECTION) & DEBUG_RENDER)
            LOCKED_LOGGING((clog << "    Image_Renderer::suspend_rendering " << thread_ID
                                 << ": unlock Rendering_Lock" << endl
        ...
        if (queue_locked) Queue_Lock.unlock();
    }
...
bool Image_Renderer::stop_rendering(bool wait)
{
    ...
    void* thread_ID = (void*)QThread::currentThreadId();
    QString pathname(object_pathname(this));
    LOCKED_LOGGING((clog << ">>> Image_Renderer::stop_rendering " << thread_ID
                         << ": wait = " << boolalpha << wait << endl
                         << "    in " << pathname << endl
...
#if ((DEBUG_SECTION) & DEBUG_RENDER)
    LOCKED_LOGGING((clog << "    in " << pathname << endl
                         << "<<< Image_Renderer::stop_rendering " << thread_ID << ": " << boolalpha
                         << done << endl));
#endif
    return done;
}
...
bool Image_Renderer::finish(int cancel_options)
{
    ...
    QString pathname(object_pathname(this)),
        description(cancel_options_descriptions(cancel_options));
    LOCKED_LOGGING((clog << ">>> Image_Renderer::finish " << thread_ID << ": " << cancel_options
                         << " - " << description << endl
                         << "    in " << pathname << endl));
#endif
...
bool Image_Renderer::image(const QString& source_name)
{
    ...
    void* thread_ID = (void*)QThread::currentThreadId();
    QString pathname(object_pathname(this));
    LOCKED_LOGGING((clog << ">>> Image_Renderer::image " << thread_ID << ": \"" << source_name
                         << '"' << endl
                         << "    in " << pathname << endl));
#endif
    ...
    if (!source_name.isEmpty())
    {
        ...
        LOCKED_LOGGING((clog << "    Image_Renderer::image " << thread_ID << ": lock Queue_Lock"
                             << endl
                             << "    in " << pathname << endl));
#endif
        QMutexLocker const qLocker(&Queue_Lock);
...
#if ((DEBUG_SECTION) & DEBUG_LOAD_IMAGE)
        LOCKED_LOGGING((clog << "    Image_Renderer::image " << thread_ID << ": unlock Queue_Lock"
                             << endl
                             << "    in " << pathname << endl));
#endif
    }
#if ((DEBUG_SECTION) & (DEBUG_LOAD_IMAGE | DEBUG_OVERVIEW))
    LOCKED_LOGGING((clog << "    in " << pathname << endl
                         << "<<< Image_Renderer::image " << thread_ID << ": " << boolalpha
                         << registered << endl));
#endif
    return registered;
}
...
void Image_Renderer::load_image()
{
    ...
    LOCKED_LOGGING((clog << ">>> Image_Renderer::load_image " << thread_ID << endl
                         << "    in " << pathname << endl
                         << "    Image_Renderer::load_image " << thread_ID << ": tryLock Queue_Lock"
                         << endl));
#endif
    QMutexLocker const qLocker(&Queue_Lock);
...
#if ((DEBUG_SECTION) & DEBUG_LOAD_IMAGE)
    LOCKED_LOGGING((clog << "    Image_Renderer::load_image " << thread_ID << ": unlock Queue_Lock"
                         << endl
                         << "    in " << pathname << endl));
#endif

#if ((DEBUG_SECTION) & DEBUG_LOAD_IMAGE)
    LOCKED_LOGGING((clog << "^^^ Image_Renderer::load_image " << thread_ID << ": emit status "
...
//	Lock Queue_Lock during image loading.
#if ((DEBUG_SECTION) & DEBUG_LOAD_IMAGE)
    LOCKED_LOGGING((clog << "    Image_Renderer::load_image " << thread_ID << ": lock Queue_Lock"
                         << endl
                         << "    in " << pathname << endl));
#endif
    // Queue_Lock.lock ();
    ...
    */
    ...
    {
#if ((DEBUG_SECTION) & DEBUG_LOAD_IMAGE)
        LOCKED_LOGGING((clog << "    Image_Renderer::load_image " << thread_ID << ": Image_Source -"
                             << endl
                             << "    " << *Image_Source << endl
                             << "    in " << pathname << endl));
#endif
    ...
    {
#if ((DEBUG_SECTION) & DEBUG_LOAD_IMAGE)
        LOCKED_LOGGING((clog << "    Image_Renderer::load_image " << thread_ID << ": Source_Name \""
                             << Source_Name << '"' << endl
                             << "    in " << pathname << endl));
#endif
#if ((DEBUG_SECTION) & DEBUG_LOAD_IMAGE)
...
//	Load the source image from the named source.
#if ((DEBUG_SECTION) & DEBUG_LOAD_IMAGE)
        LOCKED_LOGGING((clog << "    Image_Renderer::load_image " << thread_ID << ": load image \""
                             << Source_Name << '"' << endl
                             << "    in " << pathname << endl));
#endif
    ...
    if (source_image)
    {
        ...
        LOCKED_LOGGING((clog << "    Image_Renderer::render " << thread_ID << ": new Source_Image -"
                             << endl
                             << "    " << *source_image << endl
                             << "    in " << pathname << endl));
#endif
...
//	Emit signals without Queue_Lock locked.
#if ((DEBUG_SECTION) & DEBUG_LOAD_IMAGE)
    LOCKED_LOGGING((clog << "^^^ Image_Renderer::load_image " << thread_ID << ": emit image_loaded "
...
#if ((DEBUG_SECTION) & (DEBUG_LOAD_IMAGE | DEBUG_OVERVIEW))
    LOCKED_LOGGING((clog << "    Image_Renderer::load_JP2_image " << thread_ID << ": scale "
                         << scale << " to fit " << max_source_image_area() << " amount " << endl
                         << "    in " << pathname << endl));
#endif
...
#if ((DEBUG_SECTION) & (DEBUG_LOAD_IMAGE | DEBUG_OVERVIEW))
    LOCKED_LOGGING((clog << "    Image_Renderer::load_JP2_image " << thread_ID << ": JP2_Image -"
                         << endl
                         << "    " << *JP2_image << endl
                         << "    in " << pathname << endl
...
/*==============================================================================
    Cloned image
*/
Plastic_Image* Image_Renderer::image_clone(const QSize& size,
...
#endif
#if ((DEBUG_SECTION) & DEBUG_CLONE_IMAGE)
    LOCKED_LOGGING((clog << "    Image_Renderer::image_clone " << thread_ID
...
#if ((DEBUG_SECTION) & DEBUG_CLONE_IMAGE)
        LOCKED_LOGGING((clog << "    Image_Renderer::clone_image " << thread_ID
