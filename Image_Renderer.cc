...
/** Rendering Process Monitor
 ...
 add_rendering_monitor(Dynamic_Image::Rendering_Monitor*) registered
 with a Dynamic_Image to receive its rendering notices. It should be
 (@link remove_rendering_monitor(Dynamic_Image::Rendering_Monitor*)
 ...
 Dynamic_Image recieves notices at increments during image rendering.
...
public:
 ...
 DEBUG_LOCATION))
        ...
        Dynamic_Image&,
#endif
        Dynamic_Image::Rendering_Monitor::Status status,
...
{
    clog << "    " << Image_Accounting.size() << " extant Dynamic_Images";
...
{
    return image ? find_tile(image, Render_Queue) >= 0 : Render_Queue.count() != 0;
}
...
)
{
                ...
                if (file.isDir())
                    ...
                    if (!file.isReadable())
                        ...
                        else
                        {
                            ...
                            {
                                plastic_image = new Dynamic_QImage(source_image);
    ...
    LOCKED_LOGGING((
        clog << "    in " << pathname << endl
        << "<<< Image_Renderer::load_image " << thread_ID
        << ": Dynamic_Image @ " << (void*)plastic_image << endl));
#endif
...
)
{
    ...
    QString
        pathname(object_pathname(this)),
        names(Dynamic_Image::mapping_type_names(shared_mappings));
    LOCKED_LOGGING((
