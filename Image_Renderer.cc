...
/*==============================================================================
    Constants
*/
const char *const Image_Renderer::ID = "UA::HiRISE::Image_Renderer ($Revision: 1.58 $ $Date: 2013/05/22 23:27:23 $)";

const int Image_Renderer::DO_NOT_WAIT = 0, Image_Renderer::WAIT_UNTIL_DONE = (1 << 0),
          Image_Renderer::DELETE_WHEN_DONE = (1 << 1), Image_Renderer::FORCE_CANCEL = (1 << 2);

const QPoint Image_Renderer::LOW_PRIORITY_RENDERING;

const bool Image_Renderer::CANCELABLE = true;

/*==============================================================================
    Class data
*/
QMutex Image_Renderer::Rendering_Lock;
...
#endif
unsigned long Image_Renderer::Default_Min_Source_Image_Area = DEFAULT_MIN_SOURCE_IMAGE_AREA,
              Image_Renderer::Default_Max_Source_Image_Area = DEFAULT_MAX_SOURCE_IMAGE_AREA;

#ifndef DEFAULT_WAIT_SECONDS
#define DEFAULT_WAIT_SECONDS 20
#endif
int Image_Renderer::Wait_Seconds = DEFAULT_WAIT_SECONDS;
...
*/
class Image_Renderer_Rendering_Monitor : public Plastic_Image::Rendering_Monitor
{
  private:
    Image_Renderer *Owner;

  public:
    Image_Renderer_Rendering_Monitor(Image_Renderer *owner = NULL)
    {
        Owner = owner;
    }
    ...
    bool notification(
        ...
        Plastic_Image &,
#endif
        Plastic_Image::Rendering_Monitor::Status status, const QString &message, const QRect &tile_region)
    {
#if ((DEBUG_SECTION) & (DEBUG_RENDER | DEBUG_NOTIFY | DEBUG_NOTIFY_VISIBLE | DEBUG_LOCATION))
        void *thread_ID = (void *)QThread::currentThreadId();
        QString pathname(object_pathname(Owner));
#if ((DEBUG_SECTION) & DEBUG_NOTIFY_VISIBLE)
        if (Owner->Active_Tile->is_high_priority())
#endif
            clog << "++> Image_Renderer::Image_Renderer_Rendering_Monitor::notification " << thread_ID << endl
        ...
        if (Owner)
        {
                ...
                (status & Plastic_Image::Rendering_Monitor::RENDERED_DATA_MASK))
            {
                ...
                if (Owner->Active_Tile->is_high_priority())
                {
                    ...
                    char input[4];
                    clog << "vis. tile " << Owner->Active_Tile->Tile_Coordinate << " region " << tile_region << " > ";
                    cin.getline(input, 2);
                    if (input[0] == 'q')
                        exit(7);
#endif
...
#if ((DEBUG_SECTION) & (DEBUG_RENDER | DEBUG_NOTIFY | DEBUG_NOTIFY_VISIBLE | DEBUG_LOCATION))
        clog << "<++ Image_Renderer::Image_Renderer_Rendering_Monitor::notification " << thread_ID << ": true" << endl;
#endif
        return true;
    }
...
void Image_Renderer::send_status_notice(const QString &message)
{
#if ((DEBUG_SECTION) & (DEBUG_NOTIFY | DEBUG_SIGNALS | DEBUG_STATUS_NOTICE))
    void *thread_ID = (void *)QThread::currentThreadId();
    LOCKED_LOGGING((clog << "    Image_Renderer::Image_Renderer_Rendering_Monitor::notification " << thread_ID
                         << ": emit status_notice \"" << message << '"' << endl));
#endif
    //	>>> SIGNAL <<<
    emit status_notice(message);
}

void Image_Renderer::send_rendered(const QPoint &tile_coordinate, const QRect &tile_region)
{
...
namespace
{
void print_queue(const Image_Renderer::Tile_Queue &queue)
{
...
void Image_Renderer::print_render_queue() const
{
    QMutexLocker qLocker(&Queue_Lock);
    QString pathname(object_pathname(this));
    LOCK_LOG;
    if (Active_Tile)
        clog << "    Image_Renderer::Active_Tile -" << endl << "    " << *Active_Tile << endl;
    clog << "    Image_Renderer::Render_Queue" << endl << "    in " << pathname << endl;
    print_queue(Render_Queue);
    UNLOCK_LOG;
}

void Image_Renderer::print_delete_queue() const
{
    QMutexLocker qLocker(&Queue_Lock);
    QString pathname(object_pathname(this));
    LOCK_LOG;
    clog << "    Image_Renderer::Delete_Queue" << endl << "    in " << pathname << endl;
...
void Image_Renderer::image_accounting()
{
    ...
    if (Image_Accounting.size())
    {
        ...
        for (int index = 0; index < Image_Accounting.size(); index++)
        {
            if (Image_Accounting[index] == Source_Image)
                clog << "SSS ";
            else if (Image_Accounting[index] == Reference_Image)
                clog << "RRR ";
            else
                clog << "    ";
...
/*==============================================================================
    Constructors
*/
Image_Renderer::Image_Renderer(QObject *parent)
    : QObject(parent), Suspended(true), Runnable(false), Finish(false), Immediate_Mode(false),
      Source_Image(new Plastic_QImage()), Max_Source_Image_Area(Default_Max_Source_Image_Area),
      Reference_Image(Source_Image->clone(QSize(0, 0))), Active_Tile(NULL), Cancel(false),
      Image_Rendering_Monitor(new Image_Renderer_Rendering_Monitor(this))
{
    ...
    LOCKED_LOGGING((clog << ">-< Image_Renderer @ " << (void *)this << ": " << object_pathname(this) << endl
...
Image_Renderer::~Image_Renderer()
{
...
#endif
#if ((DEBUG_SECTION) & (DEBUG_CONSTRUCTORS | DEBUG_IMAGE_ACCOUNTING))
    LOCKED_LOGGING((clog << ">>> ~Image_Renderer @ " << (void *)this << ": " << pathname << endl));
#endif
    ...
    */
    ...
    {
#if ((DEBUG_SECTION) & (DEBUG_CONSTRUCTORS | DEBUG_IMAGE_ACCOUNTING))
        LOCKED_LOGGING((clog << "    delete the Reference_Image -" << endl << "    " << *Reference_Image << endl));
#endif
        delete Reference_Image;
#if defined(DEBUG_SECTION) && DEBUG_SECTION != 0
        if (!Image_Accounting.removeAll(Reference_Image))
        {
            LOCKED_LOGGING((clog << "!!! Reference_Image not accounted for" << endl << "    in " << pathname << endl));
        }
#endif
    }
    ...
    LOCK_LOG;
    image_accounting();
    clog << "    in " << pathname << endl << "<<< ~Image_Renderer @ " << (void *)this << endl;
    UNLOCK_LOG;
#endif
}
...
void Image_Renderer::immediate_mode(bool enable)
{
#if ((DEBUG_SECTION) & DEBUG_ACCESSORS)
    LOCKED_LOGGING((clog << ">-< Image_Renderer::immediate_mode " << (void *)QThread::currentThreadId() << ": "
...
bool Image_Renderer::immediate_mode() const
{
#if ((DEBUG_SECTION) & DEBUG_ACCESSORS)
    void *thread_ID = (void *)QThread::currentThreadId();
...
#if ((DEBUG_SECTION) & DEBUG_ACCESSORS)
    LOCKED_LOGGING(
        (clog << "<<< Image_Renderer::immediate_mode " << thread_ID << ": " << boolalpha << immediate_mode << endl));
#endif
...
/*==============================================================================
    Queue management
*/
void Image_Renderer::queue(Plastic_Image *image, const QPoint &tile_coordinate, const QRect &tile_region,
                           bool cancelable)
{
    if (!image)
        return;
    ...
    void *thread_ID = (void *)QThread::currentThreadId();
    QString pathname(object_pathname(this));
    LOCKED_LOGGING((clog << ">>> Image_Renderer::queue " << thread_ID << ": image -" << endl
                         << "    " << *image << endl
                         << "    " << *image_tile << endl
    ...
    {
#if ((DEBUG_SECTION) & DEBUG_QUEUE)
        LOCKED_LOGGING((clog << "    Image_Renderer::queue " << thread_ID << ": adding new tile" << endl
                             << "    in " << pathname << endl));
#endif
        add_tile(image_tile);
    }
...
#if ((DEBUG_SECTION) & DEBUG_QUEUE)
        LOCKED_LOGGING((clog << "    Image_Renderer::queue " << thread_ID << ": already in Render_Queue[" << index
                             << "] -" << endl
                             << "    " << *Render_Queue[index] << endl
                             << "    in " << pathname << endl));
#endif
        if (image_tile->status() == Render_Queue[index]->status() && image_tile->area() == Render_Queue[index]->area())
        {
//	No priority change; replace the current entry.
#if ((DEBUG_SECTION) & DEBUG_QUEUE)
            LOCKED_LOGGING((clog << "    Image_Renderer::queue " << thread_ID << ": replace Render_Queue[" << index
                                 << "] -" << endl
                                 << "    " << *Render_Queue[index] << endl
                                 << "    in " << pathname << endl));
#endif
...
#if ((DEBUG_SECTION) & DEBUG_QUEUE)
            LOCKED_LOGGING((clog << "    Image_Renderer::queue " << thread_ID << ": remove and delete Render_Queue["
                                 << index << "] -" << endl
                                 << "    " << *Render_Queue[index] << endl
                                 << "    in " << pathname << endl));
#endif
...
#if ((DEBUG_SECTION) & DEBUG_QUEUE)
            LOCKED_LOGGING((clog << "    Image_Renderer::queue " << thread_ID << ": add new tile" << endl
                                 << "    in " << pathname << endl));
#endif
            add_tile(image_tile);
        }
...
#if ((DEBUG_SECTION) & DEBUG_QUEUE)
    LOCK_LOG;
    clog << "    Image_Renderer::queue " << thread_ID << endl << "    in " << pathname << endl;
    if (Active_Tile)
        clog << "    Active_Tile -" << endl << "    " << *Active_Tile << endl;
    clog << "    Render_Queue -" << endl;
    print_queue(Render_Queue);
    clog << "    Image_Renderer::queue " << thread_ID << ": unlock Queue_Lock" << endl << "    in " << pathname << endl;
    UNLOCK_LOG;
#endif

#if ((DEBUG_SECTION) & DEBUG_QUEUE)
    LOCKED_LOGGING((clog << "    in " << pathname << endl << "<<< Image_Renderer::queue " << thread_ID << endl));
#endif
}
...
bool Image_Renderer::is_rendering(Plastic_Image *image) const
{
    ...
    bool rendering;
    if (image)
        rendering = Active_Tile && Active_Tile->Image == image;
    else
...
bool Image_Renderer::cancel(Plastic_Image *image, int cancel_options)
{
    if (!image)
        return false;
    ...
    clog << ">>> Image_Renderer::cancel " << thread_ID << ": image -" << endl
         << "    " << *image << endl
         << "    cancel_options = " << cancel_options << " - " << cancel_options_descriptions(cancel_options) << endl
         << "    lock Queue_Lock" << endl;
    UNLOCK_LOG;
#endif
    ...
    QString pathname(object_pathname(this));
    LOCK_LOG;
    clog << "    Image_Renderer::cancel " << thread_ID << endl << "    in " << pathname << endl;
    if (Active_Tile)
        clog << "    Active_Tile -" << endl << "    " << *Active_Tile << endl;
    clog << "    Render_Queue -" << endl;
    print_queue(Render_Queue);
...
//	Cancel rendering of the active tile.
        ...
        LOCKED_LOGGING((clog << "    Image_Renderer::cancel " << thread_ID << ": abort" << endl));
#endif
...
#if ((DEBUG_SECTION) & DEBUG_QUEUE)
            LOCKED_LOGGING((clog << "    Image_Renderer::cancel " << thread_ID << ": queue for deletion Render_Queue["
                                 << index << "] -" << endl
                                 << "    " << *Render_Queue[index] << endl));
#endif
...
#if ((DEBUG_SECTION) & DEBUG_QUEUE)
    LOCKED_LOGGING((clog << "<<< Image_Renderer::cancel " << thread_ID << ": " << boolalpha << canceled << endl));
#endif
    return canceled;
}

bool Image_Renderer::cancel(int cancel_options)
{
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_QUEUE))
    void *thread_ID = (void *)QThread::currentThreadId();
    LOCK_LOG;
    clog << ">>> Image_Renderer::cancel " << thread_ID << ": " << cancel_options << " - "
...
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_QUEUE))
    LOCKED_LOGGING((clog << "    Image_Renderer::cancel " << thread_ID << ": unlock Queue_Lock" << endl));
#endif

#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_QUEUE))
    LOCKED_LOGGING((clog << "<<< Image_Renderer::cancel " << thread_ID << ": " << boolalpha << done << endl));
#endif
    return done;
}

bool Image_Renderer::reset(int cancel_options)
{
#if ((DEBUG_SECTION) & DEBUG_QUEUE)
    void *thread_ID = (void *)QThread::currentThreadId();
    QString pathname(object_pathname(this)), description(cancel_options_descriptions(cancel_options));
    LOCKED_LOGGING((clog << ">>> Image_Renderer::reset " << thread_ID << ": " << cancel_options << " - " << description
                         << endl
                         << "    in " << pathname << endl
                         << "    lock Queue_Lock" << endl));
#endif
    ...
    bool done = clear(cancel_options);
#if ((DEBUG_SECTION) & DEBUG_QUEUE)
    LOCKED_LOGGING((clog << "    Image_Renderer::reset " << thread_ID << ": unlock Queue_Lock" << endl
                         << "    in " << pathname << endl));
#endif

#if ((DEBUG_SECTION) & DEBUG_QUEUE)
    LOCKED_LOGGING((clog << "    in " << pathname << endl
                         << "<<< Image_Renderer::reset " << thread_ID << ": " << boolalpha << done << endl));
#endif
    return done;
}
...
void Image_Renderer::add_tile(Image_Tile *image_tile)
{
//	>>> CAUTION: The Queue_Lock is expected to be locked.
    ...
    QString pathname(object_pathname(this));
    LOCK_LOG;
    clog << ">>> Image_Renderer::add_tile " << thread_ID << ": " << *image_tile << endl
         << "    in " << pathname << endl;
    if (Active_Tile)
        clog << "    Active_Tile -" << endl << "    " << *Active_Tile << endl;
    clog << "    Render_Queue -" << endl;
    ...
    {
#if ((DEBUG_SECTION) & DEBUG_QUEUE)
        LOCKED_LOGGING((clog << "    add tile with image of Active_Tile -" << endl << "    " << *Active_Tile << endl));
#endif
    ...
    if (Render_Queue.isEmpty() || image_tile->is_low_priority() || image_tile->Tile_Region.isEmpty())
    {
//	Empty queue or low priority tile.
#if ((DEBUG_SECTION) & DEBUG_QUEUE)
        LOCKED_LOGGING((clog << "    Image_Renderer::add_tile " << thread_ID << ": append "
...
#if ((DEBUG_SECTION) & DEBUG_QUEUE)
        LOCKED_LOGGING((clog << "    Image_Renderer::add_tile " << thread_ID << ": insert tile at Render_Queue index "
                             << index << endl
                             << "    in " << pathname << endl));
#endif
...
//	Start rendering.
#if ((DEBUG_SECTION) & DEBUG_QUEUE)
        LOCKED_LOGGING((clog << "    Image_Renderer::add_tile " << thread_ID << ": start rendering" << endl
                             << "    in " << pathname << endl));
#endif
        start_rendering();
    }
    ...
    clog << "    Render_Queue -" << endl;
    print_queue(Render_Queue);
    clog << "    in " << pathname << endl << "<<< Image_Renderer::add_tile " << thread_ID << endl;
    UNLOCK_LOG;
#endif
}

bool Image_Renderer::clear(int cancel_options)
{
//	>>> CAUTION: The Queue_Lock is expected to be locked.
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_QUEUE))
    void *thread_ID = (void *)QThread::currentThreadId();
    QString pathname(object_pathname(this)), description(cancel_options_descriptions(cancel_options));
    LOCK_LOG;
    clog << ">>> Image_Renderer::clear " << thread_ID << ": " << cancel_options << " - " << description << endl
         << "    in " << pathname << endl;
    if (Active_Tile)
        clog << "    Active_Tile -" << endl << "    " << *Active_Tile << endl;
    clog << "    Render_Queue -" << endl;
...
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_QUEUE))
            LOCKED_LOGGING((clog << "    Image_Renderer::clear " << thread_ID << ": queue for deletion Render_Queue["
                                 << index << "] -" << endl
                                 << "    " << *Render_Queue[index] << endl
                                 << "    in " << pathname << endl));
#endif
...
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_QUEUE))
    LOCK_LOG;
    clog << "    Image_Renderer::clear " << thread_ID << endl << "    in " << pathname << endl;
    if (Active_Tile)
        clog << "    Active_Tile -" << endl << "    " << *Active_Tile << endl;
    ...
    if (!done && (cancel_options & WAIT_UNTIL_DONE))
    {
        ...
        LOCKED_LOGGING((clog << "    Image_Renderer::clear " << thread_ID
                             << ": emit error - "
...
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_QUEUE))
    LOCKED_LOGGING((clog << "    in " << pathname << endl
                         << "<<< Image_Renderer::clear " << thread_ID << ": " << boolalpha << done << endl));
#endif
    return done;
}

bool Image_Renderer::abort(int cancel_options)
{
//	>>> CAUTION: The Queue_Lock is expected to be locked.
#if ((DEBUG_SECTION) & DEBUG_QUEUE)
    void *thread_ID = (void *)QThread::currentThreadId();
    QString pathname(object_pathname(this)), description(cancel_options_descriptions(cancel_options));
    LOCKED_LOGGING((clog << ">>> Image_Renderer::abort " << thread_ID << ": " << cancel_options << " - " << description
                         << endl
                         << "    in " << pathname << endl));
#endif
    bool done = true;
    ...
    {
#if ((DEBUG_SECTION) & DEBUG_QUEUE)
        LOCKED_LOGGING((clog << "    Image_Renderer::abort " << thread_ID << ": cancel rendering for " << *Active_Tile
                             << endl
                             << "    in " << pathname << endl));
#endif
...
//	Wait for any rendering to complete.
#if ((DEBUG_SECTION) & DEBUG_QUEUE)
            LOCKED_LOGGING((clog << "    Image_Renderer::abort " << thread_ID << ": try lock Rendering_Lock" << endl
                                 << "    in " << pathname << endl
            ...
            {
#if ((DEBUG_SECTION) & DEBUG_QUEUE)
                LOCKED_LOGGING((clog << "    Image_Renderer::abort " << thread_ID << ": unlock Rendering_Lock" << endl
                                     << "    in " << pathname << endl));
#endif
            ...
            {
#if ((DEBUG_SECTION) & DEBUG_QUEUE)
                LOCKED_LOGGING((clog << "!!! Image_Renderer::abort " << thread_ID
                                     << ": failed to obtain Rendering_Lock after " << Wait_Seconds << " seconds!"
                                     << endl
                                     << "    in " << pathname << endl));
#endif
                done = false;
...
#if ((DEBUG_SECTION) & DEBUG_QUEUE)
            LOCKED_LOGGING((clog << "    Image_Renderer::abort " << thread_ID << ": didn't wait" << endl
                                 << "    in " << pathname << endl));
#endif
            done = false;
        }
...
#if ((DEBUG_SECTION) & DEBUG_QUEUE)
    LOCKED_LOGGING((clog << "    in " << pathname << endl
                         << "<<< Image_Renderer::abort " << thread_ID << ": " << boolalpha << done << endl));
#endif
    return done;
}

bool Image_Renderer::delete_image(Plastic_Image *image)
{
    if (!image)
        return false;
    ...
    void *thread_ID = (void *)QThread::currentThreadId();
    LOCKED_LOGGING((clog << ">>> Image_Renderer::delete_image " << thread_ID << ": image -" << endl
                         << "    " << *image << endl
                         << "    lock Queue_Lock" << endl));
#endif
    QMutexLocker qLocker(&Queue_Lock);
    ...
    {
#if ((DEBUG_SECTION) & (DEBUG_DELETE_TILES | DEBUG_QUEUE))
        LOCKED_LOGGING((clog << "    Image_Renderer::delete_image " << thread_ID << ": queue for deletion Render_Queue["
                             << index << "] -" << endl
                             << "    " << *Render_Queue[index] << endl));
#endif
        Render_Queue[index]->Delete_Image_When_Done = true;
    ...
    {
#if ((DEBUG_SECTION) & (DEBUG_DELETE_TILES | DEBUG_QUEUE))
        LOCKED_LOGGING((clog << "    Image_Renderer::delete_image " << thread_ID << ": Active_Tile -" << endl
                             << "    " << *Active_Tile << endl));
#endif
    ...
    {
#if ((DEBUG_SECTION) & (DEBUG_DELETE_TILES | DEBUG_QUEUE))
        LOCKED_LOGGING((clog << "    Image_Renderer::delete_image " << thread_ID
                             << ": tile image and queue for deletion" << endl));
#endif
        delete_tile(new Image_Tile(image, QPoint(), true, true));
    }
#if ((DEBUG_SECTION) & (DEBUG_DELETE_TILES | DEBUG_QUEUE))
    LOCKED_LOGGING((clog << "    Image_Renderer::delete_image " << thread_ID << ": unlock Queue_Lock" << endl));
#endif

#if ((DEBUG_SECTION) & (DEBUG_DELETE_TILES | DEBUG_QUEUE))
    LOCKED_LOGGING((clog << "<<< Image_Renderer::delete_image " << thread_ID << ": " << boolalpha << deleted << endl));
#endif
    return deleted;
}

void Image_Renderer::delete_tile(Image_Tile *image_tile)
{
//	>>> CAUTION: The Queue_Lock is expected to be locked.
#if ((DEBUG_SECTION) & (DEBUG_DELETE_TILES | DEBUG_QUEUE))
    void *thread_ID = (void *)QThread::currentThreadId();
    QString pathname(object_pathname(this));
    LOCKED_LOGGING((clog << ">>> Image_Renderer::delete_tile " << thread_ID << ": " << *image_tile << endl
                         << "    in " << pathname << endl));
#endif
...
#if ((DEBUG_SECTION) & (DEBUG_DELETE_TILES | DEBUG_QUEUE))
            LOCKED_LOGGING((clog << "    Image_Renderer::delete_tile " << thread_ID
...
//	Different tiles.
#if ((DEBUG_SECTION) & (DEBUG_DELETE_TILES | DEBUG_QUEUE))
                LOCKED_LOGGING((clog << "    Image_Renderer::delete_tile " << thread_ID << ": different image tile -"
                                     << endl
                                     << "    " << *Delete_Queue[index] << endl));
#endif
...
#if ((DEBUG_SECTION) & (DEBUG_DELETE_TILES | DEBUG_QUEUE))
                LOCKED_LOGGING(
                    (clog << "    Image_Renderer::delete_tile " << thread_ID << ": delete image tile" << endl));
#endif
...
#if ((DEBUG_SECTION) & (DEBUG_DELETE_TILES | DEBUG_QUEUE))
    LOCKED_LOGGING((clog << "    Image_Renderer::delete_tile " << thread_ID << ": queue for deletion" << endl));
#endif
...
void Image_Renderer::delete_tiles()
{
//	>>> CAUTION: The Queue_Lock is expected to be locked by the Renderer thread.
#if ((DEBUG_SECTION) & (DEBUG_DELETE_TILES | DEBUG_QUEUE))
    void *thread_ID = (void *)QThread::currentThreadId();
    QString pathname(object_pathname(this));
    LOCKED_LOGGING((clog << ">>> Image_Renderer::delete_tiles " << thread_ID << endl << "    in " << pathname << endl));
#endif
    bool locked = false; // Queue_Lock.tryLock();
#if ((DEBUG_SECTION) & (DEBUG_DELETE_TILES | DEBUG_QUEUE))
    LOCK_LOG;
    clog << "    Image_Renderer::delete_tiles " << thread_ID << ": Queue_Lock was " << (locked ? "not " : "")
         << "locked" << endl
         << "    in " << pathname << endl
         << "    Delete_Queue -" << endl;
    print_queue(Delete_Queue);
...
#endif
#if defined(DEBUG_SECTION) && DEBUG_SECTION != 0
        if (Delete_Queue[index]->Delete_Image_When_Done && !Image_Accounting.removeAll(Delete_Queue[index]->Image))
        {
...
#if ((DEBUG_SECTION) & (DEBUG_DELETE_TILES | DEBUG_QUEUE))
    LOCKED_LOGGING((clog << "    Image_Renderer::delete_tiles " << thread_ID << ": unlock Queue_Lock" << endl
                         << "    in " << pathname << endl));
#endif
...
/*==============================================================================
    Rendering
*/
void Image_Renderer::run_rendering()
{
#if ((DEBUG_SECTION) & DEBUG_RENDER)
    void *thread_ID = (void *)QThread::currentThreadId();
    QString pathname(object_pathname(this));
    LOCKED_LOGGING((clog << ">>> Image_Renderer::run_rendering " << thread_ID << endl
                         << "    in " << pathname << endl));
#endif
    Finish = false;
    ...
    */
...
#if ((DEBUG_SECTION) & DEBUG_RENDER)
    LOCKED_LOGGING((clog << "    Image_Renderer::run_rendering " << thread_ID << ": enter the rendering loop ..."
                         << endl
                         << "    in " << pathname << endl));
#endif
    render();
    ...
    {
#if ((DEBUG_SECTION) & DEBUG_RENDER)
        LOCKED_LOGGING((clog << "    Image_Renderer::run_rendering " << thread_ID << ": relock Queue_Lock" << endl
                             << "    in " << pathname << endl));
#endif
        Queue_Lock.lock();
    }
...
bool Image_Renderer::is_ready()
{
...
#endif
#if ((DEBUG_SECTION) & DEBUG_RENDER)
    LOCKED_LOGGING((clog << ">>> Image_Renderer::is_ready " << thread_ID << endl
    ...
    {
#if ((DEBUG_SECTION) & (DEBUG_RENDER | DEBUG_SIGNALS | DEBUG_STATUS))
        LOCKED_LOGGING((clog << "^^^ Image_Renderer::is_ready " << thread_ID << ": emit status " << NOT_RENDERING
...
#if ((DEBUG_SECTION) & DEBUG_RENDER)
    LOCKED_LOGGING((clog << "    Image_Renderer::is_ready " << thread_ID << ": unlock Ready_Lock" << endl
                         << "    in " << pathname << endl));
#endif
    Ready_Lock.unlock();
#if ((DEBUG_SECTION) & DEBUG_RENDER)
    LOCKED_LOGGING((clog << "    in " << pathname << endl
                         << "<<< Image_Renderer::is_ready " << thread_ID << ": " << boolalpha << (Runnable && !Finish)
                         << endl));
#endif
...
#endif
#if ((DEBUG_SECTION) & DEBUG_RENDER)
    LOCKED_LOGGING((clog << ">>> Image_Renderer::render " << thread_ID << endl << "    in " << pathname << endl));
#endif
...
//	Acquire the Queue_Lock.
#if ((DEBUG_SECTION) & DEBUG_RENDER)
        LOCKED_LOGGING((clog << "    Image_Renderer::render " << thread_ID << ": lock Queue_Lock" << endl
                             << "    in " << pathname << endl));
#endif
        Queue_Lock.lock();
        ...
        if (Render_Queue.isEmpty())
        {
            ...
            LOCKED_LOGGING((clog << "    Image_Renderer::render " << thread_ID << ": empty Render_Queue" << endl
                                 << "    suspend_rendering" << endl
                                 << "    in " << pathname << endl));
#endif
            suspend_rendering();
#if ((DEBUG_SECTION) & DEBUG_RENDER)
            LOCKED_LOGGING((clog << "    Image_Renderer::render " << thread_ID << ": unlock Queue_Lock" << endl
                                 << "    in " << pathname << endl));
#endif
            Queue_Lock.unlock();
            continue;
        ...
        LOCK_LOG;
        clog << "--> Image_Renderer::render " << thread_ID << endl << "    in " << pathname << endl;
        if (Active_Tile)
            clog << "    Active_Tile -" << endl << "    " << *Active_Tile << endl;
        clog << "    Render_Queue -" << endl;
        print_queue(Render_Queue);
        ...
        */
        ...
        if (!(Active_Tile && Active_Tile->Image))
            continue;
...
#endif
#if ((DEBUG_SECTION) & (DEBUG_RENDER | DEBUG_QUEUE | DEBUG_OVERVIEW))
        LOCKED_LOGGING((clog << "    Image_Renderer::render " << thread_ID << ": Active_Tile " << *Active_Tile << endl
        ...
        mark_image(Active_Tile->Image, label, Active_Tile->Image->height() - TILE_MARKINGS_ANTE_Y - 12,
                   TILE_MARKINGS_COLOR);
#endif

//	Rendering begins .......................................................
#if ((DEBUG_SECTION) & DEBUG_RENDER)
        LOCKED_LOGGING((clog << "    Image_Renderer::render " << thread_ID << ": lock Rendering_Lock" << endl
                             << "    in " << pathname << endl));
#endif
        Rendering_Lock.lock();
...
*/
        ...
        {
#if ((DEBUG_SECTION) & (DEBUG_RENDER | DEBUG_SIGNALS | DEBUG_STATUS))
            LOCKED_LOGGING((clog << "^^^ Image_Renderer::render " << thread_ID << ": emit status " << tile_status
...
//	Render the tile.
#if ((DEBUG_SECTION) & (DEBUG_RENDER | DEBUG_OVERVIEW))
            LOCKED_LOGGING((clog << "==> Image_Renderer::render " << thread_ID << ": rendering -" << endl
            ...
            catch (Plastic_Image::Render_Exception &except)
            {
                ...
                LOCKED_LOGGING((clog << "^^^ Image_Renderer::render " << thread_ID << ": emit error -" << endl
                                     << except.message() << endl
                                     << "    in " << pathname << endl));
#endif
                //	>>> SIGNAL<<<
...
#if ((DEBUG_SECTION) & DEBUG_RENDER)
            LOCKED_LOGGING((clog << "<== Image_Renderer::render " << thread_ID << ": rendering complete " << boolalpha
                                 << complete << endl
            ...
            mark_image(Active_Tile->Image, label, Active_Tile->Image->height() - TILE_MARKINGS_POST_Y - 12,
                       TILE_MARKINGS_COLOR);
#endif
        }
...
*/
#if ((DEBUG_SECTION) & DEBUG_RENDER)
        LOCKED_LOGGING((clog << "    Image_Renderer::render " << thread_ID << ": relock Queue_Lock" << endl
                             << "    in " << pathname << endl));
#endif
        Queue_Lock.lock();
        ...
        canceled |= Cancel;
#if ((DEBUG_SECTION) & (DEBUG_RENDER | DEBUG_OVERVIEW))
        LOCKED_LOGGING((clog << "<-- Image_Renderer::render " << thread_ID << ": rendering "
                             << (canceled ? "canceled " : "") << (complete ? "complete" : "incomplete") << " -" << endl
                             << "    " << *(Active_Tile->Image) << endl
                             << "    in " << pathname << endl));
#endif
#if ((DEBUG_SECTION) & DEBUG_TILE_MARKINGS)
        if (canceled || !complete)
        {
            ...
            mark_image(Active_Tile->Image, label, TILE_MARKINGS_CANCELED_Y, TILE_MARKINGS_CANCELED_COLOR,
                       Qt::transparent);
            mark_image(Active_Tile->Image, label, Active_Tile->Image->height() - TILE_MARKINGS_CANCELED_Y - 12,
        ...
        else
        {
            if (tile_status == RENDERING_HIGH_PRIORITY || !Active_Tile->Cancelable)
            {
                ...
                {
#if ((DEBUG_SECTION) & (DEBUG_RENDER | DEBUG_SIGNALS))
                    LOCKED_LOGGING((clog << "^^^ Image_Renderer::render " << thread_ID << ": emit rendered "
                                         << *Active_Tile << endl
                                         << "    in " << pathname << endl));
#endif
                    //	>>> SIGNAL <<<
                ...
                {
#if ((DEBUG_SECTION) & (DEBUG_RENDER | DEBUG_SIGNALS))
                    LOCKED_LOGGING((clog << "^^^ Image_Renderer::render " << thread_ID
...
#if ((DEBUG_SECTION) & DEBUG_RENDER)
        LOCKED_LOGGING((clog << "    Image_Renderer::render " << thread_ID << ": queue for deletion Active_Tile "
                             << *Active_Tile << endl
                             << "    in " << pathname << endl));
#endif
...
#if ((DEBUG_SECTION) & DEBUG_RENDER)
    LOCKED_LOGGING((clog << "    in " << pathname << endl << "<<< Image_Renderer::render " << thread_ID << endl));
#endif
}

void Image_Renderer::start_rendering()
{
#if ((DEBUG_SECTION) & (DEBUG_RENDER | DEBUG_OVERVIEW))
    void *thread_ID = (void *)QThread::currentThreadId();
    QString pathname(object_pathname(this));
#endif
#if ((DEBUG_SECTION) & (DEBUG_RENDER | DEBUG_OVERVIEW))
    LOCKED_LOGGING((clog << ">>> Image_Renderer::start_rendering " << thread_ID << endl
                         << "    in " << pathname << endl));
#endif
#if ((DEBUG_SECTION) & DEBUG_RENDER)
    LOCKED_LOGGING((clog << "    Image_Renderer::start_rendering " << thread_ID << ": lock Ready_Lock" << endl
                         << "    in " << pathname << endl));
#endif
    Ready_Lock.lock();
#if ((DEBUG_SECTION) & DEBUG_RENDER)
...
bool Image_Renderer::suspend_rendering(bool wait)
{
#if ((DEBUG_SECTION) & DEBUG_RENDER)
    void *thread_ID = (void *)QThread::currentThreadId();
    QString pathname(object_pathname(this));
    LOCKED_LOGGING((clog << ">>> Image_Renderer::suspend_rendering " << thread_ID << ": wait = " << boolalpha << wait
                         << endl
                         << "    in " << pathname << endl));
#endif
...
//	Wait for any rendering to complete.
#if ((DEBUG_SECTION) & DEBUG_RENDER)
        LOCKED_LOGGING((clog << "    Image_Renderer::suspend_rendering " << thread_ID << ": try lock Rendering_Lock"
                             << endl
                             << "    in " << pathname << endl
                             << "    wait up to " << Wait_Seconds << " seconds" << endl));
#endif
        ...
        {
#if ((DEBUG_SECTION) & DEBUG_RENDER)
            LOCKED_LOGGING((clog << "    Image_Renderer::suspend_rendering " << thread_ID << ": unlock Rendering_Lock"
                                 << endl
                                 << "    in " << pathname << endl));
#endif
            Rendering_Lock.unlock();
...
#if ((DEBUG_SECTION) & DEBUG_RENDER)
    LOCKED_LOGGING((clog << "    in " << pathname << endl
                         << "<<< Image_Renderer::suspend_rendering " << thread_ID << ": " << boolalpha << done
                         << endl));
#endif
    return done;
}

bool Image_Renderer::suspended() const
{
    QMutexLocker ready_lock(&Ready_Lock);
    return Suspended;
}

bool Image_Renderer::stop_rendering(bool wait)
{
#if ((DEBUG_SECTION) & DEBUG_RENDER)
    void *thread_ID = (void *)QThread::currentThreadId();
    QString pathname(object_pathname(this));
    LOCKED_LOGGING((clog << ">>> Image_Renderer::stop_rendering " << thread_ID << ": wait = " << boolalpha << wait
                         << endl
                         << "    in " << pathname << endl
                         << "    Image_Renderer::stop_rendering " << thread_ID << ": unlock Ready_Lock" << endl));
#endif
    Ready_Lock.lock();
...
#if ((DEBUG_SECTION) & DEBUG_RENDER)
    LOCKED_LOGGING((clog << "    in " << pathname << endl
                         << "<<< Image_Renderer::stop_rendering " << thread_ID << ": " << boolalpha << done << endl));
#endif
    return done;
}

bool Image_Renderer::finish(int cancel_options)
{
#if ((DEBUG_SECTION) & (DEBUG_CONSTRUCTORS | DEBUG_RENDER))
    void *thread_ID = (void *)QThread::currentThreadId();
    QString pathname(object_pathname(this)), description(cancel_options_descriptions(cancel_options));
    LOCKED_LOGGING((clog << ">>> Image_Renderer::finish " << thread_ID << ": " << cancel_options << " - " << description
                         << endl
                         << "    in " << pathname << endl));
#endif
...
/*==============================================================================
    Source image
*/
bool Image_Renderer::image(const QString &source_name)
{
#if ((DEBUG_SECTION) & (DEBUG_LOAD_IMAGE | DEBUG_OVERVIEW))
    void *thread_ID = (void *)QThread::currentThreadId();
    QString pathname(object_pathname(this));
    LOCKED_LOGGING((clog << ">>> Image_Renderer::image " << thread_ID << ": \"" << source_name << '"' << endl
                         << "    in " << pathname << endl));
#endif
    ...
    if (!source_name.isEmpty())
    {
        ...
        LOCKED_LOGGING((clog << "    Image_Renderer::image " << thread_ID << ": lock Queue_Lock" << endl
                             << "    in " << pathname << endl));
#endif
        QMutexLocker qLocker(&Queue_Lock);
#if ((DEBUG_SECTION) & DEBUG_LOAD_IMAGE)
...
#if ((DEBUG_SECTION) & DEBUG_LOAD_IMAGE)
        LOCKED_LOGGING((clog << "    Image_Renderer::image " << thread_ID << ": unlock Queue_Lock" << endl
                             << "    in " << pathname << endl));
#endif
    }
#if ((DEBUG_SECTION) & (DEBUG_LOAD_IMAGE | DEBUG_OVERVIEW))
    LOCKED_LOGGING((clog << "    in " << pathname << endl
                         << "<<< Image_Renderer::image " << thread_ID << ": " << boolalpha << registered << endl));
#endif
    return registered;
}

bool Image_Renderer::image(const Shared_Image &source_image)
{
#if ((DEBUG_SECTION) & (DEBUG_LOAD_IMAGE | DEBUG_OVERVIEW))
    void *thread_ID = (void *)QThread::currentThreadId();
    QString pathname(object_pathname(this));
...
void Image_Renderer::load_image()
{
#if ((DEBUG_SECTION) & (DEBUG_LOAD_IMAGE | DEBUG_OVERVIEW | DEBUG_IMAGE_ACCOUNTING | DEBUG_STATUS))
    void *thread_ID = (void *)QThread::currentThreadId();
    QString pathname(object_pathname(this));
#endif
#if ((DEBUG_SECTION) & DEBUG_LOAD_IMAGE)
    LOCKED_LOGGING((clog << ">>> Image_Renderer::load_image " << thread_ID << endl
                         << "    in " << pathname << endl
                         << "    Image_Renderer::load_image " << thread_ID << ": tryLock Queue_Lock" << endl));
#endif
    QMutexLocker qLocker(&Queue_Lock);
    ...
    {
#if ((DEBUG_SECTION) & DEBUG_LOAD_IMAGE)
        LOCKED_LOGGING((clog << "    Image_Renderer::load_image " << thread_ID << ": no image to load" << endl
                             << "    in " << pathname << endl
...
#if ((DEBUG_SECTION) & DEBUG_LOAD_IMAGE)
    LOCKED_LOGGING((clog << "    Image_Renderer::load_image " << thread_ID << ": unlock Queue_Lock" << endl
                         << "    in " << pathname << endl));
#endif

#if ((DEBUG_SECTION) & (DEBUG_LOAD_IMAGE | DEBUG_OVERVIEW | DEBUG_SIGNALS | DEBUG_STATUS))
    LOCKED_LOGGING((clog << "^^^ Image_Renderer::load_image " << thread_ID << ": emit status " << LOADING_IMAGE << " - "
...
//	Lock Queue_Lock during image loading.
#if ((DEBUG_SECTION) & DEBUG_LOAD_IMAGE)
    LOCKED_LOGGING((clog << "    Image_Renderer::load_image " << thread_ID << ": lock Queue_Lock" << endl
                         << "    in " << pathname << endl));
#endif
    // Queue_Lock.lock ();
    ...
    */
    ...
    {
#if ((DEBUG_SECTION) & (DEBUG_LOAD_IMAGE | DEBUG_OVERVIEW))
        LOCKED_LOGGING((clog << "    Image_Renderer::load_image " << thread_ID << ": Image_Source -" << endl
                             << "    " << *Image_Source << endl
                             << "    in " << pathname << endl));
#endif
    ...
    {
#if ((DEBUG_SECTION) & (DEBUG_LOAD_IMAGE | DEBUG_OVERVIEW))
        LOCKED_LOGGING((clog << "    Image_Renderer::load_image " << thread_ID << ": Source_Name \"" << Source_Name
                             << '"' << endl
                             << "    in " << pathname << endl));
#endif
#if ((DEBUG_SECTION) & DEBUG_LOAD_IMAGE)
        LOCKED_LOGGING((clog << "    Image_Renderer::load_image " << thread_ID << ": lock Rendering_Lock" << endl
                             << "    in " << pathname << endl));
#endif
        Rendering_Lock.lock();
...
//	Load the source image from the named source.
#if ((DEBUG_SECTION) & DEBUG_LOAD_IMAGE)
        LOCKED_LOGGING((clog << "    Image_Renderer::load_image " << thread_ID << ": load image \"" << Source_Name
                             << '"' << endl
                             << "    in " << pathname << endl));
#endif
...
#if ((DEBUG_SECTION) & DEBUG_LOAD_IMAGE)
        LOCKED_LOGGING((clog << "    Image_Renderer::load_image " << thread_ID << ": unlock Rendering_Lock" << endl
                             << "    in " << pathname << endl));
#endif
        Rendering_Lock.unlock();
    }
    ...
    if (source_image)
    {
        ...
        LOCKED_LOGGING((clog << "    Image_Renderer::render " << thread_ID << ": new Source_Image -" << endl
                             << "    " << *source_image << endl
                             << "    in " << pathname << endl));
#endif
...
#if ((DEBUG_SECTION) & (DEBUG_LOAD_IMAGE | DEBUG_OVERVIEW))
            LOCKED_LOGGING((clog << "    Image_Renderer::render " << thread_ID << ": old Reference_Image -" << endl
...
#if ((DEBUG_SECTION) & (DEBUG_LOAD_IMAGE | DEBUG_IMAGE_ACCOUNTING | DEBUG_OVERVIEW))
            LOCKED_LOGGING((clog << "    Image_Renderer::render " << thread_ID << ": new Reference_Image -" << endl
                                 << "    " << *Reference_Image << endl
                                 << "    in " << pathname << endl));
#endif
...
#if ((DEBUG_SECTION) & (DEBUG_LOAD_IMAGE | DEBUG_OVERVIEW))
            LOCKED_LOGGING((clog << "    Image_Renderer::render " << thread_ID << ": old Source_Image -" << endl
                                 << "    " << *Source_Image << endl));
#endif
...
#if ((DEBUG_SECTION) & (DEBUG_LOAD_IMAGE | DEBUG_OVERVIEW))
            LOCKED_LOGGING(
                (clog << "    Image_Renderer::load_image " << thread_ID << ": source image clone failed." << endl));
#endif
            // delete source_image;
        }
    }
...
//	Emit signals without Queue_Lock locked.
#if ((DEBUG_SECTION) & (DEBUG_LOAD_IMAGE | DEBUG_OVERVIEW | DEBUG_SIGNALS | DEBUG_STATUS))
    LOCKED_LOGGING((clog << "^^^ Image_Renderer::load_image " << thread_ID << ": emit image_loaded " << boolalpha
                         << loaded << endl
                         << "    in " << pathname << endl));
#endif
    //	>>> SIGNAL <<<
...
#if ((DEBUG_SECTION) & DEBUG_LOAD_IMAGE)
    LOCKED_LOGGING((clog << "    in " << pathname << endl
                         << "<<< Image_Renderer::load_image " << thread_ID << ": loaded = " << boolalpha << loaded
                         << endl));
#endif
}

Plastic_Image *Image_Renderer::load_image(const QString &source_name)
{
#if ((DEBUG_SECTION) & DEBUG_LOAD_IMAGE)
    void *thread_ID = (void *)QThread::currentThreadId();
    QString pathname(object_pathname(this));
    LOCKED_LOGGING((clog << ">>> Image_Renderer::load_image " << thread_ID << ": " << source_name << endl
                         << "    in " << pathname << endl));
#endif
    Plastic_Image *plastic_image = NULL;
    if (!source_name.isEmpty())
    {
...
#if ((DEBUG_SECTION) & DEBUG_LOAD_IMAGE)
                    LOCKED_LOGGING((clog << "    Image_Renderer::load_image " << thread_ID << ": loaded QImage @ "
...
// delete source_image;
#if ((DEBUG_SECTION) & DEBUG_LOAD_IMAGE)
                    LOCKED_LOGGING((clog << "   Image_Renderer::load_image " << thread_ID
            ...
            if (report)
            {
                ...
                LOCKED_LOGGING((clog << "^^^ Image_Renderer::load_image " << thread_ID << ": emit error -" << endl
                                     << error_message << endl
                                     << "    in " << pathname << endl));
#endif
                //	>>> SIGNAL <<<
...
#if ((DEBUG_SECTION) & DEBUG_LOAD_IMAGE)
    LOCKED_LOGGING((clog << "    in " << pathname << endl
                         << "<<< Image_Renderer::load_image " << thread_ID << ": Plastic_Image @ "
                         << (void *)plastic_image << endl));
#endif
    return plastic_image;
}

JP2_Image *Image_Renderer::load_JP2_image(const QString &source_name)
{
#if ((DEBUG_SECTION) & (DEBUG_LOAD_IMAGE | DEBUG_OVERVIEW))
    void *thread_ID = (void *)QThread::currentThreadId();
    QString pathname(object_pathname(this));
    LOCKED_LOGGING((clog << ">>> Image_Renderer::load_JP2_image " << thread_ID << ": " << source_name << endl
                         << "    in " << pathname << endl));
#endif
    QString report;
    JP2_Reader *JP2_reader = NULL;
#if ((DEBUG_SECTION) & DEBUG_LOAD_IMAGE)
    LOCKED_LOGGING((clog << "    Image_Renderer::load_JP2_image " << thread_ID << ": constructing a JP2::reader on \""
    ...
    Error_Report:
        ...
        error_message += "\".\n";
#if ((DEBUG_SECTION) & DEBUG_SIGNALS)
        LOCKED_LOGGING((clog << "^^^ Image_Renderer::load_JP2_image " << thread_ID << ": emit error -" << endl
                             << error_message << endl
                             << "    in " << pathname << endl));
#endif
        //	>>> SIGNAL <<<
        ...
        {
#if ((DEBUG_SECTION) & DEBUG_LOAD_IMAGE)
            LOCKED_LOGGING((clog << "    Image_Renderer::load_JP2_image " << thread_ID << ": delete JP2_Reader @ "
...
#if ((DEBUG_SECTION) & DEBUG_LOAD_IMAGE)
        LOCKED_LOGGING((clog << "    in " << pathname << endl << "<<< Image_Renderer::load_JP2_image: NULL" << endl));
#endif
        return NULL;
    }
#if ((DEBUG_SECTION) & DEBUG_LOAD_IMAGE)
    LOCKED_LOGGING((clog << "    Image_Renderer::load_JP2_image " << thread_ID << ": JP2_Reader @ "
                         << (void *)JP2_reader << " -" << endl
    ...
    double scale = scale_to_area(source_size, max_source_image_area());
#if ((DEBUG_SECTION) & (DEBUG_LOAD_IMAGE | DEBUG_OVERVIEW))
    LOCKED_LOGGING((clog << "    Image_Renderer::load_JP2_image " << thread_ID << ": scale " << scale << " to fit "
                         << max_source_image_area() << " amount " << endl
                         << "    in " << pathname << endl));
#endif
    if (scale < .95)
        source_size *= scale;
#if ((DEBUG_SECTION) & DEBUG_LOAD_IMAGE)
    LOCKED_LOGGING((clog << "    Image_Renderer::load_JP2_image " << thread_ID
...
#if ((DEBUG_SECTION) & (DEBUG_LOAD_IMAGE | DEBUG_OVERVIEW))
    LOCKED_LOGGING((clog << "    Image_Renderer::load_JP2_image " << thread_ID << ": JP2_Image -" << endl
                         << "    " << *JP2_image << endl
                         << "    in " << pathname << endl
...
/*==============================================================================
    Cloned image
*/
Plastic_Image *Image_Renderer::image_clone(const QSize &size, Plastic_Image::Mapping_Type shared_mappings)
{
#if ((DEBUG_SECTION) & DEBUG_CLONE_IMAGE)
    void *thread_ID = (void *)QThread::currentThreadId();
    QString pathname(object_pathname(this)), names(Plastic_Image::mapping_type_names(shared_mappings));
    LOCKED_LOGGING((clog << ">>> Image_Renderer::image_clone " << thread_ID << ":" << endl
                         << "               size = " << size << endl
                         << "    shared_mappings = " << shared_mappings << " - " << names << endl));
#endif
#if ((DEBUG_SECTION) & DEBUG_CLONE_IMAGE)
    LOCKED_LOGGING((clog << "    Image_Renderer::image_clone " << thread_ID << ": cloning Reference image -" << endl
                         << "    " << *Reference_Image << endl
                         << "    in " << pathname << endl));
#endif
    /*	>>> WARNING <<<
...
#if ((DEBUG_SECTION) & DEBUG_CLONE_IMAGE)
    LOCK_LOG;
    clog << "    Image_Renderer::image_clone " << thread_ID << ": cloned_image -" << endl;
    if (cloned_image)
        clog << "    " << *cloned_image << endl;
    else
        clog << "    NULL" << endl;
    clog << "    in " << pathname << endl
         << "<<< Image_Renderer::image_clone " << thread_ID << ": " << (void *)cloned_image << endl;
    UNLOCK_LOG;
#endif
    return cloned_image;
}

Plastic_Image *Image_Renderer::clone_image(Plastic_Image *source_image, const QSize &image_size,
                                           Plastic_Image::Mapping_Type shared_mappings)
{
    ...
    QString pathname(object_pathname(this));
    LOCK_LOG;
    clog << ">>> Image_Renderer::clone_image " << thread_ID << ": source_image -" << endl;
    if (source_image)
        clog << "    " << *source_image << endl;
    else
        clog << "    NULL" << endl;
    ...
    if (source_image)
    {
...
#if ((DEBUG_SECTION) & DEBUG_CLONE_IMAGE)
        LOCKED_LOGGING((clog << "    Image_Renderer::clone_image " << thread_ID << ": clone the source image" << endl
                             << "    in " << pathname << endl));
#endif
        try
        {
        ...
        if (!error_message.isEmpty())
        {
            ...
            if (source_image->source_name().isEmpty())
                ...
                message += QString(" \"") + source_image->source_name() + "\".\n";
            error_message = message + error_message;
#if ((DEBUG_SECTION) & (DEBUG_CLONE_IMAGE | DEBUG_SIGNALS))
            LOCKED_LOGGING((clog << "^^^ Image_Renderer::clone_image " << thread_ID << ": emit error -" << endl
                                 << error_message << endl
                                 << "    in " << pathname << endl));
#endif
            //	>>> SIGNAL <<<
...
#if ((DEBUG_SECTION) & DEBUG_CLONE_IMAGE)
    LOCK_LOG;
    clog << "    Image_Renderer::clone_image: " << thread_ID << endl << "    in " << pathname << endl << "    cloned ";
    if (cloned_image)
        clog << *cloned_image << endl;
    else
...
/*==============================================================================
    Utilities
*/
double Image_Renderer::scale_to_area(const QSize &size, unsigned long area)
{
...
QString Image_Renderer::status_description(int condition)
{
    QString description;
...
QString Image_Renderer::cancel_options_descriptions(int cancel_options)
{
