...
/*==============================================================================
    Constants
*/
const char *const Plastic_Image::ID = "UA::HiRISE::Plastic_Image ($Revision: 2.38 $ $Date: 2014/05/27 17:13:51 $)";
...
#endif
const QImage::Format Plastic_Image::IMAGE_FORMAT = PLASTIC_IMAGE_FORMAT;

#ifndef IMAGE_METADATA_GROUP_NAME
#define IMAGE_METADATA_GROUP_NAME "Image"
#endif
Plastic_Image::Name_String Plastic_Image::IMAGE_METADATA_GROUP = IMAGE_METADATA_GROUP_NAME,
                           Plastic_Image::IMAGE_SOURCE_PARAMETER = "Source",
                           /*
                               These parameter names should be the same as those for JP2_Metadata.
                           */
    Plastic_Image::IMAGE_WIDTH_PARAMETER = "Width", Plastic_Image::IMAGE_HEIGHT_PARAMETER = "Height",
                           Plastic_Image::IMAGE_BANDS_PARAMETER = "Image_Bands",
                           Plastic_Image::VALUE_BITS_PARAMETER = "Value_Bits";

const Plastic_Image::Pixel_Datum Plastic_Image::UNDEFINED_PIXEL_VALUE = (Plastic_Image::Pixel_Datum)-1;

const unsigned int Plastic_Image::MAXIMUM_PIXEL_DATUM_PRECISION = 16;

const Plastic_Image::Mapping_Type Plastic_Image::NO_MAPPINGS = 0, Plastic_Image::IDENTICAL_MAPPINGS = NO_MAPPINGS,
                                  Plastic_Image::BAND_MAP = (1 << 0), Plastic_Image::TRANSFORMS = (1 << 1),
                                  Plastic_Image::DATA_MAPS = (1 << 2),
                                  Plastic_Image::ALL_MAPPINGS = (BAND_MAP | TRANSFORMS | DATA_MAPS);

QString Plastic_Image::mapping_type_names(Mapping_Type mapping_type)
{
    QString names;
    if (mapping_type)
    {
...
const QString Plastic_Image::Rendering_Monitor::Status_Message[] = {
    //	0: INFO_ONLY
    "Information only.",
    //	1: LOW_QUALITY_DATA
    ...
    //	unused
    "Low quality rendering canceled.", "Top quality rendering canceled.", "Rendering canceled.",
    //	8: DONE
    "Data rendering is done.", "", "", "", "", "", "", "",
    //	DATA_MAPPING
    "Mapping source data to display data.", "", "Mapping source data to display data."};
...
QRgb Plastic_Image::Default_Background_Color = QColor(_DEFAULT_BACKGROUND_COLOR_).rgba();

#ifndef DEFAULT_AUTO_UPDATE
#define DEFAULT_AUTO_UPDATE false
#endif
bool Plastic_Image::Default_Auto_Update = DEFAULT_AUTO_UPDATE;

#ifndef DEFAULT_RENDERING_INCREMENT_LINES
#define DEFAULT_RENDERING_INCREMENT_LINES 100
#endif
unsigned int Plastic_Image::Default_Rendering_Increment_Lines = DEFAULT_RENDERING_INCREMENT_LINES;
...
Plastic_Image::Plastic_Image(const QSize &image_size, const unsigned int *band_map, const QTransform **transforms,
    ...
    : QImage((image_size.isValid() ? image_size : QSize(0, 0)), IMAGE_FORMAT), Object_Lock(), Metadata(NULL),
      ...
      Data_Maps(const_cast<Data_Map **>(data_maps)), Locally_Owned_and_Operated(NO_MAPPINGS),
      Mapping_Differences(IDENTICAL_MAPPINGS), Needs_Update(ALL_MAPPINGS), Needs_Update_Shadow(NO_MAPPINGS),
      Rendering_Increment_Lines(Default_Rendering_Increment_Lines), Background_Color(Default_Background_Color),
      Closed(false), Rendering(NULL), Cancel_Update(false)
{
...
#if ((DEBUG_SECTION) & DEBUG_CONSTRUCTORS)
    clog << "    Locally_Owned_and_Operated = " << mapping_type_names(Locally_Owned_and_Operated) << " ("
         << Locally_Owned_and_Operated << ')' << endl
         << "<<< Plastic_Image" << endl;
#endif
}

void Plastic_Image::initialize()
{
#if ((DEBUG_SECTION) & DEBUG_CONSTRUCTORS)
    clog << ">>> Plastic_Image::initialize" << endl
         << "    Locally_Owned_and_Operated = " << mapping_type_names(Locally_Owned_and_Operated) << " ("
         << Locally_Owned_and_Operated << ')' << endl;
#endif
    ...
    */
    ...
    if (Locally_Owned_and_Operated & TRANSFORMS || !Geo_Transforms)
        source_transform_reset();
    if (Locally_Owned_and_Operated & BAND_MAP || !Band_Map)
        source_band_map_reset();
    if (Locally_Owned_and_Operated & DATA_MAPS || !Data_Maps)
...
Plastic_Image::~Plastic_Image()
{
    ...
    if (Locally_Owned_and_Operated & TRANSFORMS && Geo_Transforms)
    {
        ...
        while (++band < 3)
        {
            ...
            {
#if ((DEBUG_SECTION) & DEBUG_CONSTRUCTORS)
                clog << "    delete Geo_Transforms[" << band << "] @ " << (void *)Geo_Transforms[band] << endl;
#endif
    ...
    if (Locally_Owned_and_Operated & DATA_MAPS && Data_Maps)
    {
        ...
        while (++band < 3)
        {
            ...
            {
#if ((DEBUG_SECTION) & DEBUG_CONSTRUCTORS)
                clog << "    delete Data_Maps[" << band << "] @ " << (void *)Data_Maps[band] << endl;
#endif
...
/*==============================================================================
    Metadata
*/
bool Plastic_Image::source_name(const QString &name)
{
// QMutexLocker
//	object_lock (&Object_Lock);
    ...
    if (!closed())
    {
        ...
        if ((changed = Source_Name != name))
        {
            ...
            if (Image_Metadata)
            {
                Parameter *parameter(Image_Metadata->find(IMAGE_SOURCE_PARAMETER));
#if ((DEBUG_SECTION) & DEBUG_METADATA)
                clog << "    Metadata @ " << (void *)Metadata << endl
                     << "    " << IMAGE_SOURCE_PARAMETER << " parameter " << (parameter ? "updated" : "added") << endl;
#endif
                if (parameter)
...
idaeim::PVL::Aggregate *Plastic_Image::metadata()
{
#if ((DEBUG_SECTION) & DEBUG_METADATA)
...
#endif
        (*Image_Metadata)
            .add(Assignment(IMAGE_SOURCE_PARAMETER) = String(source_name().toStdString(), String::TEXT))
            .add(Assignment(IMAGE_WIDTH_PARAMETER) = Integer(image_size.width(), Value::UNSIGNED).units("columns"))
            .add(Assignment(IMAGE_HEIGHT_PARAMETER) = Integer(image_size.height(), Value::UNSIGNED).units("rows"))
            .add(Assignment(IMAGE_BANDS_PARAMETER) = Integer(source_bands(), Value::UNSIGNED))
            .add(Assignment(VALUE_BITS_PARAMETER) = Integer(source_precision_bits(), Value::UNSIGNED).units("bits"));
        Metadata->poke_back(Image_Metadata);
#if ((DEBUG_SECTION) & DEBUG_METADATA)
        clog << "          Metadata -" << endl << *Metadata;
#endif
    }
#if ((DEBUG_SECTION) & DEBUG_METADATA)
...
bool Plastic_Image::add_metadata_monitor(Metadata_Monitor *monitor)
{
    bool added = false;
    if (monitor)
    {
...
Plastic_Image::Triplet Plastic_Image::source_pixel(const QPoint &point) const
{
    unsigned int x = static_cast<unsigned int>(point.x()), y = static_cast<unsigned int>(point.y());
    return Triplet(source_pixel_value(x, y, 0), source_pixel_value(x, y, 1), source_pixel_value(x, y, 2));
}

/*------------------------------------------------------------------------------
    Source band by display band mapping
*/
bool Plastic_Image::source_band_map(const unsigned int *band_map, bool shared)
{
    ...
    bool changed = false;
    unsigned int band = -1, bands = source_bands();
...
//	No shared map; initialize the local map.
    ...
    while (++band < 3)
    {
        if (band_map[band] >= bands)
        {
            ...
            message << ID << endl
                    ...
                    << "The image source only has " << bands << " band" << plural(bands) << '.';
    ...
    if (shared)
    {
        ...
        clog << "    delete Band_Map @ " << (void *)Band_Map << endl;
#endif
        if (band_map != Band_Map && (Locally_Owned_and_Operated & BAND_MAP))
        {
#if ((DEBUG_SECTION) & DEBUG_BAND_MAP)
            clog << "    delete Band_Map @ " << (void *)Band_Map << endl;
#endif
...
bool Plastic_Image::source_band_map_reset()
{
#if ((DEBUG_SECTION) & DEBUG_BAND_MAP)
...
bool Plastic_Image::reset_band_map(unsigned int *band_map)
{
#if ((DEBUG_SECTION) & DEBUG_BAND_MAP)
    ...
    if (band_map)
    {
        unsigned int band = 0, bands = source_bands();
#if ((DEBUG_SECTION) & DEBUG_BAND_MAP)
...
#if ((DEBUG_SECTION) & DEBUG_BAND_MAP)
        clog << "    band_map @ " << (void *)band_map << ": " << band_map[0] << ", " << band_map[1] << ", "
             << band_map[2] << endl;
#endif
    }
#if ((DEBUG_SECTION) & DEBUG_BAND_MAP)
    clog << "    band map" << band_map << endl << "<<< Plastic_Image::reset_band_map: " << changed << endl;
#endif
    return changed;
}
...
*/
bool Plastic_Image::set_source_transform(const QTransform &transform, int band)
{
...
bool Plastic_Image::source_transforms(const QTransform **transforms, bool shared)
{
#if ((DEBUG_SECTION) & DEBUG_TRANSFORMS)
    ...
    bool changed = false, locally_owned = Locally_Owned_and_Operated & TRANSFORMS;
    int band = -1;
    ...
    if (transforms)
    {
        if (shared)
        {
            ...
            while (++band < 3)
            {
                ...
                if (locally_owned && Geo_Transforms[band])
                    delete Geo_Transforms[band];
            }
...
QTransform *Plastic_Image::source_transform(int band) const
{
#if ((DEBUG_SECTION) & DEBUG_TRANSFORMS)
...
QTransform Plastic_Image::invert_keeping_origin(const QTransform &transform)
{
    QTransform xform(transform.m11(), transform.m12(), transform.m13(), transform.m21(), transform.m22(),
                     transform.m23(),
    ...
    {
        ostringstream message;
        message << ID << endl << "Unable to invert the geometric transformation -" << transform;
        throw runtime_error(message.str());
    }
    //	Restore the origin offset.
    xform.setMatrix(xform.m11(), xform.m12(), xform.m13(), xform.m21(), xform.m22(), xform.m23(), transform.m31(),
                    transform.m32(), xform.m33());
    return xform;
}

QTransform Plastic_Image::invert_clipping_origin(const QTransform &transform)
{
    QTransform xform(transform.m11(), transform.m12(), transform.m13(), transform.m21(), transform.m22(),
                     transform.m23(),
                     //	Remove the origin offset.
    ...
    xform.setMatrix(xform.m11(), xform.m12(), xform.m13(), xform.m21(), xform.m22(), xform.m23(),
                    ((transform.m31() < 0.0) ? (floor(transform.m31()) - transform.m31())
                                             : (transform.m31() - floor(transform.m31()))),
                    ((transform.m32() < 0.0) ? (floor(transform.m32()) - transform.m32())
                                             : (transform.m32() - floor(transform.m32()))),
                    xform.m33());
    return xform;
}

bool Plastic_Image::source_transform_reset(int band)
{
#if ((DEBUG_SECTION) & DEBUG_TRANSFORMS)
...
bool Plastic_Image::source_origin(const QPointF &origin, int band)
{
#if ((DEBUG_SECTION) & DEBUG_TRANSFORMS)
    ...
    bool changed = false, do_update = auto_update(false);
    ...
    while (band < bands)
    {
        ...
        if (origin.x() != transform->m31() || origin.y() != transform->m32())
        {
            transform->setMatrix(transform->m11(), transform->m12(), transform->m13(), transform->m21(),
                                 transform->m22(), transform->m23(), origin.x(), origin.y(), transform->m33()),
                changed = true;
        }
        ++band;
    }
...
#if ((DEBUG_SECTION) & DEBUG_TRANSFORMS)
    clog << "<<< Plastic_Image::source_origin: " << static_cast<int>(Geo_Transforms[band]->m31()) << "x, "
         << static_cast<int>(Geo_Transforms[band]->m32()) << 'y' << endl;
#endif
    return QPointF(Geo_Transforms[band]->m31(), Geo_Transforms[band]->m32());
}

bool Plastic_Image::source_scaling(double scale_horizontal, double scale_vertical, int band)
{
#if ((DEBUG_SECTION) & DEBUG_TRANSFORMS)
    ...
    while (band < bands)
    {
        transform = Geo_Transforms[band];
#if ((DEBUG_SECTION) & DEBUG_TRANSFORMS)
        clog << "    " << band << ": " << transform->m11() << "x, " << transform->m22() << 'y' << endl;
#endif
        if (scale_horizontal != transform->m11() || scale_vertical != transform->m22())
        {
            transform->setMatrix(scale_horizontal, transform->m12(), transform->m13(), transform->m21(), scale_vertical,
...
#if ((DEBUG_SECTION) & DEBUG_TRANSFORMS)
    clog << "<<< Plastic_Image::source_scaling: " << Geo_Transforms[band]->m11() << "x, " << Geo_Transforms[band]->m22()
         << 'y' << endl;
#endif
    if (scale_horizontal)
...
QRect Plastic_Image::image_region(int band) const
{
...
QSize Plastic_Image::displayed_size(int band) const
{
    double scale_width, scale_height;
    source_scaling(&scale_width, &scale_height, band);
    QPoint origin(round_down(source_origin()));
...
*/
bool Plastic_Image::source_data_map(const Data_Map &data_map, int band)
{
    ...
    bool changed = false, single_band = true;
    int bands = band + 1;
    if (band < 0)
    {
        single_band = false;
    ...
    while (band < bands)
    {
        ...
        else if (*Data_Maps[band] != data_map)
        {
...
bool Plastic_Image::source_data_maps(const Data_Map **data_maps, bool shared)
{
#if ((DEBUG_SECTION) & DEBUG_DATA_MAPPING)
    ...
    bool changed = false, locally_owned = Locally_Owned_and_Operated & DATA_MAPS;
    unsigned int band = -1;

    if (data_maps)
    {
        if (!data_maps[0] || !data_maps[1] || !data_maps[2])
        {
            ostringstream message;
            message << ID << endl << "Unable to assign source data maps from a NULL map.";
            throw invalid_argument(message.str());
        }
        ...
        if (shared)
        {
            ...
            while (++band < 3)
            {
                ...
                if (locally_owned && Data_Maps[band])
                    delete Data_Maps[band];
            }
...
#if ((DEBUG_SECTION) & DEBUG_BAND_MAP)
                clog << "    Data_Maps[" << band << "] @ " << (void *)Data_Maps[band] << endl;
#endif
            }
        }
    }
...
bool Plastic_Image::source_data_map_reset(int band)
{
#if ((DEBUG_SECTION) & DEBUG_DATA_MAPPING)
    ...
    if (MAXIMUM_PIXEL_DATUM_PRECISION < source_precision_bits())
    {
        ostringstream message;
        message << ID << endl
                << "Can't allocate a data map for " << source_precision_bits() << "-bit image data;" << endl
                << "the maximum supported is " << MAXIMUM_PIXEL_DATUM_PRECISION << "-bit image data.";
        throw out_of_range(message.str());
    }

    bool changed = true;
    Data_Map data_map(source_data_map_size());
...
bool Plastic_Image::reset_data_map(Data_Map &data_map)
{
#if ((DEBUG_SECTION) & DEBUG_DATA_MAPPING)
    clog << ">>> Plastic_Image::reset_data_map: @ " << (void *)&data_map << endl;
#endif
    bool changed = false;
    double counter = 0, increment = 0;
    int map_size = data_map.size();
    if (map_size > 1)
    {
        increment = static_cast<double>(1 << (sizeof(Data_Map::value_type) << 3)) / (map_size - 1);
#if ((DEBUG_SECTION) & DEBUG_DATA_MAPPING)
    ...
    */
    ...
    for (int index = 0; index < map_size; ++index)
    {
        ...
        for (counter += increment; counter >= 1; counter -= 1)
            ++value;
    }
#if ((DEBUG_SECTION) & DEBUG_DATA_MAPPING)
    clog << dec << setfill(' ') << "<<< Plastic_Image::reset_data_map: " << changed << endl;
#endif
    return changed;
}

bool Plastic_Image::assign_data_map(Data_Map &to_map, const Data_Map &from_map)
{
#if ((DEBUG_SECTION) & DEBUG_DATA_MAPPING)
    clog << ">>> Plastic_Image::assign_data_map: @ " << (void *)&to_map << " = @ " << (void *)&from_map << endl;
#endif
    if (from_map.size() > to_map.size())
    {
        ostringstream message;
        message << ID << endl
                << "Can't assign a data map of size " << from_map.size() << " to a data map of size " << to_map.size()
                << '.';
        throw invalid_argument(message.str());
    }
...
#if ((DEBUG_SECTION) & DEBUG_DATA_MAPPING)
    clog << dec << setfill(' ') << "<<< Plastic_Image::assign_data_map: " << changed << endl;
#endif
    return changed;
}

bool Plastic_Image::apply_data_map(const Data_Map &to_map, const Data_Map &from_map)
{
    if (from_map.size() > to_map.size())
    {
        ostringstream message;
        message << ID << endl
                << "Can't apply a data map of size " << from_map.size() << " to a data map of size " << to_map.size()
                << '.';
        throw invalid_argument(message.str());
    }

    bool changed = false;
    int map_size = to_map.size();
    Data_Map::value_type *data = const_cast<Data_Map::value_type *>(to_map.data());
    for (int index = 0; index < map_size; ++index, ++data)
    {
...
bool Plastic_Image::different_data_maps(const Data_Map **data_maps) const
{
#if ((DEBUG_SECTION) & DEBUG_DATA_MAPPING)
...
#if ((DEBUG_SECTION) & DEBUG_DATA_MAPPING)
            clog << "    Data_Maps[" << band << "] @ " << (void *)Data_Maps[band] << endl
                 << "    data_maps[" << band << "] @ " << (void *)data_maps[band] << endl;
#endif
            if (Data_Maps[band] != data_maps[band] && *Data_Maps[band] != *data_maps[band])
            {
#if ((DEBUG_SECTION) & DEBUG_DATA_MAPPING)
...
unsigned int Plastic_Image::source_data_map_size() const
{
    unsigned int map_size = source_precision_bits();
    if (map_size)
        map_size = 1 << map_size;
    return map_size;
}
...
/*------------------------------------------------------------------------------
    Histograms
*/
unsigned long long Plastic_Image::source_histograms(QVector<Histogram *> histograms, const QRect &source_region) const
{
#if ((DEBUG_SECTION) & (DEBUG_HISTOGRAMS | DEBUG_PRINT_HISTOGRAMS))
    clog << ">>> Plastic_Image::source_histograms:" << endl << "    source_region = " << source_region << endl;
#endif
    ...
    {
#if ((DEBUG_SECTION) & (DEBUG_HISTOGRAMS | DEBUG_PRINT_HISTOGRAMS))
        clog << "    empty source_region" << endl << "<<< Plastic_Image::source_histograms: 0" << endl;
#endif
        return 0;
    }
...
//	The histogram region does not intersect with the image region.
    ...
    Histogram *histogram;
    Pixel_Datum pixel_value;
    unsigned long long count, max_count = 0;
    int start_line = selected_region.top() - 1, line = 0, end_line = start_line + selected_region.height() + 1,
        start_sample = selected_region.left() - 1, sample = 0, end_sample = start_sample + selected_region.width() + 1,
        source_band;
#if ((DEBUG_SECTION) & (DEBUG_HISTOGRAMS | DEBUG_PRINT_HISTOGRAMS))
    clog << "      first = " << (start_line + 1) << "x, " << (start_sample + 1) << 'y' << endl
         << "       last = " << (end_sample - 1) << "x, " << (end_line - 1) << 'y' << endl;
#endif
    for (int band = 0; band < 3; ++band)
    {
...
bool Plastic_Image::refresh_source_histogram(QVector<Histogram *> &histograms, int band) const
{
#if ((DEBUG_SECTION) & (DEBUG_HISTOGRAMS | DEBUG_PRINT_HISTOGRAMS))
    clog << ">>> Plastic_Image::refresh_source_histogram: " << band << endl;
#endif
    bool refresh = true;
    int entries = 1 << source_precision_bits(), other_band = band;
...
#if ((DEBUG_SECTION) & (DEBUG_HISTOGRAMS | DEBUG_PRINT_HISTOGRAMS))
        clog << "    band " << band << " and band " << other_band << " are both mapped to band " << Band_Map[band]
             << endl;
#endif
        if (!histograms[band])
        {
...
#if ((DEBUG_SECTION) & (DEBUG_HISTOGRAMS | DEBUG_PRINT_HISTOGRAMS))
            clog << "    NULL histogram[" << band << "] set to histogram[" << other_band << "] @ "
                 << (void *)histograms[other_band] << endl;
#endif
        }
        else if (histograms[band] != histograms[other_band])
        {
            if (histograms[band]->size() < histograms[other_band]->size())
            {
#if ((DEBUG_SECTION) & (DEBUG_HISTOGRAMS | DEBUG_PRINT_HISTOGRAMS))
                clog << "    histogram[ " << band << "] resized from " << histograms[band]->size() << " to "
            ...
            *histograms[band] = *histograms[other_band];
#if ((DEBUG_SECTION) & (DEBUG_HISTOGRAMS | DEBUG_PRINT_HISTOGRAMS))
            clog << "    histogram[" << band << "] copied from histogram[" << other_band << ']' << endl;
#endif
        }
#if ((DEBUG_SECTION) & (DEBUG_HISTOGRAMS | DEBUG_PRINT_HISTOGRAMS))
        else
            clog << "    histogram[" << band << "] is histogram[" << other_band << "] @ "
                 << (void *)histograms[other_band] << endl;
#endif
        refresh = false;
    }
...
#if ((DEBUG_SECTION) & (DEBUG_HISTOGRAMS | DEBUG_PRINT_HISTOGRAMS))
            clog << "    new histogram[ " << band << "] @ " << (void *)histograms[band] << " with " << entries
                 << " entries" << endl;
#endif
        }
        else if (histograms[band]->size() < entries)
        {
#if ((DEBUG_SECTION) & (DEBUG_HISTOGRAMS | DEBUG_PRINT_HISTOGRAMS))
            clog << "    histogram[ " << band << "] resized from " << histograms[band]->size() << " to " << entries
                 << " entries" << endl;
#endif
            histograms[band]->resize(entries);
        }
...
unsigned long long Plastic_Image::display_histograms(QVector<Histogram *> histograms, const QRect &display_region) const
{
#if ((DEBUG_SECTION) & (DEBUG_HISTOGRAMS | DEBUG_PRINT_HISTOGRAMS))
    clog << ">>> Plastic_Image::display_histograms:" << endl << "    display_region = " << display_region << endl;
#endif
    if (histograms.size() < 3)
    {
#if ((DEBUG_SECTION) & (DEBUG_HISTOGRAMS | DEBUG_PRINT_HISTOGRAMS))
...
//	Allocate a Histogram initialized with NULLs.
            ...
            clog << "      new band " << band << " Histogram allocated with 256 entries" << endl;
#endif
            histograms[band] = new Histogram(256);
        }
        else if (histograms[band]->size() < 256)
        {
#if ((DEBUG_SECTION) & DEBUG_HISTOGRAMS)
            clog << "      band " << band << " histogram resized to 256 entries" << endl;
#endif
    ...
    clog << ">>> Plastic_Image::display_histograms: " << display_region << endl
         << "    histograms @ " << (void *)histograms[0] << ", " << (void *)histograms[1] << ", "
         << (void *)histograms[2] << endl;
#endif
    ...
    {
#if ((DEBUG_SECTION) & (DEBUG_HISTOGRAMS | DEBUG_PRINT_HISTOGRAMS))
        clog << "    empty display_region" << endl << "<<< Plastic_Image::display_histograms: 0" << endl;
#endif
        return 0;
    }
...
#endif
    if (display_region.left() > (selected_region.left() + selected_region.width()) ||
        (display_region.left() + display_region.width()) < selected_region.left() ||
        display_region.top() > (selected_region.top() + selected_region.height()) ||
        (display_region.top() + display_region.height()) < selected_region.top())
    {
//	The histogram region does not intersect with the display region.
    ...
    /*	>>> CAUTION <<< Implicit data sharing.
        ...
        declared const.
    */
    const QRgb *image_display_data =
                   reinterpret_cast<const QRgb *>(bits()) + (selected_region.top() * width()) + selected_region.left(),
               *display_data;
    int line = selected_region.top() - 1, end_line = line + selected_region.height() + 1,
        start_sample = selected_region.left() - 1, sample = 0, end_sample = start_sample + selected_region.width() + 1;
#if ((DEBUG_SECTION) & (DEBUG_HISTOGRAMS | DEBUG_PRINT_HISTOGRAMS))
    clog << "      first = " << (line + 1) << "x, " << (start_sample + 1) << 'y' << endl
         << "       last = " << (end_sample - 1) << "x, " << (end_line - 1) << 'y' << endl;
#endif
    while (++line < end_line)
    {
...
#if ((DEBUG_SECTION) & DEBUG_PRINT_HISTOGRAMS)
    clog << histograms[0] << histograms[1] << histograms[2];
#endif
#if ((DEBUG_SECTION) & (DEBUG_HISTOGRAMS | DEBUG_PRINT_HISTOGRAMS))
    clog << "<<< Plastic_Image::display_histograms: "
         << ((unsigned long long)selected_region.width() * selected_region.height()) << endl;
#endif
    return ((unsigned long long)selected_region.width() * selected_region.height());
}

/*------------------------------------------------------------------------------
    Band fill
*/
void Plastic_Image::fill(QRgb color, int band)
{
    if (band < 0 || band > 2)
        return;

    QRgb band_mask = 0xFF << (16 - (band << 3)), fill_value = color & band_mask;
    for (QRgb
             /*
         ...
         display_data < end_data; ++display_data)
...
*/
bool Plastic_Image::update() noexcept(false)
// throw (Render_Exception, std::bad_exception)
{
    ...
    if (!Closed)
    {
        ...
        clog << "       Rendering = " << Rendering << endl
             << "    Needs_Update = " << mapping_type_names(Needs_Update) << " (" << Needs_Update << ')' << endl;
#endif
        if (Rendering)
...
bool Plastic_Image::needs_update(Mapping_Type changed) noexcept(false)
// throw (Render_Exception, std::bad_exception)
{
#if ((DEBUG_SECTION) & (DEBUG_UPDATE | DEBUG_MANIPULATORS))
...
#if ((DEBUG_SECTION) & (DEBUG_UPDATE | DEBUG_MANIPULATORS))
            clog << "    Needs_Update_Shadow = " << mapping_type_names(Needs_Update_Shadow) << " ("
...
bool Plastic_Image::render_image() noexcept(false)
// throw (Render_Exception, std::bad_exception)
{
    ...
    {
        ostringstream message;
        message << "Request to render \"" << source_name() << '"' << endl << "while rendering is in progress.";
    ...
    */
    ...
    QTransform transform, *transforms[3] = {NULL, NULL, NULL};
    QRgb pixel_value,
        /*
            N.B.: The image_data method returns a pointer to writable
            (non-const) image display data. This avoids having the
            ...
            being used elsewhere.
        */
        *display_data = image_data(), background_value[3];
    unsigned int band_map[3];
    ...
    while (++band < 3)
    {
        ...
        try
        {
            transform = invert_keeping_origin(*(Rendering->Geo_Transforms[band]));
        }
        catch (runtime_error &except)
        {
            ...
            message //	invert_keeping_origin provides the ID.
                << except.what() << "While preparing to render image source -" << endl
                << source_name();
            throw Render_Exception(message.str());
        }
#if ((DEBUG_SECTION) & (DEBUG_MANIPULATORS | DEBUG_RENDER))
        clog << "    band " << band << " -" << endl << "    transform -" << *source_transform(band);
        ...
        if (band_map[band] > 3)
        {
            ...
            message << ID << endl
                    ...
                    << source_name() << endl
                    << "Band map entry " << band << " refers to non-display band " << band_map[band] << '.';
            throw Render_Exception(message.str());
        }
    ...
    Pixel_Datum datum, max_value = source_data_map_size() - 1;
    int source_line, source_sample, display_line = -1, display_lines = height(), display_sample,
                                    display_samples = width();
    unsigned int rendered_lines = 0, rendering_increment = rendering_increment_lines();
    if (rendering_increment &&
        //	"Close enough" margin.
        ((rendering_increment >> 3) == 0 ||
         (rendering_increment + (rendering_increment >> 3)) >= (unsigned int)display_lines))
    ...
    Rendering_Monitor::Status status = Rendering_Monitor::TOP_QUALITY_DATA;
    QRect rendered_region(0, 0, display_samples, 0);
    ...
    while (++display_line < display_lines)
    {
        ...
        while (++display_sample < display_samples)
        {
            ...
            {
                if (transforms[band])
                    transforms[band]->map(display_sample, display_line, &source_sample, &source_line);
                else
                {
                    source_sample = display_sample;
                    source_line = display_line;
                }
                datum = source_pixel_value(source_sample, source_line, band_map[band]);
#if ((DEBUG_SECTION) & DEBUG_PIXEL_DATA)
#if ((DEBUG_SECTION) & DEBUG_ONE_LINE)
                if (!display_line)
#endif
                    clog << "      " << band << "->" << band_map[band] << ": image " << setw(3) << display_sample
                         << "x, " << setw(3) << display_line << "y <- source " << setw(3) << source_sample << "x, "
                         << setw(3) << source_line << "y = " << setfill('0') << hex << setw(2) << datum << " -> ";
#endif
                if (datum == UNDEFINED_PIXEL_VALUE || datum > max_value)
                {
#if ((DEBUG_SECTION) & DEBUG_PIXEL_DATA)
#if ((DEBUG_SECTION) & DEBUG_ONE_LINE)
                    if (!display_line)
                ...
                else
                {
                    ...
                    if (!display_line)
#endif
                        clog << setw(2) << static_cast<Pixel_Datum>(data_maps[band]->at(datum)) << dec << setfill(' ')
                             << endl;
#endif
            ...
            if (!display_line)
#endif
                clog << "      display_data @ " << setfill('0') << hex << setw(sizeof(display_data) << 1)
                     << (void *)display_data << " = " << setw(8) << pixel_value << dec << setfill(' ') << endl;
#endif
            *display_data++ = pixel_value;
        }
        if (rendering_increment && ++rendered_lines == rendering_increment)
        {
            rendered_region.setHeight(static_cast<int>(rendered_lines));
            if (display_line == display_lines)
                status = Rendering_Monitor::DONE;
#if ((DEBUG_SECTION) & (DEBUG_MANIPULATORS | DEBUG_RENDER | DEBUG_NOTIFY | DEBUG_LOCATION))
...
#endif
            if (!notify_rendering_monitors(status, Rendering_Monitor::Status_Message[status], rendered_region))
                break;
    ...
    */
    bool canceled = cancel_update(false), completed = (display_line == display_lines);

    if (rendered_lines)
    {
        rendered_region.setHeight(static_cast<int>(rendered_lines));
        status = (display_line == display_lines) ? Rendering_Monitor::DONE : Rendering_Monitor::CANCELED;
#if ((DEBUG_SECTION) & (DEBUG_MANIPULATORS | DEBUG_RENDER | DEBUG_NOTIFY | DEBUG_LOCATION))
        clog << "    Plastic_Image::render_image: notify_rendering_monitors"
                " status "
             << status << " \"" << Rendering_Monitor::Status_Message[status] << '"' << endl
             << "    region = " << rendered_region << endl;
#endif
        notify_rendering_monitors(status, Rendering_Monitor::Status_Message[status], rendered_region);
    }
    ...
    //	Tile markings.
    QString label = QString(" image @ %1: so %2, %3; s %4")
                        .arg((ulong)this, 0, 16)
                        .arg(source_origin().x())
                        .arg(source_origin().y())
...
bool Plastic_Image::add_rendering_monitor(Rendering_Monitor *monitor)
{
    bool added = false;
    if (monitor)
    {
                                              ...
                                              const QRect &region)
{
    ...
    //	Tile markings.
    ostringstream label;
    label << " incr. region @ " << (void *)this << ": " << region << "; st " << status;
    mark_image(this, QString::fromStdString(label.str()), region.y(), TILE_MARKINGS_COLOR);
#endif

    bool cancel = Cancel_Update;

    for (int entry = 0; entry < Rendering_Monitors.size(); entry++)
        if (Rendering_Monitors[entry] != NULL)
            cancel |= !Rendering_Monitors[entry]->notification(*this, status, message, region);
...
*/
Plastic_Image::Rendering_Data::Rendering_Data(const Plastic_Image &image)
{
#if ((DEBUG_SECTION) & (DEBUG_CONSTRUCTORS | DEBUG_RENDER | DEBUG_LOCATION))
    LOCKED_LOGGING((clog << ">>> Plastic_Image::Rendering_Data @ " << (void *)this << endl
                         << "    owner - " << image << endl));
#endif
...
#if ((DEBUG_SECTION) & (DEBUG_CONSTRUCTORS | DEBUG_RENDER | DEBUG_LOCATION))
    LOCKED_LOGGING((clog << "          Band_Map @ " << (void *)(image.Band_Map) << endl
                         << "    Geo_Transforms @ " << (void *)(image.Geo_Transforms) << endl
                         << "         Data_Maps @ " << (void *)(image.Data_Maps) << endl));
#endif
    for (int band = 0; band < 3; band++)
    {
        Band_Map[band] = image.Band_Map[band];
#if ((DEBUG_SECTION) & (DEBUG_CONSTRUCTORS | DEBUG_RENDER | DEBUG_LOCATION))
        LOCKED_LOGGING((clog << "    Band_Map[" << band << "] = " << image.Band_Map[band] << endl));
#endif
        Geo_Transforms[band] = new QTransform(*(image.Geo_Transforms[band]));
        /*
        ...
        */
        Data_Maps[band] = new Plastic_Image::Data_Map(*image.Data_Maps[band]);
    }
...
#if ((DEBUG_SECTION) & (DEBUG_CONSTRUCTORS | DEBUG_RENDER | DEBUG_LOCATION))
    LOCKED_LOGGING((clog << "    Locally_Owned_and_Operated = 0x" << hex << Locally_Owned_and_Operated << endl
                         << "           Mapping_Differences = 0x" << Mapping_Differences << endl
                         << "                  Needs_Update = 0x" << Needs_Update << dec << endl
...
QPointF Plastic_Image::Rendering_Data::origin(int band) const
{
    return QPointF(Geo_Transforms[band]->m31(), Geo_Transforms[band]->m32());
}

QSizeF Plastic_Image::Rendering_Data::scaling(int band) const
{
    return QSizeF(Geo_Transforms[band]->m11(), Geo_Transforms[band]->m22());
}

bool Plastic_Image::is_rendering(bool rendering)
{
#if ((DEBUG_SECTION) & (DEBUG_RENDER | DEBUG_LOCATION))
...
#if ((DEBUG_SECTION) & (DEBUG_RENDER | DEBUG_LOCATION))
            LOCKED_LOGGING((clog << "    new Rendering_Data @ " << (void *)Rendering << endl
                                 << "    for " << *this << endl));
#endif
...
//	Rendering has ended.
#if ((DEBUG_SECTION) & (DEBUG_RENDER | DEBUG_LOCATION))
        LOCKED_LOGGING((clog << "    delete Rendering_Data @ " << (void *)Rendering << endl
                             << "    for " << *this << endl));
#endif
        delete Rendering;
        Rendering = NULL;
...
#if ((DEBUG_SECTION) & (DEBUG_RENDER | DEBUG_LOCATION))
            LOCKED_LOGGING((clog << "    the image was Closed" << endl));
#endif
            close();
        }
    }
#if ((DEBUG_SECTION) & (DEBUG_RENDER | DEBUG_LOCATION))
    LOCKED_LOGGING((clog << "<<< Plastic_Image::is_rendering: " << was_rendering << endl));
#endif
...
QRgb *Plastic_Image::image_data() const
{
...
void Plastic_Image::Update_Locker::start()
{
    ...
    if (!Updating || Initiator_Thread != current_thread)
    {
        /*
        ...
        */
        ...
        {
#if ((DEBUG_SECTION) & DEBUG_UPDATE)
            LOCKED_LOGGING((clog << "    Update_Lock.unlock in anticipation of blocking"
                                    " Object_Lock.lock"
                                 << endl));
#endif
            /*
    ...
    ++Updating;
#if ((DEBUG_SECTION) & DEBUG_UPDATE)
    LOCKED_LOGGING((clog << "    Updating = " << Updating << endl
...
void Plastic_Image::Update_Locker::end(Update_End_Condition condition)
{
    ...
    if (Updating)
    {
        ...
        {
            LOCKED_LOGGING((clog << "    still Updating = " << Updating << endl));
        }
#endif
    }
#if ((DEBUG_SECTION) & DEBUG_UPDATE)
    LOCKED_LOGGING((clog << "<<< Plastic_Image::Update_Locker::end " << thread_ID << endl));
#endif
}
...
/*==============================================================================
    Utility Functions
*/
std::ostream &operator<<(std::ostream &stream, const Plastic_Image &image)
{
    return stream << "image @ " << (void *)(&image) << " source name \"" << image.source_name() << '"' << std::endl
                  << "    size " << image.size() << '/' << image.source_size() << ", region " << image.image_region()
                  << ", scaling " << image.source_scaling() << ", needs update "
                  << image.mapping_type_names(image.needs_update()) << " (" << image.needs_update() << ')'
                  << (image.update_canceled() ? " canceled" : "");
}

std::ostream &operator<<(std::ostream &stream, const Plastic_Image::Triplet &triplet)
{
    return stream << triplet.Datum[0] << "r, " << triplet.Datum[1] << "g, " << triplet.Datum[2] << 'b';
}
...
void display_line_data(const Plastic_Image &image, int line)
{
    ...
    if (line < image_size.rheight())
    {
        ...
        */
        const QRgb *pixels = reinterpret_cast<const QRgb *>(image.scanLine(line));
        clog << "line " << line << " @ " << (void *)pixels << " -" << endl << hex << setfill('0');
        for (int index = 0; index < image_size.rwidth(); index++, pixels++)
        {
            if (!(index % 16))
            {
                if (index)
                    clog << endl;
                clog << dec << setfill(' ') << setw(5) << index << ": " << hex << setfill('0');
            }
            clog << "  " << setw(2) << qRed(*pixels) << ',' << setw(2) << qGreen(*pixels) << ',' << setw(2)
                 << qBlue(*pixels);
        }
...
{
    QSize image_size(image.size());
    if (image_size.rheight() && image_size.rwidth())
    {
        int line = -1, matches = 0;
        const QRgb *last_line_pixels, *this_line_pixels;

        clog << ">>> display_image_data" << endl << "    " << image << endl;
        while (++line < image_size.rheight())
        {
            if (line)
            {
                ...
                */
                last_line_pixels = reinterpret_cast<const QRgb *>(image.scanLine(line - 1));
                this_line_pixels = reinterpret_cast<const QRgb *>(image.scanLine(line));
                int index;
                for (index = 0; index < image_size.rwidth(); index++)
...
std::ostream &operator<<(std::ostream &stream, const QTransform &transform)
{
    return stream << std::endl
                  << "    m11 = " << std::setw(6) << transform.m11() << ", m12 = " << std::setw(6) << transform.m12()
                  << ", m13 = " << std::setw(6) << transform.m13() << std::endl
                  << "    m21 = " << std::setw(6) << transform.m21() << ", m22 = " << std::setw(6) << transform.m22()
                  << ", m23 = " << std::setw(6) << transform.m23() << std::endl
                  << "    m31 = " << std::setw(6) << transform.m31() << ", m32 = " << std::setw(6) << transform.m32()
                  << ", m33 = " << std::setw(6) << transform.m33() << std::endl;
}

std::ostream &operator<<(std::ostream &stream, const Plastic_Image::Histogram *const histogram)
{
    stream << "    Histogram data -";
    if (histogram)
    {
        for (int entry = 0; entry < histogram->size(); entry++)
        {
            if (!(entry % 16))
                stream << endl << dec << setw(9) << entry << ":" << hex;
...
std::ostream &operator<<(std::ostream &stream, const unsigned int *band_map)
{
    stream << " @ ";
    if (band_map)
        stream << (void *)band_map << " - " << band_map[0] << ", " << band_map[1] << ", " << band_map[2];
    else
        stream << "NULL";
    return stream;
}

void mark_image(Plastic_Image *image, const QString &label, int top, const QColor &text_color,
                const QColor &field_color)
{
#if ((DEBUG_SECTION) & DEBUG_HELPERS)
    clog << ">>> mark_image" << endl;
#endif
    static QMutex Helping;
    if (image->isNull())
        return;
    ...
    QPainter painter;
    painter.begin(image);
    QRect rect(0, top, image->width(), 12);
    QColor rect_color(field_color);
