...
namespace HiRISE
{
Drawn_Line::Drawn_Line(int weight, QWidget *parent)
    : QWidget(parent), Weight(weight), Orientation(Qt::Horizontal), Alignment(Qt::AlignCenter),
...
Drawn_Line &Drawn_Line::operator=(const Drawn_Line &drawn_line)
{
    if (this != &drawn_line)
    {
...
Drawn_Line &Drawn_Line::weight(int weight)
{
    if (weight < 0)
        weight = 0;
    if (Weight != weight)
...
Drawn_Line &Drawn_Line::orientation(Qt::Orientation orient)
{
...
Drawn_Line &Drawn_Line::alignment(Qt::Alignment align)
{
    if (Alignment != align)
    {
        Alignment = align;
...
Drawn_Line &Drawn_Line::brush(const QBrush &brush)
{
    if (Brush != brush)
    {
        Brush = brush;
...
void Drawn_Line::paintEvent(QPaintEvent *)
{
    if (Weight)
    {
        QSize extent(size());
