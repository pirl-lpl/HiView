...
/*==============================================================================
    Constants
*/
const char *const Parameter_Tree_Model::ID =
    "UA::HiRISE::Parameter_Tree_Model ($Revision: 1.23 $ $Date: 2012/11/13 22:29:37 $)";
...
const char *const Parameter_Tree_Model::DEFAULT_ROOT_NAME = _DEFAULT_ROOT_NAME_;

/*==============================================================================
    Constructors
*/
Parameter_Tree_Model::Parameter_Tree_Model(Aggregate *root, QObject *parent)
    : QAbstractItemModel(parent), Root(root), Root_Visible(false), Root_Name(tr(DEFAULT_ROOT_NAME))
{
#if (DEBUG_SECTION & DEBUG_CONSTRUCTORS)
    clog << "<-> Parameter_Tree_Model::Parameter_Tree_Model @ " << (void *)this << ": root @ " << (void *)root << endl;
#endif
#if (DEBUG_SECTION != 0)
    clog << boolalpha;
...
Parameter_Tree_Model::Parameter_Tree_Model(const Parameter_Tree_Model &model)
    : QAbstractItemModel(), Root(model.Root), Root_Visible(model.Root_Visible), Root_Name(model.Root_Name)
{
#if (DEBUG_SECTION & DEBUG_CONSTRUCTORS)
    clog << "<-> Parameter_Tree_Model::Parameter_Tree_Model: copy @ " << (void *)(&model) << " with root @ "
         << (void *)Root << endl;
#endif
#if (DEBUG_SECTION != 0)
    clog << boolalpha;
...
Parameter_Tree_Model &Parameter_Tree_Model::operator=(const Parameter_Tree_Model &model)
{
    if (this != &model)
    {
        Root = model.Root;
...
*/
QModelIndex Parameter_Tree_Model::index(int row, int column, const QModelIndex &parent_index) const
{
#if (DEBUG_SECTION & DEBUG_MODEL)
    clog << ">>> Parameter_Tree_Model::index: row " << row << " col " << column << endl
         << "    parent " << parent_index << endl
         << "      is valid = " << parent_index.isValid() << endl;
#endif
    QModelIndex index;
    if (column >= 0 && column <= 1 && parent_index.column() != 1) //	Value column has no children.
    {
        ...
        {
            //	Root parent.
            if (Root_Visible && row == 0)
            {
#if (DEBUG_SECTION & DEBUG_MODEL)
                clog << "    createIndex (" << row << ", " << column << ", " << (void *)(Root) << ')' << endl
                     << "    at Root";
#endif
        ...
        if (parent && row < (int)parent->size())
        {
// Set the internal pointer to the Parameter for the row.
#if (DEBUG_SECTION & DEBUG_MODEL)
            clog << "    createIndex (" << row << ", " << column << ", " << (void *)(&(*parent)[row]) << ')' << endl
                 << "    parameter " << parent->name() << '/' << parent->at(row).name() << endl;
#endif
...
QVariant Parameter_Tree_Model::data(const QModelIndex &index, int role) const
{
#if (DEBUG_SECTION & DEBUG_DATA)
    clog << ">>> Parameter_Tree_Model::data:" << endl
         << "    " << index << endl
         << "    role = " << role << " (" << qPrintable(QString(roleNames().value(role))) << ')' << endl;
#endif
    QVariant datum;
    ...
    {
        Parameter *parameter = static_cast<Parameter *>(index.internalPointer());
        if (index.column() == 0)
        {
            //	Name column.
            if (Root_Visible && Root == parameter)
            {
#if (DEBUG_SECTION & DEBUG_DATA)
        ...
        else if (index.column() == 1)
        {
            ...
            {
#if (DEBUG_SECTION & DEBUG_DATA)
                clog << "    " << parameter->type_name() << " Aggregate " << parameter->name() << endl;
#endif
...
#if (DEBUG_SECTION & DEBUG_DATA)
    clog << "<<< Parameter_Tree_Model::data: " << qPrintable(datum.toString()) << endl;
#endif
    return datum;
}

QModelIndex Parameter_Tree_Model::parent(const QModelIndex &index) const
{
#if (DEBUG_SECTION & DEBUG_MODEL)
    clog << ">>> Parameter_Tree_Model::parent: " << index << endl << "      is valid = " << index.isValid() << endl;
#endif
    ...
    if (index.isValid())
    {
        ...
        if (parameter && (parameter != Root || Root_Visible))
...
int Parameter_Tree_Model::rowCount(const QModelIndex &index) const
{
#if (DEBUG_SECTION & DEBUG_MODEL)
    clog << ">>> Parameter_Tree_Model::rowCount: " << index << endl << "      is valid = " << index.isValid() << endl;
#endif
    int count = 0;
    ...
    if (index.isValid())
    {
        if (index.column() == 0)
        {
            ...
            {
#if (DEBUG_SECTION & DEBUG_MODEL)
                clog << "    " << parameter->type_name() << " parameter " << parameter->name() << endl;
#endif
                if (parameter->is_Aggregate())
...
QVariant Parameter_Tree_Model::headerData(int section, Qt::Orientation orientation, int role) const
{
    QVariant data;
    if (orientation == Qt::Horizontal && role == Qt::DisplayRole && section >= 0 && section <= 1)
...
/*==============================================================================
    Accessors
*/
void Parameter_Tree_Model::root_visible(bool enable)
{
#if (DEBUG_SECTION & DEBUG_MODEL)
...
void Parameter_Tree_Model::root_name(const QString &name)
{
#if (DEBUG_SECTION & DEBUG_MODEL)
...
QString Parameter_Tree_Model::comments(const QModelIndex &index) const
{
    QString comments;
    Parameter *parameter = static_cast<Parameter *>(index.internalPointer());
    if (parameter)
...
/*==============================================================================
    Helpers
*/
int Parameter_Tree_Model::parameter_row(Parameter *parameter) const
{
    int row = -1; //	Not found.
    if (parameter)
    {
        Aggregate *parent = parameter->parent();
        if (parent)
        {
            row = parent->size();
            while (row--)
...
{
    stream << "QModelIndex = row " << index.row() << " col " << index.column() << " object @ "
