...
/*==============================================================================
    Constants
*/
const char *const Graph_Tracker::ID = "UA::HiRISE::Graph_Tracker ($Revision: 1.8 $ $Date: 2013/09/24 18:43:16 $)";

#ifndef DEFAULT_GRAPH_TRACKER_SELECTION_MODE
#define DEFAULT_GRAPH_TRACKER_SELECTION_MODE QwtPicker::PointSelection | QwtPicker::ClickSelection
#endif

/*==============================================================================
    Constructors
*/
Graph_Tracker::Graph_Tracker(QwtPlotCanvas *canvas)
...
/*==============================================================================
    Helpers
*/
void Graph_Tracker::set_cursor(const QCursor &cursor)
{
    parentWidget()->setCursor(cursor);
}

void Graph_Tracker::set_cursor_position(const QPoint &graph_position)
{
    QCursor::setPos(parentWidget()->mapToGlobal(transform(QPointF(graph_position))));
}

/*==============================================================================
    QwtPlotPicker override
*/
QwtText Graph_Tracker::trackerText(const QPoint &coordinate) const
{
...
void Graph_Tracker::widgetMouseMoveEvent(QMouseEvent *event)
{
#if ((DEBUG_SECTION) & DEBUG_EVENTS)
    clog << ">-< Graph_Tracker::widgetMouseMoveEvent: position = " << event->pos() << endl;
#endif
    QwtPlotPicker::widgetMouseMoveEvent(event);

    QPoint point(invTransform(event->pos()).toPoint());
#if ((DEBUG_SECTION) & DEBUG_EVENTS)
...
void Graph_Tracker::widgetLeaveEvent(QEvent *event)
{
#if ((DEBUG_SECTION) & DEBUG_EVENTS)
