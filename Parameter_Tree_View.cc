...
/*==============================================================================
    Constants
*/
const char *const Parameter_Tree_View::ID =
    "UA::HiRISE::Parameter_Tree_View ($Revision: 1.8 $ $Date: 2012/08/31 06:54:34 $)";

#ifndef PARAMETER_VIEW_DEFAULT_ANIMATION
#define PARAMETER_VIEW_DEFAULT_ANIMATION true
#endif
const bool Parameter_Tree_View::DEFAULT_ANIMATION = PARAMETER_VIEW_DEFAULT_ANIMATION;

/*==============================================================================
    Constructors
*/
Parameter_Tree_View::Parameter_Tree_View(Aggregate *parameters, QWidget *parent)
    : QTreeView(parent), Tree_Model(NULL), Tree_Model_Locally_Owned(false)
{
#if (DEBUG_SECTION & DEBUG_CONSTRUCTORS)
    clog << ">>> Parameter_Tree_View" << endl;
    if (parameters)
        clog << "    parameters -" << endl << *parameters << endl;
    else
...
Parameter_Tree_View::Parameter_Tree_View(Parameter_Tree_Model *model, QWidget *parent)
    : QTreeView(parent), Tree_Model(model), Tree_Model_Locally_Owned(false)
{
#if (DEBUG_SECTION & DEBUG_CONSTRUCTORS)
    clog << ">>> Parameter_Tree_View" << endl << "    model parameters -" << endl << *(model->parameters()) << endl;
#endif
...
{
    if (Tree_Model_Locally_Owned && Tree_Model)
        delete Tree_Model;
}

void Parameter_Tree_View::initialize()
{
...
/*==============================================================================
    Accessors
*/
void Parameter_Tree_View::parameters(Aggregate *params)
{
#if (DEBUG_SECTION & DEBUG_PARAMETERS)
    clog << ">>> Parameter_Tree_View::parameters" << endl;
    if (params)
        clog << "    params -" << endl << *params;
    else
        clog << "    no params" << endl;
...
Aggregate *Parameter_Tree_View::parameters() const
{
    return static_cast<Parameter_Tree_Model *>(model())->parameters();
}

void Parameter_Tree_View::setModel(QAbstractItemModel *model)
{
    Parameter_Tree_Model *parameter_model = dynamic_cast<Parameter_Tree_Model *>(model);
    if (parameter_model)
    {
...
bool Parameter_Tree_View::root_visible() const
{
    return static_cast<Parameter_Tree_Model *>(model())->root_visible();
}

void Parameter_Tree_View::root_visible(bool enable)
{
    static_cast<Parameter_Tree_Model *>(model())->root_visible(enable);
}

QString Parameter_Tree_View::root_name() const
{
    return static_cast<Parameter_Tree_Model *>(model())->root_name();
}

void Parameter_Tree_View::root_name(const QString &name)
{
    static_cast<Parameter_Tree_Model *>(model())->root_name(name);
}

QString Parameter_Tree_View::default_root_name()
{
    return Parameter_Tree_Model::default_root_name();
}

QString Parameter_Tree_View::comments(const QModelIndex &index) const
{
    return static_cast<Parameter_Tree_Model *>(model())->comments(index);
}

Parameter_Tree_View::Parameter_List Parameter_Tree_View::selected_parameters() const
{
    Parameter_List parameter_list;
    QModelIndexList model_list(selectionModel()->selectedRows());
    for (int index = 0; index < model_list.size(); ++index)
        parameter_list << static_cast<Parameter *>(model_list[index].internalPointer());
    return parameter_list;
}

void Parameter_Tree_View::setAnimated(bool enable)
{
    if (enable != isAnimated())
    {
        QTreeView::setAnimated(enable);

        Parameter_Tree_Model *parameter_model = static_cast<Parameter_Tree_Model *>(model());
        QTreeView *subtree;
        QModelIndex index, parent;
        int row = 0, row_count = 1;
...
/*==============================================================================
    Helpers
*/
bool Parameter_Tree_View::subtrees()
{
...
#endif
    Parameter_Tree_Model *parameter_model = static_cast<Parameter_Tree_Model *>(model());
    QTreeView *subtree = NULL;
    Value_Tree_Model *array_model;
    QModelIndex index, parent;
    Parameter *parameter;
    int row = 0, row_count = 1,
...
//	Returned to the root.
        ...
        parameter = static_cast<Parameter *>(index.internalPointer());
        if (parameter && parameter->is_Assignment() && parameter->value().is_Array())
        {
#if (DEBUG_SECTION & DEBUG_ARRAY_SUBTREES)
            clog << "      Array" << endl;
#endif
            ...
            array_model = new Value_Tree_Model(static_cast<Array *>(&parameter->value()));
