	...
	Region_Drag_Offset(-1, -1)
{
	...
	*/
	...
	connect(Image_View,
			...
			(const Dynamic_Image::Triplet&, const Dynamic_Image::Triplet&)),
...
)
{
	...
	for (int
		 ...
		 ++band)
	{
		if (image_pixel.Datum[band] == Dynamic_Image::UNDEFINED_PIXEL_VALUE)
	...
	{
		if (display_values &&
			display_pixel.Datum[band] != Dynamic_Image::UNDEFINED_PIXEL_VALUE)
			Display_Value[band]
