...
#endif
bool Navigator_Tool::Default_Show_All_Region_Origins = NAVIGATOR_SHOW_ALL_REGION_ORIGINS;

#ifndef NAVIGATOR_SHOW_ALL_SCALINGS
#define NAVIGATOR_SHOW_ALL_SCALINGS false
#endif
bool Navigator_Tool::Default_Show_All_Scalings = NAVIGATOR_SHOW_ALL_SCALINGS;

#ifndef NAVIGATOR_DEFAULT_SCALING_X_Y_DISTINCT
#define NAVIGATOR_DEFAULT_SCALING_X_Y_DISTINCT false
#endif
bool Navigator_Tool::Default_Scaling_X_Y_Distinct = NAVIGATOR_DEFAULT_SCALING_X_Y_DISTINCT;
      ...
      Region_Drag_Offset(-1, -1)
{
    ...
    QWidget *image_widget, *info_widget;
...
QWidget* Navigator_Tool::info_panel()
{
...
#endif
    layout->addWidget(new Drawn_Line(Heading_Line_Weight), row, 0, 1, -1, Qt::AlignTop);
    ...
    layout->addWidget(Source_Name, row, 0, 1, -1, Qt::AlignLeft | Qt::AlignVCenter);
    ...
    clog << "    " << row << ": Separator" << endl;
#endif
    layout->addWidget(new Drawn_Line(Heading_Line_Weight), row, 0, 1, -1, Qt::AlignBottom);
    layout->setRowMinimumHeight(row, 10);
...
#endif
    layout->addWidget(label = new QLabel(tr("Display:")), display_band_row, label_col,
                      Qt::AlignRight | Qt::AlignVCenter);
    label->setToolTip(tr("Displayed image bands"));
    source_band_row = ++row;
    layout->addWidget(Source_Bands = new QLabel(tr("Source:")), source_band_row, label_col,
                      Qt::AlignRight | Qt::AlignVCenter);
    ...
    clog << "    " << row << ": Separator" << endl;
#endif
    layout->addWidget(new Drawn_Line(1), row, 0, 1, -1, Qt::AlignBottom);
    layout->setRowMinimumHeight(row, 10);
    ...
    layout->addWidget(Source_Values = new QLabel(tr("Source:")), source_value_row, label_col,
                      Qt::AlignRight | Qt::AlignVCenter);
    Source_Values->setToolTip(tr("Source image value at cursor location"));

    for (band = 0, col = X_col; band < 3; band++, col += 2)
    {
        //	Band names.
        ...
        label->setPalette(QPalette(QColor(DISPLAY_BAND_COLORS[band]).lighter()));
        label->setAutoFillBackground(true);
        layout->addWidget(label, display_band_row, col, 1, 2, Qt::AlignVCenter);
        ...
        layout->addWidget(Image_Band[band], source_band_row, col, 1, 2, Qt::AlignVCenter);
        connect(Image_Band[band], SIGNAL(valueChanged(int)), SLOT(band_mapping_changed()));

        //	Source values.
        Source_Value[band] = new QLabel;
        Source_Value[band]->setAlignment(Qt::AlignRight);
        layout->addWidget(Source_Value[band], source_value_row, col, Qt::AlignRight);

        //	Display values.
        Display_Value[band] = new QLabel;
        Display_Value[band]->setAlignment(Qt::AlignRight);
        layout->addWidget(Display_Value[band], display_value_row, col, Qt::AlignRight);
    }
    ...
    clog << "    " << row << ": Separator" << endl;
#endif
    layout->addWidget(new Drawn_Line(Heading_Line_Weight), row, label_col, 1, -1, Qt::AlignBottom);
    layout->setRowMinimumHeight(row, 10);
    ...
    layout->addWidget(Region_Size_Y = new QLabel, row, Y_col, Qt::AlignRight | Qt::AlignVCenter);
    ...
    for (band = 0; band < 3; band++, row++)
    {
        ...
        layout->addWidget(Region_Origin_X[band], row, X_col, 1, 2, Qt::AlignVCenter);
        connect(Region_Origin_X[band], SIGNAL(valueChanged(int)), SLOT(region_origin_changed()));
        ...
        layout->addWidget(Region_Origin_Y[band], row, Y_col, 1, 2, Qt::AlignVCenter);
        connect(Region_Origin_Y[band], SIGNAL(valueChanged(int)), SLOT(region_origin_changed()));
...
//	Source Location
    ...
    for (band = 0; band < 3; band++, row++)
    {
        ...
        Scaling_X[band]->setRange(Image_Viewer::min_scale(), Image_Viewer::max_scale());
        ...
        layout->addWidget(Scaling_X[band], row, X_col, 1, 2, Qt::AlignVCenter);
        connect(Scaling_X[band], SIGNAL(valueChanged(double)), SLOT(image_scaling_changed()));
        ...
        Scaling_Y[band]->setRange(Image_Viewer::min_scale(), Image_Viewer::max_scale());
        ...
        layout->addWidget(Scaling_Y[band], row, Y_col, 1, 2, Qt::AlignVCenter);
        connect(Scaling_Y[band], SIGNAL(valueChanged(double)), SLOT(image_scaling_changed()));
...
//	Apply actions
    ...
    connect(Apply, SIGNAL(clicked(bool)), SLOT(apply()));
    ...
    Apply_When->setCurrentIndex(Default_Immediate_Mode ? IMMEDIATE_MODE : DEFERRED_MODE);
    connect(Apply_When, SIGNAL(currentIndexChanged(int)), SLOT(apply_when_changed(int)));
    accept_actions_layout->addWidget(Apply_When);

    accept_actions_layout->addStretch(100);

    layout->addLayout(accept_actions_layout, row, label_col, 1, -1);
...
void Navigator_Tool::reset_info()
{
#if ((DEBUG_SECTION) & DEBUG_INITIALIZE)
    ...
    for (int band = 0; band < 3; ++band)
    {
        ...
        if (band)
        {
            Region_Origin_X[band]->setVisible((band < source_bands) && Show_All_Region_Origins);
            Region_Origin_Y[band]->setVisible((band < source_bands) && Show_All_Region_Origins);
        }
        ...
        */
        ...
        Image_Band[band]->setRange(HiView_Utilities::band_index_to_number(0),
                                   HiView_Utilities::band_index_to_number(source_bands - 1));
        Image_Band[band]->setValue(HiView_Utilities::band_index_to_number(band_map[band]));
    }
...
void Navigator_Tool::refresh_band_numbers()
{
    ...
    {
#if ((DEBUG_SECTION) & (DEBUG_INITIALIZE | DEBUG_SLOTS))
        clog << "    band " << band << "->" << band_map[band] << " number "
...
void Navigator_Tool::show_all_region_origins(bool enabled)
{
...
void Navigator_Tool::show_all_scalings(bool enabled)
{
...
void Navigator_Tool::scaling_X_Y_distinct(bool enabled, int band)
{
    if (band < 3)
    {
...
void Navigator_Tool::image_loaded(bool successful)
{
#if ((DEBUG_SECTION) & (DEBUG_MANIPULATORS | DEBUG_OVERVIEW))
                                       ...
                                       const Plastic_Image::Triplet& image_pixel)
{
    ...
    for (int band = 0; band < 3; ++band)
    {
        ...
        {
            Source_Value[band]->setNum(static_cast<int>(image_pixel.Datum[band]));
            display_values = true;
        }
    }
    for (int band = 0; band < 3; ++band)
...
*/
void Navigator_Tool::region_origin_changed()
{
#if ((DEBUG_SECTION) & DEBUG_SLOTS)
...
void Navigator_Tool::band_mapping_changed()
{
    ...
    if (Band_Map_Reset_Button)
        Band_Map_Reset_Button->setVisible(
            Image_Band[0]->value() != HiView_Utilities::band_index_to_number(Initial_Band_Map[0]) ||
            Image_Band[1]->value() != HiView_Utilities::band_index_to_number(Initial_Band_Map[1]) ||
            Image_Band[2]->value() != HiView_Utilities::band_index_to_number(Initial_Band_Map[2]));
...
{
    if (Image_Band[0]->value() != HiView_Utilities::band_index_to_number(Initial_Band_Map[0]) ||
        Image_Band[1]->value() != HiView_Utilities::band_index_to_number(Initial_Band_Map[1]) ||
        Image_Band[2]->value() != HiView_Utilities::band_index_to_number(Initial_Band_Map[2]))
    {
        Received_Knowledge = true;
        Image_Band[0]->setValue(HiView_Utilities::band_index_to_number(Initial_Band_Map[0]));
        ...
        if (Changes_Pending & SCALING)
        {
            ...
            if (Show_All_Scalings)
            {
                for (band = 0; band < 3; band++)
                {
                    ...
                    center.ry() = Region_Origin_Y[band]->value() + (Region_Size.rheight() >> 1);
...
//	>>> SIGNAL <<<
            ...
            else
            {
                ...
                center.ry() = Region_Origin_Y[0]->value() + (Region_Size.rheight() >> 1);
...
//	>>> SIGNAL <<<
        ...
        if (Changes_Pending & BAND_MAPPING)
        {
            ...
            band_map[0] = HiView_Utilities::band_number_to_index(Image_Band[0]->value());
            band_map[1] = HiView_Utilities::band_number_to_index(Image_Band[1]->value());
            band_map[2] = HiView_Utilities::band_number_to_index(Image_Band[2]->value());
//	>>> SIGNAL <<<
#if ((DEBUG_SECTION) & DEBUG_SIGNALS)
...
void Navigator_Tool::apply_when_changed(int mode)
{
#if ((DEBUG_SECTION) & DEBUG_SLOTS)
...
#endif
    QSizeF scaling(Image_View->image_scaling());
    QSize region_size(Region_Size);
    region_size.rwidth() *= scaling.rwidth();
    region_size.rheight() *= scaling.rheight();
    QPointF region_origin(Region_Origin_X[0]->value(), Region_Origin_Y[0]->value());
...
#endif
        QPoint position(round_down(Image_View->map_display_to_image(
            Image_View->image_display()->mapFromGlobal(event->globalPosition()).toPoint())));
#if ((DEBUG_SECTION) & DEBUG_EVENTS)
...
void Navigator_Tool::mouseDoubleClickEvent(QMouseEvent* event)
{
        ...
        Image_View->control_mode() == Image_Viewer::NO_CONTROL_MODE)
    {
        ...
        {
            position = round_down(Image_View->image_display()->map_display_to_image(position));
#if ((DEBUG_SECTION) & DEBUG_EVENTS)
