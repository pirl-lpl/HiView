...
/*==============================================================================
    Constants
*/
const char* const Image_Info_Panel::ID =
...
const char* RESTORE_LONGITUDE_FORMAT_KEY = "Restore_Longitude_Format";
const char* RESTORE_LATITUDE_FORMAT_KEY = "Restore_Latitude_Format";
const char* RESTORE_LONGITUDE_DIRECTION_KEY = "Restore_Longitude_Direction";
const char* SHOW_SCRIPT_KEY = "Scripts_Show_Script";
const char* CURRENT_SCRIPT_KEY = "Scripts_Current_Script";

/*==============================================================================
    Constructors
*/
Image_Info_Panel::Image_Info_Panel(QWidget* parent)
    : QFrame(parent),
      Image_Values(true),
      Use_Avg_Rgb(false),
      Image_Bands(3),
...
QWidget* Image_Info_Panel::image_data_panel()
{
#if ((DEBUG_SECTION) & DEBUG_CONSTRUCTORS)
    ...
    clog << "    layout margins = " << left << "l, " << top << "t, " << right << "r, " << bottom
         << 'b' << endl;
#endif
    ...
    if (Image_Values) Data_Source->setText(Image_Data);
    else Data_Source->setText(Display_Data);
    ...
    const char* band_label[] = {"<font color='red'><b>r</b></font>",
                                "<font color='green'><b>g</b></font>",
                                "<font color='blue'><b>b</b></font>"};
    for (int band = 0; band < 3; band++)
    {
        if (band) layout->addSpacing(ITEM_SPACING);
        ...
        clog << "    Pixel_Value[" << band << "] sizeHint = " << Pixel_Value[band]->sizeHint()
             << endl;
#endif
        ...
        clog << "    Pixel_Label[" << band << "] sizeHint = " << Pixel_Label[band]->sizeHint()
             << endl;
#endif
    }
    ...
    clog << "    sizeHint = " << panel->sizeHint() << endl
         ...
         << "     margins = " << left << "l, " << top << "t, " << right << "r, " << bottom << 'b'
         << endl
...
QWidget* Image_Info_Panel::world_location_panel()
{
#if ((DEBUG_SECTION) & DEBUG_CONSTRUCTORS)
    ...
    int left, top, right, bottom;
    layout->getContentsMargins(&left, &top, &right, &bottom);
    clog << "    projection name layout margins = " << left << "l, " << top << "t, " << right
         << "r, " << bottom << 'b' << endl;
#endif
...
#if ((DEBUG_SECTION) & DEBUG_CONSTRUCTORS)
    layout->getContentsMargins(&left, &top, &right, &bottom);
    clog << "    locations layout margins = " << left << "l, " << top << "t, " << right << "r, "
         << bottom << 'b' << endl;
#endif
    ...
    int location_value_width = Longitude->sizeHint().width();
    ...
    clog << "    Longitude sizeHint = " << Longitude->sizeHint() << endl;
#endif
    ...
    label->setAutoFillBackground(true);
    label->setPalette(colors);
    clog << "    lon. label sizeHint = " << label->sizeHint() << endl;
#endif
    layout->addWidget(label);
    ...
    clog << "    Latitude sizeHint = " << Latitude->sizeHint() << endl;
#endif
    ...
    label->setAutoFillBackground(true);
    label->setPalette(colors);
    clog << "    label sizeHint = " << label->sizeHint() << endl;
#endif
    layout->addWidget(label);
    ...
    label->setAutoFillBackground(true);
    label->setPalette(colors);
    clog << "    lat. label sizeHint = " << label->sizeHint() << endl;
#endif
    layout->addWidget(label);

#if ((DEBUG_SECTION) & DEBUG_CONSTRUCTORS)
    layout->getContentsMargins(&left, &top, &right, &bottom);
    clog << "     locations margins = " << left << "l, " << top << "t, " << right << "r, " << bottom
         << 'b' << endl;
    // panel_layout->getContentsMargins (&left, &top, &right, &bottom);
    clog << "        panel sizeHint = " << panel->sizeHint() << endl
         << "            panel size = " << panel->size() << endl
         << "         panel margins = " << left << "l, " << top << "t, " << right << "r, " << bottom
         << 'b' << endl;
    clog << "<<< Image_Info_Panel::world_location_panel" << endl;
#endif
    return panel;
...
/*==============================================================================
    Manipulators
*/
void Image_Info_Panel::image_bands(int bands)
{
    if (bands < 1) bands = 0;
    else if (bands > 3) bands = 3;
    ...
    if (Image_Values)
    {
        ...
        if (Image_Bands == 1) Pixel_Label[0]->setVisible(false);
    }
}
...
/*------------------------------------------------------------------------------
    Image Data
*/
void Image_Info_Panel::image_values(bool enabled)
{
    if (enabled != Image_Values)
    {
        Image_Values = enabled;
        int bands = Image_Values ? Image_Bands : 3;

        Data_Source->setText(Image_Values ? Image_Data : Display_Data);
        for (int band = 0; band < 3; band++)
        {
            Pixel_Value[band]->setVisible(band < bands);
            Pixel_Label[band]->setVisible(band < bands);
        }
        if (bands == 1) Pixel_Label[0]->setVisible(false);

        //	Reset all the values.
        QPoint display_location(Display_Location), image_location(Image_Location);
        Display_Location = Image_Location = QPoint(-1, -1);
        cursor_location(display_location, image_location);

        Plastic_Image::Triplet display_value(Display_Value), image_value(Image_Value);
        Display_Value.Datum[0] = Display_Value.Datum[1] = Display_Value.Datum[2] =
            Image_Value.Datum[0] = Image_Value.Datum[1] = Image_Value.Datum[2] =
                static_cast<Plastic_Image::Pixel_Datum>(-2);
        pixel_value(display_value, image_value);

        QSizeF scaling(Image_Scale);
        Image_Scale = QSizeF();
        image_scale(scaling);
    }
}

void Image_Info_Panel::cursor_location(const QPoint& display_location, const QPoint& image_location)
{
    ...
    if (Image_Values)
    {
        ...
        {
            int x = image_location.x(), y = image_location.y();

            if (x < 0 || y < 0) return;
            Coordinate coordinate(Projection::INVALID_VALUE, Projection::INVALID_VALUE);

            if (x < 0)
            {
                Location_X->clear();
                if (Evaluate_X) Script_Output->clear();
                //			Annotation_X->clear ();
            }
            else
            {
            ...
            {
                Location_Y->clear();
                if (Evaluate_Y) Script_Output->clear();
                //			Annotation_Y->clear ();
            }
            else
            {
            ...
            if (!Projector || !World_Location->isVisible()) location(coordinate);
            else location(Projector->to_world(coordinate));
        }
    }
    else
    {
...
void Image_Info_Panel::pixel_value(const Plastic_Image::Triplet& display_value,
                                   const Plastic_Image::Triplet& image_value)
{
    if (!Use_Avg_Rgb && Statistics != NULL)
    {
        // r
        Plastic_Image::Pixel_Datum value =
            Image_Values ? image_value.Datum[0] : display_value.Datum[0];
        if (value != (Image_Values ? Image_Value.Datum[0] : Display_Value.Datum[0]))
        {
            if (value == Plastic_Image::UNDEFINED_PIXEL_VALUE)
            {
                Pixel_Value[0]->clear();
                if (Evaluate_R) Script_Output->clear();
            }
            else
            {
        ...
        if (value != (Image_Values ? Image_Value.Datum[1] : Display_Value.Datum[1]))
        {
            ...
            {
                Pixel_Value[1]->clear();
                if (Evaluate_G) Script_Output->clear();
            }
            else
            {
        ...
        if (value != (Image_Values ? Image_Value.Datum[2] : Display_Value.Datum[2]))
        {
            ...
            {
                Pixel_Value[2]->clear();
                if (Evaluate_B) Script_Output->clear();
            }
            else
            {
...
void Image_Info_Panel::use_avg_pixel_value(bool use) { Use_Avg_Rgb = use; }

void Image_Info_Panel::image_scale(const QSizeF& scaling, int /* band argument is not used */
)
{
    if (Image_Scale != scaling)
    {
        ...
        QString report;
        report = QString("%1").arg(scale_x, 6, 'f', 3);
        if (scale_y != scale_x) report += QString(" / %1").arg(scale_y, 6, 'f', 3);

        Scaling->setText(report);
        Global_Object.setProperty("display.scale", scale_x);
        if (Evaluate_Scale) { evaluate_script(); }
    }
}
...
/*------------------------------------------------------------------------------
    World Data
*/
void Image_Info_Panel::projection(Projection* projector)
{
#if ((DEBUG_SECTION) & DEBUG_WORLD_DATA)
    clog << ">>> Image_Info_Panel::projection: @ " << (void*)projector << endl;
#endif
    if (Projector != projector) Projector = projector;

    bool visible = false;
    if (Projector && !Projector->is_identity())
    {
#if ((DEBUG_SECTION) & DEBUG_WORLD_DATA)
        clog << "    canonical_projection_name = " << Projector->canonical_projection_name()
             << endl;
#endif
        /*	QString
                name ("<b>");
        ...
        if (result < 0.000001) { degree_precision = 11; }
        else
        {
            int i;
            for (i = 0; result < 1; ++i) { result = result * 10; }
            degree_precision = i + 5;
        }
    }
#if ((DEBUG_SECTION) & DEBUG_WORLD_DATA)
    clog << "    World_Location->isVisible = " << World_Location->isVisible() << endl
         << "    setVisible = " << visible << endl;
#endif
    if (World_Location->isVisible() != visible) World_Location->setVisible(visible);
#if ((DEBUG_SECTION) & DEBUG_WORLD_DATA)
    clog << "<<< Image_Info_Panel::projection" << endl;
#endif
}

void Image_Info_Panel::longitude_units(int units)
{
    if (Longitude_Units != units) { Longitude_Units = units; }
    else if (!Projection::is_invalid(Longitude_Location))
    {
        //	Refresh the displayed value.
        double value = Longitude_Location;
        Longitude_Location = Projection::INVALID_VALUE;
        longitude(value);
    }
}

void Image_Info_Panel::latitude_units(int units)
{
    if (Latitude_Units != units) { Latitude_Units = units; }
    else if (!Projection::is_invalid(Latitude_Location))
    {
        //	Refresh the displayed value.
        double value = Latitude_Location;
        Latitude_Location = Projection::INVALID_VALUE;
        latitude(value);
    }
}

void Image_Info_Panel::longitude_direction(int direction)
{
    if (Longitude_Direction != direction) Longitude_Direction = direction;
    else if (!Projection::is_invalid(Longitude_Location))
    {
        //	Refresh the displayed value.
        double value = Longitude_Location;
...
void Image_Info_Panel::location(const Coordinate& coordinate)
{
#if ((DEBUG_SECTION) & DEBUG_WORLD_DATA)
...
void Image_Info_Panel::longitude(double value)
{
    if (Longitude_Location != value)
    {
        Longitude_Location = value;
        if (Projection::is_invalid(value)) Longitude->clear();
        else
        {
            if (Longitude_Direction == WEST) value = 360 - value;
            Longitude->setText(location_representation(value, Longitude_Units));
        }
    }
}

void Image_Info_Panel::latitude(double value)
{
    if (Latitude_Location != value)
    {
        Latitude_Location = value;
        if (Projection::is_invalid(value)) Latitude->clear();
        else Latitude->setText(location_representation(value, Latitude_Units));
    }
}

QString Image_Info_Panel::location_representation(double value, int units) const
{
#if ((DEBUG_SECTION) & DEBUG_WORLD_DATA)
    clog << ">>> Image_Info_Panel::location_representation: " << value << " units " << units
         << endl;
#endif
    ...
    {
        case DEGREES:
            representation = QString("%1").arg(value, 16, 'f', degree_precision);
            break;
        case HMS:
        ...
        case RADIANS:
            ...
            representation = QString("%1").arg(value, 16, 'f', degree_precision + 2);
    }
#if ((DEBUG_SECTION) & DEBUG_WORLD_DATA)
    clog << "<<< Image_Info_Panel::location_representation: " << representation << endl;
#endif
...
void Image_Info_Panel::update_region_stats()
{
    ...
    if (Evaluate_R || Evaluate_G || Evaluate_B) evaluate_script();
}

void Image_Info_Panel::add_exception(int exception)
{
    if (Exception_List.indexOf(exception) == -1) { Exception_List.push_back(exception); }
}

void Image_Info_Panel::clear_exceptions() { Exception_List.clear(); }
...
void Image_Info_Panel::array_to_string(idaeim::PVL::Array& array, QJSValue& engine_array)
{
    ...
    {
        if (i->is_Array()) array_to_string(static_cast<idaeim::PVL::Array&>(*i), engine_array);
        else
        {
            engine_array.setProperty(index,
                                     QString::fromStdString(static_cast<std::string>(*i).c_str()));
            ++index;
        }
    }
}
...
void Image_Info_Panel::get_properties(idaeim::PVL::Aggregate& metadata)
{
    ...
    for (idaeim::PVL::Aggregate::Depth_Iterator parameters = metadata.begin_depth();
         parameters != end; ++parameters)
    {
        // if is_Aggregate() the parameter contains metadata, so make recursive call to this
        // function
        ...
        {
            QString const qname = QString::fromStdString(parameters->name()).remove(':');
            ...
            else
            {
                ...
                if (value.is_Real())
                {
                    Global_Object.setProperty(name,
                                              static_cast<idaeim::PVL::Value::Real_type>(value));
                }
                else if (value.is_Integer())
                {
                    /*
                     * Note, no method signature corresponding to
                     * Value::Integer_type (long long) or Value::Unsigned_Integer_type (unsigned
                     * long long)
                     */
                    if (value.is_signed())
                    {
                ...
                {
                    // TODO check this
                    QString const tstr = QString::fromStdString(
                        static_cast<idaeim::PVL::Value::String_type>(value).c_str());

                    Global_Object.setProperty(
                        name,
                        1.0 * QDateTime::fromString(tstr, "yyyyMMdd'T'HHmmss").toMSecsSinceEpoch());
                }
                // TODO: check is_Identifier or is_Symbol ??
                else
                {
                    Global_Object.setProperty(
                        name, QString::fromStdString(static_cast<std::string>(value).c_str()));
                }
            }
        }
    }
...
{
    Global_Object.setProperty(name, static_cast<double>(data));
    if (Script.contains(name)) evaluate_script();
}
...
{
    /* moved in qt6 */
    if ((Evaluate_R || Evaluate_G || Evaluate_B) && Use_Avg_Rgb && Statistics != nullptr)
    {
        QVector<Plastic_Image::Histogram*>& histograms = Statistics->histograms();
        int const lower_limit = Statistics->lower_limit();
        int const upper_limit = histograms[0]->size() - Statistics->upper_limit() - 1;
        int count = 0;
        double result = 0.0;
...
void Image_Info_Panel::set_metadata(idaeim::PVL::Aggregate* metadata)
{
    ...
    if (metadata != nullptr)
    {
        ...
        idaeim::PVL::Parameter const* param =
            metadata->find("CORE_NULL", false, 0, idaeim::PVL::Parameter::ASSIGNMENT);
        if (param != nullptr)
        {
            idaeim::PVL::Value const& value = param->value();
            if (value.is_Integer()) { add_exception(static_cast<int>(value)); }
        }
        param = metadata->find("CORE_LOW_REPR_SATURATION", false, 0,
        ...
        param = metadata->find("CORE_LOW_INSTR_SATURATION", false, 0,
                               idaeim::PVL::Parameter::ASSIGNMENT);
        if (param != nullptr)
        {
            idaeim::PVL::Value const& value = param->value();
        ...
        param = metadata->find("CORE_HIGH_REPR_SATURATION", false, 0,
                               idaeim::PVL::Parameter::ASSIGNMENT);
        if (param != nullptr)
        {
            idaeim::PVL::Value const& value = param->value();
        ...
        param = metadata->find("CORE_HIGH_INSTR_SATURATION", false, 0,
                               idaeim::PVL::Parameter::ASSIGNMENT);
        if (param != nullptr)
        {
            idaeim::PVL::Value const& value = param->value();
...
void Image_Info_Panel::check_names()
{
    QList<QString> list = QList<QString>()
                          << "x_px" << "y_px" << "red" << "green" << "blue" << "scale";
        ...
        if (min != script.length())
        {
            ...
            switch (min_type)
            {
                case 0:
                    ...
                    if (j != -1) { script.remove(min, j - min + 1); }
                    else
                    {
                ...
                case 1:
                    j = script.indexOf('\'', min + 1);
                    if (min != -1) { script.remove(min, j - min + 1); }
                    else
                    {
                        script.remove(min, script.length() - min);
                    }
                ...
                case 3:
                    ...
                    if (j != -1) { script.remove(min, j - min + 2); }
                    else
                    {
                        script.remove(min, script.length() - min);
                    }
