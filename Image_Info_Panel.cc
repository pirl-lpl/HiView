...
Image_Info_Panel::Image_Info_Panel(QWidget *parent)
    : QFrame(parent), Image_Values(true), Use_Avg_Rgb(false), Image_Bands(3), Display_Data(tr(DISPLAY_DATA_LABEL)),
      Image_Data(tr(IMAGE_DATA_LABEL)), Projector(NULL), Statistics(NULL), Exception_List(QList<int>())
{
...
#if ((DEBUG_SECTION) & DEBUG_CONSTRUCTORS)
    clog << "    Image_Info_Panel sizeHint = " << sizeHint() << endl << "<<< Image_Info_Panel" << endl;
#endif
}
...
QWidget *Image_Info_Panel::image_data_panel()
{
    ...
    clog << "    layout margins = " << left << "l, " << top << "t, " << right << "r, " << bottom << 'b' << endl;
#endif
    ...
    const char *band_label[] = {"<font color='red'><b>r</b></font>", "<font color='green'><b>g</b></font>",
    ...
    for (int band = 0; band < 3; band++)
    {
        ...
        clog << "    Pixel_Value[" << band << "] sizeHint = " << Pixel_Value[band]->sizeHint() << endl;
#endif
        ...
        clog << "    Pixel_Label[" << band << "] sizeHint = " << Pixel_Label[band]->sizeHint() << endl;
#endif
    }
    ...
    clog << "    sizeHint = " << panel->sizeHint() << endl
         ...
         << "     margins = " << left << "l, " << top << "t, " << right << "r, " << bottom << 'b' << endl
...
QWidget *Image_Info_Panel::world_location_panel()
{
    ...
    int left, top, right, bottom;
    layout->getContentsMargins(&left, &top, &right, &bottom);
    clog << "    projection name layout margins = " << left << "l, " << top << "t, " << right << "r, " << bottom << 'b'
         << endl;
#endif
...
#if ((DEBUG_SECTION) & DEBUG_CONSTRUCTORS)
    layout->getContentsMargins(&left, &top, &right, &bottom);
    clog << "    locations layout margins = " << left << "l, " << top << "t, " << right << "r, " << bottom << 'b'
         << endl;
#endif
...
void Image_Info_Panel::image_values(bool enabled)
{
    if (enabled != Image_Values)
    {
        ...
        Display_Value.Datum[0] = Display_Value.Datum[1] = Display_Value.Datum[2] = Image_Value.Datum[0] =
            Image_Value.Datum[1] = Image_Value.Datum[2] = static_cast<Plastic_Image::Pixel_Datum>(-2);
...
void Image_Info_Panel::cursor_location(const QPoint &display_location, const QPoint &image_location)
{
    ...
    if (Image_Values)
    {
        if (image_location != Image_Location)
        {
            ...
            if (!Projector || !World_Location->isVisible())
                location(coordinate);
            else
...
)
{
    if (Image_Scale != scaling)
    {
        ...
        if (scale_y != scale_x)
            report += QString(" / %1").arg(scale_y, 6, 'f', 3);
...
void Image_Info_Panel::projection(Projection *projector)
{
    ...
    {
#if ((DEBUG_SECTION) & DEBUG_WORLD_DATA)
        clog << "    canonical_projection_name = " << Projector->canonical_projection_name() << endl;
#endif
        /*	QString
                name ("<b>");
...
void Image_Info_Panel::latitude(double value)
{
    if (Latitude_Location != value)
    {
        ...
        if (Projection::is_invalid(value))
            ...
            Latitude->setText(location_representation(value, Latitude_Units));
    }
}
...
{
#if ((DEBUG_SECTION) & DEBUG_WORLD_DATA)
    clog << ">>> Image_Info_Panel::location_representation: " << value << " units " << units << endl;
#endif
            ...
            else
            {
                ...
                else if (value.is_Date_Time())
                {
                    ...
                    Global_Object.setProperty(
                        name, 1.0 * QDateTime::fromString(tstr, "yyyyMMdd'T'HHmmss").toMSecsSinceEpoch());
                }
    ...
    for (int i = 0; i < list->size(); ++i)
    {
        ...
        while ((j = script.indexOf(list->at(i), j)) != -1)
        {
            if ((j == 0 || !(script[j - 1].isLetterOrNumber() || reject_chars_before_var.contains(script[j - 1]))) &&
                ((j + len) >= script.length() ||
                 !(script[j + len].isLetterOrNumber() || reject_chars_after_var.contains(script[j + len]))))
            {
                break;
            }
            ++j;
