...
#define DEBUG_DEFAULT (DEBUG_CONSTRUCTORS & DEBUG_CONTROL_PANEL)
...
/*==============================================================================
    Constants
*/
const char *const Data_Mapper_Tool::ID =
...
#endif
const QSize Data_Mapper_Tool::GRAPH_MIN_SIZE(DATA_MAPPER_TOOL_GRAPH_MIN_WIDTH, DATA_MAPPER_TOOL_GRAPH_MIN_HEIGHT);
...
*/
#define MAX_DISPLAY_VALUE HiView_Config::MAX_DISPLAY_VALUE
#define MAX_SOURCE_DATA_PRECISION HiView_Config::MAX_SOURCE_DATA_PRECISION
#define PERCENT_DECIMAL_PLACES HiView_Config::PERCENT_DECIMAL_PLACES

#define Panel_Frame_Style HiView_Config::Panel_Frame_Style
#define Panel_Frame_Width HiView_Config::Panel_Frame_Width
#define Label_Frame_Style HiView_Config::Label_Frame_Style
#define Label_Frame_Width HiView_Config::Label_Frame_Width
#define Label_Frame_Margin HiView_Config::Label_Frame_Margin
#define Heading_Line_Weight HiView_Config::Heading_Line_Weight

#define Crosshair_Cursor HiView_Config::Crosshair_Cursor
#define Move_Horizontal_Cursor HiView_Config::Move_Horizontal_Cursor
#define Greater_Than_Cursors HiView_Config::Greater_Than_Cursors
#define Less_Than_Cursors HiView_Config::Less_Than_Cursors

#define Default_Graph_Canvas_Color HiView_Config::Default_Graph_Canvas_Color

#define DISPLAY_BAND_NAMES HiView_Config::DISPLAY_BAND_NAMES
#define DISPLAY_BAND_COLORS HiView_Config::DISPLAY_BAND_COLORS

#define SELECTED_NONE HiView_Config::SELECTED_NONE
#define SELECTED_RED HiView_Config::SELECTED_RED
#define SELECTED_GREEN HiView_Config::SELECTED_GREEN
#define SELECTED_BLUE HiView_Config::SELECTED_BLUE

#define TRACKER_BOUND_MODE HiView_Config::TRACKER_BOUND_MODE
#define TRACKER_TRACK_MODE HiView_Config::TRACKER_TRACK_MODE
#define TRACKER_DRAG_MODE HiView_Config::TRACKER_DRAG_MODE

/*==============================================================================
    Class data members
*/
QErrorMessage *Data_Mapper_Tool::Error_Message = NULL;
...
namespace
{
const char *BAND_SELECTION_ICONS[] = {RED_BAND_SELECTION_ICON, GREEN_BAND_SELECTION_ICON, BLUE_BAND_SELECTION_ICON,
                                      ALL_BANDS_SELECTION_ICON};

//	Plot items are painted in order of increasing depth; highest on top.
const double PLOT_DEPTH[] = {3, 2, 1};
...
#endif
int Data_Mapper_Tool::Default_Selection_Distance = DEFAULT_SELECTION_DISTANCE;
...
QString Data_Mapper_Tool::Default_Graph_Data_Format = _DEFAULT_GRAPH_DATA_FORMAT_;

/*==============================================================================
    Constructor
*/
Data_Mapper_Tool::Data_Mapper_Tool(Data_Map **maps, QWidget *parent)
    : QDockWidget(tr("Data Mapper"), parent), Data_Maps(NULL), Max_Source_Value(0), Selected_Bands(SELECTED_ALL),
      Selection_Distance(Default_Selection_Distance), Signal_Changes(true), Upper_Limit(0), Lower_Limit(0),
      Operating_Mode(TRACKING_MODE), Graph(NULL), Load_File_Dialog(NULL), Save_File_Dialog(NULL),
      Graph_Data_Format(Default_Graph_Data_Format), Visible_Graph(true)
{
    ...
    Functions[0] = Functions[1] = Functions[2] = NULL;
    Nodes[0] = Nodes[1] = Nodes[2] = NULL;

    //	Initialize the data maps.
    reset_data_maps(maps);

    QWidget *panel = new QWidget(this), *sub_panel;
...
void print_nodes(Function_Nodes **nodes)
{
    int band = -1;
    while (++band < 3)
    {
...
*/
class Function : public QwtSeriesStore<QPointF>
{
  public:
    Function(Data_Mapper_Tool::Data_Map *data_map) : QwtSeriesStore(), Map(data_map)
    {
#if ((DEBUG_SECTION) & DEBUG_CONSTRUCTORS)
        clog << ">-< Data_Mapper_Tool::Function @ " << (void *)this << ": Data_Map @ " << (void *)Map << endl;
#endif
    }
    ...
    */
    ...
    void data_map(Data_Mapper_Tool::Data_Map *data_map)
    {
        Map = data_map;
    }
...
/*==============================================================================
    Accessors
*/
void Data_Mapper_Tool::data_maps(Data_Map **maps)
{
    ...
    {
#if ((DEBUG_SECTION) & DEBUG_DATA_MAPS)
        clog << "    same maps" << endl << "<<< Data_Mapper_Tool::data_maps" << endl;
#endif
        return;
    }
    ...
    clog << "    old Nodes -" << endl;
    print_nodes(Nodes);
    clog << "    Upper_Bound_Value[0] = " << Upper_Bound_Value[0]->value() << endl
         << "    Upper_Bound_Value[1] = " << Upper_Bound_Value[1]->value() << endl
         << "    Upper_Bound_Value[2] = " << Upper_Bound_Value[2]->value() << endl
         << "    Lower_Bound_Value[0] = " << Lower_Bound_Value[0]->value() << endl
         << "    Lower_Bound_Value[1] = " << Lower_Bound_Value[1]->value() << endl
         << "    Lower_Bound_Value[2] = " << Lower_Bound_Value[2]->value() << endl;
#endif
    ...
    for (int band = 0; band < 3; ++band)
    {
        ...
        if (nodes->size() != 2 || nodes->size() != maps[band]->size() || nodes->at(0).x() != 0 ||
            nodes->at(0).y() != 0 || nodes->at(1).x() != max_x() || nodes->at(1).y() != max_y())
        {
            changed = true;
            break;
        }
    ...
    {
#if ((DEBUG_SECTION) & DEBUG_DATA_MAPS)
        clog << "    reset Functions/Nodes[" << band << "] data_map = " << (void *)(Data_Maps[band]) << endl;
#endif
...
#if ((DEBUG_SECTION) & DEBUG_DATA_MAPS)
        clog << "    Nodes[" << band << "] max = " << Nodes[band]->nodes()->at(1) << endl;
#endif
...
void Data_Mapper_Tool::reset_data_maps(Data_Map **maps)
{
#if ((DEBUG_SECTION) & DEBUG_DATA_MAPS)
    clog << ">>> Data_Mapper_Tool::reset_data_maps: @ " << (void *)maps << endl;
#endif
    if (!maps || !maps[0] || !maps[0]->size() || !maps[1] || !maps[1]->size() || !maps[2] || !maps[2]->size())
    {
        ostringstream message;
        message << ID << endl << "Can't use empty data maps.";
#if ((DEBUG_SECTION) & DEBUG_DATA_MAPS)
        clog << "!!! " << message.str() << endl << "<<< Data_Mapper_Tool::reset_data_maps" << endl;
#endif
        throw invalid_argument(message.str());
    }

    int map_size = maps[0]->size();
    if (map_size > (1 << MAX_SOURCE_DATA_PRECISION))
    {
        ostringstream message;
        message << ID << endl
                << "Invalid data maps have " << map_size
                << " entries,"
                   " but the maximum is "
                << (1 << MAX_SOURCE_DATA_PRECISION) << " (" << MAX_SOURCE_DATA_PRECISION << "-bit precision).";
#if ((DEBUG_SECTION) & DEBUG_DATA_MAPS)
        clog << "!!! " << message.str() << endl << "<<< Data_Mapper_Tool::reset_data_maps" << endl;
#endif
        throw invalid_argument(message.str());
    }
    if (map_size != maps[1]->size() || map_size != maps[2]->size())
    {
        ostringstream message;
        message << ID << endl
                << "Invalid data maps have different sizes: " << map_size << ", " << maps[1]->size() << ", and "
                << maps[2]->size() << " entries.";
#if ((DEBUG_SECTION) & DEBUG_DATA_MAPS)
        clog << "!!! " << message.str() << endl << "<<< Data_Mapper_Tool::reset_data_maps" << endl;
#endif
...
void Data_Mapper_Tool::remap_data(int bounds_selection, bool unconditional)
{
#if ((DEBUG_SECTION) & DEBUG_DATA_MAPS)
    clog << ">>> Data_Mapper_Tool::remap_data: " << bounds_selection << ", " << boolalpha << unconditional << endl
         << "    pre-remap Nodes:" << endl;
    print_nodes(Nodes);
#endif
    bool upper_changed = unconditional, lower_changed = unconditional;
    Signal_Changes = false;
    int band = 3;
    if (bounds_selection == REMAP_VALUES)
    {
        QVector<int> upper_values(3, -1), lower_values(3, -1);
        while (band--)
        {
...
//	>>> SIGNAL <<<
            ...
            clog << "^^^ Data_Mapper_Tool::remap_data: "
                 ...
                 << upper_values[0] << ", " << upper_values[1] << ", " << upper_values[2] << endl;
#endif
...
//	>>> SIGNAL <<<
            ...
            clog << "^^^ Data_Mapper_Tool::remap_data: "
                 ...
                 << lower_values[0] << ", " << lower_values[1] << ", " << lower_values[2] << endl;
#endif
    ...
    {
        QVector<double> upper_percents(3, -1.0), lower_percents(3, -1.0);
        while (band--)
        {
            if (Selected_Bands & (1 << band))
            {
                upper_percents[band] = Upper_Bound_Percent_Setting[band]->value();
                upper_changed |= upper_bound_percent(upper_percents[band], band, true);
                lower_percents[band] = Lower_Bound_Percent_Setting[band]->value();
                lower_changed |= lower_bound_percent(lower_percents[band], band, true);
            }
        }
...
//	>>> SIGNAL <<<
            ...
            clog << "^^^ Data_Mapper_Tool::remap_data: "
                 ...
                 << upper_percents[0] << ", " << upper_percents[1] << ", " << upper_percents[2] << endl;
#endif
...
//	>>> SIGNAL <<<
            ...
            clog << "^^^ Data_Mapper_Tool::remap_data: "
                 ...
                 << lower_percents[0] << ", " << lower_percents[1] << ", " << lower_percents[2] << endl;
#endif
...
int Data_Mapper_Tool::max_y() const
{
    return MAX_DISPLAY_VALUE;
}

int Data_Mapper_Tool::default_selection_distance(int distance)
{
    int previous_distance = Default_Selection_Distance;
    if (distance < 0)
        distance = 0;
...
void Data_Mapper_Tool::selection_distance(int distance)
{
    if (distance < 0)
        distance = 0;
...
void Data_Mapper_Tool::band_map(const unsigned int *bands)
{
    for (int band = 0; band < 3; band++)
        Band_Mapping_Labels[band]->setText(
            QString::number(HiView_Utilities::band_index_to_number(Band_Map[band] = bands[band]))
                .prepend(tr("<b>Band "))
                .append(" - &")
                .append(tr(DISPLAY_BAND_NAMES[band])));
}

void Data_Mapper_Tool::refresh_band_numbers()
{
    band_map(Band_Map);
}

void Data_Mapper_Tool::canvas_color(QRgb color)
{
    Graph->setCanvasBackground(QBrush(color));
    Graph->replot();
}

QList<QAction *> Data_Mapper_Tool::band_selection_actions() const
{
    QList<QAction *> actions;
    actions << Band_Selection_Actions[0] << Band_Selection_Actions[1] << Band_Selection_Actions[2]
            << Band_Selection_Actions[3]; //	All bands.
    return actions;
}

QList<QAction *> Data_Mapper_Tool::presets_actions() const
{
    QList<QAction *> actions;
    actions << Limited_Linear_Action << Bounded_Linear_Action << Unbounded_Linear_Action;
    return actions;
}

QList<QAction *> Data_Mapper_Tool::file_actions() const
{
    QList<QAction *> actions;
    actions << Load_CSV_File_Action << Load_Last_CSV_File_Action << Save_File_Action << Save_Last_CSV_File_Action;
    return actions;
}
...
/*==============================================================================
    GUI elements
*/
QSize Data_Mapper_Tool::minimumSizeHint() const
{
...
QSize Data_Mapper_Tool::sizeHint() const
{
#if ((DEBUG_SECTION) & DEBUG_LAYOUT)
...
/*==============================================================================
    Event Handlers
*/
void Data_Mapper_Tool::resizeEvent(QResizeEvent *event)
{
#if ((DEBUG_SECTION) & DEBUG_EVENTS)
    clog << ">-< Data_Mapper_Tool::resizeEvent: from " << event->oldSize() << " to " << event->size() << endl;
#endif
    Previous_Size = event->oldSize();
    QDockWidget::resizeEvent(event);
}

void Data_Mapper_Tool::contextMenuEvent(QContextMenuEvent *event)
{
    //	>>> SIGNAL <<<
...
/*------------------------------------------------------------------------------
    Control Panel
*/
QWidget *Data_Mapper_Tool::control_panel()
{
...
#endif
    panel_layout->addWidget(label = new QLabel(tr("<b>Band Selections</b>")), 1, Qt::AlignLeft | Qt::AlignVCenter);
    Band_Selection_Icon_Size = label->sizeHint();
    Band_Selection_Icon_Size.rwidth() = Band_Selection_Icon_Size.rheight() +=
...
//		Band Selections.
    ...
    char band_character[2] = {' ', 0};

    for (band = 0; band < 3; band++)
    {
        Band_Selection_Actions[band] = new QAction(QString("&") + tr(DISPLAY_BAND_NAMES[band]) + tr(" Band"), this);
        Band_Selection_Icons[band] = new QIcon(BAND_SELECTION_ICONS[band]);
#if ((DEBUG_SECTION) & DEBUG_CONTROL_PANEL)
        clog << "    Band_Selection_Icons[" << band << "] @ " << (void *)Band_Selection_Icons[band] << endl;
#endif
        ...
        Band_Selection_Actions[band]->setToolTip(tr("Disable mapping modification of the ") +
                                                 tr(DISPLAY_BAND_NAMES[band]) + tr(" band"));
        *band_character = *DISPLAY_BAND_NAMES[band];
        Band_Selection_Actions[band]->setShortcut(QString("Ctrl+Shift+") + tr(band_character));
#if ((DEBUG_SECTION) & DEBUG_CONTROL_PANEL)
        clog << "      band_character " << band_character
             << ", shortcut = " << Band_Selection_Actions[band]->shortcut().toString() << endl;
#endif
        connect(Band_Selection_Actions[band], SIGNAL(toggled(bool)), SLOT(select_band()));

        Band_Selection_Buttons[band] = new Icon_Button(Band_Selection_Actions[band]->icon());
        ...
        Band_Selection_Buttons[band]->setToolTip(Band_Selection_Actions[band]->toolTip());
        connect(Band_Selection_Buttons[band], SIGNAL(toggled(bool)), Band_Selection_Actions[band],
                SLOT(setChecked(bool)));
        grid_layout->addWidget(Band_Selection_Buttons[band], band, 0, Qt::AlignCenter);

        Band_Mapping_Labels[band] = new QLabel;
        Band_Mapping_Labels[band]->setBuddy(Band_Selection_Buttons[band]);
        Band_Mapping_Labels[band]->setToolTip(tr("Source to display mapping"));
        ...
        Band_Mapping_Labels[band]->setPalette(QPalette(QColor(DISPLAY_BAND_COLORS[band]).lighter()));
        Band_Mapping_Labels[band]->setAutoFillBackground(true);
        grid_layout->addWidget(Band_Mapping_Labels[band], band, 1, Qt::AlignVCenter);
    }
    ...
    Band_Selection_Actions[3]->setToolTip(tr("Enable mapping modification of all bands"));
    Band_Selection_Actions[3]->setShortcut(tr("Ctrl+Shift+A"));
    connect(Band_Selection_Actions[3], SIGNAL(triggered()), SLOT(select_band()));
...
#endif
    panel_layout->addWidget(new QLabel(tr("<b>Mapping</b>")), 1, Qt::AlignLeft | Qt::AlignVCenter);
...
//		Presets popup menu.
    ...
    Limited_Linear_Action->setToolTip(tr("Linear mapping between the valid data limits"));
    Limited_Linear_Action->setShortcut(tr("Ctrl+Alt+L"));
    connect(Limited_Linear_Action, SIGNAL(triggered()), SLOT(limited_linear_mapping()));
    menu->addAction(Limited_Linear_Action);
    //			Bounded Linear mapping.
    Bounded_Linear_Action = new QAction(tr("&Bounded Linear"), this);
    Bounded_Linear_Action->setToolTip(tr("Linear mapping between the saturation bounds"));
    Bounded_Linear_Action->setShortcut(tr("Ctrl+Alt+B"));
    connect(Bounded_Linear_Action, SIGNAL(triggered()), SLOT(bounded_linear_mapping()));
    ...
    connect(Unbounded_Linear_Action, SIGNAL(triggered()), SLOT(linear_mapping()));
    menu->addAction(Unbounded_Linear_Action);
    button->setMenu(menu);
    horizontal_layout->addWidget(button, 1, Qt::AlignLeft | Qt::AlignVCenter);
...
//		File popup menu.
    ...
    Load_CSV_File_Action->setToolTip(tr("Load mappings of enabled bands from a file"));
    Load_CSV_File_Action->setShortcut(tr("Ctrl+Alt+O"));
    connect(Load_CSV_File_Action, SIGNAL(triggered()), SLOT(load()));
    ...
    connect(Load_Last_CSV_File_Action, SIGNAL(triggered()), SLOT(last_file_action()));
    ...
    connect(Save_File_Action, SIGNAL(triggered()), SLOT(save()));
    ...
    connect(Save_Last_CSV_File_Action, SIGNAL(triggered()), SLOT(last_file_action()));
    menu->addAction(Save_Last_CSV_File_Action);

    File_Menu_Button->setMenu(menu);
    horizontal_layout->addWidget(File_Menu_Button, 1, Qt::AlignLeft | Qt::AlignVCenter);
...
//		Heading.
    ...
    connect(Bounds_Button, SIGNAL(toggled(bool)), SLOT(saturation_controls(bool)));
    horizontal_layout->addWidget(Bounds_Button, 1, Qt::AlignLeft | Qt::AlignVCenter);
    label = new QLabel(tr("<b>Sa&turation Bounds</b>"));
    horizontal_layout->addWidget(label, 1, Qt::AlignLeft | Qt::AlignVCenter);
    ...
    Default_Contrast_Stretch_Action = new QAction(tr("Default &Constrast Stretch"), this);
    Default_Contrast_Stretch_Action->setToolTip(tr("Apply default contrast stretch percentages"));
    ...
    connect(Default_Contrast_Stretch_Action, SIGNAL(triggered()), SLOT(default_contrast_stretch()));
    ...
    connect(icon_button, SIGNAL(clicked()), Default_Contrast_Stretch_Action, SLOT(trigger()));
    horizontal_layout->addWidget(icon_button, 1, Qt::AlignLeft | Qt::AlignVCenter);

    Restore_Original_Contrast_Stretch_Action = new QAction(tr("Restore &Original Contrast Stretch"), this);
    Restore_Original_Contrast_Stretch_Action->setToolTip(tr("Restore the original contrast stretch percentages"));
    ...
    connect(Restore_Original_Contrast_Stretch_Action, SIGNAL(triggered()), SLOT(restore_original_contrast_stretch()));
    ...
    connect(icon_button, SIGNAL(clicked()), Restore_Original_Contrast_Stretch_Action, SLOT(trigger()));
    horizontal_layout->addWidget(icon_button, 1, Qt::AlignLeft | Qt::AlignVCenter);

    horizontal_layout->addStretch(100);
    panel_layout->addLayout(horizontal_layout, 1);
    //		Bounds.
    QWidget *saturation_controls = new QWidget;
    int row = -1, headings_top_row = 0, headings_bottom_row = headings_top_row + 1,
        headings_underline_row = headings_bottom_row + 1, first_upper_row = headings_underline_row + 1,
        divider_row = first_upper_row + 3, first_lower_row = divider_row + 1, labels_col = 0,
        labels_line_col = labels_col + 1, values_col = labels_line_col + 1, values_slider_col = values_col + 1,
        divider_col = values_slider_col + 1, percents_slider_col = divider_col + 1,
        percents_setting_col = percents_slider_col + 1, percents_actual_col = percents_setting_col + 1,
    ...
    grid_layout->addWidget(label, headings_top_row, values_col, 1, 2, Qt::AlignLeft | Qt::AlignVCenter);
    label = new QLabel(tr("Values"));
    label->setAlignment(Qt::AlignLeft | Qt::AlignVCenter);
    grid_layout->addWidget(label, headings_bottom_row, values_col, 1, 2, Qt::AlignLeft | Qt::AlignVCenter);

    label = new QLabel(tr("Histogram Percents"));
    label->setAlignment(Qt::AlignLeft | Qt::AlignVCenter);
    grid_layout->addWidget(label, headings_top_row, percents_slider_col, 1, 3, Qt::AlignLeft | Qt::AlignVCenter);
    ...
    connect(icon_button, SIGNAL(clicked()), Apply_Percents_Action, SLOT(trigger()));
    horizontal_layout->addWidget(icon_button, 1, Qt::AlignRight);

    //		Actual percents to settings action.
    Actual_to_Settings_Action = new QAction(tr("Percents Settings from Actual"), this);
    Actual_to_Settings_Action->setToolTip(tr("Percents Settings set from Actual Percents values"));
    Actual_to_Settings_Action->setShortcut(tr("Ctrl+Alt+Shift+P"));
    connect(Actual_to_Settings_Action, SIGNAL(triggered()), SLOT(bound()));

    grid_layout->addLayout(horizontal_layout, headings_bottom_row, percents_slider_col, 1, 2,
                           Qt::AlignLeft | Qt::AlignVCenter);

    //		Actual percents column.
    label = new QLabel(tr("Actual"));
    label->setAlignment(Qt::AlignRight | Qt::AlignVCenter);
    grid_layout->addWidget(label, headings_bottom_row, percents_actual_col, Qt::AlignRight | Qt::AlignVCenter);
    //		Actual percent column minimum width.
    label = new QLabel("100.00");
    grid_layout->setColumnMinimumWidth(percents_actual_col, label->sizeHint().width() + 5);
    delete label;

    //		Underline.
    grid_layout->addWidget(new Drawn_Line, headings_underline_row, labels_line_col, 1, padding_col - labels_line_col);

    double percent_increment = 1.0;
    for (int decimal_places = PERCENT_DECIMAL_PLACES; decimal_places; --decimal_places)
    ...
    for (band = 0; band < 3; band++)
    {
        ...
        grid_layout->addWidget(line, row, labels_line_col);

        Upper_Bound_Value[band] = new QSpinBox;
#if ((DEBUG_SECTION) & (DEBUG_CONTROL_PANEL | DEBUG_BOUNDS))
        clog << "    Upper_Bound_Value[" << band << "] @ " << (void *)Upper_Bound_Value[band] << endl;
#endif
        Upper_Bound_Value[band]->setRange(1, max_x());
        ...
        connect(Upper_Bound_Value[band], SIGNAL(valueChanged(int)), SLOT(bound()));
        grid_layout->addWidget(Upper_Bound_Value[band], row, values_col, Qt::AlignRight | Qt::AlignTop);

        Upper_Bound_Percent_Setting[band] = new QDoubleSpinBox;
#if ((DEBUG_SECTION) & (DEBUG_CONTROL_PANEL | DEBUG_BOUNDS))
        clog << "    Upper_Bound_Percent_Setting[" << band << "] @ " << (void *)Upper_Bound_Percent_Setting[band]
             << endl;
#endif
        ...
        connect(Upper_Bound_Percent_Setting[band], SIGNAL(valueChanged(double)), SLOT(bound()));
        grid_layout->addWidget(Upper_Bound_Percent_Setting[band], row, percents_setting_col,
                               Qt::AlignLeft | Qt::AlignTop);

        Upper_Bound_Percent_Actual[band] = new QLabel("  0.00");
#if ((DEBUG_SECTION) & (DEBUG_CONTROL_PANEL | DEBUG_BOUNDS))
        clog << "    Upper_Bound_Percent_Actual[" << band << "] @ " << (void *)Upper_Bound_Percent_Actual[band] << endl;
#endif
        Upper_Bound_Percent_Actual[band]->setAlignment(Qt::AlignRight | Qt::AlignVCenter);
        grid_layout->addWidget(Upper_Bound_Percent_Actual[band], row, percents_actual_col,
...
#if ((DEBUG_SECTION) & (DEBUG_CONTROL_PANEL | DEBUG_BOUNDS))
        clog << "    Lower_Bound_Value[" << band << "] @ " << (void *)Lower_Bound_Value[band] << endl;
#endif
        Lower_Bound_Value[band]->setRange(0, max_x() - 1);
        ...
        connect(Lower_Bound_Value[band], SIGNAL(valueChanged(int)), SLOT(bound()));
        grid_layout->addWidget(Lower_Bound_Value[band], row, values_col, Qt::AlignRight | Qt::AlignBottom);

        Lower_Bound_Percent_Setting[band] = new QDoubleSpinBox;
#if ((DEBUG_SECTION) & (DEBUG_CONTROL_PANEL | DEBUG_BOUNDS))
        clog << "    Lower_Bound_Percent_Setting[" << band << "] @ " << (void *)Lower_Bound_Percent_Setting[band]
             << endl;
#endif
        ...
        connect(Lower_Bound_Percent_Setting[band], SIGNAL(valueChanged(double)), SLOT(bound()));
        grid_layout->addWidget(Lower_Bound_Percent_Setting[band], row, percents_setting_col,
                               Qt::AlignLeft | Qt::AlignBottom);

        Lower_Bound_Percent_Actual[band] = new QLabel("  0.00");
#if ((DEBUG_SECTION) & (DEBUG_CONTROL_PANEL | DEBUG_BOUNDS))
        clog << "    Lower_Bound_Percent_Actual[" << band << "] @ " << (void *)Lower_Bound_Percent_Actual[band] << endl;
#endif
        Lower_Bound_Percent_Actual[band]->setAlignment(Qt::AlignRight | Qt::AlignVCenter);
        grid_layout->addWidget(Lower_Bound_Percent_Actual[band], row, percents_actual_col,
                               Qt::AlignRight | Qt::AlignVCenter);
    ...
    */
    ...
    grid_layout->addWidget(rotated_label, first_upper_row, labels_col, 3, 1, Qt::AlignHCenter | Qt::AlignTop);

    rotated_label = new Rotated_Label(tr("Lower"));
    rotated_label->setToolTip(tr("Saturation lower bounds"));
    rotated_label->text_rotation(Rotated_Label::COUNTER_CLOCKWISE);
    grid_layout->addWidget(rotated_label, first_lower_row, labels_col, 3, 1, Qt::AlignHCenter | Qt::AlignBottom);

    //		Upper bounds value slider.
    int slider_height = Upper_Bound_Value[0]->sizeHint().height() * 3;
    Upper_Bound_Value_Slider = new QwtSlider(Qt::Vertical, this);
    // QwtSlider::NoScale, QwtSlider::BgBoth);
#if ((DEBUG_SECTION) & (DEBUG_CONTROL_PANEL | DEBUG_BOUNDS))
    clog << "    Upper_Bound_Value_Slider @ " << (void *)Upper_Bound_Value_Slider << endl;
#endif
    Upper_Bound_Value_Slider->setToolTip(tr("Change all upper bounds values"));
    ...
    grid_layout->addWidget(Upper_Bound_Value_Slider, first_upper_row, values_slider_col, 3, 1);
    connect(Upper_Bound_Value_Slider, SIGNAL(valueChanged(double)), SLOT(bound()));

    //		Values-Percents dividing line.
    line = new Drawn_Line(2);
    line->orientation(Qt::Vertical);
    line->alignment(Qt::AlignVCenter);
    grid_layout->addWidget(line, 0, divider_col, -1, 1, Qt::AlignHCenter);

    //		Upper bounds percent slider.
    Upper_Bound_Percent_Slider = new QwtSlider(Qt::Vertical, this);
    //, QwtSlider::NoScale, QwtSlider::BgBoth);
#if ((DEBUG_SECTION) & (DEBUG_CONTROL_PANEL | DEBUG_BOUNDS))
    clog << "    Upper_Bound_Percent_Slider @ " << (void *)Upper_Bound_Percent_Slider << endl;
#endif
    Upper_Bound_Percent_Slider->setToolTip(tr("Change all upper bounds percentages"));
    ...
    grid_layout->addWidget(Upper_Bound_Percent_Slider, first_upper_row, percents_slider_col, 3, 1);
    connect(Upper_Bound_Percent_Slider, SIGNAL(valueChanged(double)), SLOT(bound()));

    //		Lower-Upper bounds dividing line.
    line = new Drawn_Line(1);
    line->alignment(Qt::AlignVCenter);
    grid_layout->addWidget(line, divider_row, 0, 1, padding_col, Qt::AlignVCenter);
    ...
    //, QwtSlider::NoScale, QwtSlider::BgBoth);
#if ((DEBUG_SECTION) & (DEBUG_CONTROL_PANEL | DEBUG_BOUNDS))
    clog << "    Lower_Bound_Value_Slider @ " << (void *)Lower_Bound_Value_Slider << endl;
#endif
    Lower_Bound_Value_Slider->setToolTip(tr("Change all lower bounds values"));
    ...
    grid_layout->addWidget(Lower_Bound_Value_Slider, first_lower_row, values_slider_col, 4, 1);
    connect(Lower_Bound_Value_Slider, SIGNAL(valueChanged(double)), SLOT(bound()));

    //		Lower bounds percent slider.
    Lower_Bound_Percent_Slider = new QwtSlider(Qt::Vertical, this);
    // QwtSlider::NoScale, QwtSlider::BgBoth);
#if ((DEBUG_SECTION) & (DEBUG_CONTROL_PANEL | DEBUG_BOUNDS))
    clog << "    Lower_Bound_Percent_Slider @ " << (void *)Lower_Bound_Percent_Slider << endl;
#endif
    Lower_Bound_Percent_Slider->setToolTip(tr("Change all lower bounds percentages"));
    ...
    grid_layout->addWidget(Lower_Bound_Percent_Slider, first_lower_row, percents_slider_col, 4, 1);
    connect(Lower_Bound_Percent_Slider, SIGNAL(valueChanged(double)), SLOT(bound()));
...
#if ((DEBUG_SECTION) & (DEBUG_CONTROL_PANEL | DEBUG_BOUNDS))
    clog << "    control_panel minimum size = " << panel_layout->sizeHint() << endl
...
/*..............................................................................
    Band Selection.
*/
void Data_Mapper_Tool::select_band(bool enabled, int band)
{
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_CONTROL_PANEL))
    clog << ">>> Data_Mapper_Tool::select_band: " << boolalpha << enabled << " - band " << band << endl;
#endif
    if (band < 0)
    {
        //	All bands selection.
    ...
    else if (band < 3)
    {
        if (enabled == Band_Selection_Actions[band]->isChecked())
        {
            ...
            {
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_CONTROL_PANEL))
                clog << "    set Band_Selection_Icons[" << band << "] @ " << (void *)Band_Selection_Icons[band] << endl;
#endif
            ...
            {
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_CONTROL_PANEL))
                clog << "    set Not_Selected_Icon @ " << (void *)Not_Selected_Icon << endl;
#endif
                Selected_Bands &= ~(1 << band);
                Band_Selection_Actions[band]->setIcon(*Not_Selected_Icon);
                Band_Selection_Actions[band]->setToolTip(tr("Enable mapping modification of the ") +
                                                         tr(DISPLAY_BAND_NAMES[band]) + tr(" band"));

                //	Enable select all bands.
                Band_Selection_Actions[3]->setEnabled(true);
            }
            Band_Selection_Buttons[band]->setIcon(Band_Selection_Actions[band]->icon());
            Band_Selection_Buttons[band]->setToolTip(Band_Selection_Actions[band]->toolTip());
...
void Data_Mapper_Tool::selected_bands(int selection)
{
...
void Data_Mapper_Tool::select_band()
{
    ...
    if (source)
    {
        ...
        {
            int band = 3;
            while (band-- && source != Band_Selection_Actions[band])
                ;
            if (band >= 0)
...
/*..............................................................................
    Default Mapping.
*/
bool Data_Mapper_Tool::linear_mapping(Mapping_Extent extent)
{
    ...
    if (Selected_Bands)
    {
        bool upper_changed = false, lower_changed = false;
        QVector<int> upper_values(3, -1), lower_values(3, -1);
        int band = 3;
        while (band--)
        {
            if (Selected_Bands & (1 << band))
            {
                ...
                clog << "    Nodes[" << band << "] reset = " << boolalpha << nodes_changed << endl;
...
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_DATA_MAPS | DEBUG_BOUNDS))
                clog << "    upper_values[" << band << "] = " << upper_values[band] << endl;
#endif
                if (upper_bound_value(upper_values[band], band, true))
                    ...
                    upper_values[band] = -1;
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_DATA_MAPS | DEBUG_BOUNDS))
                clog << "      changed = " << ((upper_values[band] != -1) ? "true" : "false") << endl;
                clog << "    lower_values[" << band << "] = " << lower_values[band] << endl;
#endif
                if (lower_bound_value(lower_values[band], band, true))
                    ...
                    lower_values[band] = -1;
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_DATA_MAPS | DEBUG_BOUNDS))
                clog << "      changed = " << ((lower_values[band] != -1) ? "true" : "false") << endl;
#endif
            }
        }
        Signal_Changes = true;
        ...
        {
//	>>> SIGNAL <<<
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_DATA_MAPS | DEBUG_BOUNDS | DEBUG_SIGNALS))
...
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_DATA_MAPS | DEBUG_BOUNDS))
    print_data_maps(clog);
    clog << "<<< Data_Mapper_Tool::linear_mapping: " << boolalpha << changed << endl;
#endif
    return changed;
}

bool Data_Mapper_Tool::limited_linear_mapping()
{
    return linear_mapping(LIMITED_MAPPING);
}

bool Data_Mapper_Tool::bounded_linear_mapping()
{
...
/*..............................................................................
    Saturation Bounds
*/
void Data_Mapper_Tool::saturation_controls(bool enabled)
{
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_CONTROL_PANEL))
    clog << ">>> Data_Mapper_Tool::saturation_controls: " << boolalpha << enabled << endl;
#endif
...
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_CONTROL_PANEL))
            clog << "    set Bounds_Icon @ " << (void *)Bounds_Icon << endl;
#endif
            Bounds_Button->setIcon(*Bounds_Icon);
            Bounds_Button->setToolTip(tr("Saturation bounds graph manipulation disable"));
...
void Data_Mapper_Tool::bound()
{
    ...
    if (source)
    {
        ...
        {
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_CONTROL_PANEL | DEBUG_BOUNDS))
            clog << "    Upper_Bound_Value_Slider value = " << (int)Upper_Bound_Value_Slider->value() << endl;
#endif
            QVector<int> values(3, (int)Upper_Bound_Value_Slider->value());
            while (band--)
                if (!(Selected_Bands & (1 << band)))
                    values[band] = -1;
            upper_bound_values(values);
        }
        else if (source == Lower_Bound_Value_Slider)
        {
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_CONTROL_PANEL | DEBUG_BOUNDS))
            clog << "    Lower_Bound_Value_Slider value = " << (int)Lower_Bound_Value_Slider->value() << endl;
#endif
            QVector<int> values(3, (int)Lower_Bound_Value_Slider->value());
            while (band--)
                if (!(Selected_Bands & (1 << band)))
                    values[band] = -1;
            lower_bound_values(values);
        }
        else if (source == Upper_Bound_Percent_Slider)
        {
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_CONTROL_PANEL | DEBUG_BOUNDS))
            clog << "    Upper_Bound_Percent_Slider = " << Upper_Bound_Percent_Slider->value() << endl;
#endif
            QVector<double> percents(3, Upper_Bound_Percent_Slider->value());
            while (band--)
                if (!(Selected_Bands & (1 << band)))
                    percents[band] = -1.0;
            upper_bound_percents(percents);
        }
        else if (source == Lower_Bound_Percent_Slider)
        {
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_CONTROL_PANEL | DEBUG_BOUNDS))
            clog << "    Lower_Bound_Percent_Slider = " << Lower_Bound_Percent_Slider->value() << endl;
#endif
            QVector<double> percents(3, Lower_Bound_Percent_Slider->value());
            while (band--)
                if (!(Selected_Bands & (1 << band)))
                    percents[band] = -1.0;
            lower_bound_percents(percents);
        }
        else if (source == Actual_to_Settings_Action)
        {
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_CONTROL_PANEL | DEBUG_BOUNDS))
            ...
            while (band--)
            {
                if ((Selected_Bands & (1 << band)))
                {
                    ...
                    Upper_Bound_Percent_Setting[band]->setValue(Upper_Bound_Percent_Actual[band]->text().toDouble());
                    Upper_Bound_Percent_Setting[band]->blockSignals(enabled);

                    enabled = Lower_Bound_Percent_Setting[band]->blockSignals(true);
                    Lower_Bound_Percent_Setting[band]->setValue(Lower_Bound_Percent_Actual[band]->text().toDouble());
        ...
        else
        {
            while (band--)
            {
                ...
                {
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_CONTROL_PANEL | DEBUG_BOUNDS))
                    clog << "    Upper_Bound_Value[" << band << "] = " << Upper_Bound_Value[band]->value() << endl;
#endif
                ...
                {
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_CONTROL_PANEL | DEBUG_BOUNDS))
                    clog << "    Lower_Bound_Value[" << band << "] = " << Lower_Bound_Value[band]->value() << endl;
#endif
...
#endif
                    upper_bound_percent(Upper_Bound_Percent_Setting[band]->value(), band, true);
                    break;
                }
...
#endif
                    lower_bound_percent(Lower_Bound_Percent_Setting[band]->value(), band, true);
                    break;
                }
            }
        }
...
void Data_Mapper_Tool::apply_bound_values()
{
#if ((DEBUG_SECTION) & DEBUG_DATA_MAPS)
    clog << ">>> Data_Mapper_Tool::apply_bound_values:" << endl << "    pre-apply Nodes:" << endl;
    print_nodes(Nodes);
#endif
...
/*..............................................................................
    Bounds Percents
*/
double Data_Mapper_Tool::upper_bound_percent(int band) const
{
    double percent;
    if (band >= 0 && band < 3)
...
void Data_Mapper_Tool::actual_upper_bound_percents(const QVector<double> &percents)
{
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_CONTROL_PANEL | DEBUG_BOUNDS))
    ...
    for (int band = 0; band < 3; ++band)
    {
        ...
        clog << " -> " << percent << endl;
#endif
        Upper_Bound_Percent_Actual[band]->setText(format.arg(percent, 6, 'f', 2));
    }
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_CONTROL_PANEL | DEBUG_BOUNDS))
    clog << "<<< Data_Mapper_Tool:: actual_upper_bound_percents" << endl;
#endif
...
bool Data_Mapper_Tool::upper_bound_percent(double percent, int band, bool force_change)
{
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_CONTROL_PANEL | DEBUG_BOUNDS))
    clog << ">>> Data_Mapper_Tool::upper_bound_percent: " << percent << ", band " << band << endl;
#endif
    bool changed = force_change, actually_changed = false;
    double previous_percent = upper_bound_percent(band);
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_CONTROL_PANEL | DEBUG_BOUNDS))
    ...
    if (previous_percent >= 0.0)
    {
        ...
        if (Upper_Bound_Percent_Slider->value() != percent)
        {
            ...
            enabled = true;
            for (int selected_band = 0; selected_band < 3; ++selected_band)
                if ((Selected_Bands & (1 << selected_band)) &&
                    !(enabled = (Upper_Bound_Percent_Setting[selected_band]->value() == percent)))
                    break;
            if (enabled)
            {
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_CONTROL_PANEL | DEBUG_BOUNDS))
        ...
        if (changed && Signal_Changes)
        {
            QVector<double> percents(3, -1.0);
            percents[band] = percent;
//	>>> SIGNAL <<<
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_CONTROL_PANEL | DEBUG_BOUNDS | DEBUG_SIGNALS))
...
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_CONTROL_PANEL | DEBUG_BOUNDS))
    clog << "<<< Data_Mapper_Tool::upper_bound_percent: " << boolalpha << actually_changed << endl;
#endif
    return actually_changed;
}

bool Data_Mapper_Tool::upper_bound_percents(const QVector<double> &percents)
{
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_CONTROL_PANEL | DEBUG_BOUNDS))
    clog << ">>> Data_Mapper_Tool::upper_bound_percents: " << percents[0] << ", " << percents[1] << ", " << percents[2]
         << endl;
#endif
    bool changed = false, signal_changes = Signal_Changes;
    Signal_Changes = false;

    QVector<double> changed_percents(percents);
    for (int band = 0; band < 3; ++band)
    {
...
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_CONTROL_PANEL | DEBUG_BOUNDS))
        clog << "    upper_bound_percent[" << band << "] = " << upper_bound_percent(band) << endl;
#endif
    }
...
//	>>> SIGNAL <<<
        ...
        clog << "^^^ Data_Mapper_Tool::upper_bound_percents: "
                "emit upper_bound_percents_changed "
             << changed_percents[0] << ", " << changed_percents[1] << ", " << changed_percents[2] << endl;
#endif
...
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_CONTROL_PANEL | DEBUG_BOUNDS))
    clog << "<<< Data_Mapper_Tool::upper_bound_percents: " << boolalpha << changed << endl;
#endif
    return changed;
}

void Data_Mapper_Tool::upper_default_contrast_stretch(double percent, int band)
{
    if (band < 0 || band > 2)
    {
        ostringstream message;
        message << ID << endl << "Invalid upper default contrast stretch band: " << band;
        throw invalid_argument(message.str());
    }
    if (percent < 0.0)
        percent = 0.0;
...
double Data_Mapper_Tool::lower_bound_percent(int band) const
{
    double percent;
    if (band >= 0 && band < 3)
...
void Data_Mapper_Tool::actual_lower_bound_percents(const QVector<double> &percents)
{
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_CONTROL_PANEL | DEBUG_BOUNDS))
    ...
    for (int band = 0; band < 3; ++band)
    {
        ...
        clog << " -> " << percent << endl;
#endif
        Lower_Bound_Percent_Actual[band]->setText(format.arg(percent, 6, 'f', 2));
    }
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_CONTROL_PANEL | DEBUG_BOUNDS))
    clog << "<<< Data_Mapper_Tool:: actual_lower_bound_percents" << endl;
#endif
...
bool Data_Mapper_Tool::lower_bound_percent(double percent, int band, bool force_change)
{
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_CONTROL_PANEL | DEBUG_BOUNDS))
    clog << ">>> Data_Mapper_Tool::lower_bound_percent: " << percent << ", band " << band << endl
         << "    force_change = " << boolalpha << force_change << endl;
#endif
    bool changed = force_change, actually_changed = false;
    double previous_percent = lower_bound_percent(band);
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_CONTROL_PANEL | DEBUG_BOUNDS))
    clog << "    previous_percent = " << previous_percent << endl;
#endif
    if (previous_percent >= 0.0)
    {
        ...
        if (Lower_Bound_Percent_Slider->value() != percent)
        {
            ...
            for (int selected_band = 0; selected_band < 3; ++selected_band)
                if ((Selected_Bands & (1 << selected_band)) &&
                    !(enabled = (Lower_Bound_Percent_Setting[selected_band]->value() == percent)))
                    break;
            if (enabled)
            {
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_CONTROL_PANEL | DEBUG_BOUNDS))
...
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_CONTROL_PANEL | DEBUG_BOUNDS))
    clog << "<<< Data_Mapper_Tool::lower_bound_percent: " << boolalpha << actually_changed << endl;
#endif
    return actually_changed;
}

bool Data_Mapper_Tool::lower_bound_percents(const QVector<double> &percents)
{
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_CONTROL_PANEL | DEBUG_BOUNDS))
    clog << ">>> Data_Mapper_Tool::lower_bound_percents: " << percents[0] << ", " << percents[1] << ", " << percents[2]
         << endl;
#endif
    bool changed = false, signal_changes = Signal_Changes;
    Signal_Changes = false;
...
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_CONTROL_PANEL | DEBUG_BOUNDS))
    clog << "<<< Data_Mapper_Tool::lower_bound_percents: " << boolalpha << changed << endl;
#endif
    return changed;
}

void Data_Mapper_Tool::lower_default_contrast_stretch(double percent, int band)
{
    if (band < 0 || band > 2)
    {
        ostringstream message;
        message << ID << endl << "Invalid lower default contrast stretch band: " << band;
        throw invalid_argument(message.str());
    }
    if (percent < 0.0)
        percent = 0.0;
...
void Data_Mapper_Tool::bound_percents(const QVector<double> &upper, const QVector<double> &lower)
{
    bool signal_changes = Signal_Changes;
    Signal_Changes = false;
...
void Data_Mapper_Tool::apply_bound_percents()
{
#if ((DEBUG_SECTION) & DEBUG_DATA_MAPS)
    clog << ">>> Data_Mapper_Tool::apply_bound_percents:" << endl << "    pre-apply Nodes:" << endl;
    print_nodes(Nodes);
#endif
    Signal_Changes = false;
    QVector<double> upper_percents(3, -1.0), lower_percents(3, -1.0);
    for (int band = 0; band < 3; ++band)
    {
        if (Selected_Bands & (1 << band))
        {
            upper_percents[band] = Upper_Bound_Percent_Setting[band]->value();
#if ((DEBUG_SECTION) & DEBUG_DATA_MAPS)
            clog << "    Upper_Bound_Percent_Setting[" << band << "] = " << upper_percents[band] << endl;
#endif
            upper_bound_percent(upper_percents[band], band);

            lower_percents[band] = Lower_Bound_Percent_Setting[band]->value();
#if ((DEBUG_SECTION) & DEBUG_DATA_MAPS)
            clog << "    Lower_Bound_Percent_Setting[" << band << "] = " << lower_percents[band] << endl;
#endif
...
void Data_Mapper_Tool::default_contrast_stretch()
{
    bound_percents(Upper_Default_Contrast_Stretch, Lower_Default_Contrast_Stretch);
    apply_bound_percents();
}

void Data_Mapper_Tool::restore_original_contrast_stretch()
{
...
/*..............................................................................
    Bounds Values
*/
int Data_Mapper_Tool::upper_bound_value(int band) const
{
    int value;
    if (band >= 0 && band < 3)
...
bool Data_Mapper_Tool::upper_bound_value(int value, int band, bool force_change)
{
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_CONTROL_PANEL | DEBUG_BOUNDS))
    clog << ">>> Data_Mapper_Tool::upper_bound_value: " << value << ", band " << band
         << ", force_change = " << boolalpha << force_change << endl;
#endif
    bool changed = force_change, actually_changed = false;
    int previous_value = upper_bound_value(band);
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_CONTROL_PANEL | DEBUG_BOUNDS))
    ...
    if (previous_value >= 0)
    {
        ...
        if ((int)Upper_Bound_Value_Slider->value() != value)
        {
            ...
            for (int selected_band = 0; selected_band < 3; ++selected_band)
                if ((Selected_Bands & (1 << selected_band)) &&
                    !(enabled = (Upper_Bound_Value[selected_band]->value() == value)))
                    break;
            if (enabled)
            {
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_CONTROL_PANEL | DEBUG_BOUNDS))
        ...
        {
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_CONTROL_PANEL | DEBUG_BOUNDS))
            clog << "    lower bound overlap moved from " << lower_bound_value(band) << " to " << (value - 1) << endl;
#endif
...
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_CONTROL_PANEL | DEBUG_BOUNDS))
            clog << "    reposition Nodes[" << band << "] upper_bound to " << value << endl;
#endif
...
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_CONTROL_PANEL | DEBUG_BOUNDS))
    clog << "<<< Data_Mapper_Tool::upper_bound_value: " << boolalpha << actually_changed << endl;
#endif
    return actually_changed;
}

bool Data_Mapper_Tool::upper_bound_values(const QVector<int> &values)
{
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_CONTROL_PANEL | DEBUG_BOUNDS))
    clog << ">>> Data_Mapper_Tool::upper_bound_values: " << values[0] << ", " << values[1] << ", " << values[2] << endl;
#endif
    bool changed = false, signal_changes = Signal_Changes;
    Signal_Changes = false;

    QVector<int> changed_values(values);
    for (int band = 0; band < 3; ++band)
    {
        if (values[band] >= 0)
        {
...
//	>>> SIGNAL <<<
        ...
        clog << "^^^ Data_Mapper_Tool::upper_bound_values: "
                "emit upper_bound_values_changed "
             << changed_values[0] << ", " << changed_values[1] << ", " << changed_values[2] << endl;
#endif
...
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_CONTROL_PANEL | DEBUG_BOUNDS))
    clog << "<<< Data_Mapper_Tool::upper_bound_values: " << boolalpha << changed << endl;
#endif
    return changed;
}

int Data_Mapper_Tool::lower_bound_value(int band) const
{
    int value;
    if (band >= 0 && band < 3)
...
bool Data_Mapper_Tool::lower_bound_value(int value, int band, bool force_change)
{
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_CONTROL_PANEL | DEBUG_BOUNDS))
    clog << ">>> Data_Mapper_Tool::lower_bound_value: " << value << ", band " << band << endl;
#endif
    bool changed = force_change, actually_changed = false;
    int previous_value = lower_bound_value(band);
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_CONTROL_PANEL | DEBUG_BOUNDS))
    clog << "    previous_value = " << previous_value << endl;
#endif
    if (previous_value >= 0)
    {
        ...
        if ((int)Lower_Bound_Value_Slider->value() != value)
        {
            ...
            for (int selected_band = 0; selected_band < 3; ++selected_band)
                if ((Selected_Bands & (1 << selected_band)) &&
                    !(enabled = (Lower_Bound_Value[selected_band]->value() == value)))
                    break;
            if (enabled)
            {
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_CONTROL_PANEL | DEBUG_BOUNDS))
        ...
        {
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_CONTROL_PANEL | DEBUG_BOUNDS))
            clog << "    upper bound overlap moved from " << upper_bound_value(band) << " to " << (value + 1) << endl;
#endif
...
//	Reposition the corresponding node.
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_CONTROL_PANEL | DEBUG_BOUNDS))
            clog << "    reposition Nodes[" << band << "] lower_bound to " << value << endl;
#endif
...
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_CONTROL_PANEL | DEBUG_BOUNDS))
    clog << "<<< Data_Mapper_Tool::lower_bound_value: " << boolalpha << actually_changed << endl;
#endif
    return actually_changed;
}

bool Data_Mapper_Tool::lower_bound_values(const QVector<int> &values)
{
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_CONTROL_PANEL | DEBUG_BOUNDS))
    clog << ">>> Data_Mapper_Tool::lower_bound_values: " << values[0] << ", " << values[1] << ", " << values[2] << endl;
#endif
    bool changed = false, signal_changes = Signal_Changes;
    Signal_Changes = false;
...
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_CONTROL_PANEL | DEBUG_BOUNDS))
    clog << "<<< Data_Mapper_Tool::lower_bound_values: " << boolalpha << changed << endl;
#endif
    return changed;
}

void Data_Mapper_Tool::upper_limit(int limit)
{
    ...
    {
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_BOUNDS))
        clog << "    band " << band << " upper_bound_value = " << upper_bound_value(band) << endl;
#endif
    ...
    {
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_BOUNDS))
        clog << "    changed upper_bound_values = " << values[0] << ", " << values[1] << ", " << values[2] << endl;
#endif
        upper_bound_values(values);
    }
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_BOUNDS))
...
void Data_Mapper_Tool::lower_limit(int limit)
{
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_BOUNDS))
    ...
    {
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_BOUNDS))
        clog << "    band " << band << " lower_bound_value = " << lower_bound_value(band) << endl;
#endif
    ...
    {
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_BOUNDS))
        clog << "    changed lower_bound_values = " << values[0] << ", " << values[1] << ", " << values[2] << endl;
#endif
        lower_bound_values(values);
    }
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_BOUNDS))
...
QWidget *Data_Mapper_Tool::graph_panel()
{
    ...
    for (int band = 0; band < 3; band++)
    {
...
#if ((DEBUG_SECTION) & DEBUG_PLOT_PANEL)
        clog << "    Functions[" << band << "] @ " << (void *)Functions[band] << endl;
#endif

        Function_Plots[band] = new QwtPlotCurve();
#if ((DEBUG_SECTION) & DEBUG_PLOT_PANEL)
        clog << "    Function_Plots[" << band << "] @ " << endl << (void *)Function_Plots[band] << endl;
#endif
...
#if ((DEBUG_SECTION) & DEBUG_PLOT_PANEL)
        clog << "    Nodes[" << band << "] @ " << (void *)Nodes[band] << endl;
#endif
...
#if ((DEBUG_SECTION) & DEBUG_PLOT_PANEL)
        clog << "    Node_Plots[" << band << "] @ " << (void *)Node_Plots[band] << endl;
#endif
        ...
        Node_Plots[band]->setSymbol(new QwtSymbol(QwtSymbol::Diamond, QBrush(DISPLAY_BAND_COLORS[band]),
        ...
        Nodes[band]->move_node(QPoint(max_x(), max_y()), QPoint(max_x() - (band * 25), max_y()));
#endif
    }

    //	Node selection symbol/marker.
    Selection_Symbol = new QwtSymbol(QwtSymbol::Diamond, QBrush(Qt::white), QPen(Qt::white), QSize(9, 9));
    Selection_Symbol->setBrush(QColor(DISPLAY_BAND_COLORS[0] | DISPLAY_BAND_COLORS[1] | DISPLAY_BAND_COLORS[2]));
    ...
    connect(Tracker, SIGNAL(position(const QPoint &)), SLOT(graph_position(const QPoint &)));
    connect(Tracker, SIGNAL(appended(const QPointF &)), SLOT(mouse_down(const QPointF &)));
    connect(Tracker, SIGNAL(moved(const QPointF &)), SLOT(mouse_drag(const QPointF &)));
    connect(Tracker, SIGNAL(selected(const QPointF &)), SLOT(mouse_up(const QPointF &)));
    connect(Tracker, SIGNAL(leave_widget()), SLOT(leave_graph()));
...
bool Data_Mapper_Tool::visible_graph(bool enabled)
{
#if ((DEBUG_SECTION) & DEBUG_LAYOUT)
...
void Data_Mapper_Tool::refresh_graph()
{
#if ((DEBUG_SECTION) & DEBUG_PLOT_PANEL)
...
/*------------------------------------------------------------------------------
    Graph Tracker
*/
void Data_Mapper_Tool::graph_position(const QPoint &position)
{
    ...
    if (!(Operating_Mode & DRAGGING_MODE))
    {
        ...
        else
        {
            ...
            if (Node_Bands_Selected)
            {
                ...
                else if (Node_Bands_Selected & SELECTED_GREEN)
                    This_Node.ry() = Functions[1]->y(This_Node.rx());
                else if (Node_Bands_Selected & SELECTED_BLUE)
                ...
                {
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_GRAPH_POSITION))
                    clog << "    show Selection_Marker" << endl << "    set TRACKER_DRAG_MODE" << endl;
#endif
                    Selection_Marker->show();
                    // Tracker->setSelectionFlags (TRACKER_DRAG_MODE);
                }
            }
            else if (Selection_Marker->isVisible())
            {
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_GRAPH_POSITION))
                clog << "    hide Selection_Marker" << endl << "    set TRACKER_TRACK_MODE" << endl;
#endif
                Selection_Marker->hide();
                // Tracker->setSelectionFlags (TRACKER_TRACK_MODE);
            }
...
void Data_Mapper_Tool::mouse_down(const QPointF &point)
{
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_DRAGGING))
    clog << ">>> Data_Mapper_Tool::mouse_down: " << point << endl << "    This_Node = " << This_Node << endl;
#endif
    if (Node_Bands_Selected)
    {
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_DRAGGING))
        ...
        if (Operating_Mode & SATURATION_MODE)
        {
            Function_Nodes *function_nodes = NULL;
            if (Node_Bands_Selected & SELECTED_RED)
            ...
            Tracker->set_cursor_position(QPoint(This_Node.rx(), (int)(point.y() + 0.5)));
        }
        else
        {
            QPolygon *nodes = NULL;
            int index, total_nodes, band = -1;
            while (++band < 3)
            {
                if (Node_Bands_Selected & (1 << band))
                {
...
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_DRAGGING))
                    clog << "    band " << band << " total nodes = " << total_nodes << endl;
#endif
                    index = -1;
                    while (++index < total_nodes && (nodes->at(index)).x() <= This_Node.rx())
                        ;
                    ...
                    --index;
                    if (index && nodes->at(index) == This_Node)
...
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_DRAGGING))
                    clog << "    last node[" << band << "][" << Last_Node_Index[band] << "]" << endl
                         << "    next node[" << band << "][" << Next_Node_Index[band] << "]" << endl;
#endif
                }
            }
...
void Data_Mapper_Tool::mouse_drag(const QPointF &point)
{
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_DRAGGING))
    clog << ">>> Data_Mapper_Tool::mouse_drag: " << point << endl
         << "    Node_Bands_Selected = " << Node_Bands_Selected << endl;
#endif
    if (Node_Bands_Selected && (Operating_Mode & DRAGGING_MODE))
    {
        int band;
        QPoint node, here(This_Node);
        ...
        QPoint there(This_Node);
#endif

        //	Constrain the point vertically to within the graph data range.
        if (This_Node.ry() < 0)
            This_Node.ry() = 0;
        else if (This_Node.ry() > max_y())
            This_Node.ry() = max_y();
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_DRAGGING))
        if (This_Node != there)
        {
            clog << "        This_Node = " << This_Node << " constrained vertically" << endl;
            there = This_Node;
        }
#endif
        ...
        if (Operating_Mode & SATURATION_MODE)
        {
            ...
            if (This_Node.rx() < 0)
                This_Node.rx() = 0;
            else if (This_Node.rx() > max_x())
                This_Node.rx() = max_x();
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_DRAGGING))
            if (This_Node != there)
                clog << "        This_Node = " << This_Node << " constrained horizontally" << endl;
#endif

            if (This_Node != here)
            {
                QVector<int> values(3, This_Node.rx());
                band = -1;
                while (++band < 3)
                    if (!(Node_Bands_Selected & (1 << band)))
                        values[band] = -1;
...
//	Move the Selection_Marker to the new position.
        ...
        else
        {
            ...
            int above = This_Node.ry() + selection_distance(), below = This_Node.ry() - selection_distance();

            //	Possible saturation bounds changes.
            int bounds_changed = 0;
            QVector<int> changed_bounds(3, -1);
            band = -1;
            while (++band < 3)
            {
                if (Node_Bands_Selected & (1 << band))
...
*/
...
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_DRAGGING))
                    clog << "     next node[" << band << "][" << Next_Node_Index[band] << "] = " << node << endl;
#endif
                    if (This_Node.rx() >= node.rx())
                    {
                        //	This_Node is at or beyond next node.
                        if (Next_Node_Index[band] == (Last_Node_Index[band] + 1))
                            //	This_Node is next node at max_x; constrain.
                            This_Node.rx() = node.rx();
                        else if (node.ry() > above || node.ry() < below)
                        ...
                        else
                        {
                            ...
                            This_Node.rx() = node.rx();
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_DRAGGING))
                            clog << "           This_Node = " << This_Node << " merge with next node" << endl;
#endif
                            if (Next_Node_Index[band] == Nodes[band]->last_node_index())
                            {
                                Next_Node_Index[band]--;
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_DRAGGING))
                                clog << "     next node[" << band << "][" << Next_Node_Index[band]
                                     << "] = " << Nodes[band]->nodes()->at(Next_Node_Index[band]) << endl;
#endif
                            }
                        }
                    }
...
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_DRAGGING))
                        clog << "     last node[" << band << "][" << Last_Node_Index[band] << "] = " << node << endl;
#endif
                        if (This_Node.rx() <= node.rx())
                        {
                            ...
                            else
                            {
                                ...
                                This_Node.rx() = node.rx();
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_DRAGGING))
                                clog << "           This_Node = " << This_Node << " merge with last node" << endl;
#endif
                                Next_Node_Index[band]--;
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_DRAGGING))
                                clog << "     next node[" << band << "][" << Next_Node_Index[band]
...
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_DRAGGING))
                                    clog << "     last node[" << band << "][" << Last_Node_Index[band]
                                         << "] = " << Nodes[band]->nodes()->at(Last_Node_Index[band]) << endl;
#endif
                                }
                            }
                        }
            ...
            {
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_DRAGGING))
                clog << "    node moved from " << here << " to " << This_Node << endl;
#endif
...
//	Move the node.
                        ...
                        if ((below & Function_Nodes::ADDED_NODE) && !(below & Function_Nodes::REMOVED_NODES_MASK))
                        {
...
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_DRAGGING))
                                clog << "     next node[" << band << "][" << Next_Node_Index[band]
                                     << "] = " << Nodes[band]->nodes()->at(Next_Node_Index[band])
...
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_DRAGGING))
                        clog << "    total nodes = " << Nodes[band]->size() << endl;
#endif
                    }
                }
...
//	Move the Selection_Marker to the new position.
                ...
                {
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_DRAGGING))
                    clog << "    saturation " << (This_Node.ry() ? "upper" : "lower")
                         << " bounds changed: " << changed_bounds[0] << ", " << changed_bounds[1] << ", "
...
void Data_Mapper_Tool::mouse_up(const QPointF &)
{
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_DRAGGING))
...
void Data_Mapper_Tool::leave_graph()
{
    if (Selection_Marker->isVisible())
        Selection_Marker->hide();
    if (Bound_Marker->isVisible())
...
/*==============================================================================
    Helpers
*/
void Data_Mapper_Tool::linear_interpolation(int x0, int y0, int x1, int y1, int band)
{
    if (band < 0)
    {
...
void Data_Mapper_Tool::linear_interpolation(const QPoint &start, const QPoint &end, int band)
{
    linear_interpolation(start.x(), start.y(), end.x(), end.y(), band);
}

void Data_Mapper_Tool::linear_interpolation(int band_selections, const QPoint &start, const QPoint &end)
{
    if (band_selections)
    {
        if (band_selections & SELECTED_RED)
            linear_interpolation(start.x(), start.y(), end.x(), end.y(), Data_Maps[0]);
        if (band_selections & SELECTED_GREEN)
            linear_interpolation(start.x(), start.y(), end.x(), end.y(), Data_Maps[1]);
        if (band_selections & SELECTED_BLUE)
            linear_interpolation(start.x(), start.y(), end.x(), end.y(), Data_Maps[2]);
    }
}

QPoint Data_Mapper_Tool::selected_data_point(const QPoint &position) const
{
    QPoint selection(-1, SELECTED_NONE);
    double min_distance = selection_distance() * selection_distance();
    int band, band_count, bands_selected, max_bands = 0, active_bands = selected_bands(), active_band_mask, index = -1;

    for (band = 0, band_count = 0, bands_selected = SELECTED_NONE, active_band_mask = 1; band < 3;
         ++band, active_band_mask <<= 1)
    {
...
QPoint Data_Mapper_Tool::selected_bounds(const QPoint &position) const
{
    /*	selection codes:
        ...
        y - band SELECTED_XXX flags.
    */
    QPoint selection(0, SELECTED_NONE);
    int band = 3, active_bands = selected_bands(), active_band_mask, min_distance = selection_distance(), x_distance,
        x_position = position.x();

    for (band = 0, active_band_mask = 1; band < 3; ++band, active_band_mask <<= 1)
    {
        if (active_bands & active_band_mask)
        {
            //	Lower bound.
            ...
            else if (x_distance == min_distance)
            {
                selection.rx() = -1;
                selection.ry() |= active_band_mask;
            }
            ...
            else if (x_distance == min_distance && selection.rx() >= 0) //	Lower bound has precedence.
            {
                selection.rx() = 1;
                selection.ry() |= active_band_mask;
            }
...
void Data_Mapper_Tool::save()
{
    ...
    if (!Save_File_Dialog)
    {
...
#endif
        QStringList formats(HiView_Utilities::image_writer_formats());
        //	Put the CSV format at the top of the list
        QString file_filters(HiView_Utilities::file_filter_for(Graph_Data_Format) + ";; ");
        ...
        /*	>>> Qt implementation used instead of native implementation <<<
            ...
            for use with HiView.
        */
        Save_File_Dialog = new QFileDialog(this, tr("Save Data Mapper File"), Graph_Directory, file_filters);
        Save_File_Dialog->setAcceptMode(QFileDialog::AcceptSave);
        Save_File_Dialog->setFileMode(QFileDialog::AnyFile);
        Save_File_Dialog->setOptions(QFileDialog::DontUseNativeDialog | QFileDialog::DontResolveSymlinks);
        Save_File_Dialog->setDefaultSuffix(Graph_Data_Format);
        // default is true Save_File_Dialog->setConfirmOverwrite (true);
        connect(Save_File_Dialog, SIGNAL(filterSelected(const QString &)), SLOT(graph_data_format(const QString &)));
    }
    if (Save_File_Dialog->exec())
    {
        QString pathname(Save_File_Dialog->selectedFiles().value(0));
        if (!pathname.isEmpty())
        {
            if (pathname.length() > 2 && pathname.at(0) == '~' && pathname.at(1) == QDir::separator())
...
bool Data_Mapper_Tool::save_CSV_file(const QString &pathname)
{
#if ((DEBUG_SECTION) & DEBUG_FILE)
    ...
    if (!pathname.isEmpty())
    {
        ...
        if (file.open(QIODevice::WriteOnly | QIODevice::Truncate | QIODevice::Text))
        {
            ...
            stream << "# HiView Data Mapper Graph Nodes\n"
                   << "# " << ID << '\n';
        ...
        {
            QString report("The data mapper graph file could not be opened -\n");
            report += pathname;
#if ((DEBUG_SECTION) & DEBUG_FILE)
            clog << report << endl;
#endif
...
bool Data_Mapper_Tool::save_graph_image(const QString &pathname)
{
#if ((DEBUG_SECTION) & DEBUG_FILE)
    ...
    if (!pathname.isEmpty())
    {
        QImage rendered_image(Graph->size(), QImage::Format_RGB32);
        Graph->render(&rendered_image);
        if (rendered_image.save(pathname, Graph_Data_Format.toLatin1() /*.toAscii ()*/))
            saved = true;
        else
        {
            QString report("Could not write the data mapper graph image file -\n");
            report += pathname;
#if ((DEBUG_SECTION) & DEBUG_FILE)
            clog << report << endl;
#endif
...
void Data_Mapper_Tool::graph_data_format(const QString &format)
{
    Graph_Data_Format = format.left(format.indexOf(' ')).toLower();
#if ((DEBUG_SECTION) & DEBUG_FILE)
    clog << ">-< Data_Mapper_Tool::graph_data_format: " << format << " (" << Graph_Data_Format << ')' << endl;
#endif
    Save_File_Dialog->setDefaultSuffix(Graph_Data_Format);
}

void Data_Mapper_Tool::load()
{
#if ((DEBUG_SECTION) & DEBUG_FILE)
    ...
    if (!Load_File_Dialog)
    {
        QString file_filters("All Files (*);; CSV (*.CSV *.csv)");
        ...
        /*	>>> Qt implementation used instead of native implementation <<<
            ...
            for use with HiView.
        */
        Load_File_Dialog = new QFileDialog(this, tr("Load Data Mapper File"), Graph_Directory, file_filters);
        Load_File_Dialog->setAcceptMode(QFileDialog::AcceptOpen);
        Load_File_Dialog->setFileMode(QFileDialog::ExistingFile);
        Load_File_Dialog->setOptions(QFileDialog::DontUseNativeDialog | QFileDialog::DontResolveSymlinks);
    }
    if (Load_File_Dialog->exec())
    {
        QString pathname(Load_File_Dialog->selectedFiles().value(0));
        if (!pathname.isEmpty())
        {
            Graph_Directory = QFileInfo(pathname).absolutePath();

            if (pathname.at(0) == '~' && pathname.at(1) == QDir::separator())
                //	Substitute '~' with the user's home directory pathname.
                pathname.replace(0, 1, QDir::homePath());
...
bool Data_Mapper_Tool::load_CSV_file(const QString &pathname)
{
#if ((DEBUG_SECTION) & DEBUG_FILE)
    ...
    if (!pathname.isEmpty())
    {
        ...
        QString line;
        QStringList words;
        int line_count = 0, index, band = -1, entry, x, y;
        bool OK;
        QPolygon nodes[3];
        ...
        while (!stream.atEnd())
        {
            ...
            if (line.startsWith(DISPLAY_BAND_NAMES[0], Qt::CaseInsensitive))
                band = 0;
            else if (line.startsWith(DISPLAY_BAND_NAMES[1], Qt::CaseInsensitive))
                band = 1;
            else if (line.startsWith(DISPLAY_BAND_NAMES[2], Qt::CaseInsensitive))
                band = 2;
            else if (band >= 0)
            {
                if (!(Selected_Bands & (1 << band)))
                ...
                words = line.split(QRegularExpression("[\\s,]"), Qt::SkipEmptyParts);
                if (!words.size())
                    continue;
                ...
                while (++index < words.size())
                {
                    ...
                    Invalid_Contents:
                        ...
                        QString report = QString("Invalid data mapper graph file contents -\n"
                                                 "%1\n\n"
                    ...
                    if (x <= max_x() && y <= max_y())
                    {
                        ...
                        for (entry = nodes[band].size() - 1; entry >= 0 && x <= nodes[band].at(entry).x(); entry--)
                            ;
                        if (nodes[band].size() && entry > 0 && nodes[band].at(entry).x() == x)
                            //	Duplicate x position.
                            continue;
                        nodes[band].insert(++entry, QPoint(x, y));
#if ((DEBUG_SECTION) & DEBUG_FILE)
                        clog << "    nodes[" << band << "][" << entry << "] = " << nodes[band].at(entry) << endl;
#endif
                    }
#if ((DEBUG_SECTION) & DEBUG_FILE)
                    else
                        clog << "    out of range: " << x << "x, " << y << 'y' << endl;
#endif
                }
            }
            else
                ...
                if (*(Nodes[band]->nodes()) != nodes[band])
                {
                    ...
                    *(Nodes[band]->nodes()) = nodes[band];
                    ...
                    clog << "    reset Nodes[" << band << "] -" << endl
                         << "      lower bound = " << lower_values[band] << endl;
                    for (entry = 0; entry < nodes[band].size(); entry++)
                        clog << "      " << entry << ": " << nodes[band].at(entry) << endl;
                    clog << "      upper bound = " << upper_values[band] << endl;
#endif
                }
            }
        }
...
void Data_Mapper_Tool::last_CSV_file(const QString &pathname)
{
    QFileInfo file(Last_CSV_Pathname = pathname);
...
void Data_Mapper_Tool::last_file_action()
{
    if (Last_CSV_Pathname.isEmpty())
    {
        Load_Last_CSV_File_Action->setEnabled(false);
    ...
    else
    {
        ...
        else if (source == Save_Last_CSV_File_Action)
...
/*==============================================================================
    Utilities
*/
void Data_Mapper_Tool::print_data_map(std::ostream &stream, Data_Map *map)
{
    ...
    if (map)
    {
        for (int index = 0; index < map->size(); ++index)
        {
            if (!(index % 16))
            {
                if (index)
                    stream << endl;
                stream << dec << setfill(' ') << setw(5) << index << ": " << hex << setfill('0');
            }
...
void Data_Mapper_Tool::print_data_maps(std::ostream &stream)
{
