...
#define DEBUG_ACCESSORS (DEBUG_TRANSFORMS | DEBUG_DATA_MAPS | DEBUG_BAND_MAP)
...
#define DEBUG_DEFAULT (DEBUG_ALL & ~DEBUG_PIXEL_DATUM & ~DEBUG_PIXEL_MAPPING & ~DEBUG_PIXEL_DATA)
...
/*==============================================================================
    Constants
*/
const char *const JP2_Image::ID = "UA::HiRISE::JP2_Image ($Revision: 2.33 $ $Date: 2014/05/27 17:13:51 $)";

#ifndef JP2_METADATA_GROUP_NAME
#define JP2_METADATA_GROUP_NAME "JP2"
#endif
Plastic_Image::Name_String JP2_Image::JP2_METADATA_GROUP = JP2_METADATA_GROUP_NAME;
...
*/
class JP2_Image_Rendering_Monitor : public JP2_Reader::Rendering_Monitor
{
  private:
    JP2_Image *Owner;

  public:
    JP2_Image_Rendering_Monitor(JP2_Image *owner = NULL)
    {
        Owner = owner;
    }
    ...
    */
    bool notify(JP2_Reader &
#if ((DEBUG_SECTION) & (DEBUG_RENDER | DEBUG_NOTIFY | DEBUG_LOCATION))
                    reader
#endif
                ,
                JP2_Reader::Rendering_Monitor::Status status, const std::string &message,
#if ((DEBUG_SECTION) & (DEBUG_RENDER | DEBUG_NOTIFY | DEBUG_LOCATION))
                ...
                const Cube &source_region)
    {
        ...
        if (Owner)
        {
            ...
            */
            QRect display_region(Owner->map_source_to_display(source_region));
...
//	Data rendering notification.
            ...
            else if (status == JP2_Reader::Rendering_Monitor::DONE)
            {
            ...
            continue_rendering =
                Owner->notify_rendering_monitors(static_cast<Plastic_Image::Rendering_Monitor::Status>(status),
                                                 QString::fromStdString(message), display_region);
        }
#if ((DEBUG_SECTION) & (DEBUG_RENDER | DEBUG_NOTIFY | DEBUG_LOCATION))
        clog << "<++ JP2_Image::JP2_Image_Rendering_Monitor::notify: " << boolalpha << continue_rendering << endl;
#endif
...
class Source_Data
{
  public:
    ...
    */
    unsigned int Rendered_Resolution;

    //!	Storage for pixel data rendered from the source image.
    unsigned char *Buffer;

    Source_Data(unsigned long long amount = 0)
        : Band(-1), Rendered(false), Rendered_Size(0, 0), Rendered_Resolution(0), Buffer(NULL)
    {
#if ((DEBUG_SECTION) & (DEBUG_DATA_BUFFERS | DEBUG_CONSTRUCTORS | DEBUG_BAND_MAP))
        clog << ">>> JP2_Image::Source_Data @ " << (void *)this << ": " << amount << endl;
#endif
        if (amount)
        {
            try
            ...
            catch (bad_alloc &)
            {
                ...
                message << JP2_Image::ID << endl
                        << "Couldn't allocate a " << magnitude_of(amount) << " (" << amount
...
#if ((DEBUG_SECTION) & (DEBUG_DATA_BUFFERS | DEBUG_CONSTRUCTORS | DEBUG_BAND_MAP))
        clog << "    buffer @ " << (void *)Buffer << endl << "<<< JP2_Image::Source_Data" << endl;
#endif
    }

    Source_Data &operator=(const Source_Data &source_data)
    {
        if (this != &source_data)
        {
    ...
    Source_Data &operator|=(const Source_Data &source_data)
    {
        if (Band == source_data.Band && Buffer == source_data.Buffer)
        {
            Rendered = source_data.Rendered;
            Rendered_Size = source_data.Rendered_Size;
            Rendered_Resolution = source_data.Rendered_Resolution;
...
std::ostream &operator<<(std::ostream &stream, const UA::HiRISE::Source_Data &source_data)
{
    return stream << "source band " << source_data.Band << " buffer @ " << (void *)source_data.Buffer << " is"
                  << (source_data.Rendered ? "" : " not") << " rendered with size " << source_data.Rendered_Size
                  << " at resolution level " << source_data.Rendered_Resolution;
}
#endif

void JP2_Image::assign_source_data_buffers()
{
#if ((DEBUG_SECTION) & (DEBUG_DATA_BUFFERS | DEBUG_BAND_MAP))
    LOCKED_LOGGING((clog << ">>> JP2_Image::assign_source_data_buffers" << endl
    ...
    {
#if ((DEBUG_SECTION) & (DEBUG_DATA_BUFFERS | DEBUG_BAND_MAP))
        LOCKED_LOGGING((clog << "    closed" << endl << "<<< JP2_Image::assign_source_data_buffers" << endl));
#endif
        return;
    }

#if ((DEBUG_SECTION) & (DEBUG_DATA_BUFFERS | DEBUG_BAND_MAP))
    LOCKED_LOGGING((clog << "    Data_Buffers pool -" << endl));
#endif
    Source_Data *data_buffers[3];
    int band = -1;
    while (++band < 3)
    {
//	Pool of buffers.
        ...
        {
            LOCKED_LOGGING((clog << "      display band " << band << " from " << *Data_Buffers[band] << endl));
        }
#endif
...
#if ((DEBUG_SECTION) & (DEBUG_DATA_BUFFERS | DEBUG_BAND_MAP))
    LOCKED_LOGGING((clog << "    reused -" << endl));
#endif
    int index;
    band = -1;
    while (++band < 3)
    {
        ...
        while (++index < 3)
        {
            if (data_buffers[index] && data_buffers[index]->Band == (int)Band_Map[band])
            {
                Display_Data_Buffers[band] = data_buffers[index];
                data_buffers[index] = NULL;
#if ((DEBUG_SECTION) & (DEBUG_DATA_BUFFERS | DEBUG_BAND_MAP))
                LOCKED_LOGGING(
                    (clog << "      display band " << band << " from " << *Display_Data_Buffers[band] << endl));
#endif
                break;
            }
        }
...
#if ((DEBUG_SECTION) & (DEBUG_DATA_BUFFERS | DEBUG_BAND_MAP))
                    LOCKED_LOGGING(
                        (clog << "     +display band " << index << " from " << *Display_Data_Buffers[index] << endl));
#endif
                }
            }
        }
...
#if ((DEBUG_SECTION) & (DEBUG_DATA_BUFFERS | DEBUG_BAND_MAP))
    LOCKED_LOGGING((clog << "    reassigned -" << endl));
#endif
    band = -1;
    while (++band < 3)
    {
...
//	Reassign the buffer to a different source band.
#if ((DEBUG_SECTION) & (DEBUG_DATA_BUFFERS | DEBUG_BAND_MAP))
                LOCKED_LOGGING((clog << "               reassigned " << *data_buffers[index] << endl));
#endif
                Display_Data_Buffers[band] = data_buffers[index];
...
#if ((DEBUG_SECTION) & (DEBUG_DATA_BUFFERS | DEBUG_BAND_MAP))
                LOCKED_LOGGING(
                    (clog << "      display band " << index << " from " << *Display_Data_Buffers[band] << endl));
#endif
                break;
            }
        }
        if (!Display_Data_Buffers[band])
        {
            ...
            ostringstream message;
            message << ID << endl
                    << "Couldn't assign a source data buffer for display band " << band << endl
...
void **JP2_Image::source_data_buffers()
{
#if ((DEBUG_SECTION) & (DEBUG_DATA_BUFFERS | DEBUG_RENDER))
    LOCKED_LOGGING((clog << ">>> JP2_Image::source_data_buffers" << endl << "    " << *this << endl));
#endif
...
#if ((DEBUG_SECTION) & (DEBUG_DATA_BUFFERS | DEBUG_RENDER))
        LOCKED_LOGGING(
            (clog << "    display band " << band << " from " << *Rendering_Display_Data_Buffers[band] << endl));
#endif
        Source_Data_Buffers[Rendering->Band_Map[band]] = Rendering_Display_Data_Buffers[band]->Buffer;
    }
#if ((DEBUG_SECTION) & (DEBUG_DATA_BUFFERS | DEBUG_RENDER))
    LOCKED_LOGGING((clog << "<<< JP2_Image::source_data_buffers" << endl));
#endif
    return Source_Data_Buffers;
}

bool JP2_Image::rendered_source_data(int band) const
{
#if ((DEBUG_SECTION) & (DEBUG_DATA_BUFFERS | DEBUG_RENDER))
    LOCKED_LOGGING((clog << ">>> JP2_Image::rendered_source_data: " << band << endl << "    " << *this << endl));
#endif
    bool rendered = true;
    if (band < 0)
    {
...
#if ((DEBUG_SECTION) & (DEBUG_DATA_BUFFERS | DEBUG_RENDER))
            LOCKED_LOGGING(
                (clog << "    Display_Data_Buffers[" << band << "] = " << (void *)Display_Data_Buffers[band] << endl));
            if (Display_Data_Buffers[band])
            {
                LOCKED_LOGGING(
                    (clog << "      Rendered = " << boolalpha << Display_Data_Buffers[band]->Rendered << endl));
            }
#endif
            if (!Display_Data_Buffers[band] || !Display_Data_Buffers[band]->Rendered)
            {
                rendered = false;
                break;
            }
    ...
    else if (band < 3)
    {
        ...
        if (Display_Data_Buffers[band])
        {
            LOCKED_LOGGING((clog << "      Rendered = " << boolalpha << Display_Data_Buffers[band]->Rendered << endl));
        }
#endif
        rendered = Display_Data_Buffers[band] && Display_Data_Buffers[band]->Rendered;
    }
    else
    {
        ostringstream message;
        message << ID << endl
                << "Can't determine if source data is rendered for band " << band << endl
                << "because there are only 3 display bands.";
        throw invalid_argument(message.str());
    }
#if ((DEBUG_SECTION) & (DEBUG_DATA_BUFFERS | DEBUG_RENDER))
    LOCKED_LOGGING((clog << ">>> JP2_Image::rendered_source_data: " << boolalpha << rendered << endl));
#endif
    return rendered;
}

void JP2_Image::source_data_rendered(bool rendered, int band)
{
#if ((DEBUG_SECTION) & (DEBUG_DATA_BUFFERS | DEBUG_RENDER))
    LOCKED_LOGGING(
        (clog << ">-< JP2_Image::source_data_rendered: " << boolalpha << rendered << ", band " << band << endl));
#endif
    if (Rendering_Display_Data_Buffers[0])
    {
        if (band < 0)
        {
            Rendering_Display_Data_Buffers[0]->Rendered = Rendering_Display_Data_Buffers[1]->Rendered =
                Rendering_Display_Data_Buffers[2]->Rendered = rendered;
#if ((DEBUG_SECTION) & (DEBUG_DATA_BUFFERS | DEBUG_RENDER))
            LOCKED_LOGGING(
                (clog << "    Rendering_Display_Data_Buffers[0] " << *Rendering_Display_Data_Buffers[0] << endl
                      << "    Rendering_Display_Data_Buffers[1] " << *Rendering_Display_Data_Buffers[1] << endl
                      << "    Rendering_Display_Data_Buffers[2] " << *Rendering_Display_Data_Buffers[2] << endl));
#endif
        }
        else if (band < 3)
        {
...
#if ((DEBUG_SECTION) & (DEBUG_DATA_BUFFERS | DEBUG_RENDER))
            LOCKED_LOGGING((clog << "    Rendering_Display_Data_Buffers[" << band << "] "
...
void JP2_Image::source_buffer_rendered(bool rendered, void *buffer)
{
#if ((DEBUG_SECTION) & (DEBUG_DATA_BUFFERS | DEBUG_RENDER))
    LOCKED_LOGGING((clog << ">-< JP2_Image::source_buffer_rendered: " << boolalpha << rendered << ", buffer @ "
                         << buffer << endl));
#endif
    if (Rendering_Data_Buffers[0])
    {
        ...
        else if (Rendering_Data_Buffers[1]->Buffer == buffer)
        {
            Rendering_Data_Buffers[1]->Rendered = rendered;
#if ((DEBUG_SECTION) & (DEBUG_DATA_BUFFERS | DEBUG_RENDER))
            LOCKED_LOGGING(
                (clog << "    Rendering_Display_Data_Buffers[1] " << *Rendering_Display_Data_Buffers[1] << endl));
#endif
        }
        else if (Rendering_Data_Buffers[2]->Buffer == buffer)
        {
            Rendering_Data_Buffers[2]->Rendered = rendered;
#if ((DEBUG_SECTION) & (DEBUG_DATA_BUFFERS | DEBUG_RENDER))
            LOCKED_LOGGING(
                (clog << "    Rendering_Display_Data_Buffers[2] " << *Rendering_Display_Data_Buffers[2] << endl));
#endif
        }
    }
}
...
/*==============================================================================
    Constructors
*/
JP2_Image::JP2_Image(JP2_Reader *JP2_reader, const QSize &size, const unsigned int *band_map,
                     const QTransform **transforms, const Data_Map **data_maps)
    : Plastic_Image(!size.isEmpty()
                        ? size
                        : (JP2_reader ? (QSize(JP2_reader->rendered_width(), JP2_reader->rendered_height()) *=
                                         (1.0 / JP2_reader->resolution_level()))
                                      : size),
                    band_map, transforms, data_maps),
      Source(JP2_reader), Source_Rendering_Monitor(NULL), Data_Buffer_Size(0)
{
...
JP2_Image::JP2_Image(const JP2_Reader &JP2_reader, const QSize &size, const unsigned int *band_map,
                     const QTransform **transforms, const Data_Map **data_maps)
    : Plastic_Image(!size.isEmpty() ? size
                                    : (QSize(JP2_reader.rendered_width(), JP2_reader.rendered_height()) *=
                                       (1.0 / JP2_reader.resolution_level())),
                    band_map, transforms, data_maps),
      Source(JP2_reader.clone()), Source_Rendering_Monitor(NULL), Data_Buffer_Size(0)
{
#if ((DEBUG_SECTION) & DEBUG_CONSTRUCTORS)
    clog << ">>> JP2_Reader: From JP2_Reader& @ " << (void *)&JP2_reader << endl
         << "     specified size = " << size << endl
...
JP2_Image::JP2_Image(const JP2_Image &image, const QSize &size, Mapping_Type shared_mappings)
    : Plastic_Image(!size.isEmpty() ? size : image.size(),
                    (shared_mappings & BAND_MAP) ? image.source_band_map() : NULL,
                    (shared_mappings & TRANSFORMS) ? const_cast<const QTransform **>(image.source_transforms()) : NULL,
                    (shared_mappings & DATA_MAPS) ? const_cast<const Data_Map **>(image.source_data_maps()) : NULL),
      Source(image.Source), //	Share the JP2_Reader.
      Source_Rendering_Monitor(NULL), Data_Buffer_Size(0)
{
#if ((DEBUG_SECTION) & DEBUG_CONSTRUCTORS)
    clog << ">>> JP2_Image: Copy " << image.source_name() << endl
         << "     specified size = " << size << endl
         << "         image size = " << image.size() << endl
         << "    shared_mappings = " << shared_mappings << ": " << mapping_type_names(shared_mappings) << endl
         << "    Source: ";
    if (Source)
        clog << Source->image_size() << ", " << Source->image_bands() << 'b' << endl;
    else
        clog << "NULL";
#endif

    Auto_Update = image.Auto_Update;
    bool do_update = auto_update(false);
    ...
    if (!(shared_mappings & TRANSFORMS))
        source_transforms(const_cast<const QTransform **>(image.source_transforms()));
    if (!(shared_mappings & DATA_MAPS))
        source_data_maps(const_cast<const Data_Map **>(image.source_data_maps()));
...
#endif
#if ((DEBUG_SECTION) & DEBUG_CONSTRUCTORS)
    clog << "    background_color = #" << setw(8) << setfill('0') << hex << background_color() << dec << setfill(' ')
         << endl;
#endif
...
JP2_Image::JP2_Image(const QString &name, const QSize &size)
    : Plastic_Image(size), Source(NULL), Source_Rendering_Monitor(NULL), Data_Buffer_Size(0)
{
#if ((DEBUG_SECTION) & DEBUG_CONSTRUCTORS)
    clog << ">>> JP2_Image: " << name << endl << "    specified size = " << size << endl;
#endif
    try
    {
        Source = JP2::reader(name.toStdString());
    }
    catch (exception &
#if ((DEBUG_SECTION) & DEBUG_CONSTRUCTORS)
               except
    ...
    {
#if ((DEBUG_SECTION) & DEBUG_CONSTRUCTORS)
        clog << "    Failed to construct a JP2_Reader for the source -" << endl << except.what() << endl;
#endif
    }
...
void JP2_Image::initialize()
{
    ...
    Data_Buffers[0] = Data_Buffers[1] = Data_Buffers[2] = Display_Data_Buffers[0] = Display_Data_Buffers[1] =
    ...
    if (!Source)
    {
        close();
#if ((DEBUG_SECTION) & (DEBUG_CONSTRUCTORS | DEBUG_DATA_BUFFERS | DEBUG_BAND_MAP))
        clog << "    no Source" << endl << "<<< JP2_Image::initialize" << endl;
#endif
        return;
    }

    //	Determine the size of a source data buffer.
    int image_width = Source->image_width(), image_height = Source->image_height(), bands = Source->image_bands();
    Source_Data_Buffers = new void *[bands];
#if ((DEBUG_SECTION) & (DEBUG_CONSTRUCTORS | DEBUG_DATA_BUFFERS | DEBUG_BAND_MAP))
    clog << "      image size = " << image_width << "w, " << image_height << "h, " << bands << "b, "
         << Source->rendered_pixel_bytes() << " byte" << ((Source->rendered_pixel_bytes() == 1) ? ", " : "s, ")
         << Source->rendered_pixel_bits() << " bits" << endl
         << "    display size = " << size() << endl;
#endif
    if (!image_width || !image_height || !bands || !Source->rendered_pixel_bytes())
    {
        close();
#if ((DEBUG_SECTION) & (DEBUG_CONSTRUCTORS | DEBUG_DATA_BUFFERS | DEBUG_BAND_MAP))
        clog << "    no rendered image content" << endl << "<<< JP2_Image::initialize" << endl;
#endif
        return;
    }
    /*
    ...
    */
    if (image_width > (width() << 1) || image_height > (height() << 1))
    {
    ...
    bool overflow = false;
    unsigned long long value = image_width, amount = image_width;
    amount *= image_height;
    ...
    if (overflow || amount > MAX_ALLOCATION)
    {
        close();
        ostringstream message;
        message << ID << endl
                << "The required pixel data buffer size for a " << Source->image_width() << "w, "
                << Source->image_height() << "h, " << Source->rendered_pixel_bits() << " byte image band" << endl
                << "is greater than the maximum " << magnitude_of(MAX_ALLOCATION) << " byte allocation size.";
#if ((DEBUG_SECTION) & (DEBUG_CONSTRUCTORS | DEBUG_DATA_BUFFERS | DEBUG_BAND_MAP))
        clog << message.str() << endl;
#endif
        throw out_of_range(message.str());
...
#if ((DEBUG_SECTION) & (DEBUG_CONSTRUCTORS | DEBUG_DATA_BUFFERS | DEBUG_BAND_MAP))
    clog << "    Data_Buffer_Size = " << magnitude_of(Data_Buffer_Size) << " (" << Data_Buffer_Size << ") bytes"
         << endl;
#endif
    ...
    while (++band < bands)
    {
        try
        {
            Data_Buffers[band] = new Source_Data(Data_Buffer_Size);
        }
        catch (out_of_range &)
        {
            close();
    ...
    Cube rendered_region(Source->rendered_region());
#if ((DEBUG_SECTION) & (DEBUG_CONSTRUCTORS | DEBUG_DATA_BUFFERS | DEBUG_BAND_MAP))
...
JP2_Image::~JP2_Image()
{
    ...
    for (int band = 0; band < 3; band++)
    {
        ...
        {
#if ((DEBUG_SECTION) & DEBUG_CONSTRUCTORS)
            clog << "    delete Data_Buffers[" << band << "] - " << *Data_Buffers[band] << endl;
#endif
    ...
    {
#if ((DEBUG_SECTION) & DEBUG_CONSTRUCTORS)
        clog << "    delete Source_Data_Buffers array @ " << (void *)Source_Data_Buffers << endl;
#endif
...
/*==============================================================================
    Accessors
*/
JP2_Image *JP2_Image::clone(const QSize &size, Mapping_Type shared_mappings) const
{
    return new JP2_Image(*this, size, shared_mappings);
}

const void *JP2_Image::source() const
{
    return Source;
}
...
idaeim::PVL::Aggregate *JP2_Image::metadata()
{
    ...
    else if (Source)
    {
        ...
        if (metadata)
        {
            ...
            Aggregate *JP2_metadata(new Aggregate(*(static_cast<JP2_Metadata *>(Source)->metadata_parameters())));
...
*/
QSize JP2_Image::source_size() const
{
    return Source ? QSize(Source->image_width(), Source->image_height()) : QSize(0, 0);
}

unsigned int JP2_Image::source_bands() const
{
    if (Source)
...
unsigned int JP2_Image::source_precision_bits() const
{
    if (Source)
...
Plastic_Image::Pixel_Datum JP2_Image::source_pixel_value(unsigned int x, unsigned int y, unsigned int band) const
{
#if ((DEBUG_SECTION) & DEBUG_PIXEL_DATUM)
    clog << ">>> JP2_Image::source_pixel_value: " << x << "x, " << y << "y, " << band << 'b' << endl;
#endif
    if (Source && Display_Data_Buffers[band]->Rendered && band < 3)
    {
        //	Map the image coordinate to the source data location.
        QPoint origin(round_down(source_origin(band)));
#if ((DEBUG_SECTION) & DEBUG_PIXEL_DATUM)
        clog << "       origin = " << origin << endl;
#endif
        unsigned int resolution = Display_Data_Buffers[band]->Rendered_Resolution;
        if (resolution && x >= static_cast<unsigned int>(origin.rx()) && y >= static_cast<unsigned int>(origin.ry()))
        {
            ...
            clog << "    Rendered_Resolution = " << (resolution + 1) << endl
                 << "                 offset = " << x << "x, " << y << 'y' << endl
                 << "          Rendered_Size = " << Display_Data_Buffers[band]->Rendered_Size << endl;
#endif
            if (x < static_cast<unsigned int>((Display_Data_Buffers[band]->Rendered_Size).rwidth()) &&
                y < static_cast<unsigned int>((Display_Data_Buffers[band]->Rendered_Size).rheight()))
            {
                Pixel_Datum value;
                ...
                if (Source->rendered_pixel_bytes() == 1)
                    value = *(reinterpret_cast<quint8 *>(Display_Data_Buffers[band]->Buffer) + y);
                else
                    value = *(reinterpret_cast<quint16 *>(Display_Data_Buffers[band]->Buffer) + y);
#if ((DEBUG_SECTION) & DEBUG_PIXEL_DATUM)
...
QPoint JP2_Image::map_image_to_source(const QPoint &image_coordinate, int band) const
{
#if ((DEBUG_SECTION) & (DEBUG_IMAGE_TO_SOURCE | DEBUG_HISTOGRAMS))
    LOCKED_LOGGING((clog << ">>> JP2_Image::map_image_to_source:" << endl
                         << "        image_coordinate = " << image_coordinate << ", " << band << 'b' << endl));
#endif
    if (band < 0)
        band = 0;
    if (Source && band < 3)
    {
        QPoint source_coordinate(image_coordinate), origin(round_down(source_origin(band)));
        unsigned int resolution = Display_Data_Buffers[band]->Rendered_Resolution;
#if ((DEBUG_SECTION) & (DEBUG_IMAGE_TO_SOURCE | DEBUG_HISTOGRAMS))
        clog << "                  origin = " << origin << endl << "     Rendered_Resolution = " << resolution << endl;
#endif
        if (resolution && source_coordinate.rx() >= origin.rx() && source_coordinate.ry() >= origin.ry())
        {
            //	Offset into the band data.
            ...
            clog << "       source coordinate = " << source_coordinate << endl
                 << "     Rendered_Size = " << Display_Data_Buffers[band]->Rendered_Size << endl;
#endif
            if (source_coordinate.rx() < static_cast<int>((Display_Data_Buffers[band]->Rendered_Size).rwidth()) &&
                source_coordinate.ry() < static_cast<int>((Display_Data_Buffers[band]->Rendered_Size).rheight()))
            {
#if ((DEBUG_SECTION) & (DEBUG_IMAGE_TO_SOURCE | DEBUG_HISTOGRAMS))
                clog << "<<< JP2_Image::map_image_to_source: " << source_coordinate << endl;
#endif
...
/*------------------------------------------------------------------------------
    Histograms
*/
unsigned long long JP2_Image::source_histograms(QVector<Histogram *> histograms, const QRect &source_region) const
{
#if ((DEBUG_SECTION) & (DEBUG_HISTOGRAMS | DEBUG_PRINT_HISTOGRAMS))
    clog << ">>> JP2_Image::source_histograms:" << endl << "    source_region = " << source_region << endl;
#endif
    if (histograms.size() < 3)
    {
#if ((DEBUG_SECTION) & (DEBUG_HISTOGRAMS | DEBUG_PRINT_HISTOGRAMS))
        clog << "    resize histograms set from " << histograms.size() << " to 3" << endl;
#endif
        histograms.resize(3);
    }
#if ((DEBUG_SECTION) & (DEBUG_HISTOGRAMS | DEBUG_PRINT_HISTOGRAMS))
    clog << "    histograms @ " << (void *)histograms[0] << ", " << (void *)histograms[1] << ", "
         << (void *)histograms[2] << endl;
#endif
    if (!rendered_source_data() || source_region.isEmpty())
    {
#if ((DEBUG_SECTION) & (DEBUG_HISTOGRAMS | DEBUG_PRINT_HISTOGRAMS))
        clog << "    nothing to render" << endl << "<<< JP2_Image::source_histograms: 0" << endl;
#endif
        return 0;
    }

    QRect selected_region(image_region());
#if ((DEBUG_SECTION) & (DEBUG_HISTOGRAMS | DEBUG_PRINT_HISTOGRAMS))
    ...
    if (source_region.left() > (selected_region.left() + selected_region.width()) ||
        ...
        (source_region.top() + source_region.height()) < selected_region.top())
    {
//	The histogram region does not intersect with the image region.
    ...
    QRect source_data_region;
    QPoint source_data_BR;
    unsigned long long count, max_count = 0;
    for (int band = 0; band < 3; ++band)
    {
        if (refresh_source_histogram(histograms, band))
        {
#if ((DEBUG_SECTION) & (DEBUG_HISTOGRAMS | DEBUG_PRINT_HISTOGRAMS))
            clog << "       image region UL = " << selected_region.topLeft() << endl;
#endif
            source_data_region.setTopLeft(map_image_to_source(selected_region.topLeft(), band));
#if ((DEBUG_SECTION) & (DEBUG_HISTOGRAMS | DEBUG_PRINT_HISTOGRAMS))
            clog << "        source data UL = " << source_data_region.topLeft() << endl
                 << "       image region BR = " << selected_region.bottomRight() << endl;
#endif
            source_data_region.setBottomRight(map_image_to_source(selected_region.bottomRight(), band));
#if ((DEBUG_SECTION) & (DEBUG_HISTOGRAMS | DEBUG_PRINT_HISTOGRAMS))
            clog << "        source data BR = " << source_data_region.bottomRight() << endl
                 << "    source data region = " << source_data_region << endl;
#endif
            if (source_data_region.left() < 0 || source_data_region.right() < 0)
            {
//	This can happen if rendering is in progress.
            ...
            if (Source->rendered_pixel_bytes() == 1)
                source_data_histogram<quint8>(histograms[band], source_data_region, band);
            else
                source_data_histogram<quint16>(histograms[band], source_data_region, band);

            count = (unsigned long long)source_data_region.width() * source_data_region.height();
            if (max_count < count)
                max_count = count;
...
void JP2_Image::source_data_histogram(Histogram *histogram, const QRect &source_data_region, int band) const
{
#if ((DEBUG_SECTION) & (DEBUG_HISTOGRAMS | DEBUG_PRINT_HISTOGRAMS))
    clog << ">>> JP2_Image::source_data_histogram:" << endl
         ...
         << "    band = " << band << endl;
#if ((DEBUG_SECTION) & DEBUG_PRINT_HISTOGRAMS)
    int line = source_data_region.top(),
        line_number_digits = decimal_digits(source_data_region.y() + source_data_region.height()),
        mask = (1 << source_precision_bits()) - 1, pixel_value_digits = hex_digits(mask);
#endif
#endif
    unsigned int source_data_width = (Display_Data_Buffers[band]->Rendered_Size).width(),
                 line_samples = source_data_region.width(), max_value = histogram->size();
    Pixel_Data_Type *data = reinterpret_cast<Pixel_Data_Type *>(Display_Data_Buffers[band]->Buffer) +
                            source_data_region.left() + (source_data_region.top() * source_data_width),
                    *end_data = data + (source_data_region.height() * source_data_width), *datum, value, *end_datum;
    while (data < end_data)
    {
        datum = data;
...
bool JP2_Image::source_band_map(const unsigned int *band_map, bool shared)
{
    ...
    clog << ">>> JP2_Image::source_band_map:" << band_map << endl << "    shared = " << boolalpha << shared << endl;
#endif
    bool do_update = auto_update(false), changed = Plastic_Image::source_band_map(band_map, shared);
    if (changed)
...
/*==============================================================================
    Rendering
*/
bool JP2_Image::needs_update(Mapping_Type changed) noexcept(false)
...
bool JP2_Image::render_image() noexcept(false)
// throw (Render_Exception, std::bad_exception)
{
    ...
    */
    bool canceled = cancel_update(false);

#if ((DEBUG_SECTION) & DEBUG_TILE_MARKINGS)
    //	Tile markings.
    double origin_x = Rendering->Geo_Transforms[0]->m31(), origin_y = Rendering->Geo_Transforms[0]->m32(),
           scaling = Rendering->Geo_Transforms[0]->m11();
    QString label = QString(" JP2 image @ %1: so %2, %3; s %4")
                        .arg((ulong)this, 0, 16)
                        .arg(origin_x)
                        .arg(origin_y)
                        .arg(scaling)
    ...
    mark_image(this, label, TILE_MARKINGS_Y, TILE_MARKINGS_COLOR);
    mark_image(this, label, height() - TILE_MARKINGS_Y - 12, TILE_MARKINGS_COLOR);
#endif
    ...
    is_rendering(false);
#if ((DEBUG_SECTION) & DEBUG_RENDER)
    LOCKED_LOGGING((clog << "<<< JP2_Image::render_image: " << boolalpha << rendered << endl));
#endif
    return rendered;
}

bool JP2_Image::render_source()
{
#if ((DEBUG_SECTION) & (DEBUG_RENDER | DEBUG_OVERVIEW | DEBUG_LOCATION))
    LOCKED_LOGGING((clog << ">>> JP2_Image::render_source:" << endl << "    " << *this << endl));
#endif
    bool rendered = true;
    Cube expected, actual;
    int bands = source_bands();
#if ((DEBUG_SECTION) & DEBUG_RENDER)
    LOCKED_LOGGING((clog << "    source_bands = " << bands << endl));
#endif
    if (bands > 3)
        bands = 3;

#if ((DEBUG_SECTION) & DEBUG_RENDER)
    LOCKED_LOGGING((clog << "    mapping_differences = " << mapping_differences() << endl));
#endif
...
*/
#if ((DEBUG_SECTION) & DEBUG_RENDER)
        LOCKED_LOGGING((clog << "    registering Source_Data_Buffers as Source image_data" << endl));
#endif
        ...
        int band = -1;
        while (rendered && ++band < bands)
        {
            if (rendering_resolution_and_region(band) || Rendering->Needs_Update & (TRANSFORMS & BAND_MAP))
            {
                ...
                LOCKED_LOGGING((clog << "    rendering display band " << band << " -> "
                                     << *Rendering_Display_Data_Buffers[band] << endl));
#endif
...
#if ((DEBUG_SECTION) & (DEBUG_RENDER | DEBUG_OVERVIEW))
                LOCKED_LOGGING((clog << "      rendered region " << actual << ' ' << ((actual == expected) ? '=' : '!')
                                     << "= " << expected << " expected region" << endl));
#endif
                if (actual != expected)
                    rendered = false;
                else
                    source_buffer_rendered(true, Source_Data_Buffers[Rendering->Band_Map[band]]);
            }
        }
...
#if ((DEBUG_SECTION) & (DEBUG_RENDER | DEBUG_OVERVIEW))
            LOCKED_LOGGING((clog << "    rendering all bands -" << endl
                                 << "    display band 0 -> " << *Rendering_Display_Data_Buffers[0] << endl
                                 << "    display band 1 -> " << *Rendering_Display_Data_Buffers[1] << endl
                                 << "    display band 2 -> " << *Rendering_Display_Data_Buffers[2] << endl));
#endif
...
#if ((DEBUG_SECTION) & (DEBUG_RENDER | DEBUG_OVERVIEW | DEBUG_LOCATION))
            LOCKED_LOGGING((clog << "      rendered region " << expected << ' ' << ((actual == expected) ? '=' : '!')
                                 << "= " << expected << " expected region" << endl));
#endif
        }
        else if (Rendering->Needs_Update)
            rendered = map_source();
    }
#if ((DEBUG_SECTION) & (DEBUG_RENDER | DEBUG_OVERVIEW | DEBUG_LOCATION))
    LOCKED_LOGGING((clog << "<<< JP2_Image::render_source: " << boolalpha << rendered << endl));
#endif
    return rendered;
}

bool JP2_Image::map_source()
{
#if ((DEBUG_SECTION) & (DEBUG_RENDER | DEBUG_LOCATION))
    LOCKED_LOGGING((clog << ">>> JP2_Image::map_source:" << endl << "    " << *this << endl));
#endif
    bool mapped = true;
    Rendering_Monitor::Status status = Rendering_Monitor::REMAPPING_DATA;
    int data_precision = source_precision_bytes(), display_height = height(),
        increment = Source->effective_rendering_increment_lines();
    if (increment == 0)
        increment = display_height;
#if ((DEBUG_SECTION) & (DEBUG_RENDER | DEBUG_LOCATION))
    LOCKED_LOGGING((clog << "    increment = " << increment << endl));
#endif
    QRect display_region(0, 0, width(), increment);
    while (display_region.top() < display_height)
    {
        ...
        clog << "    JP2_Image::map_source: notify_rendering_monitors"
                " status "
             << status << " \"" << Rendering_Monitor::Status_Message[status] << '"' << endl
             << "    display_region = " << display_region << endl;
        UNLOCK_LOG;
#endif
        if (!mapped ||
            !(mapped = notify_rendering_monitors(status, Rendering_Monitor::Status_Message[status], display_region)))
            break;
    ...
    notify_rendering_monitors(status, Rendering_Monitor::Status_Message[status], display_region);
#if ((DEBUG_SECTION) & (DEBUG_RENDER | DEBUG_LOCATION))
    LOCKED_LOGGING((clog << "<<< JP2_Image::map_source: " << boolalpha << mapped << endl));
#endif
    return mapped;
}

template <typename Pixel_Data_Type> bool JP2_Image::map_source_data_to_display_data(const QRect &region)
{
#if ((DEBUG_SECTION) & (DEBUG_RENDER | DEBUG_PIXEL_MAPPING | DEBUG_PIXEL_DATA | DEBUG_LOCATION))
    LOCKED_LOGGING((clog << ">>> JP2_Image::map_source_data_to_display_data: " << region << endl
                         << "    " << *this << endl));
#endif
    Pixel_Datum datum;
    QRgb pixel_value, *display_data, *image_display_data = image_data() + (region.top() * width()) + region.left();
    int source_line = region.height(); //	display lines, temp.
    if (source_line == 0 && region.top() == 0)
        source_line = height();
    int source_sample = region.width(); //	display samples, temp.
    if (source_sample == 0 && region.left() == 0)
        source_sample = width();
    int display_line = region.top() - 1, display_sample, display_line_end = region.top() + source_line,
    ...
    clog << "      display image size = " << size() << endl
         ...
         << "     region display_data @ " << (void *)image_display_data << " at " << region.left() << "x, "
         << region.top() << 'y' << endl
         << "      " << setw(2) << (sizeof(Pixel_Data_Type) << 3) << "-bit source_data -" << endl
         << "         background_color #" << setw(sizeof(QRgb)) << setfill('0') << hex << Rendering->Background_Color
         << setfill(' ') << dec << endl;
    UNLOCK_LOG;
#endif

    //	Pre-map the band-map sensitive values.
    QTransform *transforms[3];
    Pixel_Data_Type *source_data[3];
    unsigned int source_data_width[3], source_data_height[3];
    Data_Map *data_maps[3];
    QRgb background_value[3];
    int band = -1;
    while (++band < 3)
    {
        //	Geo-transform.
        ...
        //	Source data buffer.
        source_data[band] = reinterpret_cast<Pixel_Data_Type *>(Rendering_Display_Data_Buffers[band]->Buffer);

        //	Source data buffer size.
        source_data_width[band] = (Rendering_Display_Data_Buffers[band]->Rendered_Size).rwidth();
        source_data_height[band] = (Rendering_Display_Data_Buffers[band]->Rendered_Size).rheight();

        //	Source-to-Display data LUT.
        data_maps[band] = Rendering->Data_Maps[band];

        //	Background (no image) value.
        background_value[band] = Rendering->Background_Color & (0xFF << (16 - (band << 3)));

#if ((DEBUG_SECTION) & (DEBUG_RENDER | DEBUG_PIXEL_MAPPING | DEBUG_PIXEL_DATA | DEBUG_LOCATION))
        LOCK_LOG;
        if (transforms[band])
        {
            transforms[band]->map(region.left(), region.top(), &sample, &line);
        ...
        clog << "    display band " << band << " from " << *Display_Data_Buffers[band] << endl
             << "      display " << region.left() << "x, " << region.top() << 'y' << " from source " << source_sample
             << "x, " << source_line << 'y' << " @ "
             << (void *)(source_data[band] + (source_line * source_data_width[band])) << endl;
        UNLOCK_LOG;
#endif
    }
    ...
    while (++display_line < display_line_end)
    {
        ...
        while (++display_sample < display_sample_end)
        {
            ...
            while (++band < 3)
            {
                if (transforms[band])
                {
                    transforms[band]->map(display_sample, display_line, &sample, &line);
                    //	Round down to integer coordinates.
                    source_sample = round_down(sample);
                    source_line = round_down(line);
                }
                ...
                if (!display_line)
#endif
                    clog << "      " << band << "->" << Rendering->Band_Map[band] << ": display " << setw(3)
                         << display_sample << "x, " << setw(3) << display_line << "y <- source " << setw(3)
                         << source_sample << "x, " << setw(3) << source_line << "y ";
#endif
                if (source_sample < 0 || source_sample >= (int)source_data_width[band] || source_line < 0 ||
                ...
                else
                {
                    /*
                        ...
                        being assembled.
                    */
                    datum = *(source_data[band] + (source_line * source_data_width[band]) + source_sample);
                    if (datum > (unsigned int)(data_maps[band]->size()))
                    {
#if ((DEBUG_SECTION) & DEBUG_PIXEL_MAPPING)
                        clog << "= BG" << endl
                             << "Invalid source data mapping while rendering" << endl
                             << "at line " << source_line << ", sample " << source_sample << ", band " << band
                             << " with datum " << datum << " of data map size " << data_maps[band]->size();
#endif
                        pixel_value |= background_value[band];
                    }
                    else
                    {
                        ...
                        {
#endif
                            clog << "@ " << setfill(' ') << setw(sizeof(void *) << 1)
                                 << (void *)(source_data[band] + (source_line * source_data_width[band]) +
                                             source_sample)
                                 << hex << setfill('0') << setw(sizeof(Pixel_Data_Type) << 1) << datum << " -> "
                                 << setw(2) << static_cast<Pixel_Datum>(data_maps[band]->at(datum)) << dec
                                 << setfill(' ') << endl;
#if ((DEBUG_SECTION) & DEBUG_ONE_LINE)
                        }
#endif
                        ...
                        pixel_value |= static_cast<QRgb>(data_maps[band]->at(datum) << (16 - (band << 3)));
                    }
                }
            }
#if ((DEBUG_SECTION) & DEBUG_PIXEL_MAPPING)
            ...
            if (!display_line)
#endif
                clog << "        display pixel @ " << setfill(' ') << setw(sizeof(display_data) << 1)
                     << (void *)display_data << " = " << hex << setw(8) << pixel_value << dec << endl;
#endif
    ...
    if (input[0] == 'q')
        exit(7);
#endif
    LOCKED_LOGGING((clog << "<<< JP2_Image::map_source_data_to_display_data: " << boolalpha
                         << (display_line == display_line_end) << endl));
#endif
    return (display_line == display_line_end);
}

bool JP2_Image::rendering_resolution_and_region(int band)
{
#if ((DEBUG_SECTION) & (DEBUG_RENDER | DEBUG_LOCATION))
    clog << ">>> JP2_Image::rendering_resolution_and_region: " << band << endl;
#endif
    int image_band = (band < 0) ? 0 : band;
#if ((DEBUG_SECTION) & (DEBUG_RENDER | DEBUG_LOCATION))
    clog << "    effective band " << image_band << " from " << *Rendering_Display_Data_Buffers[image_band] << endl;
#endif
    bool rendering_needed = !rendered_source_data(band);

    //	Determine the resolution level to be used with the JP2_Reader.
    QSizeF scaling(source_scaling(image_band));
    double scale = qMax(scaling.rwidth(), scaling.rheight());
    unsigned int resolution = nearest_resolution_level(scale);
    ...
    QPoint origin(round_down(source_origin(image_band)));

    Rectangle region(origin.x(), origin.y(),
                     //	The region is relative to the full resolution grid.
                     width() << (resolution - 1), height() << (resolution - 1));
    ...
    rendering_needed |= Source->resolution_and_region(resolution, region);
#if ((DEBUG_SECTION) & (DEBUG_RENDER | DEBUG_LOCATION))
    ...
    (Rendering_Display_Data_Buffers[image_band]->Rendered_Size).rwidth() = Source->rendered_width();
    (Rendering_Display_Data_Buffers[image_band]->Rendered_Size).rheight() = Source->rendered_height();
...
#if ((DEBUG_SECTION) & DEBUG_RENDER)
    clog << "    Differential_Transforms -" << Differential_Transforms[image_band];
#endif
    //	Set the residual, after resolution level, scaling.
    scale = 1 << (resolution - 1);
    Differential_Transforms[image_band].scale(scale, scale);
#if ((DEBUG_SECTION) & DEBUG_RENDER)
    clog << "    at scale " << scale << " -" << Differential_Transforms[image_band];
#endif
    //	Invert, clipping the origin offset to its fractional part.
    Differential_Transforms[image_band] = invert_clipping_origin(Differential_Transforms[image_band]);
#if ((DEBUG_SECTION) & DEBUG_RENDER)
    clog << "    inverted and origin clipped - " << Differential_Transforms[image_band] << "      map 0,0 -> "
         << Differential_Transforms[image_band].map(QPointF(0, 0)) << endl
         << "      map 1,1 -> " << Differential_Transforms[image_band].map(QPointF(1, 1)) << endl
         << "      map " << (width() - 2) << ',' << (height() - 2) << " -> "
         << Differential_Transforms[image_band].map(QPointF(width() - 2, height() - 2)) << endl
         << "      map " << (width() - 1) << ',' << (height() - 1) << " -> "
         << Differential_Transforms[image_band].map(QPointF(width() - 1, height() - 1)) << endl
         << "      map " << Rendering_Display_Data_Buffers[image_band]->Rendered_Size << " -> "
         << Differential_Transforms[image_band].map(QPointF(Source->rendered_width(), Source->rendered_height()))
         << endl;
#endif
    ...
    if (band < 0)
    {
        ...
        if (Rendering_Display_Data_Buffers[2]->Rendered_Size != Rendering_Display_Data_Buffers[0]->Rendered_Size)
        {
            rendering_needed = true;
            Rendering_Display_Data_Buffers[2]->Rendered = false;
            Rendering_Display_Data_Buffers[2]->Rendered_Size = Rendering_Display_Data_Buffers[0]->Rendered_Size;
        }
        if (Rendering_Display_Data_Buffers[1]->Rendered_Size != Rendering_Display_Data_Buffers[0]->Rendered_Size)
        {
            rendering_needed = true;
            Rendering_Display_Data_Buffers[1]->Rendered = false;
            Rendering_Display_Data_Buffers[1]->Rendered_Size = Rendering_Display_Data_Buffers[0]->Rendered_Size;
        }

        Differential_Transforms[2] = Differential_Transforms[1] = Differential_Transforms[0];
    }

    //	Set the rendering increment.
    unsigned int height = (Rendering_Display_Data_Buffers[image_band]->Rendered_Size).rheight(),
                 rendering_increment = rendering_increment_lines();
    if (rendering_increment &&
        //	"Close enough" margin.
        ((rendering_increment >> 3) == 0 || (rendering_increment + (rendering_increment >> 3)) > height))
        rendering_increment = height;
    Source->rendering_increment_lines(rendering_increment);
#if ((DEBUG_SECTION) & (DEBUG_RENDER | DEBUG_LOCATION))
    clog << "    source rendering_increment_lines = " << rendering_increment << endl;
#endif

#if ((DEBUG_SECTION) & (DEBUG_RENDER | DEBUG_LOCATION))
    clog << "<<< JP2_Image::rendering_resolution_and_region: " << boolalpha << rendering_needed << endl;
#endif
...
/*==============================================================================
    Helpers
*/
bool JP2_Image::is_JP2_file(const QString &pathname)
{
    return JP2_Utilities::is_JP2_file(pathname.toStdString(), 0, 0);
}

unsigned int JP2_Image::nearest_resolution_level(double scale) const
{
#if ((DEBUG_SECTION) & DEBUG_RENDER)
    ...
    {
        ostringstream message;
        message << ID << endl << "Can't use image scale factor " << scale << '.';
        throw invalid_argument(message.str());
    }
    ...
    {
        unsigned int resolution_levels = Source->resolution_levels(),
        ...
        */
        while ((scale_level >>= 1) && ++resolution_level < resolution_levels)
            ;
    }
#if ((DEBUG_SECTION) & DEBUG_RENDER)
...
QRect JP2_Image::map_source_to_display(const PIRL::Cube &source_region) const
{
#if ((DEBUG_SECTION) & (DEBUG_RENDER | DEBUG_LOCATION))
    LOCKED_LOGGING((clog << ">>> JP2_Image::map_source_to_display: " << source_region << endl
                         << "    " << *this << endl));
#endif
    QRect region;
    ...
    {
#if ((DEBUG_SECTION) & (DEBUG_RENDER | DEBUG_LOCATION))
        LOCKED_LOGGING((clog << "    Rendering @ " << (void *)Rendering << endl << "    for " << *this << endl));
#endif
        QPointF origin(Rendering->origin(0));
        QSizeF scaling(Rendering->scaling(0));
        if (source_region.Depth != 1)
        {
            //	Find the largest display region that encompasses the source region.
            QPointF origins(Rendering->origin(1));
            ...
            QSizeF scalings(Rendering->scaling(1));
...
#if ((DEBUG_SECTION) & (DEBUG_RENDER | DEBUG_LOCATION))
        LOCKED_LOGGING((clog << "      display origin = " << origin << endl
        ...
        QRectF rectangle(
            /*
                ...
                factors.
            */
            (source_region.X - origin.rx()) * scaling.rwidth(), (source_region.Y - origin.ry()) * scaling.rheight(),
            /*
                The size of the region in the display is the size of the
                image region scaled by the image scaling factors.
            */
            source_region.Width * scaling.rwidth(), source_region.Height * scaling.rheight());

        //	The display region contains the floating point display rectangle.
        region = rectangle.toAlignedRect();
#if ((DEBUG_SECTION) & (DEBUG_RENDER | DEBUG_LOCATION))
        LOCKED_LOGGING((clog << "    display region = " << region << " (" << rectangle << ')' << endl));
#endif

        //	Clip to the display image boundaries.
        if (region.left() < 0)
            region.setLeft(0);
        else if (region.left() > width())
            region.setLeft(width() - 1);

        if (region.top() < 0)
            region.setTop(0);
        else if (region.top() > height())
        ...
        if (region.height() == 0)
        {
            LOCKED_LOGGING((clog << "!!! zero height region" << endl));
        }
#endif
    }
#if ((DEBUG_SECTION) & (DEBUG_RENDER | DEBUG_LOCATION))
    LOCKED_LOGGING((clog << "<<< JP2_Image::map_source_to_display: " << region << endl));
#endif
    return region;
}

bool JP2_Image::is_rendering(bool rendering)
{
#if ((DEBUG_SECTION) & DEBUG_RENDER)
    LOCKED_LOGGING((clog << ">>> JP2_Image::is_rendering: " << boolalpha << rendering << endl
                         << "    " << *this << endl));
#endif
...
#if ((DEBUG_SECTION) & DEBUG_RENDER)
    LOCKED_LOGGING((clog << "    was rendering - " << was_rendering << endl));
#endif
    if (was_rendering)
    {
        if (!rendering)
        {
            ...
            while (++band < 3)
            {
                ...
                {
#if ((DEBUG_SECTION) & DEBUG_RENDER)
                    LOCKED_LOGGING((clog << "    " << band << " previous " << *Data_Buffers[band] << endl
                                         << "       current " << *Rendering_Data_Buffers[band] << endl));
#endif
                    *Data_Buffers[band] |= *Rendering_Data_Buffers[band];
#if ((DEBUG_SECTION) & DEBUG_RENDER)
                    LOCKED_LOGGING((clog << "       updated " << *Data_Buffers[band] << endl));
#endif
                }
                else
                    break;
            ...
            *Rendering_Data_Buffers[band] = *Data_Buffers[band];
            ...
            while (++index < 3)
                if (Display_Data_Buffers[index] == Data_Buffers[band])
                    Rendering_Display_Data_Buffers[index] = Rendering_Data_Buffers[band];
        }
#if ((DEBUG_SECTION) & DEBUG_RENDER)
        LOCK_LOG;
