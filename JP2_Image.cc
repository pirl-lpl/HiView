...
/** Rendering Process Monitor
    ...
    Dynamic_Image::notify_rendering_monitors(Rendering_Monitor::Status,
    const QString&, const QRect&) forwarded} to the owner of the monitor
    - the Dynamic_Image base class of the JP2_Image bound to the
    JP2_Image_Rendering_Monitor object when it was created - which
    multiplexes the notice to all listeners that have {@link
    Dynamic_Image::add_rendering_monitor(Dynamic_Image::Rendering_Monitor*)
        ...
        )
    {
        ...
        if (Owner)
        {
            ...
            */
            ...
            (
                static_cast<Dynamic_Image::Rendering_Monitor::Status>(status),
...
(
    ...
    :
    Dynamic_Image(!size.isEmpty() ? size
                  : (JP2_reader ?
...
(
    ...
    :
    Dynamic_Image(!size.isEmpty() ? size : image.size(),
...
)
{
    ...
    bool
        do_update = auto_update(false),
        changed = Dynamic_Image::source_band_map(band_map, shared);
    if (changed)
...
//throw (Render_Exception, std::bad_exception)
{
    ...
    bool
        updated = Dynamic_Image::needs_update(changed);
...
)
{
    ...
    clog << "      display image size = " << size() << endl
        ...
        << "      image display data @ " << reinterpret_cast<void*>(image_data()) << endl
        << "     region display_data @ " << reinterpret_cast<void*>(image_display_data)
    ...
    while (++band < 3)
    {
        ...
        clog << "    display band " << band
            ...
            << " @ " << reinterpret_cast<void*>(source_data[band]
...
#if ((DEBUG_SECTION) & (DEBUG_RENDER | DEBUG_LOCATION))
        LOCKED_LOGGING((
            clog << "    Rendering @ " << reinterpret_cast<void*>(Rendering) << endl
            << "    for " << *this << endl));
#endif
        QPointF
...
)
{
    ...
    */
    int
        band = -1;
    bool
        was_rendering = Dynamic_Image::is_rendering(rendering);
#if ((DEBUG_SECTION) & DEBUG_RENDER)
    LOCKED_LOGGING((
