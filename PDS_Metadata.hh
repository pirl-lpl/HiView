...
*/
class PDS_Metadata : public QObject /*,
                     ...
                     */
{
  ...
  public:
    ...
    */
    ...
    static const char *const PDS_METADATA_GROUP;

    //	Structure definition parameters:

    //! PDS ID.
    static const char *const PDS_ID_PARAMETER_NAME;

    //!	Type of metadata records.
    static const char *const RECORD_TYPE_PARAMETER_NAME;

    //!	The RECORD_TYPE_PARAMETER_NAME used to define byte count based records.
    static const char *const BYTE_RECORD_TYPE;

    //!	The RECORD_TYPE_PARAMETER_NAME used to define fixed length records.
    static const char *const FIXED_LENGTH_RECORD_TYPE;

    //!	Number of records in the metadata label.
    static const char *const LABEL_RECORDS_PARAMETER_NAME;

    //!	Number of bytes per record.
    static const char *const RECORD_BYTES_PARAMETER_NAME;

    //!	Number of records in the file.
    static const char *const FILE_RECORDS_PARAMETER_NAME;

    //!	Data format.
    static const char *const INTERCHANGE_FORMAT_PARAMETER_NAME;

    //!	Name of the Image data block parameter group.
    static const char *const IMAGE_DATA_BLOCK_NAME;

    //!	Units name for storage measured in bytes.
    static const char *const BYTES_UNITS;
    ...
    */
    ...
    static const char *const IMAGE_MAP_PROJECTION_GROUP_NAME;

    //!	Type of projection.
    static const char *const PROJECTION_TYPE_PARAMETER_NAME;
    //!	The PROJECTION_TYPE_PARAMETER_NAME for equirectangular projection.
    static const char *const EQUIRECTANGULAR_PROJECTION_NAME;
    //!	The PROJECTION_TYPE_PARAMETER_NAME for polar stereographic projection.
    static const char *const POLARSTEREOGRAPHIC_PROJECTION_NAME;

    //!	Type of coordinate system.
    static const char *const COORDINATE_SYSTEM_PARAMETER_NAME;
    //!	Type of coordinate system (alternative name).
    static const char *const LATITUDE_TYPE_PARAMETER_NAME;
    //!	Planeto-centric (non-geographic) type of coordinate system.
    static const char *const PLANETOCENTRIC_PROJECTION_NAME;
    //!	Planeto-graphic (geographic) type of coordinate system.
    static const char *const PLANETOGRAPHIC_PROJECTION_NAME;

    //!	Horizontal offset of the projection origin from the image origin.
    static const char *const HORIZONATAL_OFFSET_PARAMETER_NAME;
    //!	Vertical offset of the projection origin from the image origin.
    static const char *const VERTICAL_OFFSET_PARAMETER_NAME;
    ...
    static const char *const PIXELS_PER_DEGREE_PARAMETER_NAME;
    //!	Real world size of a pixel (km by default, may be m).
    static const char *const PIXEL_SIZE_PARAMETER_NAME;

    //!	Planet's semi-major (equitorial) axis radius.
    static const char *const EQUITORIAL_RADIUS_PARAMETER_NAME;
    //!	Planet's semi-minor (polar) axis radius.
    static const char *const POLAR_RADIUS_PARAMETER_NAME;

    //!	Projection first standard parallel.
    static const char *const FIRST_STANDARD_PARALLEL_PARAMETER_NAME;
    //!	Projection second standard parallel.
    static const char *const SECOND_STANDARD_PARALLEL_PARAMETER_NAME;

    //!	Projection center longitude.
    static const char *const CENTER_LONGITUDE_PARAMETER_NAME;
    //!	Projection center latitude.
    static const char *const CENTER_LATITUDE_PARAMETER_NAME;

    //!	Image minimum latitude.
    static const char *const MINIMUM_LATITUDE_PARAMETER_NAME;
    //!	Image maximum latitude.
    static const char *const MAXIMUM_LATITUDE_PARAMETER_NAME;
    //!	Image minimum longitude.
    static const char *const MINIMUM_LONGITUDE_PARAMETER_NAME;
    //!	Image maximum longitude.
    static const char *const MAXIMUM_LONGITUDE_PARAMETER_NAME;

    //!	Positive longitude direction.
    static const char *const POSITIVE_LONGITUDE_PARAMETER_NAME;
    //!	POSITIVE_LONGITUDE_PARAMETER_NAME for easterly positive longitude.
    static const char *const POSITIVE_LONGITUDE_EAST_NAME;
    //!	POSITIVE_LONGITUDE_PARAMETER_NAME for westerly positive longitude.
    static const char *const POSITIVE_LONGITUDE_WEST_NAME;

    //!	Projection rotation.
    static const char *const PROJECTION_ROTATION_PARAMETER_NAME;

    //!	Not-applicable constant.
    static const char *const NOT_APPLICABLE_CONSTANT_PARAMETER_NAME;
    ...
    */
    inline static idaeim::PVL::Parameter *find_parameter(const idaeim::PVL::Aggregate &parameters,
                                                         const std::string &pathname, bool case_sensitive = false,
                                                         int skip = 0,
                                                         Parameter_Class_Selection parameter_class = ANY_PARAMETER)
    {
        return parameters.find(
            pathname, case_sensitive, skip,
            static_cast<idaeim::PVL::Parameter::Type>(
                ((parameter_class == ANY_PARAMETER)
                     ? idaeim::PVL::Parameter::ASSIGNMENT | idaeim::PVL::Parameter::AGGREGATE
                     : ((parameter_class == ASSIGNMENT_PARAMETER) ? idaeim::PVL::Parameter::ASSIGNMENT
    ...
    */
    inline static idaeim::PVL::Parameter *find_parameter(const idaeim::PVL::Aggregate &parameters,
                                                         const char *const pathname, bool case_sensitive = false,
                                                         int skip = 0,
                                                         Parameter_Class_Selection parameter_class = ANY_PARAMETER)
    {
        return PDS_Metadata::find_parameter(parameters, std::string(pathname), case_sensitive, skip, parameter_class);
    }
    ...
    */
    inline static idaeim::PVL::Parameter *find_parameter(const idaeim::PVL::Aggregate &parameters,
                                                         const QString &pathname, bool case_sensitive = false,
                                                         int skip = 0,
                                                         Parameter_Class_Selection parameter_class = ANY_PARAMETER)
    {
        return PDS_Metadata::find_parameter(parameters, pathname.toStdString(), case_sensitive, skip, parameter_class);
    }
    ...
    /**	Find a named Parameter within the metadata parameters.
        ...
        @see	find_parameter(const idaeim::PVL::Aggregate&, const std::string&,
            bool, int, Parameter_Class_Selection)
    */
    inline idaeim::PVL::Parameter *find_parameter(const std::string &pathname, bool case_sensitive = false,
                                                  int skip = 0,
                                                  Parameter_Class_Selection parameter_class = ANY_PARAMETER) const
    {
        return metadata() ? find_parameter(*metadata(), pathname, case_sensitive, skip, parameter_class) : NULL;
    }
    ...
    /**	Find a named Parameter within the metadata parameters.
        ...
        @see	find_parameter(const idaeim::PVL::Aggregate&, const std::string&,
            bool, int, Parameter_Class_Selection)
    */
    inline idaeim::PVL::Parameter *find_parameter(const char *const pathname, bool case_sensitive = false, int skip = 0,
                                                  Parameter_Class_Selection parameter_class = ANY_PARAMETER) const
    {
        return metadata() ? find_parameter(*metadata(), std::string(pathname), case_sensitive, skip, parameter_class)
                          : NULL;
    }
    ...
    */
    inline idaeim::PVL::Parameter *find_parameter(const QString &pathname, bool case_sensitive = false, int skip = 0,
                                                  Parameter_Class_Selection parameter_class = ANY_PARAMETER) const
    {
        return find_parameter(pathname.toStdString(), case_sensitive, skip, parameter_class);
    }
    ...
    /**	Removes a named Parameter within a Parameter Aggregate.
        ...
        @see	find_parameter(const idaeim::PVL::Aggregate&, const std::string&,
            bool, int, Parameter_Class_Selection)
    */
    inline static idaeim::PVL::Parameter *remove_parameter(const idaeim::PVL::Aggregate &parameters,
                                                           const std::string &pathname, bool case_sensitive = false,
                                                           int skip = 0,
                                                           Parameter_Class_Selection parameter_class = ANY_PARAMETER)
    {
        idaeim::PVL::Parameter *parameter(
            PDS_Metadata::find_parameter(parameters, pathname, case_sensitive, skip, parameter_class));
        if (parameter)
    ...
    */
    inline static idaeim::PVL::Parameter *remove_parameter(const idaeim::PVL::Aggregate &parameters,
                                                           ...
                                                           Parameter_Class_Selection parameter_class = ANY_PARAMETER)
    {
        return PDS_Metadata::remove_parameter(parameters, std::string(pathname), case_sensitive, skip, parameter_class);
    }
    ...
    */
    inline static idaeim::PVL::Parameter *remove_parameter(const idaeim::PVL::Aggregate &parameters,
                                                           ...
                                                           Parameter_Class_Selection parameter_class = ANY_PARAMETER)
    {
        return PDS_Metadata::remove_parameter(parameters, pathname.toStdString(), case_sensitive, skip,
                                              parameter_class);
    }
    ...
    /**	Removes a named Parameter within the metadata parameters.
        ...
        @see	remove_parameter(const idaeim::PVL::Aggregate&, const std::string&,
            bool, int, Parameter_Class_Selection)
    */
    inline idaeim::PVL::Parameter *remove_parameter(const std::string &pathname, bool case_sensitive = false,
                                                    int skip = 0,
                                                    Parameter_Class_Selection parameter_class = ANY_PARAMETER) const
    {
        return metadata() ? remove_parameter(*metadata(), pathname, case_sensitive, skip, parameter_class) : NULL;
    }
    ...
    /**	Removes a named Parameter within the metadata parameters.
        ...
        @see	remove_parameter(const std::string&, bool, int,
            Parameter_Class_Selection)
    */
    inline idaeim::PVL::Parameter *remove_parameter(const char *const pathname, bool case_sensitive = false,
                                                    int skip = 0,
                                                    Parameter_Class_Selection parameter_class = ANY_PARAMETER) const
    {
        return remove_parameter(std::string(pathname), case_sensitive, skip, parameter_class);
    }
    ...
    /**	Removes a named Parameter within the metadata parameters.
        ...
        @see	remove_parameter(const std::string&, bool, int,
            Parameter_Class_Selection)
    */
    inline idaeim::PVL::Parameter *remove_parameter(const QString &pathname, bool case_sensitive = false, int skip = 0,
                                                    Parameter_Class_Selection parameter_class = ANY_PARAMETER) const
    {
        return remove_parameter(pathname.toStdString(), case_sensitive, skip, parameter_class);
    }
    ...
    /**	Get the numeric value of a named parameter within a Parameter Aggregate.
        ...
        @see	find_parameter(const idaeim::PVL::Aggregate&, const std::string&,
            bool, int, Parameter_Class_Selection)
    */
    static double numeric_value(const idaeim::PVL::Aggregate &parameters, const std::string &pathname,
                                bool case_sensitive = false, int skip = 0);
    ...
    /**	Get the numeric value of a named parameter within a Parameter Aggregate.
        ...
        @see	find_parameter(const idaeim::PVL::Aggregate&, const std::string&,
            bool, int, Parameter_Class_Selection)
    */
    inline static double numeric_value(const idaeim::PVL::Aggregate &parameters, const char *const pathname,
                                       bool case_sensitive = false, int skip = 0)
    {
        return PDS_Metadata::numeric_value(parameters, std::string(pathname), case_sensitive, skip);
    }
    ...
    /**	Get the numeric value of a named parameter within a Parameter Aggregate.
        ...
        @see	find_parameter(const idaeim::PVL::Aggregate&, const std::string&,
            bool, int, Parameter_Class_Selection)
    */
    inline static double numeric_value(const idaeim::PVL::Aggregate &parameters, const QString &pathname,
                                       bool case_sensitive = false, int skip = 0)
    {
        return PDS_Metadata::numeric_value(parameters, pathname.toStdString(), case_sensitive, skip);
    }
    ...
    */
    ...
    /**	Get the numeric value of a named parameter within the metadata parameters.
        ...
        @see	numeric_value(const std::string&, bool, int,
            Parameter_Class_Selection)
    */
    double numeric_value(const char *const pathname, bool case_sensitive = false, int skip = 0) const
    {
    ...
    /**	Get the numeric value of a named parameter within the metadata parameters.
        ...
        @see	numeric_value(const std::string&, bool, int,
            Parameter_Class_Selection)
    */
    double numeric_value(const QString &pathname, bool case_sensitive = false, int skip = 0) const
    {
    ...
    */
    ...
    /**	Get the string value of a named parameter within a Parameter Aggregate.
        ...
        @see	find_parameter(const idaeim::PVL::Aggregate&, const std::string&,
            bool, int, Parameter_Class_Selection)
    */
    static std::string string_value(const idaeim::PVL::Aggregate &parameters, const std::string &pathname,
                                    bool case_sensitive = false, int skip = 0);
    ...
    /**	Get the string value of a named parameter within a Parameter Aggregate.
        ...
        @see	string_value(const idaeim::PVL::Aggregate&, const std::string&,
            bool, int)
    */
    inline static std::string string_value(const idaeim::PVL::Aggregate &parameters, const char *const pathname,
                                           bool case_sensitive = false, int skip = 0)
    {
        return PDS_Metadata::string_value(parameters, std::string(pathname), case_sensitive, skip);
    }
    ...
    /**	Get the string value of a named parameter within a Parameter Aggregate.
        ...
        @see	string_value(const idaeim::PVL::Aggregate&, const std::string&,
            bool, int)
    */
    inline static QString string_value(const idaeim::PVL::Aggregate &parameters, const QString &pathname,
                                       bool case_sensitive = false, int skip = 0)
    {
        return QString::fromStdString(
            PDS_Metadata::string_value(parameters, pathname.toStdString(), case_sensitive, skip));
    }
    ...
    */
    ...
    /**	Get the string value of a named parameter within the metadata parameters.
        ...
        @see	string_value(const idaeim::PVL::Aggregate&, const std::string&,
            bool, int)
    */
    inline std::string string_value(const char *const pathname, bool case_sensitive = false, int skip = 0)
    {
    ...
    /**	Get the string value of a named parameter within the metadata parameters.
        ...
        @see	string_value(const idaeim::PVL::Aggregate&, const std::string&,
            bool, int)
    */
    inline QString string_value(const QString &pathname, bool case_sensitive = false, int skip = 0)
    {
        return QString::fromStdString(string_value(pathname.toStdString(), case_sensitive, skip));
    }
    ...
    */
    template <typename T>
    bool parameter_value(const idaeim::PVL::Aggregate &parameters, const std::string &pathname, T &value,
                         bool case_sensitive = false, int skip = 0)
    {
        idaeim::PVL::Parameter *parameter(
            parameters.find(pathname, case_sensitive, skip, idaeim::PVL::Parameter::ASSIGNMENT));
        if (!parameter || parameter->value().is_Array())
            return false;
    ...
    */
    template <typename T>
    bool parameter_value(const idaeim::PVL::Aggregate &parameters, const char *const pathname, T &value,
                         bool case_sensitive = false, int skip = 0)
    {
        return PDS_Metadata::parameter_value(parameters, std::string(pathname), value, case_sensitive, skip);
    }
    ...
    */
    template <typename T>
    bool parameter_value(const idaeim::PVL::Aggregate &parameters, const QString &pathname, T &value,
                         bool case_sensitive = false, int skip = 0)
    {
        return PDS_Metadata::parameter_value(parameters, pathname.toStdString(), value, case_sensitive, skip);
    }
    ...
    */
    template <typename T>
    bool parameter_value(const std::string &pathname, T &value, bool case_sensitive = false, int skip = 0) const
    {
        return metadata() ? parameter_value(*metadata(), pathname, value, case_sensitive, skip) : false;
    }
    ...
    */
    template <typename T>
    bool parameter_value(const char *const pathname, T &value, bool case_sensitive = false, int skip = 0) const
    {
    ...
    */
    template <typename T>
    bool parameter_value(const QString &pathname, T &value, bool case_sensitive = false, int skip = 0) const
    {
    ...
    */
    ...
    {
        return QString::fromStdString(PDS_Metadata::absolute_pathname(pathname.toStdString()));
    }
