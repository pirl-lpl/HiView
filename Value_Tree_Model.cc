...
/*==============================================================================
    Constants
*/
const char *const Value_Tree_Model::ID =
    "UA::HiRISE::Value_Tree_Model ($Revision: 1.11 $ $Date: 2012/05/10 05:23:04 $)";

/*==============================================================================
    Constructors
*/
Value_Tree_Model::Value_Tree_Model(Array *root, QObject *parent) : QAbstractItemModel(parent), Root(root)
{
#if (DEBUG_SECTION & DEBUG_CONSTRUCTORS)
    clog << "<-> Value_Tree_Model::Value_Tree_Model: root @ " << (void *)root << endl;
#endif
#if (DEBUG_SECTION != 0)
    clog << boolalpha;
...
Value_Tree_Model::Value_Tree_Model(const Value_Tree_Model &model) : QAbstractItemModel(), Root(model.Root)
{
#if (DEBUG_SECTION & DEBUG_CONSTRUCTORS)
    clog << "<-> Value_Tree_Model::Value_Tree_Model: copy @ " << (void *)(&model) << " with root @ " << (void *)Root
         << endl;
#endif
#if (DEBUG_SECTION != 0)
    clog << boolalpha;
...
Value_Tree_Model &Value_Tree_Model::operator=(const Value_Tree_Model &model)
{
    if (this != &model)
        Root = model.Root;
    return *this;
...
*/
QModelIndex Value_Tree_Model::index(int row, int column, const QModelIndex &parent_index) const
{
...
QVariant Value_Tree_Model::data(const QModelIndex &index, int role) const
{
    ...
    {
        Value *value = static_cast<Value *>(index.internalPointer());
        if (value->is_Array())
...
#if (DEBUG_SECTION & DEBUG_DATA)
    clog << "<<< Value_Tree_Model::data: " << qPrintable(datum.toString()) << endl;
#endif
    return datum;
}

QModelIndex Value_Tree_Model::parent(const QModelIndex &index) const
{
#if (DEBUG_SECTION & DEBUG_MODEL)
    clog << ">>> Value_Tree_Model::parent: " << index << endl << "      is valid = " << index.isValid() << endl;
#endif
...
int Value_Tree_Model::rowCount(const QModelIndex &parent) const
{
    if (!parent.isValid())
        return 1;

    Value *referenced_value = static_cast<Value *>(parent.internalPointer());
...
/*==============================================================================
    Accessors
*/
QString Value_Tree_Model::units(const QModelIndex &index) const
{
    QString units;
    Value *value = static_cast<Value *>(index.internalPointer());
    if (value)
...
/*==============================================================================
    Helpers
*/
int Value_Tree_Model::value_row(Value *value) const
{
    int row = -1; //	Not found.
    if (value)
    {
