...
using idaeim::PVL::Array, idaeim::PVL::Aggregate;
...
class Image_Info_Panel : public QFrame
{
 ...
 public:
    ...
    */
    ...
    /*==============================================================================
        Constructors
    */
    explicit Image_Info_Panel(QWidget* parent = nullptr);
    ...
    */
    ...
    Q_SLOT void image_values(bool enabled);

    Q_SLOT void cursor_location(const QPoint& display_location, const QPoint& image_location);

    Q_SLOT void pixel_value(const Plastic_Image::Triplet& display_value,
                            const Plastic_Image::Triplet& image_value);

    Q_SLOT void image_scale(const QSizeF& scaling, int band = -1);

    Q_SLOT void projection(Projection* projection);

    Q_SLOT void set_metadata(idaeim::PVL::Aggregate* metadata);

    Q_SLOT void longitude_units(int units);
    Q_SLOT void latitude_units(int units);

    Q_SLOT void longitude_direction(int direction);

    Q_SLOT void location(const Coordinate& coordinate);

    Q_SLOT void script_changed(const QString& script);

    Q_SLOT void show_script_changed(bool show_script);
 ...
 private:
    ...
    /*==============================================================================
        Data
    */
    bool Image_Values, Use_Avg_Rgb;
    int Image_Bands;
    QPoint Display_Location, Image_Location;
    Plastic_Image::Triplet Display_Value, Image_Value;
    QSizeF Image_Scale;

    QString Display_Data, Image_Data;
    QLabel *Data_Source, *Location_X, *Annotation_X, *Location_Y, *Annotation_Y, *Pixel_Value[3],
        *Pixel_Label[3], *Scaling, *Script_Output, *Script_Value;

    QSize Data_Source_Size;
    int Latitude_Units, Longitude_Units, Longitude_Direction;
    double Longitude_Location, Latitude_Location;
    Projection* Projector;
    QWidget* World_Location;

    QWidget* Script_Panel;
    QLabel *Projection_Name, *Longitude, *Latitude;
    ...
    // QScriptValue
    QJSValue Global_Object;

    bool Evaluate_X, Evaluate_Y, Evaluate_R, Evaluate_G, Evaluate_B, Evaluate_Scale;
