...
*/
class Data_Mapper_Tool : public QDockWidget
{
  ...
  public:
    ...
    */
    ...
    static const QSize GRAPH_MIN_SIZE;
  ...
  private:
    /**	Set the saturation upper bound value.
        ...
        @return	true if the specified value is different than the previous
            value.
    */
    bool upper_bound_value(int value, int band, bool force_change = false);
    ...
    */
    bool lower_bound_value(int value, int band, bool force_change = false);
  ...
  public:
    /**	Set the saturation bounds percent values but do not apply them.
        ...
        @param	lower	A QVector of three doubles in display band order
            ...
            greater than 100.
    */
    void bound_percents(const QVector<double> &upper, const QVector<double> &lower);
    ...
    */
    ...
    {
        return (band >= 0 && band < 3) ? Upper_Default_Contrast_Stretch[band] : -1.0;
    }
    ...
    */
    ...
    {
        return (band >= 0 && band < 3) ? Lower_Default_Contrast_Stretch[band] : -1.0;
    }
  ...
  private:
    ...
    */
  public:
    inline static void linear_interpolation(int x0, int y0, int x1, int y1, Data_Map *data_map)
    {
        Function_Nodes::linear_interpolation(x0, y0, x1, y1, data_map);
    }
    inline static void linear_interpolation(const QPoint &start, const QPoint &end, Data_Map *data_map)
    {
        Function_Nodes::linear_interpolation(start, end, data_map);
    }
    void linear_interpolation(int x0, int y0, int x1, int y1, int band = -1);
    void linear_interpolation(const QPoint &start, const QPoint &end, int band = -1);
    void linear_interpolation(int band_selections, const QPoint &start, const QPoint &end);
    ...
    */
    void remap_data(int bounds_selection = REMAP_VALUES, bool unconditional = false);
  ...
  private:
    ...
    Icon_Button *Band_Selection_Buttons[3];
    QAction *Band_Selection_Actions[4]; //	The extra entry selects all bands.
    QIcon *Band_Selection_Icons[4], *Not_Selected_Icon;
    QSize Band_Selection_Icon_Size;
    QLabel *Band_Mapping_Labels[3];
    unsigned int Band_Map[3];

    //		Selection distance.
    int Selection_Distance;
    static int Default_Selection_Distance;

    //		Saturation Bounds.
    Icon_Button *Bounds_Button;
    QIcon *Bounds_Icon;
    QwtSlider *Upper_Bound_Percent_Slider, *Lower_Bound_Percent_Slider;
    QwtSlider *Upper_Bound_Value_Slider, *Lower_Bound_Value_Slider;
    QSpinBox *Upper_Bound_Value[3], *Lower_Bound_Value[3];
    QDoubleSpinBox *Upper_Bound_Percent_Setting[3], *Lower_Bound_Percent_Setting[3];
    QLabel *Upper_Bound_Percent_Actual[3], *Lower_Bound_Percent_Actual[3];
    QVector<double> Upper_Default_Contrast_Stretch, Lower_Default_Contrast_Stretch;
    QAction *Default_Contrast_Stretch_Action, *Restore_Original_Contrast_Stretch_Action, *Apply_Percents_Action,
        *Actual_to_Settings_Action;
    QwtPlotMarker *Bound_Marker;
    bool Signal_Changes;
    ...
    */
    ...
    QwtPlotCurve *Function_Plots[3], *Node_Plots[3];
    ...
    Function_Nodes *Nodes[3];

    QwtSymbol *Selection_Symbol;
    QwtPlotMarker *Selection_Marker;

    //	Bands selected during node dragging.
    int Node_Bands_Selected;
    ...
    */
    ...
    /*	The indices of the nodes immediately before and after This_Node.
        ...
        indices.
    */
    int Last_Node_Index[3], Next_Node_Index[3];

    Graph_Tracker *Tracker;
    /*
    QwtPlotPanner
        *Panner;
    QwtPlotZoomer
    ...
    */
    ...
    //	Mapping presets.
    QAction *Limited_Linear_Action, *Bounded_Linear_Action, *Unbounded_Linear_Action;

    //	Load/Save files.
    QPushButton *File_Menu_Button;
    QFileDialog *Load_File_Dialog, *Save_File_Dialog;
    QAction *Load_CSV_File_Action, *Load_Last_CSV_File_Action, *Save_File_Action, *Save_Last_CSV_File_Action;
    QString Graph_Directory;
    QString Graph_Data_Format;
    static QString Default_Graph_Data_Format;
    QString Last_CSV_Pathname;
