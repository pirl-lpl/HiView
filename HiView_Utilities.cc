                                        ...
                                        const QUrl& model_URL)
{
...
#if ((DEBUG_SECTION) & DEBUG_METADATA)
    LOCKED_LOGGING((clog << "    source URL: " << URL.toString() << endl));
#endif
    ...
    {
        QString name(model_URL.scheme());
        if (name.isEmpty() || name.compare("file", Qt::CaseInsensitive) == 0)
        {
            //	Model file pathname.
...
QStringList HiView_Utilities::image_reader_formats()
{
