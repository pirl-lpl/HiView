...
/**	An <i>Image_Viewer</i> provides a QWidget (in a QFrame) for the
 ...
 Dynamic_Image abstract class. The Image_Viewer is optimized for
 ...
 Tiled_Image_Display dynamically manages a grid of Dynamic_Image tile
 ...
 Dynamic_Image implementation (a Dynamic_Image is a QImage
 ...
 implementation of a Dynamic_Image is constructed; otherwise a plain
 Dynamic_QImage implementation is constructed. A JP2_Image takes full
 advantage of the selective rendering capabilities of the JPEG2000
 codestream. A Dynamic_QImage will always be displayed as a single
...
public:
    ...
    /*------------------------------------------------------------------------------
     Pixel values
     */
    inline Dynamic_Image::Pixel_Datum image_pixel_datum
    (unsigned int x, unsigned int y, unsigned int band) const
    {
        return Image_Display->image_pixel_datum(x, y, band);
    }
    inline Dynamic_Image::Triplet image_pixel(const QPoint& coordinate) const
    {
    ...
    inline Dynamic_Image::Triplet display_pixel(const QPoint coordinate) const
    {
...
signals:
    ...
    /**	Signals the image pixel values at the cursor position.
     ...
     @param	display_pixel	A Dynamic_Image::Triplet containing the
     {@link display_pixel(const QPoint&) display image pixel value} at
     the cursor position. This will be the {@link
     Dynamic_Image::UNDEFINED_PIXEL_VALUE} if the cursor is not
     located within the displayed image region.
     @param	image_pixel	A Dynamic_Image::Triplet containing the {@link
    ...
    (const Dynamic_Image::Triplet& display_pixel,
     const Dynamic_Image::Triplet& image_pixel);
