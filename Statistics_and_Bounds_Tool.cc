...
#endif
int Statistics_and_Bounds_Tool::Default_Selection_Distance = DEFAULT_SELECTION_DISTANCE;
...
/*==============================================================================
    Constructors
*/
Statistics_and_Bounds_Tool::Statistics_and_Bounds_Tool(const QString &title, QWidget *parent, Qt::WindowFlags flags)
    : Statistics_Tool(title, parent, flags), Selected_Bounds(SELECTED_NONE), Bounds_Moving(false),
      Selected_Bands(SELECTED_ALL), Selection_Distance(Default_Selection_Distance)
{
    if (title.isEmpty())
    ...
    Upper_Bound_Value[0] = Upper_Bound_Value[1] = Upper_Bound_Value[2] = max_x();
    Lower_Bound_Value[0] = Lower_Bound_Value[1] = Lower_Bound_Value[2] = 0;
    Upper_Bound_Percent[0] = Upper_Bound_Percent[1] = Upper_Bound_Percent[2] = Lower_Bound_Percent[0] =
        Lower_Bound_Percent[1] = Lower_Bound_Percent[2] = 0;
    ...
    */
    ...
    while (band--)
    {
        ...
        Upper_Bound_Marker[band]->setSymbol(new QwtSymbol(QwtSymbol::Rect, QBrush(DISPLAY_BAND_COLORS[band]),
                                                          QPen(Qt::white), QSize(2, (3 - band) * 20)));
        ...
        Lower_Bound_Marker[band]->setSymbol(new QwtSymbol(QwtSymbol::Rect, QBrush(DISPLAY_BAND_COLORS[band]),
                                                          QPen(Qt::black), QSize(2, (3 - band) * 20)));
...
/*==============================================================================
    Accessors
*/
void Statistics_and_Bounds_Tool::bands_selected(int band_selections)
{
    Selected_Bands = band_selections;
}

void Statistics_and_Bounds_Tool::selection_distance(int distance)
{
    if (distance < 0)
        distance = 0;
...
bool Statistics_and_Bounds_Tool::refresh(const QRect &selected_region)
{
    ...
    {
        percent = Statistics.upper_percent_at_value(Upper_Bound_Value[band],
                                                    //	N.B.: Display band mapped to Statistics set index.
                                                    Display_to_Stats_Map[band]);
#if ((DEBUG_SECTION) & (DEBUG_REFRESH | DEBUG_BOUNDS))
        clog << "    " << band << ": " << Upper_Bound_Percent[band] << " -> " << percent << endl;
#endif
...
//	>>> SIGNAL <<<
    ...
    for (int band = 0; band < 3; ++band)
    {
        percent = Statistics.lower_percent_at_value(Lower_Bound_Value[band],
                                                    //	N.B.: Display band mapped to Statistics set index.
                                                    Display_to_Stats_Map[band]);
#if ((DEBUG_SECTION) & (DEBUG_REFRESH | DEBUG_BOUNDS))
        clog << "    " << band << ": " << Lower_Bound_Percent[band] << " -> " << percent << endl;
#endif
...
*/
void Statistics_and_Bounds_Tool::upper_bound_percents(const QVector<double> &percents)
{
#if ((DEBUG_SECTION) & DEBUG_BOUNDS)
    clog << ">>> Statistics_and_Bounds_Tool::upper_bound_percents: " << percents[0] << ", " << percents[1] << ", "
         << percents[2] << endl;
#endif
    bool changed = false;
    int value;
    QVector<int> changed_values(3, -1);
    QVector<double> changed_percents(3, -1.0);
    for (int band = 0; band < 3; ++band)
    {
        ...
        if (upper_bound_value(value, band))
        {
            ...
            clog << "    changed_percents[" << band << "] = " << changed_percents[band] << endl;
#endif
        }
    }
...
#if ((DEBUG_SECTION) & DEBUG_BOUNDS)
    clog << "<<< Statistics_and_Bounds_Tool::upper_bound_percent" << endl;
#endif
}

void Statistics_and_Bounds_Tool::lower_bound_percents(const QVector<double> &percents)
{
#if ((DEBUG_SECTION) & DEBUG_BOUNDS)
    clog << ">>> Statistics_and_Bounds_Tool::lower_bound_percents: " << percents[0] << ", " << percents[1] << ", "
         << percents[2] << endl;
#endif
    bool changed = false;
    int value;
...
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_BOUNDS))
            clog << "    percent[" << band << "] = " << changed_percents[band] << endl;
#endif
        }
    }
...
#if ((DEBUG_SECTION) & DEBUG_BOUNDS)
    clog << "<<< Statistics_and_Bounds_Tool::lower_bound_percent" << endl;
#endif
}

void Statistics_and_Bounds_Tool::upper_bound_values(const QVector<int> &values)
{
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_BOUNDS))
    clog << ">>> Statistics_and_Bounds_Tool::upper_bound_values: " << values[0] << ", " << values[1] << ", "
         << values[2] << endl;
#endif
    bool changed = false;
    QVector<int> changed_values(3, -1);
...
bool Statistics_and_Bounds_Tool::upper_bound_value(int value, int band)
{
#if ((DEBUG_SECTION) & DEBUG_BOUNDS)
    clog << ">>> Statistics_and_Bounds_Tool::upper_bound_value: value " << value << ", band " << band << endl;
#endif
    bool changed = false;
    if (value >= 0)
    {
        if (value < 1)
            ...
            value = max_x();
#if ((DEBUG_SECTION) & DEBUG_BOUNDS)
        clog << "    previous Upper_Bound_Value[" << band << "] = " << Upper_Bound_Value[band] << endl;
#endif
...
//	If the value changed then the percent is different.
#if ((DEBUG_SECTION) & DEBUG_BOUNDS)
            clog << "    previous Upper_Bound_Percent[" << band << "] = " << Upper_Bound_Percent[band] << endl;
#endif
            Upper_Bound_Percent[band] =
                Statistics.upper_percent_at_value(value,
                                                  ...
                                                  Display_to_Stats_Map[band]);
#if ((DEBUG_SECTION) & DEBUG_BOUNDS)
            clog << "         new Upper_Bound_Percent[" << band << "] = " << Upper_Bound_Percent[band] << endl;
#endif
        }
    }
#if ((DEBUG_SECTION) & DEBUG_BOUNDS)
    clog << "<<< Statistics_and_Bounds_Tool::upper_bound_value: " << boolalpha << changed << endl;
#endif
    return changed;
}

void Statistics_and_Bounds_Tool::lower_bound_values(const QVector<int> &values)
{
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_BOUNDS))
    clog << ">>> Statistics_and_Bounds_Tool::lower_bound_values: " << values[0] << ", " << values[1] << ", "
         << values[2] << endl;
#endif
    bool changed = false;
    QVector<int> changed_values(3, -1);
...
bool Statistics_and_Bounds_Tool::lower_bound_value(int value, int band)
{
#if ((DEBUG_SECTION) & DEBUG_BOUNDS)
    clog << ">>> Statistics_and_Bounds_Tool::lower_bound_value: value " << value << ", band " << band << endl;
#endif
    bool changed = false;
    if (value >= 0)
    {
        if (value >= max_x())
            value = max_x() - 1;
#if ((DEBUG_SECTION) & DEBUG_BOUNDS)
        clog << "    Lower_Bound_Value = " << Lower_Bound_Value[band] << endl;
#endif
        if (Lower_Bound_Value[band] != value)
        {
            ...
            Lower_Bound_Percent[band] =
                Statistics.lower_percent_at_value(value,
                                                  ...
                                                  Display_to_Stats_Map[band]);
#if ((DEBUG_SECTION) & DEBUG_BOUNDS)
            clog << "    Lower_Bound_Percent = " << Lower_Bound_Percent[band] << endl;
#endif
        }
    }
#if ((DEBUG_SECTION) & DEBUG_BOUNDS)
    clog << "<<< Statistics_and_Bounds_Tool::lower_bound_value: " << boolalpha << changed << endl;
#endif
    return changed;
}

void Statistics_and_Bounds_Tool::upper_limit(int limit)
{
...
void Statistics_and_Bounds_Tool::lower_limit(int limit)
{
...
/*------------------------------------------------------------------------------
    Graph Tracker
*/
void Statistics_and_Bounds_Tool::graph_position(const QPoint &position)
{
    ...
    if (!Bounds_Moving)
    {
        ...
        if (Selected_Bounds)
        {
            ...
            if (Selected_Bounds & SELECTED_LOWER_BOUND)
                //	Lower bound.
                cursor = Greater_Than_Cursors[Selected_Bounds & SELECTED_ALL];
            else
                //	Upper bound.
                cursor = Less_Than_Cursors[Selected_Bounds & SELECTED_ALL];
        ...
        else if (Bound_Marker->isVisible())
        {
...
void Statistics_and_Bounds_Tool::mouse_down(const QPointF &
...
void Statistics_and_Bounds_Tool::mouse_drag(const QPointF &point)
{
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_DRAGGING))
    clog << ">>> Statistics_and_Bounds_Tool::mouse_drag: " << point << endl;
#endif
    if (Selected_Bounds && Bounds_Moving)
    {
        int band;
        QPoint position(static_cast<int>(point.x() + 0.5), static_cast<int>(point.y() + 0.5));
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_DRAGGING))
        clog << "    Last_Position = " << Last_Position << endl << "         position = " << position << endl;
        QPoint there(position);
#endif

        //	Constrain the point within the graph data range.
        if (position.ry() < 0)
            position.ry() = 0;
        else if (position.ry() > max_y())
            position.ry() = max_y();
        if (position.rx() < 0)
            position.rx() = 0;
        else if (position.rx() > max_x())
            position.rx() = max_x();
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_DRAGGING))
        if (position != there)
        ...
        {
            QVector<int> values(3, (int)position.rx());
            band = -1;
            while (++band < 3)
...
//	Move the bound.
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_DRAGGING))
            clog << "    move selection bounds to " << position.rx() << endl;
#endif
            if (Selected_Bounds & SELECTED_LOWER_BOUND)
...
void Statistics_and_Bounds_Tool::mouse_up(const QPointF &)
{
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_DRAGGING))
...
void Statistics_and_Bounds_Tool::leave_graph()
{
    if (Bound_Marker->isVisible())
        Bound_Marker->hide();
}

int Statistics_and_Bounds_Tool::selected_bounds(const QPoint &position) const
{
    int selection = SELECTED_NONE, band = 3, active_bands = selected_bands(), active_band_mask,
        min_distance = selection_distance(), x_distance, x_position = position.x();
...
int Statistics_and_Bounds_Tool::saturation_bound(int selection)
{
    int band = 3;
    while (band--)
        if (selection & (1 << band))
            return (selection & SELECTED_LOWER_BOUND) ? Lower_Bound_Value[band] : Upper_Bound_Value[band];
    return -1;
}
