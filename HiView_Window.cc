...
#define Shift_Region_Cursor HiView_Config::Shift_Region_Cursor
#define Shift_Region_Horizontal_Cursor HiView_Config::Shift_Region_Horizontal_Cursor
#define Shift_Region_Vertical_Cursor HiView_Config::Shift_Region_Vertical_Cursor
#define Shift_Region_FDiag_Cursor HiView_Config::Shift_Region_FDiag_Cursor
#define Shift_Region_BDiag_Cursor HiView_Config::Shift_Region_BDiag_Cursor
...
//	Image viewer pane.
    ...
    Region_Overlay = new QRubberBand(QRubberBand::Rectangle, Image_View->image_display());
...
*/
...
#if ((DEBUG_SECTION) & (DEBUG_CONSTRUCTORS | DEBUG_INITIALIZE))
    clog << "    Startup_Stage set to STARTUP_SPLASH (" << STARTUP_SPLASH << ')' << endl;
#endif
    ...
    {
#if ((DEBUG_SECTION) & (DEBUG_CONSTRUCTORS | DEBUG_INITIALIZE))
        clog << "    initial load failed" << endl << "    show ..." << endl;
#endif
        show();
...
#if ((DEBUG_SECTION) & (DEBUG_CONSTRUCTORS | DEBUG_INITIALIZE))
    LOCKED_LOGGING((clog << "<<< HiView_Window" << endl));
#endif
}
...
//	N.B. The menu actions must be created before they are configured.
    ...
    */
    ...
    Source_Directory = settings.value("Source_Directory", QString()).toString();
    QFileInfo file_info(Source_Directory);
    if (Source_Directory.isEmpty() || !file_info.exists() || !file_info.isDir())
        Source_Directory = QDir::currentPath();

    QString pathname(settings.value("Save_Image_Directory", QString()).toString());
...
void HiView_Window::save_configuration()
{
#if ((DEBUG_SECTION) & DEBUG_INITIALIZE)
...
#ifdef __APPLE__
    settings.setValue("View_SpeechRecog", View_SpeechRecog_Action->isChecked());
#endif
    settings.setValue("View_Image_Info", View_Image_Info_Action->isChecked());
    settings.setValue("View_Image_Metadata", View_Image_Metadata_Action->isChecked());
    settings.setValue("View_Status_Bar", View_Status_Bar_Action->isChecked());
    /*
    ...
    */
    ...
    {
        QString pathname(Image_Save_Dialog->pathname());
        if (!pathname.isEmpty())
        {
...
#if ((DEBUG_SECTION) & DEBUG_INITIALIZE)
        clog << endl << "    Saturation_Lower_Bound_Percents =";
#endif
        for (int band = 0; band < 3; ++band)
        {
#if ((DEBUG_SECTION) & DEBUG_INITIALIZE)
...
#if ((DEBUG_SECTION) & DEBUG_INITIALIZE)
        clog << endl << "    Last_CSV_File = \"" << Data_Mapper->last_CSV_file() << '"' << endl;
#endif
        settings.setValue("Last_CSV_File", Data_Mapper->last_CSV_file());
    }
...
void HiView_Window::save_layout()
{
...
void HiView_Window::restore_layout()
{
    ...
    QString application_geometry(qApp->applicationName() + "_Geometry"),
        application_state(qApp->applicationName() + "_State");
    if (Restore_Layout && Image_Metadata_Dialog && Navigator && Statistics && Data_Mapper &&
        settings.contains(application_geometry) && settings.contains(application_state))
    {
#if ((DEBUG_SECTION) & (DEBUG_LAYOUT | DEBUG_INITIALIZE | DEBUG_MENUS | DEBUG_LOAD_IMAGE))
        clog << "    restoring geometry and dock states" << endl;
#endif
        restoreGeometry(settings.value(windowTitle() + "_Geometry").toByteArray());
        restoreState(settings.value(windowTitle() + "_State").toByteArray());
    }
    else
    {
//	Don't restore the image info, metadata, and status bar panels.
#if ((DEBUG_SECTION) & (DEBUG_LAYOUT | DEBUG_INITIALIZE | DEBUG_MENUS | DEBUG_LOAD_IMAGE))
        clog << "    not restoring image info, metadata, and status bar panels" << endl;
#endif
...
/*==============================================================================
    Menus
*/
void HiView_Window::create_menus()
{
    ...
    connect(action, SIGNAL(triggered()), SLOT(showMinimized()));
    addAction(action);
    ...
    connect(Open_URL_Action, SIGNAL(triggered()), SLOT(open_URL()));
    ...
    */
    ...
    connect(Preferences_Action, SIGNAL(triggered()), Preferences, SLOT(show()));
...
//	N.B.: QKeySequence::Quit is not present in Qt v4.5.x
    ...
    connect(View_Image_Info_Action,
            ...
            */
            SIGNAL(triggered(bool)), SLOT(view_image_info(bool)));
    ...
    connect(View_Image_Metadata_Action, SIGNAL(triggered(bool)), SLOT(view_image_metadata(bool)));
    ...
    connect(Fit_to_Image_Action, SIGNAL(triggered()), SLOT(fit_window_to_image()));
    ...
    connect(View_SpeechRecog_Action, SIGNAL(triggered(bool)), SLOT(recognizer_toggled(bool)));
    ...
    */
    ...
    connect(Floating_Position, SIGNAL(triggered()), SLOT(tool_position()));
    ...
    connect(Left_Position, SIGNAL(triggered()), SLOT(tool_position()));
...
void HiView_Window::view_image_info(bool enabled)
{
...
void HiView_Window::view_image_metadata(bool enabled)
{
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_MENUS))
    ...
    else if (View_Image_Metadata_Action->isEnabled() && Image_Metadata_Dialog &&
...
void HiView_Window::fit_window_to_image()
{
    ...
    QSize scaled_size(Image_View->scaled_image_size());
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_MENUS | DEBUG_LAYOUT))
...
void HiView_Window::update_window_fit_action()
{
...
/*==============================================================================
    Source Selections
*/
void HiView_Window::create_source_selections()
{
...
void HiView_Window::display_image_name(const QString &source_name)
{
    ...
    QString name(source_name);
    if (!name.isEmpty() && !HiView_Utilities::is_URL(name))
...
#endif
    QStringList source_list(Preferences->source_list());
...
void HiView_Window::source_selections(const QStringList &source_list)
{
    ...
    for (int index = 0; index < source_list.count(); ++index)
        clog << "    " << index << ": " << source_list.at(index) << endl;
#endif
...
//	Not a signal from Preferences; update the list.
    ...
    else if (!Source_Name.isEmpty() && (source_list.isEmpty() || source_list.at(0) != Source_Name))
    {
...
*/
    ...
    else if (*Source_Selections != source_list)
    {
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_SOURCE_SELECTIONS))
...
*/
    ...
    */
        ...
        if (!source_is_URL)
        {
            if (name.at(0) == '~' && name.at(1) == QDir::separator())
...
bool HiView_Window::JPIP_source(QString &source_name) const
{
    ...
    if (HiView_Utilities::is_URL(source_name))
    {
        ...
        {
            is_JPIP = true;
            if (URL.port() < 0 && Preferences->JPIP_server_port() > 0)
        ...
        else
        {
          ...
          */
            if (!path.isEmpty())
            {
                ...
                QString hostname /*(Preferences->HTTP_to_JPIP_hostname ());
                    if (hostname.isEmpty ())
                        hostname*/
                    = URL.host();
...
void HiView_Window::make_way_for(QDockWidget *tool)
{
    ...
    if (!tool->isWindow())
    {
        ...
        Qt::DockWidgetArea dock_area = dockWidgetArea(tool);
...
*/
        ...
        int
            ...
            */
            ...
            current_height = height(), max_height = frameSize().height();
        if (max_height > screen_height || max_height < current_height)
        {
        ...
        clog << "                   tool isWindow = " << tool->isWindow() << endl
             ...
             << "                     tool height = " << tool->height() << endl
             << "              tool minimumHeight = " << tool->minimumHeight() << endl
            ...
            !Statistics->isWindow() && vertically_overlapping_docks(dock_area, dockWidgetArea(Statistics)))
        {
            ...
            clog << "    hide Statistics" << endl
                 << "               Statistics height = " << Statistics->height() << endl
                 << "        Statistics minimumHeight = " << Statistics->minimumHeight() << endl
                 << "                  current_height = " << current_height << endl;
#endif
            ...
            !Data_Mapper->isWindow() && vertically_overlapping_docks(dock_area, dockWidgetArea(Data_Mapper)))
        {
            ...
            clog << "    hide Data_Mapper" << endl
                 << "              Data_Mapper height = " << Data_Mapper->height() << endl
                 << "       Data_Mapper minimumHeight = " << Data_Mapper->minimumHeight() << endl
                 << "                  current_height = " << current_height << endl;
#endif
            ...
            !Navigator->isWindow() && vertically_overlapping_docks(dock_area, dockWidgetArea(Navigator)))
        {
            ...
            clog << "    hide Navigator" << endl
                 << "                Navigator height = " << Navigator->height() << endl
                 << "         Navigator minimumHeight = " << Navigator->minimumHeight() << endl;
#endif
...
#endif
    if (tool->isVisible() && (tool != Navigator || Navigator_Fit))
    {
        Qt::DockWidgetArea dock_area = dockWidgetArea(tool);
...
//	Apply contraints.
            ...
            */
            ...
            if (Navigator->isVisible() && !Navigator->isWindow() && dockWidgetArea(Navigator) == dock_area)
            {
                ...
                {
#if ((DEBUG_SECTION) & (DEBUG_LAYOUT | DEBUG_SLOTS))
                    clog << "    Navigator previous_size = " << Navigator->previous_size() << endl;
#endif
                    max_size.rwidth() = qMax(max_size.rwidth(), Navigator->previous_size().width());
                }
                else
                {
#if ((DEBUG_SECTION) & (DEBUG_LAYOUT | DEBUG_SLOTS))
                    clog << "             Navigator size = " << Navigator->previous_size() << endl;
#endif
                    max_size.rwidth() = qMax(max_size.rwidth(), Navigator->width());
                }
            }
            if (tool == Data_Mapper && Data_Mapper->isVisible() && !Data_Mapper->isWindow() &&
                dockWidgetArea(Data_Mapper) == dock_area)
            {
#if ((DEBUG_SECTION) & (DEBUG_LAYOUT | DEBUG_SLOTS))
                clog << "    Data_Mapper previous_size = " << Data_Mapper->previous_size() << endl;
#endif
                max_size.rwidth() = qMax(max_size.rwidth(), Data_Mapper->previous_size().width());
            }
            if (tool == Statistics && Statistics->isVisible() && !Statistics->isWindow() &&
                dockWidgetArea(Statistics) == dock_area)
            {
#if ((DEBUG_SECTION) & (DEBUG_LAYOUT | DEBUG_SLOTS))
                clog << "    Statistics previous_size = " << Statistics->previous_size() << endl;
#endif
                max_size.rwidth() = qMax(max_size.rwidth(), Statistics->previous_size().width());
            }
#if ((DEBUG_SECTION) & (DEBUG_LAYOUT | DEBUG_SLOTS))
...
void HiView_Window::tool_position()
{
    ...
    if (Selected_Tool)
    {
        ...
        else if (source == Floating_Position)
        {
#if ((DEBUG_SECTION) & DEBUG_TOOLS_POSITION)
            clog << "    Floating_Position - setFloating " << (!is_floating) << endl;
#endif
            Selected_Tool->setFloating(!is_floating);
        }
        else
        {
            Qt::DockWidgetArea selected_area = Qt::NoDockWidgetArea;
            ...
            else if (source == Bottom_Position)
            {
#if ((DEBUG_SECTION) & DEBUG_TOOLS_POSITION)
...
/*------------------------------------------------------------------------------
    Navigator
*/
void HiView_Window::create_navigator()
{
...
void HiView_Window::view_navigator(bool enabled)
{
...
void HiView_Window::navigator_visibility_changed(bool visible)
{
...
/*------------------------------------------------------------------------------
    Statistics
*/
void HiView_Window::create_statistics_panels()
{
...
void HiView_Window::view_statistics(bool enabled)
{
...
void HiView_Window::statistics_visibility_changed(bool visible)
{
...
void HiView_Window::statistics_section_changed(int section_index)
{
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_STATISTICS))
...
bool HiView_Window::refresh_source_statistics()
{
#if ((DEBUG_SECTION) & DEBUG_STATISTICS)
...
#if ((DEBUG_SECTION) & DEBUG_STATISTICS)
        clog << "    displayed_image_region = " << Image_View->displayed_image_region() << endl
...
void HiView_Window::refresh_display_statistics()
{
#if ((DEBUG_SECTION) & DEBUG_STATISTICS)
    clog << ">>> HiView_Window::refresh_display_statistics" << endl;
#endif
    if (Statistics && Statistics->display_statistics()->isVisible())
    {
        QRect selected_region(selected_display_region());
#if ((DEBUG_SECTION) & DEBUG_STATISTICS)
        clog << "       image_display_region = " << Image_View->image_display_region() << endl
...
QRect HiView_Window::selected_source_region() const
{
    if (Selected_Image_Region.isEmpty())
    {
...
QRect HiView_Window::selected_display_region() const
{
    if (Selected_Image_Region.isEmpty())
    {
        if (Selection_Modification)
...
/*------------------------------------------------------------------------------
    Data_Mapper
*/
void HiView_Window::create_data_mapper()
{
    ...
    Qt::DockWidgetArea dock_area = Qt::LeftDockWidgetArea;
    if (Navigator && (Navigator->sizeHint().height() + Data_Mapper->sizeHint().height() + 20) >
    ...
    connect(Preferences, SIGNAL(contrast_stretch_lower_changed(double, int)), Data_Mapper,
    ...
    QVector<double> upper(3, -1.0), lower(3, -1.0);
    QList<QVariant> values;
    ...
    Data_Mapper->last_CSV_file(settings.value("Last_CSV_File", QString()).toString());
    ...
    if (Statistics)
    {
        ...
        Statistics->source_statistics()->stats().limits(lower_limit, upper_limit);
    }
...
void HiView_Window::view_data_mapper(bool enabled)
{
...
void HiView_Window::data_mapper_visibility_changed(bool visible)
{
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_DATA_MAPPER))
    clog << ">>> HiView_Window::data_mapper_visibility_changed: " << visible << endl;
#endif
...
/*------------------------------------------------------------------------------
    Toolbar
*/
void HiView_Window::create_toolbar()
{
    /*	HiView Toolbar.
...
/*==============================================================================
    Image Viewer
*/
void HiView_Window::create_image_viewer()
{
    ...
    */
    ...
    for (int index = 0; index < scale_actions.size(); index++)
    ...
    connect(Preferences, SIGNAL(min_scale_changed(double)), Image_View, SLOT(min_scale(double)));
    Image_View->max_scale(Preferences->max_scale());
    connect(Preferences, SIGNAL(max_scale_changed(double)), Image_View, SLOT(max_scale(double)));
    Image_View->scaling_minor_increment(Preferences->scaling_minor_increment());
    connect(Preferences, SIGNAL(scaling_minor_increment_changed(double)), Image_View,
            SLOT(scaling_minor_increment(double)));
    Image_View->scaling_major_increment(Preferences->scaling_major_increment());
    connect(Preferences, SIGNAL(scaling_major_increment_changed(double)), Image_View,
            SLOT(scaling_major_increment(double)));
    Image_View->tile_size(Preferences->tile_size());
    connect(Preferences, SIGNAL(tile_size_changed(int)), Image_View, SLOT(tile_size(int)));
    Image_View->rendering_increment_lines(Preferences->rendering_increment_lines());
...
/*------------------------------------------------------------------------------
    Image loading
*/
bool HiView_Window::load_initial_source()
{
#if ((DEBUG_SECTION) & (DEBUG_INITIALIZE | DEBUG_LOAD_IMAGE))
    clog << ">>> HiView_Window::load_initial_source" << endl
         << "    Preferences restore_last_source = " << Preferences->restore_last_source() << endl
         << "    Initial_Source = \"" << Initial_Source << '"' << endl
         << "    Startup_Stage set to STARTUP_COMPLETE (" << STARTUP_COMPLETE << ')' << endl;
#endif
    ...
    {
        if (URL_source(Initial_Source) && !JPIP_source(Initial_Source) &&
            /*
            ...
            */
            (Initial_Source.startsWith("http://", Qt::CaseInsensitive) ||
             Initial_Source.startsWith("https://", Qt::CaseInsensitive)))
        {
//	HTTP URL.
#if ((DEBUG_SECTION) & (DEBUG_INITIALIZE | DEBUG_LOAD_IMAGE | DEBUG_OVERVIEW))
            clog << "    load_URL Initial_Source = \"" << Initial_Source << '"' << endl;
#endif
            /*
...
#endif
            QSizeF scaling(Initial_Scaling);
            //	Reset Initial_Scaling.
            Initial_Scaling.rwidth() = Initial_Scaling.rheight() = -1.0;
...
void HiView_Window::open(const QString &source_name)
{
    ...
    */
    QString name(source_name);
    if (URL_source(name) && !JPIP_source(name) &&
        /*
...
void HiView_Window::open_file()
{
    ...
    if (!Open_File_Dialog)
    {
        QString file_filters("All Files (*);; ");
    ...
    {
        QString source_name(Open_File_Dialog->selectedFiles().value(0));
        if (!source_name.isEmpty())
        {
...
//	N.B.: Massaging of the source_name is done by the open method.
    ...
    if (source_name.isEmpty())
    {
        ...
        clog << "    empty source name" << endl << "<<< HiView_Window::load_image: false" << endl;
#endif
        return false;
    }
...
#if ((DEBUG_SECTION) & DEBUG_LOAD_IMAGE)
        LOCKED_LOGGING((clog << "    viewport_size = " << displayed_size << endl));
#endif
    }
#if ((DEBUG_SECTION) & DEBUG_LOAD_IMAGE)
...
bool HiView_Window::load_image(const QImage &source_image, const QSizeF &scaling)
{
    ...
    if (image_scaling.isEmpty())
    {
        if (!Initial_Scaling.isEmpty() && Startup_Stage == STARTUP_COMPLETE)
        {
            image_scaling = Initial_Scaling;
#if ((DEBUG_SECTION) & DEBUG_LOAD_IMAGE)
            LOCKED_LOGGING((clog << "    using Initial_Scaling = " << Initial_Scaling << endl));
#endif
            //	Reset Initial_Scaling so it won't be used again.
            Initial_Scaling.rwidth() = Initial_Scaling.rheight() = -1.0;
        }
        else if (Preferences->initial_scale() != Preferences->INITIAL_SCALE_AUTO_FIT)
        {
//	Use the Preferences setting for the initial scaling.
#if ((DEBUG_SECTION) & DEBUG_LOAD_IMAGE)
            LOCKED_LOGGING((clog << "    using Preferences initial_scale = " << Preferences->initial_scale() << endl));
...
#if ((DEBUG_SECTION) & (DEBUG_LOAD_IMAGE | DEBUG_DROP_IMAGE | DEBUG_SLOTS))
    clog << "    initiating network data get" << endl;
#endif
    Network_Reply = Network_Access_Manager->get(QNetworkRequest(QUrl(source_name)));
#if ((DEBUG_SECTION) & (DEBUG_LOAD_IMAGE | DEBUG_DROP_IMAGE | DEBUG_SLOTS | DEBUG_OVERVIEW))
...
void HiView_Window::image_loaded(bool successful)
{
#if ((DEBUG_SECTION) & (DEBUG_LOAD_IMAGE | DEBUG_SLOTS | DEBUG_INITIALIZE | DEBUG_OVERVIEW))
    LOCKED_LOGGING((clog << ">>> HiView_Window::image_loaded: " << successful << endl
    ...
    if (successful)
    {
        ...
        */
        ...
        {
#if ((DEBUG_SECTION) & (DEBUG_LOAD_IMAGE | DEBUG_SLOTS))
            LOCKED_LOGGING((clog << "    Data_Mapper selected_bands SELECTED_ALL" << endl));
#endif
    ...
    if (Startup_Stage)
    {
        ...
        else
        {
            ...
            if (Image_Activity_Indicator)
            {
                ...
                Image_Activity_Indicator->state_color(ACTIVITY_VISIBLE_RENDERING, Qt::yellow);
                Image_Activity_Indicator->state(ACTIVITY_VISIBLE_RENDERING);
            }
#if ((DEBUG_SECTION) & (DEBUG_LOAD_IMAGE | DEBUG_SLOTS | DEBUG_INITIALIZE | DEBUG_OVERVIEW))
            LOCKED_LOGGING((clog << "    wait " << SPLASH_IMAGE_DWELL_MS << " ms ..." << endl));
#endif
            //	Delay before continue_startup.
            QTimer::singleShot(SPLASH_IMAGE_DWELL_MS, this, SLOT(continue_startup()));
...
void HiView_Window::continue_startup()
{
#if ((DEBUG_SECTION) & (DEBUG_LOAD_IMAGE | DEBUG_SLOTS | DEBUG_INITIALIZE))
    LOCKED_LOGGING((clog << ">>> HiView_Window::continue_startup" << endl));
#endif
    if (Image_Activity_Indicator)
    {
        Image_Activity_Indicator->state(ACTIVITY_OFF);
...
void HiView_Window::load_image_start()
{
...
void HiView_Window::image_load_actions(bool enabled)
{
#if ((DEBUG_SECTION) & DEBUG_LOAD_IMAGE)
...
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_RENDERING_STATUS))
    LOCKED_LOGGING((clog << "<<< HiView_Window::rendering_status" << endl));
#endif
}

void HiView_Window::image_viewer_state_change(int state)
{
#if ((DEBUG_SECTION) & (DEBUG_STATE | DEBUG_SLOTS))
    LOCKED_LOGGING((clog << ">>> HiView_Window::image_viewer_state_change: " << state << " - "
    ...
    if (!Image_Loading)
    {
        if (Image_Activity_Indicator)
        {
            ...
            else if (state & Image_Viewer::RENDERING_VISIBLE_TILES_STATE)
        ...
        if ((state & Image_Viewer::COMPLETED_WITHOUT_RENDERING_STATE) ||
            ...
            */
            state == (Image_Viewer::IMAGE_MOVE_STATE | Image_Viewer::RENDERING_BACKGROUND_STATE))
        {
            condition = state & Image_Viewer::STATE_TYPE_MASK;
            if (condition == Image_Viewer::DATA_MAPPING_STATE)
            {
#if ((DEBUG_SECTION) & (DEBUG_STATE | DEBUG_SLOTS))
                LOCKED_LOGGING((clog << "    refresh_display_statistics ..." << endl));
#endif
                refresh_display_statistics();
            }
...
#if ((DEBUG_SECTION) & (DEBUG_STATE | DEBUG_SLOTS))
    LOCKED_LOGGING((clog << "<<< HiView_Window::image_viewer_state_change" << endl));
#endif
}

void HiView_Window::activity_indicator_clicked(int)
{
#if ((DEBUG_SECTION) & DEBUG_LOAD_IMAGE)
...
/*==============================================================================
    Metadata
*/
void HiView_Window::reset_metadata()
{
    ...
    if (metadata && Preferences->get_PDS_metadata() && !Source_Name.isEmpty())
    {
#if ((DEBUG_SECTION) & (DEBUG_METADATA | DEBUG_LOAD_IMAGE))
...
*/
#if ((DEBUG_SECTION) & DEBUG_METADATA)
    LOCKED_LOGGING((clog << "<<< HiView_Window::PDS_metadata" << endl));
#endif
}
...
bool HiView_Window::save_image()
{
    ...
    if (!Image_Save_Dialog)
    {
        ...
        connect(Image_Save_Thread, SIGNAL(done(bool)), SLOT(save_image_done(bool)));
    }

    if (Image_Save_Dialog->reset() && Image_Save_Dialog->exec())
    {
        QString pathname(Image_Save_Dialog->pathname()), format(Image_Save_Dialog->image_format());
        QSize image_size(Image_Save_Dialog->image_size());
#if ((DEBUG_SECTION) & (DEBUG_SAVE_IMAGE | DEBUG_MENUS))
        clog << "      pathname = " << pathname << endl
             << "        format = " << format << endl
             << "    image_size = " << image_size << endl;
#endif
        if (!pathname.isEmpty() && !format.isEmpty() && !image_size.isEmpty())
        {
            QString message;
            try
            {
...
void HiView_Window::save_image_done(bool completed)
{
    ...
    if (completed)
        show_status_message(tr("Saved %1 image file: %2")
                                .arg(Image_Save_Thread->image_format())
                                .arg(Image_Save_Thread->image_pathname()));
    else
    {
        QString message(tr("Did not save the %1 image file -\n%2")
                            .arg(Image_Save_Thread->image_format())
                            .arg(wrapped_pathname(Image_Save_Thread->image_pathname(), 0, font())));
...
#endif
    if (!Image_Loading && (mime_data->hasImage() || mime_data->hasUrls() ||
...
#if ((DEBUG_SECTION) & DEBUG_MOUSE_EVENTS)
                    clog << "    Selected_Image_Region start = " << position << endl;
#endif
                }
...
#if ((DEBUG_SECTION) & DEBUG_MOUSE_EVENTS)
                    clog << "    move selection w/ offset = " << Selection_Start << endl;
#endif
                }
...
//	Drag started outside the image area.
                ...
                QRectF selected_image_region(Selected_Image_Region);
#if ((DEBUG_SECTION) & DEBUG_MOUSE_EVENTS)
                ...
                {
                    QSize image_size(Image_View->image_size());
                    ...
                    else if (Selected_Image_Region.right() >= image_size.rwidth())
                    {
#if ((DEBUG_SECTION) & DEBUG_MOUSE_EVENTS)
                    ...
                    else if (Selected_Image_Region.bottom() >= image_size.rheight())
                    {
#if ((DEBUG_SECTION) & DEBUG_MOUSE_EVENTS)
                ...
                else
                {
                    ...
                    {
                        display_position -= Image_View->map_image_to_display(Selection_Start);
...
//	Initialize the selection location.
                    ...
                    int selection = Selection_Modification;
                    ...
                    {
#if ((DEBUG_SECTION) & DEBUG_MOUSE_EVENTS)
                        clog << "    Selection_Modification changed from " << selection << " to "
                             << Selection_Modification << endl;
#endif
                        set_selection_cursor(Selection_Modification);
                    }
                }
...
#endif
    if (!Selection_Modification && !Selected_Image_Region.isEmpty() &&
...
void HiView_Window::mouseReleaseEvent(QMouseEvent *event)
{
    ...
    set_selection_cursor(selection_modification(round_down(Image_View->map_display_to_image(
...
void HiView_Window::reset_region_overlay()
{
#if ((DEBUG_SECTION) & DEBUG_MOUSE_EVENTS)
    ...
    if (Region_Overlay)
    {
        ...
        if (Location != NULL && Location->projection() != NULL && !Location->projection()->is_identity())
        {
            ...
            Selected_Area_Text = QString("%1 x %2 m").arg(projected_width).arg(projected_height);
...
void HiView_Window::reset_selected_region()
{
...
void HiView_Window::tool_context_menu_requested(QDockWidget *tool, QContextMenuEvent *event)
{
    ...
    if (Selected_Tool)
    {
        ...
        if (dock_area == Qt::RightDockWidgetArea || !(allowed_areas & Qt::RightDockWidgetArea))
            actions.at(TOOL_POSITION_RIGHT)->setEnabled(false);
        if (dock_area == Qt::TopDockWidgetArea || !(allowed_areas & Qt::TopDockWidgetArea))
            actions.at(TOOL_POSITION_TOP)->setEnabled(false);
        if (dock_area == Qt::BottomDockWidgetArea || !(allowed_areas & Qt::BottomDockWidgetArea))
