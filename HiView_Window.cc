...
HiView_Window::HiView_Window(
    ...
    Image_Saved(NULL),
    Evaluator(new Script_Evaluator(NULL))
{
#if defined(DEBUG_SECTION) && DEBUG_SECTION != 0
    clog << boolalpha;
#endif
...
#if ((DEBUG_SECTION) & (DEBUG_CONSTRUCTORS | DEBUG_INITIALIZE))
    clog << "    shared Error_Message @ " << reinterpret_cast<void*>(Error_Message) << endl;
#endif
    ...
    */
    ...
    Dynamic_Image::default_auto_update(false);
...
#if ((DEBUG_SECTION) & (DEBUG_CONSTRUCTORS | DEBUG_INITIALIZE | DEBUG_DROP_IMAGE))
    clog << "    Network_Access_Manager @ " << reinterpret_cast<void*>(Network_Access_Manager) << endl;
#endif
    ...
    */
    ...
    // Evaluator->setData(Statistics->source_statistics ()->stats ());
    ...
    */
    ...
    clog << "    load SPLASH_IMAGE \"" << SPLASH_IMAGE
        << "\" image @ " << reinterpret_cast<void*>(&default_image) << endl;
#endif
...
HiView_Window::configure()
{
    ...
    */
    ...
    View_Tooltips_Action->setChecked(settings.value("View_Tooltips",
                                     DEFAULT_VIEW_TOOLTIPS)
                                     .toBool());

#ifdef Q_OS_MACOS
    View_SpeechRecog_Action->setChecked(settings.value("View_SpeechRecog",
                                        DEFAULT_VIEW_SPEECH_RECOG)
                                        .toBool());
#endif

    View_Image_Info_Action->setChecked(settings.value("View_Image_Info",
                                       DEFAULT_VIEW_IMAGE_INFO)
                                       .toBool());

    View_Image_Metadata_Action->setChecked(settings.value("View_Image_Metadata",
                                           DEFAULT_VIEW_IMAGE_METADATA)
                                           .toBool());

    View_Status_Bar_Action->setChecked(settings.value("View_Status_Bar",
                                       DEFAULT_VIEW_STATUS_BAR)
                                       .toBool());

    Navigator_Tool::default_immediate_mode(settings.value("Navigator_Immediate_Mode",
...
HiView_Window::create_menus()
{
    ...
    */
    ...
    View_Status_Bar_Action->setShortcut(tr("Ctrl+S"));
...
void HiView_Window::toggle_distance_tool(bool enabled)
{
    ...
    else
    {
        ...
        else
        {
            ...
            Evaluator->setProperty(Script_Evaluator::VAR_LENGTH_PIXELS, 0);
            Evaluator->setProperty(Script_Evaluator::VAR_LENGTH_METERS, 0);
            Evaluator->run();
        }
    }
}
    ...
    bool enabled)
{
    ...
    else if (View_Image_Info_Action->isEnabled() &&
             Image_Info)
    ...
    bool enabled)
{
    ...
    else if (View_Image_Metadata_Action->isEnabled() &&
             Image_Metadata_Dialog &&
...
HiView_Window::create_navigator()
{
    ...
    // Image pixel value.
    connect(Image_View,
            SIGNAL(image_pixel_value(const Dynamic_Image::Triplet&,
            const Dynamic_Image::Triplet&)),
            Navigator,
            SLOT(image_pixel_value(const Dynamic_Image::Triplet&,
            const Dynamic_Image::Triplet&)));
    ...
    bool enabled)
{
    ...
    else if (View_Navigator_Action->isEnabled() &&
             Navigator)
    ...
    bool enabled)
{
    ...
    else if (View_Statistics_Action->isEnabled() &&
             Statistics)
...
HiView_Window::refresh_source_statistics()
{
    ...
    if (Statistics)
    {
        ...
        if (selected_region.intersects(round_down(Image_View->displayed_image_region())) &&
            ...
            Image_View->source_data_histograms(Statistics->source_statistics()
            ->stats()
            .histograms(),
            selected_region))
        {
        ...
        if (refreshed)
        {
            uint pixel_count = selected_region.width() * selected_region.height();

            Evaluator->setProperty(Script_Evaluator::VAR_COUNT_PIXELS, pixel_count);
            Evaluator->setProperty(Script_Evaluator::VAR_WIDTH_PIXELS, selected_region.width());
            Evaluator->setProperty(Script_Evaluator::VAR_HEIGHT_PIXELS, selected_region.height());
            // Evaluator->run();
            // send data to engine
            qDebug() << "setting data";
            ...
            // Evaluator->setProperty( Script_Evaluator::VAR_BAND0_VAL, Statistics->source_statistics()->stats().mean_value_excluding_exceptions(0));
            // Evaluator->setProperty( Script_Evaluator::VAR_BAND1_VAL, Statistics->source_statistics()->stats().mean_value_excluding_exceptions(1));
            // Evaluator->setProperty( Script_Evaluator::VAR_BAND2_VAL, Statistics->source_statistics()->stats().mean_value_excluding_exceptions(2));
        }
    }
...
HiView_Window::refresh_display_statistics()
{
        ...
        Statistics->display_statistics()->isVisible())
    {
        ...
        if (selected_region.intersects(Image_View->image_display_region()) &&
            // Refresh the histograms data.
            Image_View->display_data_histograms(Statistics->display_statistics()
            ->stats()
            .histograms(),
            selected_region))
        {
...
HiView_Window::create_data_mapper()
{
    ...
    if (Navigator &&
        (Navigator->sizeHint().height() + Data_Mapper->sizeHint().height() + 20) > QGuiApplication::primaryScreen()->availableGeometry().height())
    {
        ...
        if (band < values.count())
        {
            ...
            if (!OK)
            {
                QMessageBox::warning((isVisible() ? this : NULL),
                                     ...
                                     tr("The ") + "Saturation_Upper_Bound_Percents[" + QString::number(band) + "] \"" + values[band].toString() + tr("\" value is invalid - a number is required.\n\n") + tr("This value is ignored."));
                upper[band] = -1.0;
            }
        }
    }
         ...
         ++band)
    {
        if (band < values.count())
        {
            ...
            if (!OK)
            {
                QMessageBox::warning((isVisible() ? this : NULL),
                                     tr("HiView Configuration"),
                                     tr("The ") + "Saturation_Lower_Bound_Percents[" + QString::number(band) + "] \"" + values[band].toString() + tr("\" value is invalid - a number is required.\n\n") + tr("This value is ignored."));
                lower[band] = -1.0;
            }
        }
    }
    ...
    if (Statistics)
    {
        ...
        connect(Data_Mapper,
                SIGNAL(upper_bound_values_changed(const QVector<int> &)),
                Statistics->source_statistics(),
                SLOT(upper_bound_values(const QVector<int> &)));
        connect(Data_Mapper,
                SIGNAL(lower_bound_values_changed(const QVector<int> &)),
                Statistics->source_statistics(),
                SLOT(lower_bound_values(const QVector<int> &)));
        connect(Data_Mapper,
                SIGNAL(upper_bound_percents_changed(const QVector<double> &)),
                Statistics->source_statistics(),
                SLOT(upper_bound_percents(const QVector<double> &)));
        connect(Data_Mapper,
                SIGNAL(lower_bound_percents_changed(const QVector<double> &)),
                Statistics->source_statistics(),
        ...
        if (!OK)
        {
            QMessageBox::warning((isVisible() ? this : NULL),
                                 tr("HiView Configuration"),
                                 tr("The Statistics_Upper_Limit_Offset \"") + settings.value("Statistics_Upper_Limit_Offset").toString() + tr("\" value is invalid - a number is required.\n\n") + tr("The default value of 0 is being used."));
            upper_limit = 0;
        }
        lower_limit =
        ...
        if (!OK)
        {
            QMessageBox::warning((isVisible() ? this : NULL),
                                 tr("HiView Configuration"),
                                 tr("The Statistics_Lower_Limit_Offset \"") + settings.value("Statistics_Lower_Limit_Offset").toString() + tr("\" value is invalid - a number is required.\n\n") + tr("The default value of 0 is being used."));
            lower_limit = 0;
        }
        Statistics->source_statistics()
            ->stats()
    ...
    bool enabled)
{
    ...
    else if (View_Data_Mapper_Action->isEnabled() &&
             Data_Mapper)
    ...
    */
    ...
    if (Image_Info)
    {
        ...
        connect(Image_View,
                ...
                const Dynamic_Image::Triplet&)),
                Image_Info,
                SLOT(pixel_value(const Dynamic_Image::Triplet&,
                const Dynamic_Image::Triplet&)));
        connect(Image_View,
                SIGNAL(image_scaled(const QSizeF&, int)),
                Image_Info,
    ...
    const QString& source_name)
{
    ...
    */
    ...
    else
    {
        ...
        if (!JPIP_source(name) && HiView_Utilities::is_jpip_passthru_link(name))
        {
            QString Requested_Link = HiView_Utilities::parse_jpip_passthru_link(name);
...
HiView_Window::open_file()
{
    ...
    if (Open_File_Dialog->exec())
    {
        ...
        if (!source_name.isEmpty())
        {
            ...
            if (HiView_Utilities::is_jpip_passthru_link(source_name))
            {
                QString Requested_Link = HiView_Utilities::parse_jpip_passthru_link(source_name);
                if (!Requested_Link.isEmpty())
    ...
    bool successful)
{
    ...
    if (successful)
    {
        ...
        */
        ...
        if (Statistics)
        {
            ...
            LOCKED_LOGGING((
                ...
                << endl));
#endif
            Statistics->source_statistics()->data_structure(Image_View->image_bands(), Image_View->image_data_precision());
#if ((DEBUG_SECTION) & (DEBUG_LOAD_IMAGE | DEBUG_SLOTS))
            LOCKED_LOGGING((
        ...
        */
        ...
        add_source_selection(Source_Name_Loading);
...
HiView_Window::reset_metadata()
{
    ...
    Evaluator->clearScriptProperties();
...
#endif
        Evaluator->setMetadata(metadata);
        Image_Info->projection(Location->projection());
        Image_View->projection(Location->projection());
    }
    /*
        ...
        Image_Save_Dialog->exec())
    {
            ...
            !image_size.isEmpty())
        {
            ...
            try
            {
                ...
                else if (!(Image_Saved = Dynamic_Image_Factory::create(Image_View->image_name(), image_size)))
                {
                    message =
                        tr("Could not create the %1x%2 output image.\n")
                        .arg(image_size.width())
                        .arg(image_size.height());
                    message += Dynamic_Image_Factory::error_message();
                }
                ...
                if (Image_Saved)
                {
                    ...
                    else
                    {
                        ...
                        Image_Saved->source_data_maps(const_cast<const Dynamic_Image::Data_Map**>(Image_View->image()->source_data_maps()));
...
#if ((DEBUG_SECTION) & (DEBUG_SAVE_IMAGE | DEBUG_MENUS))
                clog << "    Dynamic_Image assembly failed" << endl;
#endif
            }
        }
    }
    ...
    QMouseEvent* event)
{
    ...
    {
        QPoint
            display_position(Image_View->image_display()->mapFromGlobal(event->globalPos()));
#if ((DEBUG_SECTION) & DEBUG_MOUSE_EVENTS)
...
void HiView_Window::update_line()
{
    if (Distance_Tool)
    {
        ...
        Evaluator->setProperty(Script_Evaluator::VAR_LENGTH_PIXELS, length);
        ...
        if (Location != NULL && Location->projection() != NULL && !Location->projection()->is_identity())
        {
            ...
            Evaluator->setProperty(Script_Evaluator::VAR_LENGTH_METERS, projected_length);
        }
...
HiView_Window::reset_region_overlay()
{
    ...
    if (Selected_Image_Region.isEmpty())
    {
        ...
        if (Location != NULL && Location->projection() != NULL && !Location->projection()->is_identity())
        {
            ...
            auto pixel_size = Location->projection()->pixel_size();
            auto projected_width = source_region.width() * pixel_size;
            auto projected_height = source_region.height() * pixel_size;

            Evaluator->setProperty(Script_Evaluator::VAR_AREA_METERS2, projected_width * projected_height);
            Evaluator->setProperty(Script_Evaluator::VAR_WIDTH_METERS, projected_width);
            Evaluator->setProperty(Script_Evaluator::VAR_HEIGHT_METERS, projected_height);
        }
#if ((DEBUG_SECTION) & DEBUG_MOUSE_EVENTS)
    ...
    if (Region_Overlay)
    {
        ...
        Dynamic_Image::Triplet pixel_value = Image_View->display_pixel(topLeft);
