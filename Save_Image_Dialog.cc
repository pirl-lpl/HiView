...
/*==============================================================================
    Constants
*/
const char *const Save_Image_Dialog::ID =
...
QString Save_Image_Dialog::Default_Image_Format(_DEFAULT_SAVE_IMAGE_FORMAT_);

QString Save_Image_Dialog::Default_Directory;
...
namespace
{
QSize Thumbnail_Max_Size;
}

/*==============================================================================
    Constructor
*/
Save_Image_Dialog::Save_Image_Dialog(const Image_Viewer &image_viewer, QWidget *parent)
    : QFileDialog(parent, tr("Save Image"), Default_Directory, HiView_Utilities::image_writer_formats_file_filters()),
      Image_View(&image_viewer), Thumbnail(NULL)
{
    ...
    setOptions(QFileDialog::DontUseNativeDialog | QFileDialog::DontResolveSymlinks);
    ...
    connect(this, SIGNAL(filterSelected(const QString &)), SLOT(filter_selected(const QString &)));
    connect(this, SIGNAL(fileSelected(const QString &)), SLOT(file_selected(const QString &)));

    /*==============================================================================
        Add-on panel
    */
    QLayout *layout = QFileDialog::layout();
    QGridLayout *dialog_grid_layout = dynamic_cast<QGridLayout *>(layout);
    if (dialog_grid_layout)
    {
        Displayed_Image_Size = Image_View->displayed_image_region().size();
        QSize dialog_size(QFileDialog::sizeHint());
        Thumbnail_Max_Size = dialog_size / 2;

        Drawn_Line *line;
        QWidget *panel = new QWidget;
        QGridLayout *grid_layout = new QGridLayout(panel);
        int row = -1;

        //	Heading.
        dialog_grid_layout->addWidget(line = new Drawn_Line(Heading_Line_Weight), dialog_grid_layout->rowCount(), 0, 1,
                                      -1, Qt::AlignBottom);
        line->alignment(Qt::AlignBottom);
        dialog_size.rheight() += Heading_Line_Weight;
        ++row;
        grid_layout->addWidget(new QLabel(tr("<b>Image Size</b>")), row, 0, Qt::AlignLeft | Qt::AlignVCenter);

        //	Width.
        ++row;
        grid_layout->addWidget(new QLabel(tr("Width:")), row, 1, Qt::AlignRight | Qt::AlignVCenter);
        grid_layout->addWidget(Image_Width = new QSpinBox, row, 2, Qt::AlignLeft | Qt::AlignVCenter);

        //	Height.
        ++row;
        grid_layout->addWidget(new QLabel(tr("Height:")), row, 0, -1, 1, Qt::AlignRight | Qt::AlignVCenter);
        grid_layout->addWidget(Image_Height = new QSpinBox, row, 1, -1, 1, Qt::AlignLeft | Qt::AlignVCenter);

        //	Scale.
        grid_layout->addWidget(new QLabel(tr("Scale:")), row, 3, Qt::AlignRight | Qt::AlignVCenter);
        grid_layout->addWidget(Image_Scale = new QDoubleSpinBox, row, 4, Qt::AlignRight | Qt::AlignVCenter);
        Image_Scale->setDecimals(4);
        Image_Scale->setRange(0.0001, 100.0);
        Image_Scale->setSingleStep(0.01);
        grid_layout->addWidget(new QLabel(tr("Max:")), row + 1, 3, Qt::AlignRight | Qt::AlignVCenter);
        grid_layout->addWidget(Max_Scale = new QLabel("    "), row + 1, 4, Qt::AlignLeft | Qt::AlignVCenter);
        Max_Scale->setToolTip(tr("Max scale at which the image will fit in memory."));
        //	Padding.
        ...
        grid_layout->addWidget(Thumbnail, row, 2, -1, 1, Qt::AlignCenter);

        //	Padding.
        grid_layout->setColumnMinimumWidth(5, 10);
        grid_layout->setColumnStretch(5, 100);

        dialog_grid_layout->addWidget(panel, dialog_grid_layout->rowCount(), 0, 1, -1,
        ...
        connect(Image_Width, SIGNAL(valueChanged(int)), SLOT(image_width(int)));
        connect(Image_Height, SIGNAL(valueChanged(int)), SLOT(image_height(int)));
        connect(Image_Scale, SIGNAL(valueChanged(double)), SLOT(image_scale(double)));
    }
#if ((DEBUG_SECTION) & DEBUG_CONSTRUCTORS)
    clog << "<<< Save_Image_Dialog" << endl;
#endif
...
/*==============================================================================
    Accessors
*/
bool Save_Image_Dialog::image_format(const QString &format)
{
    bool selected = false;
    if (!format.isEmpty() && HiView_Utilities::image_writer_formats().contains(format, Qt::CaseInsensitive))
    {
        Image_Format = format;
...
QSize Save_Image_Dialog::image_size() const
{
    return QSize(Image_Width->value(), Image_Height->value());
}

double Save_Image_Dialog::image_scale() const
{
...
/*==============================================================================
    Manipulators
*/
bool Save_Image_Dialog::reset()
{
#if ((DEBUG_SECTION) & DEBUG_RESET)
    clog << ">>> Save_Image_Dialog::reset" << endl;
#endif
    Displayed_Image_Size = Image_View->displayed_image_region().size();
    QSize image_display_size(Image_View->image_display_region().size());
    ...
    */
    ...
    if (Displayed_Image_Size.isEmpty())
        ...
        Image_Scale->setMaximum(INT_MAX /
                                (Displayed_Image_Size.rwidth() * Displayed_Image_Size.rheight() * sizeof(int)));
    ...
    Image_Width->setMaximum((int)((Image_Scale->maximum() * Displayed_Image_Size.rwidth()) + 0.5));
    Image_Width->setMinimum((int)((Image_Scale->minimum() * Displayed_Image_Size.rwidth()) - 0.5));
    Image_Width->setValue(image_display_size.rwidth());
    Image_Width->blockSignals(enabled);

    enabled = Image_Height->blockSignals(true);
    Image_Height->setMaximum((int)((Image_Scale->maximum() * Displayed_Image_Size.rheight()) + 0.5));
    Image_Height->setMinimum((int)((Image_Scale->minimum() * Displayed_Image_Size.rheight()) - 0.5));
    ...
    QSize thumbnail_size(image_display_size);
    ...
    Plastic_Image *thumbnail_image = NULL;
    if (enabled)
    {
        QString message;
        try
        {
            ...
            else if (!(thumbnail_image = Plastic_Image_Factory::create(Image_View->image_name(), thumbnail_size)))
            {
                message = tr("Could not create a %1x%2 thumbnail of the image.\n")
            ...
            if (thumbnail_image)
            {
                thumbnail_image->source_band_map(Image_View->image()->source_band_map());
                thumbnail_image->source_data_maps(
                    const_cast<const Plastic_Image::Data_Map **>(Image_View->image()->source_data_maps()));
                thumbnail_image->source_origin(Image_View->displayed_image_origin());
                thumbnail_image->source_scale((double)thumbnail_size.rwidth() / Displayed_Image_Size.rwidth());
                thumbnail_image->update();
            }
        }
        catch (Plastic_Image::Render_Exception &except)
        {
            message = tr("Could not render a %1x%2 thumbnail of the image")
                          .arg(thumbnail_size.width())
                          .arg(thumbnail_size.height());
            if (Image_View->image_name().isEmpty())
                message += '.';
        ...
        {
            message = tr("Unexpected exception while generating thumbnail image!");
        }
        if (!thumbnail_image)
        {
...
/*==============================================================================
    Slots
*/
void Save_Image_Dialog::file_selected(const QString &pathname)
{
    ...
    if (!pathname.isEmpty())
    {
        ...
        if (name.length() > 2 && name.at(0) == '~' && name.at(1) == QDir::separator())
...
void Save_Image_Dialog::filter_selected(const QString &format)
{
    Image_Format = format.left(format.indexOf(' ')).toLower();
#if ((DEBUG_SECTION) & DEBUG_SLOTS)
    clog << ">-< Save_Image_Dialog::filter_selected: \"" << format << "\" - " << Image_Format << ')' << endl;
#endif
    setDefaultSuffix(Image_Format);
}

void Save_Image_Dialog::image_width(int width)
{
#if ((DEBUG_SECTION) & DEBUG_SIZE)
    clog << ">>> Save_Image_Dialog::image_width: " << width << endl;
#endif
    bool enabled;
    double scale = (double)width / Displayed_Image_Size.rwidth();
#if ((DEBUG_SECTION) & DEBUG_SIZE)
...
#if ((DEBUG_SECTION) & DEBUG_SIZE)
        clog << "    scale reset to " << scale << endl << "    width reset to " << width << endl;
#endif
        enabled = Image_Width->blockSignals(true);
    ...
    int height = scale * Displayed_Image_Size.rheight();
#if ((DEBUG_SECTION) & DEBUG_SIZE)
...
void Save_Image_Dialog::image_height(int height)
{
#if ((DEBUG_SECTION) & DEBUG_SIZE)
    clog << ">>> Save_Image_Dialog::image_height: " << height << endl;
#endif
    bool enabled;
    double scale = (double)height / Displayed_Image_Size.rheight();
#if ((DEBUG_SECTION) & DEBUG_SIZE)
    clog << "    scale = " << scale << endl;
#endif
    if (scale > Image_Scale->maximum())
...
#if ((DEBUG_SECTION) & DEBUG_SIZE)
        clog << "    scale reset to " << scale << endl << "    height reset to " << height << endl;
#endif
        enabled = Image_Width->blockSignals(true);
        Image_Height->setValue(height);
        Image_Height->blockSignals(enabled);
    ...
    int width = scale * Displayed_Image_Size.rwidth();
#if ((DEBUG_SECTION) & DEBUG_SIZE)
...
void Save_Image_Dialog::image_scale(double scale)
{
#if ((DEBUG_SECTION) & DEBUG_SIZE)
...
void Save_Image_Dialog::min_scale(double scale)
{
    if (scale > 0.0)
        Image_Scale->setMinimum(scale);
}

void Save_Image_Dialog::max_scale(double scale)
{
    if (scale > 0.0)
