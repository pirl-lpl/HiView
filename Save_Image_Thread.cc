...
/*==============================================================================
    Constants
*/
const char *const Save_Image_Thread::ID =
    "UA::HiRISE::Save_Image_Thread ($Revision: 1.8 $ $Date: 2012/06/15 01:16:07 $)";

/*==============================================================================
    Constructors
*/
Save_Image_Thread::Save_Image_Thread(QObject *parent) : QThread(parent), Dialog(NULL), Message(NULL), Saved(false)
{
    setObjectName("Save_Image_Thread");
#if ((DEBUG_SECTION) & DEBUG_CONSTRUCTORS)
    clog << ">>> Save_Image_Thread" << endl;
#endif
    connect(this, SIGNAL(finished()), SLOT(thread_finished()));
/*connect (this,
    SIGNAL (terminated ()),
...
/*==============================================================================
    Accessors
*/
Save_Image_Thread::Save_Sequence Save_Image_Thread::save_sequence() const
{
    QMutexLocker sequence_lock(&Sequence_Lock);
    return Sequence;
}

bool Save_Image_Thread::image_saved() const
{
    QMutexLocker sequence_lock(&Sequence_Lock);
    return Saved;
}

QString Save_Image_Thread::failure_message() const
{
    QMutexLocker sequence_lock(&Sequence_Lock);
    return Failure_Message;
}

void Save_Image_Thread::invalidate()
{
    QMutexLocker sequence_lock(&Sequence_Lock);
    Image = NULL;
    Pathname.clear();
...
/*==============================================================================
    Run the thread
*/
void Save_Image_Thread::run()
{
#if ((DEBUG_SECTION) & DEBUG_RUN)
    LOCKED_LOGGING((clog << ">>> Save_Image_Thread::run" << endl));
#endif
    Sequence_Lock.lock();
    Saved = false;
    Failure_Message.clear();

    if (Image && !Pathname.isEmpty() && !Format.isEmpty())
    {
        setTerminationEnabled(true);

        Sequence = SAVE_IMAGE_RENDER;
        //	Copy so the user can change them while the thread is running.
        Plastic_Image *plastic_image = Image;
        QString pathname(Pathname), format(Format), message;
#if ((DEBUG_SECTION) & DEBUG_RUN)
        LOCKED_LOGGING((clog << "    Save_Image_Thread::run: SAVE_IMAGE_RENDER" << endl));
#endif
...
#if ((DEBUG_SECTION) & DEBUG_RUN)
            LOCKED_LOGGING((clog << "    " << message << endl));
#endif
            rendered = false;
        }
        ...
        if (rendered)
        {
...
#if ((DEBUG_SECTION) & DEBUG_RUN)
            LOCKED_LOGGING((clog << "    Save_Image_Thread::run: SAVE_IMAGE_WRITE" << endl));
#endif
            ...
            Sequence_Lock.lock();
#if ((DEBUG_SECTION) & DEBUG_RUN)
            LOCKED_LOGGING((clog << "    Save_Image_Thread::run: SAVE_IMAGE_DONE" << endl));
#endif
        }
        else
        {
...
#if ((DEBUG_SECTION) & DEBUG_RUN)
        LOCKED_LOGGING((clog << "    " << Failure_Message << endl));
#endif
    }
...
bool Save_Image_Thread::image(Plastic_Image *plastic_image, const QString &pathname, const QString &format)
{
#if ((DEBUG_SECTION) & DEBUG_RUN)
    clog << ">>> Save_Image_Thread::image:" << endl << "    plastic_";
    if (plastic_image)
        clog << *plastic_image << endl;
    else
        clog << "image NULL" << endl;
    clog << "    pathname = " << pathname << endl << "    format = " << format << endl;
#endif
    QMutexLocker sequence_lock(&Sequence_Lock);
    if (plastic_image)
        Image = plastic_image;
...
#if ((DEBUG_SECTION) & DEBUG_RUN)
    clog << "    current save info -" << endl << "    plastic_";
    if (Image)
        clog << *Image << endl;
    else
        clog << "image NULL" << endl;
    clog << "    Pathname = " << Pathname << endl << "    Format = " << Format << endl;
#endif
    bool ready = (Image && !Pathname.isEmpty() && !Format.isEmpty());
#if ((DEBUG_SECTION) & DEBUG_RUN)
...
bool Save_Image_Thread::save_image(Plastic_Image *plastic_image, const QString &pathname, const QString &format)
{
#if ((DEBUG_SECTION) & DEBUG_RUN)
    ...
    if (image(plastic_image, pathname, format) && !isRunning())
    {
        if (!Dialog)
        {
#if ((DEBUG_SECTION) & DEBUG_RUN)
            ...
            Dialog_Layout->addWidget(Activity = new Activity_Indicator, 0, Qt::AlignCenter);
            Dialog_Layout->addWidget(Format_Label = new QLabel, 1, Qt::AlignLeft | Qt::AlignVCenter);
            Dialog_Layout->addWidget(Pathname_Label = new QLabel, 100, Qt::AlignLeft | Qt::AlignVCenter);
            connect(Activity, SIGNAL(button_clicked(int)), SLOT(cancel()));
            connect(this, SIGNAL(canceled(bool)), SLOT(save_canceled(bool)));
        }
        Format_Label->setText(Format + tr(" file:"));

        QString pathname(wrapped_pathname(Pathname, 0, Dialog->font()));
...
/*==============================================================================
    Qt slots
*/
bool Save_Image_Thread::show_dialog(bool enabled)
{
    bool shown = false;
    if (Dialog)
    {
...
void Save_Image_Thread::thread_finished()
{
    if (Dialog)
    {
...
//	>>> SIGNAL <<<
    ...
    LOCKED_LOGGING((clog << "^^^ Save_Image_Thread::thread_finished: "
                            "emit done "
                         << boolalpha << Saved << endl));
#endif
    emit done(Saved);
}

void Save_Image_Thread::thread_terminated()
{
...
//	>>> SIGNAL <<<
#if ((DEBUG_SECTION) & (DEBUG_RUN | DEBUG_SIGNALS))
    LOCKED_LOGGING((clog << "^^^ Save_Image_Thread::thread_terminated: "
                            "emit done false "
                         << endl));
#endif
    emit done(false);
...
bool Save_Image_Thread::cancel()
{
#if ((DEBUG_SECTION) & DEBUG_RUN)
    LOCKED_LOGGING((clog << ">>> Save_Image_Thread::cancel" << endl));
#endif
    bool completed = true;
    if (isRunning())
    {
        ...
        */
        ...
        else if (Sequence == SAVE_IMAGE_WRITE)
...
//	>>> SIGNAL <<<
#if ((DEBUG_SECTION) & (DEBUG_RUN | DEBUG_SIGNALS))
    LOCKED_LOGGING((clog << "^^^ Save_Image_Thread::cancel: "
                            "emit canceled "
...
#if ((DEBUG_SECTION) & DEBUG_RUN)
    LOCKED_LOGGING((clog << "<<< Save_Image_Thread::cancel" << boolalpha << completed << endl));
#endif
    return completed;
}

void Save_Image_Thread::save_canceled(bool completed)
{
    if (!completed && Dialog)
    {
        if (!Message)
