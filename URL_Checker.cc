...
/*==============================================================================
    Constants
*/
const char *const URL_Checker::ID = "UA::HiRISE::URL_Checker ($Revision: 1.8 $ $Date: 2012/08/08 03:33:40 $)";
...
/*==============================================================================
    Accessors
*/
bool URL_Checker::checking() const
{
...
/*==============================================================================
    Run the thread
*/
bool URL_Checker::check(const QUrl &URL, bool synchronous)
{
    ...
    if (!isRunning() && !URL.isEmpty())
    {
        ...
        if (URL.scheme().compare("HTTPS", Qt::CaseInsensitive) == 0 && !URL.host().isEmpty() && !URL.path().isEmpty())
        {
//	HTTP server check.
#if ((DEBUG_SECTION) & DEBUG_CHECK)
            ...
            {
#if ((DEBUG_SECTION) & DEBUG_CHECK)
                LOCKED_LOGGING((clog << "    URL_Checker::check: waiting for thread to finish" << endl));
#endif
...
#endif
            QFileInfo file(QDir::toNativeSeparators(URL.path()));
            if (file.exists())
            {
                if (file.isReadable())
...
#if ((DEBUG_SECTION) & DEBUG_CHECK)
            LOCKED_LOGGING(
                (clog << "    URL_Checker::check: emit checked " << boolalpha << (result == ACCESSIBLE_URL) << endl));
#endif
...
#if ((DEBUG_SECTION) & DEBUG_CHECK)
    LOCKED_LOGGING((clog << "    URL_Checker::check: result = " << result << " - " << status_description(result) << endl
                         << "<<< URL_Checker::check: " << boolalpha << (result == ACCESSIBLE_URL) << endl));
#endif
...
//	>>> SIGNAL <<<
#if ((DEBUG_SECTION) & DEBUG_CHECK)
    LOCKED_LOGGING((clog << "    URL_Checker::run: emit checked " << boolalpha << (result == ACCESSIBLE_URL) << endl));
#endif
    emit checked(result == ACCESSIBLE_URL);
