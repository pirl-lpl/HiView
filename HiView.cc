...
//!	Application name.
extern const char *const APPLICATION_NAME = APP_NAME_STRING;
...
//!	Organization name.
extern const char *const ORGANIZATION_NAME = ORG_NAME_STRING;
...
extern const char *const HOST_MACHINE = MACHINE_STRING;
...
extern const char *const HOST_OS = HOST_OS_STRING;
...
void usage(int exit_status = BAD_SYNTAX, bool list_descriptions = false)
{
    ...
    if (list_descriptions)
        cout << endl << "Viewer for images obtained from local file or remote server sources." << endl << endl;
    ...
    if (list_descriptions)
        cout << "    The initial scaling factor(s) will be applied when the specified" << endl
    ...
    if (list_descriptions)
        cout << "    Do (not) restore the GUI layout geometry that was saved from the" << endl
    ...
    cout << "-Help" << endl;
    if (list_descriptions)
        cout << "    Print this help description and exit." << endl << endl;
...
int main(int arg_count, char **arg_list)
{
    ...
    clog << Command_Name << ": " << APPLICATION_ID << endl << HOST_OS << ' ' << HOST_MACHINE << endl;
#endif
    ...
    //	Application object.
    HiView_Application application(arg_count, arg_list);
        ...
        if (arg_list[count][0] == '-')
        {
            ...
            case 'S':
                if (toupper(arg_list[count][2]) == 'C')
                {
                    ...
                    {
                    Bad_Scale:
                        cout << "A scale factor was expected for the " << arg_list[count - 1] << " option but \""
            ...
            case 'V': //	-Version
                cout << APPLICATION_ID << endl << "Build system: " << HOST_OS << ' ' << HOST_MACHINE << endl;
                exit(SUCCESS);
    ...
    HiView_Window main_window(Source_Name, Scaling, Restore_Layout);
