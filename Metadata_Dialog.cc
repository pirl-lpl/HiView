...
/*==============================================================================
    Constants
*/
const char *const Metadata_Dialog::ID = "UA::HiRISE::Metadata_Dialog ($Revision: 1.18 $ $Date: 2012/09/16 07:41:32 $)";
...
#endif
const int Metadata_Dialog::DEFAULT_WIDTH = METADATA_DIALOG_DEFAULT_WIDTH,
...
/*==============================================================================
    Static data
*/
QErrorMessage *Metadata_Dialog::Error_Message = NULL;

/*==============================================================================
    Constructors
*/
Metadata_Dialog::Metadata_Dialog(Aggregate *parameters, QWidget *parent, Qt::WindowFlags flags)
    : QDialog(parent, flags), Tree_View(new Parameter_Tree_View(parameters)), Comments(new QTextEdit), Save_Dialog(NULL)
{
    initialize();
}

Metadata_Dialog::Metadata_Dialog(QWidget *parent, Qt::WindowFlags flags)
    : QDialog(parent, flags), Tree_View(new Parameter_Tree_View), Comments(new QTextEdit), Save_Dialog(NULL)
{
    initialize();
}

void Metadata_Dialog::initialize()
{
...
//	Comments:
    ...
    */
    ...
    grid->addWidget(new Drawn_Line(Heading_Line_Weight), 0, 1, Qt::AlignBottom);
    grid->addWidget(new QLabel(tr("<b>Comments</b>")), 1, 1, Qt::AlignLeft | Qt::AlignVCenter);

    Comments->setReadOnly(true);
    //	Frame styling.
    Comments->setFrameStyle(Panel_Frame_Style);
    Comments->setLineWidth(Panel_Frame_Width);
    grid->addWidget(Comments, 2, 0, 1, -1);
...
//	Vertical QSplitter.
    ...
    int adjustment = splitter_sizes[1] - comments->minimumSizeHint().height();
...
/*==============================================================================
    Menus
*/
QMenuBar *Metadata_Dialog::create_menus()
{
#if (DEBUG_SECTION & DEBUG_CONSTRUCTORS)
    ...
    connect(action, SIGNAL(triggered()), SLOT(save_metadata()));
    ...
    connect(Save_Selected_Action, SIGNAL(triggered()), SLOT(save_selected_metadata()));
...
/*==============================================================================
    Accessors
*/
void Metadata_Dialog::parameters(Aggregate *params)
{
    Tree_View->parameters(params);
    //	Reconnect to the selectionChanged signal if the selectionModel changed.
    connect(Tree_View->selectionModel(), SIGNAL(selectionChanged(const QItemSelection &, const QItemSelection &)),
            SLOT(selection_changed(const QItemSelection &, const QItemSelection &)), Qt::UniqueConnection);
}

idaeim::PVL::Aggregate *Metadata_Dialog::parameters() const
{
    return Tree_View->parameters();
}

bool Metadata_Dialog::root_visible() const
{
    return Tree_View->root_visible();
}

void Metadata_Dialog::root_visible(bool enable)
{
    Tree_View->root_visible(enable);
}

QString Metadata_Dialog::root_name() const
{
    return Tree_View->root_name();
}

void Metadata_Dialog::root_name(const QString &new_name)
{
    Tree_View->root_name(new_name);
}

QString Metadata_Dialog::default_root_name()
{
    return Parameter_Tree_View::default_root_name();
}

QSize Metadata_Dialog::sizeHint() const
{
    return QSize(DEFAULT_WIDTH, DEFAULT_HEIGHT);
}

bool Metadata_Dialog::alternating_row_colors() const
{
    return Tree_View->alternatingRowColors();
}

void Metadata_Dialog::alternating_row_colors(bool enable)
{
...
/*==============================================================================
    Slots
*/
void Metadata_Dialog::selection_changed(const QItemSelection &selected, const QItemSelection & /*deselected*/
)
{
    ...
    else
    {
        ...
        clog << "    comments from " << list[0] << endl
             << "    parameter " << static_cast<Parameter *>(list[0].internalPointer())->name() << endl
...
bool Metadata_Dialog::save_selected_metadata()
{
    bool saved(false);
    Parameter_Tree_View::Parameter_List selected_parameters(Tree_View->selected_parameters());
    ...
    else
    {
        ...
        if (selected_parameters[0])
            title.append(tr("Save ")).append(selected_parameters[0]->name().c_str()).append(tr(" Metadata"));
        saved = save_metadata(selected_parameters[0], title);
    }
    return saved;
}

bool Metadata_Dialog::save_metadata(idaeim::PVL::Parameter *metadata, const QString &title)
{
    ...
    if (metadata)
    {
        if (!Save_Dialog)
        {
            ...
            Save_Dialog->setOptions(QFileDialog::DontUseNativeDialog | QFileDialog::DontResolveSymlinks);
        ...
        if (Save_Dialog->exec())
        {
            QString pathname(Save_Dialog->selectedFiles().value(0).trimmed());
            if (!pathname.isEmpty())
            {
#if (DEBUG_SECTION & DEBUG_SAVE)
                clog << "    metadata name = " << metadata->name() << endl
                     << "    metadata parent @ " << (void *)metadata->parent() << endl;
                if (metadata->parent())
                    clog << "    metadata parent name = " << metadata->parent()->name() << endl;
#endif
                //	Check for special container Aggregates.
                bool tmp_name(true);
                string name(metadata->name());
                if (name.empty() || name == PDS_Metadata::PDS_METADATA_GROUP ||
...
/*==============================================================================
    Utilities
*/
bool Metadata_Dialog::save(const Parameter *const metadata, const QString &pathname)
{
#if (DEBUG_SECTION & DEBUG_SAVE)
    clog << ">>> Metadata_Dialog::save: " << pathname << endl;
#endif
    bool saved(false);
    if (metadata && !pathname.isEmpty())
    {
        QString message;
        QFileInfo *file_info(new QFileInfo(pathname));
        if (!file_info->exists() || (file_info->isFile() && file_info->isWritable()))
        {
            ofstream file(pathname.toStdString().c_str());
            if (file)
            {
                try
                {
        ...
        if (!message.isEmpty())
        {
            message.prepend('\n').prepend(pathname).prepend("Unable to save the metadata to file\n");
            if (!Error_Message)
...
/*==============================================================================
    Event Handler
*/
bool Metadata_Dialog::event(QEvent *event)
{
    bool visible = false;
    switch (event->type())
    {
