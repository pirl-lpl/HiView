...
(
    ...
    Tile_Grid_Images(new QList<QList<Dynamic_Image*>*>),
    ...
    Tile_Image_Size(Tile_Display_Size)
{
    ...
    clog << ">>> Tiled_Image_Display @ " << reinterpret_cast<void*>(this)
...
#if ((DEBUG_SECTION) & DEBUG_CONSTRUCTORS)
    LOCKED_LOGGING((
        clog << ">>> ~Tiled_Image_Display: @ " << reinterpret_cast<void*>(this) << endl));
#endif
    // Stop all rendering.
    Renderer->finish
    (Tile_Renderer::FORCE_CANCEL |
     Tile_Renderer::WAIT_UNTIL_DONE);
    // Delete all of the tiles.
    clear_tiles();

#if ((DEBUG_SECTION) & DEBUG_CONSTRUCTORS)
    LOCKED_LOGGING((
        clog << "    delete Tile_Grid_Images @ " << reinterpret_cast<void*>(Tile_Grid_Images) << endl));
#endif
    delete Tile_Grid_Images;

#if ((DEBUG_SECTION) & DEBUG_CONSTRUCTORS)
    LOCKED_LOGGING((
        clog << "    delete Renderer @ " << reinterpret_cast<void*>(Renderer) << endl));
#endif
    delete Renderer;
#if ((DEBUG_SECTION) & DEBUG_CONSTRUCTORS)
    LOCKED_LOGGING((
...
#endif
        registered = Renderer->image(Shared_Image(new Dynamic_QImage()));
    }
    else
...
)
{
    ...
    if (successful)
    {
        ...
        */
        ...
        if (!dynamic_cast<Dynamic_QImage*>(Reference_Image) &&
...
)
{
    ...
    clog << "    current color = 0x" << setw(8)
        ...
        << setfill(' ') << dec << endl;
#endif
    if (color != Dynamic_Image::default_background_color())
    {
        Dynamic_Image::default_background_color(color);
...
{
    return Tile_Renderer::default_max_source_image_area();
}
...
{
    Tile_Renderer::default_max_source_image_area(area);
}
...
) const
{
    ...
    LOCKED_LOGGING((
        clog << "<<< Tiled_Image_Display::image_pixel_datum: "
        << Dynamic_Image::UNDEFINED_PIXEL_VALUE << endl));
#endif
    return Dynamic_Image::UNDEFINED_PIXEL_VALUE;
}
...
#endif
    if (value)
        return Dynamic_Image::Triplet(value);
    return Dynamic_Image::Triplet();
}
...
)
{
    ...
    if (tile_size != Tile_Display_Size)
    {
        ...
        Renderer->reset(Tile_Renderer::WAIT_UNTIL_DONE);
    ...
    if (clipped_origin != current_origin)
    {
        ...
        if (band <= 0)
        {
                ...
                offset.ry() < 0)
            {
                ...
                if (!Image_Loading)
                    // Cancel all rendering.
                    Renderer->reset(Tile_Renderer::DO_NOT_WAIT);
    ...
    while (true)
    {
        ...
        if (!(tiles = Tile_Grid_Images->at(tile_grid.ry())))
        {
                ...
                tile_locations[0].ry() < size_of_image.rheight())
            {
                ...
                (tile_grid.ry(), tiles = new QList<Dynamic_Image*>);
...
#endif
                        Renderer->cancel(tile_image, Tile_Renderer::DO_NOT_WAIT);
                        if (Tile_Image_Pool.size() < Tile_Image_Pool_Max)
                        {
#if ((DEBUG_SECTION) & (DEBUG_RESET_TILES | DEBUG_IMAGE_GEOMETRY))
                            LOCKED_LOGGING((
                        ...
                        if (Tile_Image_Pool.isEmpty())
                        {
                            tile_image =
                                ...
                                (Tile_Display_Size,
                                 ...
                                 Dynamic_Image::BAND_MAP |
                                 Dynamic_Image::DATA_MAPS);
                        ...
                        !Image_Loading)
                    {
                        ...
                        */
                        ...
                        Renderer->queue(tile_image,
                                        ...
                                        Tile_Renderer::LOW_PRIORITY_RENDERING : tile_grid),
                                        displayed_tile_region);
                    }
                }
#if ((DEBUG_SECTION) & (DEBUG_RESET_TILES | DEBUG_IMAGE_GEOMETRY))
...
)
{
    ...
    if (changed)
    {
        ...
        image_update_needed(Dynamic_Image::BAND_MAP);
...
)
{
    ...
    */
    ...
    if (changed)
    {
        ...
        // Apply the update to the tile images.
        image_update_needed(Dynamic_Image::DATA_MAPS);
...
)
{
    ...
    LOCKED_LOGGING((
        ...
        << " - " << Dynamic_Image::mapping_type_names(update_type) << endl
        << "    in " << object_pathname(this) << endl));
#endif
    int
        tile_rows = Tile_Grid_Size.rheight(),
...
)
{
    ...
    if (status == NOT_RENDERING ||
        (status & Tile_Renderer::RENDERING_CANCELED))
    {
        Renderer->clean_up();
