...
#endif
const int Tiled_Image_Display::MINIMUM_TILE_DIMENSION = MINIMUM_IMAGE_TILE_DIMENSION;
...
#endif
double Tiled_Image_Display::Min_Scale = IMAGE_VIEWER_MIN_SCALE;
#ifndef IMAGE_VIEWER_MAX_SCALE
#define IMAGE_VIEWER_MAX_SCALE 10.0
#endif
double Tiled_Image_Display::Max_Scale = IMAGE_VIEWER_MAX_SCALE;

#ifndef DEFAULT_SOURCE_IMAGE_RENDERING
#define DEFAULT_SOURCE_IMAGE_RENDERING true
#endif
bool Tiled_Image_Display::Default_Source_Image_Rendering = DEFAULT_SOURCE_IMAGE_RENDERING;
...
//	Renderer.
    ...
    loaded(true);
#if ((DEBUG_SECTION) & DEBUG_CONSTRUCTORS)
    LOCKED_LOGGING((clog << "<<< Tiled_Image_Display: " << object_pathname(this) << endl));
#endif
}
...
Tiled_Image_Display::~Tiled_Image_Display()
{
    ...
    delete Renderer;
#if ((DEBUG_SECTION) & DEBUG_CONSTRUCTORS)
    LOCKED_LOGGING((clog << "<<< ~Tiled_Image_Display" << endl));
#endif
}
...
bool Tiled_Image_Display::image(const QString& source_name, const QSizeF& scaling)
{
    ...
    LOCKED_LOGGING((clog << ">>> Tiled_Image_Display::image: \"" << source_name << '"' << endl
...
//	Cancel and clear any rendering in progress and stop rendering.
    ...
    LOCKED_LOGGING((clog << "    " << pathname << endl
                         << "<<< Tiled_Image_Display::image: " << boolalpha << registered << endl));
#endif
    return registered;
}
...
#if ((DEBUG_SECTION) & DEBUG_LOAD_IMAGE)
        LOCKED_LOGGING((clog << "    Providing an empty image" << endl));
#endif
...
void Tiled_Image_Display::loaded(bool successful)
{
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_LOAD_IMAGE | DEBUG_OVERVIEW))
    QString pathname(object_pathname(this));
    LOCKED_LOGGING((clog << ">>> Tiled_Image_Display::loaded: " << boolalpha << successful << endl
                         << "    in " << pathname << endl));
#endif
...
//	Stop any rendering in progress (should have been done at load request).
        ...
        LOCKED_LOGGING((clog << "    Tiled_Image_Display::loaded: stop_rendering" << endl));
#endif
...
//	Switch over to the new Source and Reference images.
        ...
        if (Image_Loading)
            ...
            state |= IMAGE_MOVE_STATE | BAND_MAPPING_STATE | DATA_MAPPING_STATE;
...
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_LOAD_IMAGE))
            LOCKED_LOGGING((clog << "    preferred display size = " << display_size << endl));
#endif
            if (display_size.rwidth() == 0 || display_size.rheight() == 0)
            {
        ...
        QSizeF displayed_size(Initial_Scaling);
        displayed_size.rwidth() *= image_width();
        displayed_size.rheight() *= image_height();
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_LOAD_IMAGE | DEBUG_OVERVIEW))
        LOCKED_LOGGING((clog << "    Initial_Scaling = " << Initial_Scaling << endl
...
*/
        ...
        Tile_Display_Offset.rx() = Tile_Display_Offset.ry() = 0;
...
void Tiled_Image_Display::background_color(QRgb color)
{
    ...
    if (color != Plastic_Image::default_background_color())
    {
        ...
        int tile_rows = Tile_Grid_Size.rheight(), tile_cols = Tile_Grid_Size.rwidth();
                                                 ...
                                                 const QRect& image_region) const
{
    ...
    {
#if ((DEBUG_SECTION) & DEBUG_HISTOGRAMS)
        clog << "    tiled_image_region = " << round_down(tiled_image_region()) << endl
...
Done:
#if ((DEBUG_SECTION) & DEBUG_HISTOGRAMS)
    clog << "<<< Tiled_Image_Display::source_data_histograms: " << boolalpha << completed << endl;
#endif
    return completed;
}
                                                  ...
                                                  const QRect& display_region) const
{
    ...
    if (display_region.isEmpty() || Tile_Display_Size.isEmpty() || Tile_Grid_Size.isEmpty())
    {
#if ((DEBUG_SECTION) & DEBUG_HISTOGRAMS)
        clog << "    empty " << (display_region.isEmpty() ? "display region" : "tile grid") << endl
...
#endif
    if (tile_grid.rx() < 0 || tile_grid_limit.rx() < 0)
    {
#if ((DEBUG_SECTION) & DEBUG_HISTOGRAMS)
    ...
    while (true)
    {
        if ((tiles = Tile_Grid_Images->at(tile_grid.ry())))
        {
            ...
            {
#if ((DEBUG_SECTION) & DEBUG_HISTOGRAMS)
                clog << "      " << tile_grid << " tile display region = " << tile_region
                     << ", selected = " << (tile_region & display_region) << endl;
#endif
...
/*==============================================================================
    Accesors
*/
Plastic_Image::Pixel_Datum Tiled_Image_Display::image_pixel_datum(unsigned int x, unsigned int y,
...
#if ((DEBUG_SECTION) & DEBUG_PIXEL_DATUM)
    LOCKED_LOGGING((clog << "      tile grid = " << point << endl));
#endif
        ...
        point.ry() < Tile_Grid_Size.height())
    {
        ...
        if (tiles)
        {
            ...
            if (tile_image && !tile_image->needs_update())
            {
                ...
                Plastic_Image::Pixel_Datum datum = tile_image->source_pixel_value(x, y, band);
                clog << "<<< Tiled_Image_Display::image_pixel_datum: " << datum << endl;
                UNLOCK_LOG;
                return datum;
#else
...
QPointF Tiled_Image_Display::displayed_image_origin(int band) const
{
    ...
    QPointF point(Displayed_Image_Region.topLeft());
    point += displayed_tile_grid_origin(band);
    point -= displayed_tile_grid_origin(0);
#if ((DEBUG_SECTION) & (DEBUG_MAP_POINTS | DEBUG_ORIGIN))
    LOCKED_LOGGING((clog << "<<< Tiled_Image_Display::displayed_image_origin: " << point << endl));
#endif
    return point;
}

QPoint Tiled_Image_Display::calculate_lower_right_origin_limit() const
{
#if ((DEBUG_SECTION) & (DEBUG_MAP_POINTS | DEBUG_ORIGIN))
    LOCKED_LOGGING((clog << ">>> Tiled_Image_Display::calculate_lower_right_origin_limit" << endl));
#endif
    QPointF limit(image_width() - 1, image_height() - 1);
#if ((DEBUG_SECTION) & (DEBUG_MAP_POINTS | DEBUG_ORIGIN))
    LOCKED_LOGGING((clog << "       image LR limit = " << limit << endl));
#endif
...
QRectF Tiled_Image_Display::displayed_image_region(int band) const
{
    ...
    QRectF region(calculate_displayed_image_region(band));
#if ((DEBUG_SECTION) & (DEBUG_MAP_POINTS | DEBUG_REGION))
    LOCKED_LOGGING((clog << "<<< Tiled_Image_Display::displayed_image_region: " << region << endl));
#endif
    return region;
}
...
bool Tiled_Image_Display::reset_displayed_image_region()
{
#if ((DEBUG_SECTION) & (DEBUG_MAP_POINTS | DEBUG_ORIGIN | DEBUG_REGION))
    LOCKED_LOGGING((clog << ">>> Tiled_Image_Display::reset_displayed_image_region:" << endl
                         << "        Displayed_Image_Region = " << Displayed_Image_Region << endl));
#endif
    QRectF region(calculate_displayed_image_region(0));
#if ((DEBUG_SECTION) & (DEBUG_MAP_POINTS | DEBUG_ORIGIN | DEBUG_REGION))
    LOCKED_LOGGING((clog << "    calculated displayed_image_region = " << region << endl));
#endif
    bool changed = false;
    if (region != Displayed_Image_Region)
    {
        QSize region_size(round_down(region.size()));
        if (region_size.rwidth() != round_down(Displayed_Image_Region.width()) ||
            region_size.rheight() != round_down(Displayed_Image_Region.height()))
            changed = true;
...
QRectF Tiled_Image_Display::calculate_displayed_image_region(int band) const
{
    ...
    QSizeF size_of_image(image_size());
#if ((DEBUG_SECTION) & (DEBUG_MAP_POINTS | DEBUG_ORIGIN | DEBUG_REGION))
...
QPointF Tiled_Image_Display::map_display_to_image(const QPoint& coordinate, int band) const
{
...
#if ((DEBUG_SECTION) & DEBUG_MAP_POINTS)
    LOCKED_LOGGING((clog << "           Tile_Display_Offset = " << Tile_Display_Offset << endl
    ...
    point += displayed_tile_grid_origin(band);
#if ((DEBUG_SECTION) & DEBUG_MAP_POINTS)
    LOCKED_LOGGING((clog << "<<< Tiled_Image_Display::map_display_to_image: " << point << endl));
#endif
    return point;
}
...
#if ((DEBUG_SECTION) & DEBUG_MAP_POINTS)
    QPointF tile_grid_origin(displayed_tile_grid_origin(band));
    LOCKED_LOGGING((clog << "    displayed_tile_grid_origin = " << tile_grid_origin << endl
                         << "       image coordinate offset = " << point << endl));
#endif
...
#if ((DEBUG_SECTION) & DEBUG_MAP_POINTS)
    LOCKED_LOGGING((clog << "    displayed_tile_grid_origin = " << point << endl
                         << "               tile_image_size = " << extent << endl));
#endif
    point.rx() += extent.rwidth() * (coordinate.x() - 1);
    point.ry() += extent.rheight() * (coordinate.y() - 1);
#if ((DEBUG_SECTION) & DEBUG_MAP_POINTS)
    LOCKED_LOGGING((clog << "<<< Tiled_Image_Display::map_tile_to_image: " << point << endl));
#endif
    return point;
}
...
#if ((DEBUG_SECTION) & DEBUG_MAP_POINTS)
    LOCKED_LOGGING((clog << "    tiled_image_region = " << region << endl));
#endif
    if (region.contains(coordinate))
    {
        QPointF point(coordinate);
        point -= region.topLeft();
#if ((DEBUG_SECTION) & DEBUG_MAP_POINTS)
        LOCKED_LOGGING((clog << "       Tile_Image_Size = " << Tile_Image_Size << endl));
#endif
        if (band == 0)
        {
        ...
        {
            QSizeF tile_size(tile_image_size(band));
...
QPoint Tiled_Image_Display::map_image_to_tile_offset(const QPointF& coordinate, int band) const
{
    ...
    */
    ...
    if (region.contains(coordinate))
    {
        ...
        QSizeF tile_size(tile_image_size(band));
#if ((DEBUG_SECTION) & (DEBUG_MAP_POINTS | DEBUG_ORIGIN))
        LOCKED_LOGGING((clog << "     tile grid offset coordinate = " << point << endl
                             << "                 tile_image_size = " << tile_size << endl));
#endif

        //	Tile offset in image space.
        point.rx() -= static_cast<int>(point.rx() / tile_size.rwidth()) * tile_size.rwidth();
        point.ry() -= static_cast<int>(point.ry() / tile_size.rheight()) * tile_size.rheight();
#if ((DEBUG_SECTION) & (DEBUG_MAP_POINTS | DEBUG_ORIGIN))
        LOCKED_LOGGING((clog << "    tile image offset coordinate = " << point << endl));
#endif
...
QPoint Tiled_Image_Display::map_display_to_tile(const QPoint& coordinate) const
{
    ...
    point.rx()++;
    point.ry()++;
    if (point.rx() < 0 || point.ry() < 0 || point.rx() >= Tile_Grid_Size.width() ||
        point.ry() >= Tile_Grid_Size.height())
...
bool Tiled_Image_Display::reset_tile_image_size()
{
...
void Tiled_Image_Display::print_tile_grid() const
{
    ...
    int tile_region_left_edge = tile_region.x();
    int rows = Tile_Grid_Images->size(), row = -1, cols, col;
...
{
    int entries = Tile_Image_Pool.size();
...
bool Tiled_Image_Display::resize_tile_grid()
{
    ...
    new_rows += 2;
#if ((DEBUG_SECTION) & DEBUG_TILE_GRID)
    LOCKED_LOGGING((clog << "            image size = " << image_size() << endl
...
//	Add empty tile row.
#if ((DEBUG_SECTION) & DEBUG_TILE_GRID)
                LOCKED_LOGGING((clog << "      adding row " << tile_row << endl));
#endif
...
#if ((DEBUG_SECTION) & DEBUG_TILE_GRID)
                                LOCKED_LOGGING((clog << "        move image to pool -" << endl
                                                     << "        " << *tile_image << endl));
#endif
...
#if ((DEBUG_SECTION) & DEBUG_MAP_POINTS)
    LOCKED_LOGGING((clog << "<<< Tiled_Image_Display::tile_display_region: " << region << endl));
#endif
    return region;
}
...
QPoint Tiled_Image_Display::tile_display_origin(const QPoint& coordinate) const
{
    ...
    origin.ry()--;
#if ((DEBUG_SECTION) & DEBUG_MAP_POINTS)
    LOCKED_LOGGING((clog << "            active grid = " << origin << endl));
#endif
...
bool Tiled_Image_Display::reset_tiled_image_region()
{
    reset_tile_image_size();
...
QRectF Tiled_Image_Display::calculate_tiled_image_region(int band) const
{
    QPointF origin(displayed_tile_grid_origin(band));
    QSizeF extent(calculate_tile_image_size(band));
...
QString move_change_description(int change)
{
    switch (change)
    {
        case NO_CHANGE:
...
bool Tiled_Image_Display::move_image(const QPoint& origin, int band)
{
    ...
    bool image_region_resized = false;

    QPoint clipped_origin(origin);
    ...
    if (clipped_origin != current_origin)
    {
        ...
        if (band <= 0)
        {
            ...
            QPoint offset(map_image_to_tile(location, band));
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_MOVE | DEBUG_ORIGIN))
            LOCKED_LOGGING((clog << "    map_image_to_tile = " << offset << endl));
#endif
            if (offset.rx() < 0 || offset.ry() < 0)
            {
                //	Now for someplace completely different.
                change = RENDER_TILES;
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_MOVE | DEBUG_ORIGIN))
                LOCKED_LOGGING((clog << "    reset tile grid" << endl));
#endif
                if (!Image_Loading)
                    //	Cancel all rendering.
                ...
                offset.ry()--;
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_MOVE | DEBUG_ORIGIN))
                LOCKED_LOGGING((clog << "    reset tile grid offset = " << offset << endl));
#endif
            ...
            else
            {
                ...
                offset.ry()--;
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_MOVE | DEBUG_ORIGIN))
                LOCKED_LOGGING((clog << "    shift tile grid offset = " << offset << endl));
#endif
                if (!offset.isNull())
                {
                    change = RENDER_TILES;
                    ...
                    if (offset.ry())
                    {
                        ...
                        if (offset.ry() > 0)
                            //	Rotate up.
                            while (offset.ry()--)
                                Tile_Grid_Images->append(Tile_Grid_Images->takeFirst());
                        else
                            //	Rotate down.
                            while (offset.ry()++)
                                Tile_Grid_Images->prepend(Tile_Grid_Images->takeLast());
                    }
                    if (offset.rx())
                    {
                        //	Shift tile columns.
...
*/
        ...
        else
        {
            ...
            */
            ...
            displayed_tile_grid_origin(location, band);
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_MOVE | DEBUG_ORIGIN))
            LOCKED_LOGGING((clog << "    change to non-reference band" << endl
                                 << "    displayed_tile_grid_origin = " << location << endl));
#endif
        }
...
//	>>> SIGNAL <<<
        ...
        LOCKED_LOGGING((clog << "^^^ Tiled_Image_Display::move_image: "
                                "emit image_moved"
                             << endl
...
int Tiled_Image_Display::reset_tiles()
{
#if ((DEBUG_SECTION) & (DEBUG_RESET_TILES | DEBUG_IMAGE_GEOMETRY))
    QString pathname(object_pathname(this));
    LOCKED_LOGGING((clog << ">>> Tiled_Image_Display::reset_tiles" << endl
                         << "    in " << pathname << endl
                         << "    Tile_Grid_Size = " << Tile_Grid_Size << endl));
#endif
    int changed = NO_TILES_RESET;
    bool origin_changed, scaling_changed;
    int tile_rows = Tile_Grid_Size.rheight(), tile_cols = Tile_Grid_Size.rwidth();
    if (!tile_rows || !tile_cols)
    {
#if ((DEBUG_SECTION) & (DEBUG_RESET_TILES | DEBUG_IMAGE_GEOMETRY))
    ...
    QPoint tile_grid, displayed_tile_origin;
    QRect viewport(rect()),
    ...
    int tile_region_left_edge = tile_region.x();
#if ((DEBUG_SECTION) & (DEBUG_RESET_TILES | DEBUG_IMAGE_GEOMETRY))
    LOCKED_LOGGING((clog << "    Display geometry -" << endl
                         << "    Tile_Display_Offset = " << Tile_Display_Offset << endl
                         << "      Tile_Display_Size = " << Tile_Display_Size << endl
                         << "               viewport = " << rect() << endl));
#endif
    ...
    while (true)
    {
        ...
        if (!(tiles = Tile_Grid_Images->at(tile_grid.ry())))
        {
            if (tile_locations[0].ry() >= 0 && tile_locations[0].ry() < size_of_image.rheight())
            {
...
#if ((DEBUG_SECTION) & (DEBUG_RESET_TILES | DEBUG_IMAGE_GEOMETRY))
            LOCKED_LOGGING((clog << "--- deactivating row -" << endl));
#endif
            tile_grid.rx() = tile_cols;
            while (tile_grid.rx()--)
            {
...
//	Deactivate the tile.
                    ...
                    {
#if ((DEBUG_SECTION) & (DEBUG_RESET_TILES | DEBUG_IMAGE_GEOMETRY))
                        LOCKED_LOGGING((clog << "      move to pool " << *tile_image << endl));
#endif
                        Tile_Image_Pool.append(tile_image);
                    }
                    else
...
//	Activate the tile.
#if ((DEBUG_SECTION) & (DEBUG_RESET_TILES | DEBUG_IMAGE_GEOMETRY))
                        LOCKED_LOGGING((clog << "+++++ activating column -" << endl));
#endif
...
//	Reset the tile image.
                    ...
                    if (!changed && (origin_changed || scaling_changed))
                    ...
                    //	Tile markings.
                    QString label = QString(" reset @ %1: g %2, %3; o %4, %5, s %6")
                    ...
                    mark_image(tile_image, label, TILE_MARKINGS_RESET_Y, TILE_MARKINGS_RESET_COLOR);
                    mark_image(tile_image, label, tile_image->height() - TILE_MARKINGS_RESET_Y,
                               TILE_MARKINGS_RESET_COLOR);
#endif

                    if (tile_image->needs_update() && !Image_Loading)
                    {
#if ((DEBUG_SECTION) & (DEBUG_RESET_TILES | DEBUG_IMAGE_GEOMETRY))
                        LOCKED_LOGGING((clog << "    Tiled_Image_Display::reset_tiles: "
...
void Tiled_Image_Display::clear_tiles()
{
    ...
    Tile_Grid_Size.rwidth() = Tile_Grid_Size.rheight() = 0;
...
//	Clear the tile image pool.
    ...
    Image_Tile::tile_accounting();
    Renderer->image_accounting();
    clog << "    in " << pathname << endl << "<<< Tiled_Image_Display::clear_tiles" << endl;
    UNLOCK_LOG;
#endif
}
...
#if ((DEBUG_SECTION) & DEBUG_SCALING)
        QPoint display_center(map_image_to_display(center));
        LOCKED_LOGGING((clog << "    center display point = " << display_center << endl));
#endif
...
//	New image origin.
#if ((DEBUG_SECTION) & DEBUG_SCALING)
            LOCKED_LOGGING((clog << "        old displayed_image_origin = " << origin << endl));
#endif
            if (origin != center)
            {
...
#if ((DEBUG_SECTION) & DEBUG_SCALING)
            LOCKED_LOGGING((clog << "                    clipped origin = " << origin << endl));
#endif
        }
...
*/
        ...
        QPointF grid_origin(static_cast<int>(origin.rx() / size.rwidth()) * size.rwidth(),
                            static_cast<int>(origin.ry() / size.rheight()) * size.rheight());
#if ((DEBUG_SECTION) & DEBUG_SCALING)
        LOCKED_LOGGING((clog << "    new displayed_tile_grid_origin = " << grid_origin << endl));
#endif
...
//	>>> SIGNAL <<<
        ...
        LOCKED_LOGGING((clog << "^^^ Tiled_Image_Display::scale_image: "
                                "emit image_scaled"
                             << endl
                             << "    scaling = " << scaling << endl
                             << "       band = " << endl));
...
QSize Tiled_Image_Display::scaled_image_size(int band) const
{
#if ((DEBUG_SECTION) & DEBUG_ACCESSORS)
    ...
    if (band < 0)
    {
        QRect inclusive(round_down(displayed_tile_grid_origin(0)), scaled_image_size(0)), band_rect;
...
void Tiled_Image_Display::min_scale(double scale_factor)
{
    if (scale_factor <= 0.0)
    {
        ostringstream message;
        message << ID << endl << "Invalid minimum scale factor - " << scale_factor;
        throw invalid_argument(message.str());
    }
    Min_Scale = scale_factor;
}

void Tiled_Image_Display::max_scale(double scale_factor)
{
    if (scale_factor < 1.0)
    {
        ostringstream message;
        message << ID << endl << "Invalid maximum scale factor - " << scale_factor;
        throw invalid_argument(message.str());
    }
...
bool Tiled_Image_Display::map_bands(const unsigned int* band_map)
{
    ...
    {
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_MAP_BANDS))
        LOCKED_LOGGING((clog << "    no " << (band_map ? "tiles" : "band_map") << endl
...
//	Cancel all rendering.
        ...
        LOCKED_LOGGING((clog << "    reset Renderer" << endl));
#endif
        Renderer->reset();
...
#if ((DEBUG_SECTION) & DEBUG_MAP_DATA)
    LOCKED_LOGGING((clog << "<<< Tiled_Image_Display::map_data: " << boolalpha << changed << endl));
#endif
    return changed;
}

void Tiled_Image_Display::image_update_needed(Mapping_Type update_type)
{
#if ((DEBUG_SECTION) & (DEBUG_MAP_BANDS | DEBUG_MAP_DATA))
    LOCKED_LOGGING((clog << ">>> Tiled_Image_Display::image_update_needed: " << update_type << " - "
    ...
    {
#if ((DEBUG_SECTION) & (DEBUG_MAP_BANDS | DEBUG_MAP_DATA))
        LOCKED_LOGGING((clog << "    tile row " << tile_grid.ry() << endl));
#endif
        if ((tiles = Tile_Grid_Images->at(tile_grid.ry())))
        {
            tile_grid.rx() = 0;
            ...
            {
#if ((DEBUG_SECTION) & (DEBUG_MAP_BANDS | DEBUG_MAP_DATA))
                LOCKED_LOGGING((clog << "      tile column " << tile_grid.rx() << endl));
#endif
...
#if ((DEBUG_SECTION) & (DEBUG_MAP_BANDS | DEBUG_MAP_DATA))
    LOCKED_LOGGING((clog << "<<< Tiled_Image_Display::image_update_needed" << endl));
#endif
}
...
void Tiled_Image_Display::state_change_start(int state)
{
    ...
    LOCKED_LOGGING((clog << ">>> Tiled_Image_Display::state_change_start: " << state << " - "
...
#endif
    int state_type = state & STATE_TYPE_MASK;
    if (state_type)
    {
...
//	>>> SIGNAL <<<
        ...
        LOCKED_LOGGING((clog << "^^^ Tiled_Image_Display::state_change_start: "
                                "emit state_change: "
                             << state << " - " << state_change_description(state) << endl
                             << "    in " << object_pathname(this) << endl));
#endif
...
#if ((DEBUG_SECTION) & DEBUG_STATE)
    LOCKED_LOGGING((clog << "    Renderer::start_rendering ..." << endl));
#endif
    Renderer->start_rendering();

#if ((DEBUG_SECTION) & DEBUG_STATE)
    LOCKED_LOGGING((clog << "<<< Tiled_Image_Display::state_change_start" << endl));
#endif
}

void Tiled_Image_Display::state_change_completed(int qualifier)
{
    /*
...
//	>>> SIGNAL <<<
        ...
        LOCKED_LOGGING((clog << "^^^ Tiled_Image_Display::state_change_completed: "
                                "emit state_change: "
                             << qualifier << " - " << state_change_description(qualifier) << endl
                             << "    in " << object_pathname(this) << endl));
#endif
...
#if ((DEBUG_SECTION) & DEBUG_STATE)
    LOCKED_LOGGING((clog << "<<< Tiled_Image_Display::state_change_completed" << endl));
#endif
}

QString Tiled_Image_Display::state_change_description(int state)
{
    QString names;
    if (state)
    {
...
void Tiled_Image_Display::renderer_status(int status)
{
#if ((DEBUG_SECTION) & DEBUG_STATE)
...
//	>>> SIGNAL <<<
#if ((DEBUG_SECTION) & (DEBUG_STATE | DEBUG_SIGNALS))
    LOCKED_LOGGING((clog << "^^^ Tiled_Image_Display::renderer_status: "
                            "emit rendering_status "
                         << status << " - " << rendering_status_description(status) << endl));
#endif
...
QString Tiled_Image_Display::rendering_status_description(int status)
{
    QString description;
...
void Tiled_Image_Display::cancel_rendering()
{
#if ((DEBUG_SECTION) & (DEBUG_SIGNALS | DEBUG_STATE))
    LOCKED_LOGGING((clog << ">>> Tiled_Image_Display::cancel_rendering" << endl));
#endif
//	Cancel all rendering.
#if ((DEBUG_SECTION) & (DEBUG_SIGNALS | DEBUG_STATE))
...
#if ((DEBUG_SECTION) & (DEBUG_SLOTS | DEBUG_RENDERED | DEBUG_PAINT))
        LOCKED_LOGGING((clog << "    update ..." << endl));
#endif
        update();
...
void Tiled_Image_Display::paintEvent(QPaintEvent* event)
{
    ...
    QString pathname(object_pathname(this));
    QSizeF scaled_size(scaled_image_size());
    LOCKED_LOGGING((clog << ">>> Tiled_Image_Display::paintEvent: " << event->rect() << endl
                         << "    in " << pathname << endl
                         << "              display size = " << size() << endl
                         << "         Tile_Display_Size = " << Tile_Display_Size << endl
    ...
    if (Tile_Display_Size.isEmpty() || Tile_Grid_Size.isEmpty())
    {
    ...
    QPoint tile_grid(map_display_to_tile(paint_region.topLeft())),
        //	N.B.: bottomRight is (left + width - 1, top + height - 1)
        tile_grid_limit(map_display_to_tile(paint_region.bottomRight())), painted_limit(-1, -1);
...
//	The Source image has not yet been rendered.
        ...
        LOCKED_LOGGING((clog << "    Tiled_Image_Display::paintEvent: "
    ...
    QRectF source_region(map_tile_to_image(tile_grid) *= source_scaling.rwidth(), source_size);
    source_region.translate(1, 1);

    double source_x_origin = source_region.x();
    //	Source image size.
    source_size = Source_Image->source_size();
    source_size *= source_scaling.rwidth();

    //	Background fill color.
    QColor background_fill_color(
#if ((DEBUG_SECTION) & DEBUG_TILE_MARKINGS)
    ...
    {
#if ((DEBUG_SECTION) & (DEBUG_EVENTS | DEBUG_PAINT))
        LOCKED_LOGGING((clog << "    row " << tile_grid.ry() << endl));
#endif
        if ((tiles = Tile_Grid_Images->at(tile_grid.ry())))
        {
            tile_grid.rx() = tile_col_start;
                ...
                if ((tile_image = tiles->at(tile_grid.rx())))
                {
                    ...
                    if (tile_image->needs_update())
                    {
                        ...
                        LOCKED_LOGGING((clog << "        queue tile image for rendering -" << endl
                                             << *tile_image << endl
                                             << "    tile grid " << tile_grid << ", tile region "
                                             << displayed_tile_region << endl));
#endif
                        Renderer->queue(tile_image, tile_grid, displayed_tile_region);
...
//	Paint the source image region on the display.
                        ...
                        {
#if ((DEBUG_SECTION) & (DEBUG_EVENTS | DEBUG_PAINT))
                            LOCKED_LOGGING((clog << "------> background fill" << endl));
#endif
...
//	Paint the tile image region on the display viewport.
...
#endif
#if ((DEBUG_SECTION) & DEBUG_TILE_MARKINGS)
                        label = QString(" paint @ %1: ").arg((ulong)tile_image, 0, 16);
#endif
                        painter.drawImage(tile_region, *tile_image);
                    }
#if ((DEBUG_SECTION) & DEBUG_TILE_MARKINGS)
                    //	Tile markings.
                    painter.drawLine(tile_region.topLeft(), tile_region.bottomRight());
                    painter.drawLine(tile_region.bottomLeft(), tile_region.topRight());
                    label += QString("gc %1, %2; so %3, %4; to %5, %6")
                                 .arg(tile_grid.rx())
                                 .arg(tile_grid.ry())
                                 .arg(tile_image->source_origin().x())
                                 .arg(tile_image->source_origin().y())
                    ...
                    {
                        rect.moveTop(tile_region.bottom() - TILE_MARKINGS_PAINT_Y);
                        painter.fillRect(rect, Qt::white);
                        painter.drawText(rect, Qt::AlignLeft | Qt::AlignVCenter, label);
                    }
                ...
                {
                    LOCKED_LOGGING((clog << "        empty" << endl));
                }
#endif
...
//	Paint any unpainted update area with background.
#if ((DEBUG_SECTION) & (DEBUG_EVENTS | DEBUG_PAINT))
    LOCKED_LOGGING((clog << "       painted region bottom = " << painted_limit.ry() << endl
                         << "         paint region bottom = " << paint_region.bottom() << endl));
#endif
...
#if ((DEBUG_SECTION) & (DEBUG_EVENTS | DEBUG_PAINT))
        LOCKED_LOGGING((clog << "    bottom background region = " << tile_region << endl));
#endif
        painter.fillRect(tile_region, background_fill_color);
    }
#if ((DEBUG_SECTION) & (DEBUG_EVENTS | DEBUG_PAINT))
    LOCKED_LOGGING((clog << "        painted region right = " << painted_limit.rx() << endl
                         << "          paint region right = " << paint_region.right() << endl));
#endif
    if (painted_limit.rx() < paint_region.right())
    {
        ...
        tile_region.setRight(paint_region.right());
#if ((DEBUG_SECTION) & (DEBUG_EVENTS | DEBUG_PAINT))
        LOCKED_LOGGING((clog << "      side background region = " << tile_region << endl));
#endif
        painter.fillRect(tile_region, background_fill_color);
    }
#if ((DEBUG_SECTION) & DEBUG_PROMPT)
...
//	>>> SIGNAL <<<
        ...
        LOCKED_LOGGING((clog << "^^^ Tiled_Image_Display::mouseMoveEvent: "
                                "emit image_cursor_moved"
                             << endl
                             << "    display position = " << event->pos() << endl
...
#if ((DEBUG_SECTION) & (DEBUG_EVENTS | DEBUG_REGION | DEBUG_OVERVIEW))
    QString pathname(object_pathname(this));
    QRectF display_region(displayed_image_region());
    LOCKED_LOGGING((clog << ">>> Tiled_Image_Display::resizeEvent:" << endl
                         << "    in " << pathname << endl
...
//	>>> SIGNAL <<<
    ...
    LOCKED_LOGGING((clog << "^^^ Tiled_Image_Display::resizeEvent: "
                            "emit display_viewport_resized "
                         << size() << endl));
#endif
...
//	>>> SIGNAL <<<
#if ((DEBUG_SECTION) & (DEBUG_SIGNALS | DEBUG_EVENTS | DEBUG_REGION | DEBUG_STATE))
        LOCKED_LOGGING((clog << "^^^ Tiled_Image_Display::displayed_image_region: "
                                "emit displayed_image_region_resized: "
                             << displayed_image_region_size() << endl));
#endif
        emit displayed_image_region_resized(displayed_image_region_size());
...
//	Note: A display update is done automatically for a resizeEvent.
    ...
    LOCKED_LOGGING((clog << "    displayed_image_region = " << display_region << endl
                         << "    in " << pathname << endl
